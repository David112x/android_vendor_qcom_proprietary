...
public:
        ...
        SIZE_T bufferSize)
    {
        ...
        else
        {
            ...
            {
                CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: The buffer is not sufficiently large to hold UnusedSpace tag",
                              m_writerId);
                result = CamxResultEInvalidArg;
            }
            else
            {
                CAMX_LOG_VERBOSE(CamxLogGroupDebugData, "Writer: %u: Set buffer: %p size: %zu", m_writerId, pBuffer, bufferSize);
        ...
        DebugDataTagType type)
    {
        ...
        if (m_bufferRemaining <= 0)
        {
            ...
            CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Insufficient size to write data", m_writerId);
            return result;
        }
        ...
        if (CamxResultSuccess != result)
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData,
                          "Writer: %u: Error! Unrecognized type or the given type is not a custom type: %u",
                          m_writerId, type);
        ...
        if (CamxResultSuccess == result)
        {
            if (m_sanitySize != GetSizeOfType(type))
            {
                CAMX_LOG_WARN(CamxLogGroupDebugData,
                              "Writer: %u: "
                              "Implementation Error! Typedef tag-type: [Dec:%d | Hex:0x%x], total-field size: %u bytes. "
                              "Expected size returned by GetSizeOfType(): %u bytes. This is caused by incorrect code change "
                              "by developer",
                              m_writerId,
                              type, type,
                              m_sanitySize,
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Error, failed to add the type definition tag-ID 0x%x",
                          m_writerId, type);
        }
        return result;
    }
    ...
    /// UpdateWriterId
    ///
    /// @brief  Updates writer ID.
    ///
    /// @param  Id  WriterId
    ///
    /// @return CamxResult
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    CamxResult UpdateWriterId(
        WriterId Id)
    {
        m_writerId = Id;
        ...
        DebugDataTagType typeEnd)
    {
        ...
        if (begin >= end)
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Error, invalid input!", m_writerId);
            result = CamxResultEInvalidArg;
        }
        else
        {
        ...
        SIZE_T           dataSize)
    {
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Called with invalid argument of UnusedSpace",
                          m_writerId);
            result = CamxResultEInvalidArg;
        }
        else if (NULL == pTag)
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Debug data buffer is NULL", m_writerId);
            result = CamxResultEInvalidState;
        }
        else if (NULL == pData && dataSize > 0)
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData,
                          "Writer: %u: Invalid! Tag ID %u has no payload but tag size indicates presence of a payload",
                          m_writerId, id);
            result = CamxResultEInvalidArg;
        }
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData,
                          "Writer: %u: Insufficient size to write data for tag ID: %u. Free space left: %zu bytes. "
                          "payload size: %u bytes",
                          m_writerId, id, sizeLeft, dataSize);
        ...
        if (CamxResultSuccess == result)
        {
            ...
            for (UINT32 i = 0; i < CAMX_ARRAY_SIZE(DebugDataTagIDDeprecated); i++)
            {
                ...
                {
                    CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Deprecated tag ID %u!", m_writerId, id);
                    result = CamxResultEUnsupported;
                    break;
                }
            }
        ...
        if (CamxResultSuccess == result)
        {
            if (count == 0)
            {
                CAMX_LOG_VERBOSE(CamxLogGroupDebugData, "Writer: %u: Skipping tag ID: %u because the data count is zero",
                                 m_writerId, id)
            ...
            {
                CAMX_LOG_WARN(CamxLogGroupDebugData,
                              "Writer: %u: Payload size of %u does not match the expected size of %u. Tag ID %u, Type: %u",
                              m_writerId, dataSize, expectedSize, id, type);
                result = CamxResultEInvalidArg;
            }
            else
            {
                ...
                // Reference logs, but very verbose
                // CAMX_LOG_VERBOSE(CamxLogGroupDebugData,
                //                  "Writer: %u: Add TagID: %d (0x%x), type: %u (0x%x) count %u, space left: %zu",
                //                  m_writerId, id, id, type, type, count, (m_bufferSize - m_bytesWritten));
...
protected:
        ...
        UINT16              count)
    {
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Invalid field count of %u. Value must be greater than zero",
                          m_writerId, count);
            result = FALSE;
        }
        else if (DebugDataTagType::TypeDefinition == fieldType)
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData, "Writer: %u: Invalid field type! Cannot use typedefs as a field type",
                          m_writerId);
            result = FALSE;
        }
        ...
        UINT16              dimension2)
    {
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupDebugData,
                          "Writer: %u: Invalid dimensions for the 2D array! Values must be greater than zero",
                          m_writerId);
            return FALSE;
        }
    ...
    CamxResult SealUnusedSpace()
    {
        ...
        else
        {
            ...
            if ((m_bufferSize < m_bytesWritten))
            {
                unusedBytes = 0;
            }
            else if (m_bufferSize  > UINT32_MAX + m_bytesWritten)
            {
                unusedBytes = UINT32_MAX;
            }
            else
            {
                unusedBytes = m_bufferSize - m_bytesWritten;
            }
            pTag->count = static_cast<TagCount>(unusedBytes);
        }
