...
typedef struct
{
    union
    {
        ...
        struct
        {
            ...
            uint32_t highestbankbit_lv2_en : 1; // don't care in pre-Kona
            uint32_t highestbankbit_lv3_en : 1; // don't care in pre-Kona
        } fields;
    } mode;
} UbwcFetchModeCfg;
...
typedef struct
{
    union
    {
        ...
        struct
        {
            ...
            uint32_t ubwc_bankspread_en      : 1; // don't care in Kona and beyond
            uint32_t ubwc_highestbankbit_en  : 1; // don't care in Kona and beyond
            uint32_t ubwc_highestbank_lv1_en : 1;
            uint32_t ubwc_highestbankbit_val : 5; // Use pre-Kona convention: value of actual
                                                  // bit number; e.g. if Bit 16 is to be used,
                                                  // ubwc_highestbankbit_val should be set to 16
            uint32_t reserved                : 3; // renamed as reserved as original field is
                                                  // controlled by UBWC-related fields in ImageInfo
            uint32_t ubwc_highestbank_lv2_en : 1; // don't care in pre-Kona
            uint32_t ubwc_highestbank_lv3_en : 1; // don't care in pre-Kona
            uint32_t un_used                 : 1;
            uint32_t ubwc_8channel_en        : 1;
        } fields;
    } mode;
...
/**
   ...
   */
#define FW_API_VERSION_MAJOR_VAL          2
#define FW_API_VERSION_MINOR_VAL          0
#define FW_API_VERSION_REVISION_VAL       88
...
#define _STR(s)   #s
#define FW_API_VERSION_STR                "API_VER=" _XSTR(FW_API_VERSION_MAJOR_VAL) "." _XSTR(FW_API_VERSION_MINOR_VAL) "." _XSTR(FW_API_VERSION_REVISION_VAL)
