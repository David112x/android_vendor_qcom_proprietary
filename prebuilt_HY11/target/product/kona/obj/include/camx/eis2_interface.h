...
/**
*  @brief   This function allocates memory for EIS 2.x and initializes the algorithm.
*
*  @param [out] eis2_handle     EIS 2.x handle
...
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_get_stabilization_margin_ex(
...
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_get_stabilization_crop_ratio_ex(
    const is_get_stabilization_margin* in,
...
*
*  @param [in]  eis2_handle             EIS 2.x context
...
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_get_stabilization_margin(
    void* eis2_handle,
...
*
*  @param [in,out]  eis2_handle         EIS 2.x context opaque handle
...
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_get_gyro_time_interval(
    void* eis2_handle,
...
*           Should be called every frame after SOF, gyro and OIS output will use as input to eis2_process().
*
*  @param [in,out]  eis2_handle         EIS 2.x handle
*  @param [in]      frame_time_input    frame time input to pre-process
*  @param [in]      active_sensor_idx   Active sensor index
*  @param [out]     gyro_req_interval   time interval of gyro samples that should be available for processing this frame
...
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_get_gyro_ois_time_interval(
    void* eis2_handle,
    const frame_times_t* frame_time_input,
    uint32_t active_sensor_idx,
    gyro_times_t* gyro_req_interval,
...
*           Should be called every frame after SOF. Gyro, OIS, and iFOVC output will use as input to eis2_process().
*
*  @param [in,out]  eis2_handle                 EIS 2.x handle
*  @param [in]      frame_time_input            frame time input to pre-process
*  @param [in]      active_sensor_idx           Active sensor index
...
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_get_time_intervals(
    void* eis2_handle,
    const frame_times_t* frame_time_input,
    uint32_t active_sensor_idx,
...
*
*  @param [in,out]  eis2_handle     EIS 2.x handle
*  @param [in]      eis2_input      Input to EIS 2.x algorithm
*  @param [out]     is_output       Output of EIS 2.x algorithm
*
*  @return 0 in case of success, otherwise failed.
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_process(void* eis2_handle, const is_input_t* eis2_input, is_output_type* is_output);
...
**/
EIS_VISIBILITY_PUBLIC
int32_t eis2_deinitialize(void** eis2_handle);

#ifdef __cplusplus
}
#endif

#endif //__EIS2_INTERFACE_H__
