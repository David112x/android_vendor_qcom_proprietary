...
enum class GeoLocationDataStatus {
  UNKNOWN,
  TIMEOUT, /* Modem gets time out on retrieving the Longitude and Latitude from GPS engine */
  NO_CIVIC_ADDRESS, /* Geo Location information does not contain the relevant data (ex: country code) */
  ENGINE_LOCK,      /* GPS is disabled from UI */
  RESOLVED          /* No GPS errors */
};
inline std::string toString(const GeoLocationDataStatus& o) {
  if (o == GeoLocationDataStatus::UNKNOWN) {
    return "UNKNOWN";
  }
  if (o == GeoLocationDataStatus::TIMEOUT) {
    return "TIMEOUT";
  }
  if (o == GeoLocationDataStatus::NO_CIVIC_ADDRESS) {
    return "NO_CIVIC_ADDRESS";
  }
  if (o == GeoLocationDataStatus::ENGINE_LOCK) {
    return "ENGINE_LOCK";
  }
  if (o == GeoLocationDataStatus::RESOLVED) {
    return "RESOLVED";
  }
  return "<invalid>";
}
...
class ImsSubConfigInfo : public qcril::interfaces::BasePayload {
  ...
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR_VECTOR(bool, ImsStackEnabled);
...
class Location : public qcril::interfaces::BasePayload {
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(float, Radius);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(double, Latitude);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(double, Longitude);
public:
  static const int LOCATION_NOT_SET = -1; // location is not set if radius is equal
  static const int POINT_LOCATION = 0; // location is a point if radius is equal

  Location() {
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR(Radius);
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR(Latitude);
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR(Longitude);
  }
  Location(const Location &from) {
    QCRIL_INTERFACE_COPY_OPTIONAL_MEMBER_VAR(from, Radius);
...
class CallComposerInfo : public qcril::interfaces::BasePayload {
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(int, Id);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(Priority, Priority);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR_VECTOR(uint16_t, Subject);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(std::string, ImageUrl);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(Location, Location);
public:
  CallComposerInfo() {
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR(Id);
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR(Priority);
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR_VECTOR(Subject);
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR(ImageUrl);
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR(Location);
  }
  CallComposerInfo(const CallComposerInfo &from) {
    QCRIL_INTERFACE_COPY_OPTIONAL_MEMBER_VAR(from, Id);
    QCRIL_INTERFACE_COPY_OPTIONAL_MEMBER_VAR(from, Priority);
    QCRIL_INTERFACE_COPY_OPTIONAL_MEMBER_VAR_VECTOR(from, Subject);
    QCRIL_INTERFACE_COPY_OPTIONAL_MEMBER_VAR(from, ImageUrl);
    QCRIL_INTERFACE_COPY_OPTIONAL_MEMBER_VAR(from, Location);
  }
