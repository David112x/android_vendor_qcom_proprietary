...
/*
 * Request to stop the monitoring an Expression for OsAppId
 *
 * Response:
 *   errorCode    : Valid error codes
 *   responseData : nullptr
 */
class QcRilRequestCancelSubscribeMessage
    : public QcRilRequestMessage,
      public add_message_id<QcRilRequestCancelSubscribeMessage> {
private:
  std::optional<std::string> mOsAppId;
  std::optional<std::string> mExpression;

public:
  static constexpr const char *MESSAGE_NAME = "QcRilRequestCancelSubscribeMessage";

  QcRilRequestCancelSubscribeMessage() = delete;

  ~QcRilRequestCancelSubscribeMessage() {}

  inline QcRilRequestCancelSubscribeMessage(std::shared_ptr<MessageContext> context)
