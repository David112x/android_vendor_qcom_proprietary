                                        ...
                                        public add_message_id<QcRilRequestSetMbnConfigMessage>
{
  public:
    ...
    explicit QcRilRequestSetMbnConfigMessage(std::shared_ptr<MessageContext> context, uint32_t subId,
                                             qcril::interfaces::qcril_pdc_mbn_type mbnType,
                                             const std::string& mbnFilePath,
                                             const std::vector<uint8_t>& configId)
        : QcRilRequestMessage(get_class_message_id(), context),
          mSubId(subId),
          mMbnType(mbnType),
          mFilePath(mbnFilePath),
          mConfigId(configId) {
      mName = MESSAGE_NAME;
    }
