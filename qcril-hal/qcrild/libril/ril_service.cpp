...
#ifdef RIL_FOR_LOW_RAM
rildata::AccessNetwork_t convertRadioTechToAccessNetwork(RadioTechnology tech);
#endif
...
#ifdef RIL_FOR_LOW_RAM
unordered_map<rildata::DataCallFailCause_t, V1_0::DataCallFailCause> failCauseMap = {
    ...
    auto msg =
      ...
      if (msg) {
        GenericCallback<SetupDataCallResponse_t> cb([serial](std::shared_ptr<Message>,
            ...
            if (rsp != nullptr) {
                auto indMsg = std::make_shared<SetupDataCallRadioResponseIndMessage_1_0>(*rsp, serial, status);
                if (indMsg != nullptr) {
                    indMsg->broadcast();
                } else {
...
using namespace rildata;
    ...
    if (msg)
    {
        GenericCallback<ResponseError_t> cb([serial](std::shared_ptr<Message>,
            ...
            if (rsp != nullptr)
            {
                auto indMsg = std::make_shared<DeactivateDataCallRadioResponseIndMessage_1_0>(*rsp, serial, status);
                if (indMsg != nullptr)
                {
                    indMsg->broadcast();
                }
        ...
        ([this, serial](std::shared_ptr<Message> solicitedMsg, Message::Callback::Status status,
            ...
            if (solicitedMsg && responseDataPtr)
            {
                    ...
                    (responseDataPtr->respErr == ResponseError_t::NO_ERROR))
                {
                    ...
                    {
                        dcResultList[i].status = convertDcFailStatusToHidlDcFailCause(entry.cause);
            ...
            RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, getDataCallListResponse, responseInfo, dcResultList);
        }));
    if (msg)
    {
        msg->setCallback(&cb);
...
using namespace rildata;
    ...
    for (size_t i = 0; i < profiles.size(); i++)
    {
        ...
        t.protocol = profiles[i].protocol;
        t.roamingProtocol = profiles[i].roamingProtocol;
    ...
    if (msg)
    {
        GenericCallback<RIL_Errno> cb(
            ([this, serial](std::shared_ptr<Message> solicitedMsg, Message::Callback::Status status,
                ...
                RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, setDataProfileResponse, responseInfo);
            }));
        msg->setCallback(&cb);
        msg->dispatch();
    }
    ...
    {
        RadioResponseInfo rsp{RadioResponseType::SOLICITED, serial, RadioError::NO_MEMORY};
        RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, setDataProfileResponse, rsp);
    }
#else
...
Return<void> RadioImpl::startLceService(int32_t serial, int32_t reportInterval, bool pullMode) {
    ...
    if (msg) {
        GenericCallback<RIL_LceStatusInfo> cb([this, serial](std::shared_ptr<Message>,
            ...
            RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, startLceServiceResponse, responseInfo, result);
        });
        msg->setCallback(&cb);
        msg->dispatch();
    } else {
        RadioResponseInfo rsp{RadioResponseType::SOLICITED, serial, RadioError::NO_MEMORY};
        LceStatusInfo result = {};
        RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, startLceServiceResponse, rsp, result);
    }
#else
...
Return<void> RadioImpl::stopLceService(int32_t serial) {
    ...
    if (msg) {
        GenericCallback<RIL_LceStatusInfo> cb([this, serial](std::shared_ptr<Message>,
            ...
            RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, stopLceServiceResponse, responseInfo, result);
        });
        msg->setCallback(&cb);
        msg->dispatch();
    } else {
        RadioResponseInfo rsp{RadioResponseType::SOLICITED, serial, RadioError::NO_MEMORY};
        LceStatusInfo result = {};
        RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, stopLceServiceResponse, rsp, result);
    }
#else
...
Return<void> RadioImpl::pullLceData(int32_t serial) {
    ...
    if (msg)
    {
        GenericCallback<RIL_LceDataInfo> cb([this, serial](std::shared_ptr<Message>,
            ...
            RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, pullLceDataResponse, responseInfo, result);
        });
        msg->setCallback(&cb);
        msg->dispatch();
    }
    else
    {
        RadioResponseInfo rsp{RadioResponseType::SOLICITED, serial, RadioError::NO_MEMORY};
        LceDataInfo result = {};
        RADIO_HIDL_SEND_RESPONSE(mSlotId, mRadioResponse, pullLceDataResponse, rsp, result);
    }
#else
...
using namespace rildata;
    if( mRadioResponse == nullptr)
    {
      QCRIL_LOG_DEBUG("mRadioResponseV1_0 is null");
      return Void();
    }
    auto ret = V1_1::IRadioResponse::castFrom(mRadioResponse);
        if (!ret.isOk()) {
            RLOGE("%s: ret.isOk() == false for radioService[%d]", __FUNCTION__, mSlotId);
            return Void();
        }
    sp<V1_1::IRadioResponse> radioResponseV1_1 = ret;
    ...
    switch((KeepaliveType_t)keepalive.type) {
        case KeepaliveType_t::NATT_IPV4:
            if (keepalive.sourceAddress.size() != 4 ||
                ...
                RADIO_HIDL_SEND_RESPONSE(mSlotId, radioResponseV1_1, startKeepaliveResponse, responseInfo, ka_status);
                return Void();
            }
            break;
    ...
    if (msg) {
        ...
        RADIO_HIDL_SEND_RESPONSE(mSlotId, radioResponseV1_1, startKeepaliveResponse, rsp, ka_status);
    }
#else
...
using namespace rildata;
    if( mRadioResponse == nullptr)
    {
      QCRIL_LOG_DEBUG("mRadioResponse is null");
      return Void();
    }
    ...
    if (msg) {
        GenericCallback<ResponseError_t> cb([this, serial](std::shared_ptr<Message> msg,
            ...
            QCRIL_LOG_DEBUG("startKeepAlivecb sending resp");
            sp<V1_1::IRadioResponse> radioResponseV1_1 = ret;
            RADIO_HIDL_SEND_RESPONSE(mSlotId, radioResponseV1_1, stopKeepaliveResponse, responseInfo);
        });
        msg->setCallback(&cb);
        msg->dispatch();
    } else {
        ...
        sp<V1_1::IRadioResponse> radioResponseV1_1 = ret;
        RadioResponseInfo rsp{RadioResponseType::SOLICITED, serial, RadioError::NO_MEMORY};
        RADIO_HIDL_SEND_RESPONSE(mSlotId, radioResponseV1_1, stopKeepaliveResponse, rsp);
    }
#else
...
void RadioImpl::setupDataCallResponse(RadioResponseInfo responseInfo, V1_0::SetupDataCallResult dcResult) {
    QCRIL_LOG_FUNC_ENTRY();

    auto rsp = mRadioResponse;
    if (rsp) {
        QCRIL_LOG_INFO("setupDataCallResponse");
        Return<void> retStatus = rsp->setupDataCallResponse(responseInfo, dcResult);
        checkReturnStatus(retStatus);
    } else {
        RLOGE("setupDataCallResponse: radioService[%d]->mRadioResponse == NULL", mSlotId);
    }

    QCRIL_LOG_FUNC_RETURN();
}

void RadioImpl::deactivateDataCallResponse(RadioResponseInfo responseInfo) {
    QCRIL_LOG_FUNC_ENTRY();
...
void RadioImpl::dataCallListChanged( RadioIndicationType indType, hidl_vec<V1_0::SetupDataCallResult> dcResultList) {
    QCRIL_LOG_FUNC_ENTRY();
    auto ind = mRadioIndication;
    QCRIL_LOG_INFO("ind null: %s", ind ? "false" : "true" );
    if (ind) {
        Return<void> ret = ind->dataCallListChanged(RadioIndicationType::UNSOLICITED, dcResultList);
        checkReturnStatus(ret);
    } else {
        RLOGE("dataCallListChanged: radioService->mRadioResponse == NULL");
    }
    QCRIL_LOG_FUNC_RETURN();
}


void RadioImpl::KeepAliveStatusInd(RadioIndicationType indType, const ::android::hardware::radio::V1_1::KeepaliveStatus ks) {
  QCRIL_LOG_FUNC_ENTRY();

  auto ind = mRadioIndication;

  auto ret = V1_1::IRadioIndication::castFrom(ind);
  if(ind) {
    if (!ret.isOk()) {
        RLOGE("%s: ret.isOk() == false for radioService[%d]", __FUNCTION__, mSlotId);
        return;
    ...
    sp<V1_1::IRadioIndication> radioIndicationV1_1 = ret;
    Return<void> retStatus = radioIndicationV1_1->keepaliveStatus(indType, ks);
    checkReturnStatus(retStatus);
  } else {
