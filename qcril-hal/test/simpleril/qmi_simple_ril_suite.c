...
static int pbm_main();
static int command_option_main(int argc, char *argv[]);
...
int main(int argc, char *argv[])
    ...
    for(i = 1; i < argc; i++)
    {
        if ((0 == strcmp(argv[i], "-c")) || (0 == strcmp(argv[i], "--command")))
        {
            return command_option_main(argc-(i+1), argv+(i+1));
        }
        else if(0 == strcmp(argv[i], "pbm"))
        {
            pbm = TRUE;
        }
...
/* QMI DMS Test code begins */
static qmi_client_error_type modem_dms_init();
static qmi_client_error_type modem_dms_release();
static qmi_client_error_type dms_send_msg_sync(
    unsigned int       msg_id,
    void               *req_c_struct,
...
char* qmi_simple_ril_dms_get_modem_mode_str(int state);

static int command_option_modem_command_handler(char *arg) {
  int result = 1;
  // init DMS
  qmi_client_error_type qmi_error = modem_dms_init();
  if (qmi_error != QMI_NO_ERR)
  {
    printf("ERROR: Failed to initialize DMS modem subsystem, qmi_error:", qmi_error);
    return 1;
  }
  ...
  if (0 == strcmp(arg, "status"))
  {
    ...
    // sync request
    qmi_error = dms_send_msg_sync(
        QMI_DMS_GET_OPERATING_MODE_REQ_V01,
        NULL,
        0,
        &qmi_response,
        sizeof(qmi_response));
    qmi_util_logln1("qmi_error", qmi_error);
    qmi_util_logln2("qmi_response.resp(result, error)",
    ...
    {
      printf("%s\n", qmi_simple_ril_dms_get_modem_mode_str(qmi_response.operating_mode));
      result = 0;
    } else {
      printf("ERROR: Failed to get modem status");
      result = 1;
    }
  }
  else
  {
    ...
    if ((0 == strcmp("online", arg)) || (0 == strcmp("on", arg)))
    {
      qmi_request.operating_mode = DMS_OP_MODE_ONLINE_V01;
    }
    else if ((0 == strcmp("offline", arg) ) || (0 == strcmp("off", arg) ))
    {
      qmi_request.operating_mode = DMS_OP_MODE_OFFLINE_V01;
    }
        ...
        qmi_response.resp.error == QMI_ERR_NONE_V01)
    {
      ...
      printf("ERROR: Failed to set modem status\n");
      qmi_util_logln0("Failed to set modem status");
      result = 1;
    }
  }
cleanup:
...
/* Main function to handle the command from command line option '-c' */
int command_option_main(int argc, char *argv[])
{
  if (argc > 0 && argv[0] != NULL)
  {
    if (0 == strcmp(argv[0], "modem"))
    {
      if (argv[1] != NULL)
      {
        return command_option_modem_command_handler(argv[1]);
      }
    }
  }
...
static qmi_idl_service_object_type dms_service_obj;
...
/* This function is responsible for initializing the QMI connection to QMI DMS service */
qmi_client_error_type modem_dms_init()
{
    int                   time_out = 4;
    qmi_client_error_type rc;
    ...
    if (rc != QMI_NO_ERR )
    {
         qmi_util_logln1("Error: connection not Initialized...Error Code:",rc);
         rc = qmi_client_release(dms_user_handle);
         if (rc < 0 )
         {
            qmi_util_logln0("Release not successful");
         }
         else
         {
            qmi_util_logln0("QMI client release successful");
         }
    }
    else
    {
        qmi_util_logln1("Connection Initialized....User Handle:", (int)dms_user_handle);
    }
    return rc;
}

/* Release the QMI DMS service */
qmi_client_error_type modem_dms_release()
{
  qmi_client_error_type rc = qmi_client_release(dms_user_handle);
  if (rc < 0 )
  {
    qmi_util_logln0("Release not successful \n");
  }
...
qmi_client_error_type dms_send_msg_sync(
    unsigned int       msg_id,
    void               *req_c_struct,
    unsigned int       req_c_struct_len,
    void               *resp_c_struct,
    unsigned int       resp_c_struct_len)
{
    ...
    if (rc != QMI_NO_ERR)
    {
        printf("qmi_client_send_msg_sync error %d\n", rc);
    }
    return rc;
}
