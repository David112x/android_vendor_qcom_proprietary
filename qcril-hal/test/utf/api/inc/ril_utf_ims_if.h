...
//==================================================
  ...
  */
  void update_ims_unsol_auto_call_rejection_ind(ims_CallType callType, ims_CallFailCause
          autoRejectionCause, uint16_t sipErrorCode,
...
void set_ims_request_set_supp_svc_response_resp_provisionStatus(ims_ServiceClassProvisionStatus status);
  ...
  */
   ...
   */
  void update_default_ims_unsol_retrieving_geolocation_data_status(
      enum ims_GeoLocationDataStatus geoLocationDataStatus);

  /*
     ims_MsgId_UNSOL_CALL_COMPOSER_INFO
  */
  ims_CallComposer* update_default_ims_unsol_call_composer_info();

  /*
     ims_MsgId_UNSOL_CALL_COMPOSER_AUTO_REJECTED
  */
  ims_CallComposerAutoRejected* update_default_ims_unsol_call_composer_auto_rejection_ind();
