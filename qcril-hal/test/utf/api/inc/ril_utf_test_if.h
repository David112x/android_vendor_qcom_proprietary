...
typedef class ril_utf_test_t {
  ...
  void validate_ims_unsol_registration_block_status(ril_utf_node_base_t *exp_node,
      struct ril_utf_msg_t *data);
  bool compare_call_rejection_info(const ims_AutoCallRejectionInfo& exp,
      const ims_AutoCallRejectionInfo& recv);
  void validate_ims_unsol_auto_call_rejection_ind(ril_utf_node_base_t *exp_node,
      struct ril_utf_msg_t *data);
  void validate_ims_unsol_retrieving_geolocation_data_status(ril_utf_node_base_t *exp_node,
      struct ril_utf_msg_t *data);
  bool compare_call_composer_info(const ims_CallComposer& expected,
        const ims_CallComposer& received);
  bool compare_call_composer_location(const ims_CallComposerLocation& expected,
        const ims_CallComposerLocation& received);
  void validate_ims_unsol_call_composer_info_ind(ril_utf_node_base_t *exp_node,
      struct ril_utf_msg_t *data);
  void validate_ims_unsol_auto_call_rejection_with_call_composer(
