                                   ...
                                   struct ril_utf_msg_t *data)
{
  ...
  else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_RETRIEVING_GEOLOCATION_DATA_STATUS)
  {
    validate_ims_unsol_retrieving_geolocation_data_status(exp_node, data);
  }
  else if ( exp_node->get_msg_id() == ims_MsgId_UNSOL_CALL_COMPOSER_INFO )
  {
    validate_ims_unsol_call_composer_info_ind( exp_node, data );
  }
  else if ( exp_node->get_msg_id() == ims_MsgId_UNSOL_CALL_COMPOSER_AUTO_REJECTED )
  {
    validate_ims_unsol_auto_call_rejection_with_call_composer( exp_node, data );
  }
  else
  {
...
bool ril_utf_test_t::compare_call_rejection_info(const ims_AutoCallRejectionInfo& exp,
    const ims_AutoCallRejectionInfo& recv) {
                                                     ...
                                                     struct ril_utf_msg_t *data)
{
  ...
  if (compare_call_rejection_info(*exp, *recv)) {
      exp_node->set_expect_status(met);
  } else {
      RIL_UTF_DEBUG("\n QMI_RIL_UTL: Error, messages do not match");
      exp_node->set_expect_status(not_met);
...
void ril_utf_test_t::validate_ims_unsol_retrieving_geolocation_data_status(
    ril_utf_node_base_t* exp_node, struct ril_utf_msg_t* data) {
  int validation_success = TRUE;
  ims_GeoLocationDataStatusInfo* exp = (ims_GeoLocationDataStatusInfo*)exp_node->get_payload();
  ims_GeoLocationDataStatusInfo* recv = (ims_GeoLocationDataStatusInfo*)data->payload;

  do {
    if (exp->geoLocationDataStatus != recv->geoLocationDataStatus) {
      RIL_UTF_DEBUG("\n QMI_RIL_UTF: geoLocationDataStatus exp = %d and recv = %d\n",
                    exp->geoLocationDataStatus, recv->geoLocationDataStatus);
      validation_success = FALSE;
      break;
    }
  } while (0);
...
bool ril_utf_test_t::compare_call_composer_location(
        const ims_CallComposerLocation& expected,
        const ims_CallComposerLocation& received) {
    if (expected.radius != received.radius) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: radius, expected = %f, received = %f",
                expected.radius, received.radius);
        return false;
    }
    if (expected.radius == -1) return true; // location info is not available, skip
    if (expected.latitude != received.latitude) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: latitude, expected = %f, received = %f",
                expected.latitude, received.latitude);
        return false;
    }
    if (expected.longitude != received.longitude) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: longitude, expected = %f, received = %f",
                expected.longitude, received.longitude);
        return false;
    }
    return true;
}

bool ril_utf_test_t::compare_call_composer_info(const ims_CallComposer& expected,
        const ims_CallComposer& received) {
    if (expected.call_id != received.call_id) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: call_id, expected = %d, received = %d",
                expected.call_id, received.call_id);
        return false;
    }
    if (expected.priority != received.priority) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: priority, expected = %d, received = %d",
                expected.priority, received.priority);
        return false;
    }
    if (expected.subject_len != received.subject_len) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: subject_len, expected = %d, received = %d",
                expected.subject_len, received.subject_len);
        return false;
    }
    if (memcmp(expected.subject, received.subject,
            sizeof(uint16_t)*expected.subject_len)) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: subject content does not match");
        print_msg_in_hex(expected.subject, sizeof(uint16_t)*expected.subject_len);
        print_msg_in_hex(received.subject, sizeof(uint16_t)*received.subject_len);
        return false;
    }
    if (!compare_call_composer_location(expected.location, received.location))
        return false;
    if (strncmp(expected.image_url, received.image_url,
            IMS_CALL_COMPOSER_PIC_URL_MAX_LEN)) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF image_url, expected = %s, received = %s",
                expected.image_url, received.image_url);
        return false;
    }
    return true;
}

void ril_utf_test_t::validate_ims_unsol_call_composer_info_ind(ril_utf_node_base_t *exp_node,
         struct ril_utf_msg_t *data)
{
    auto exp = static_cast<ims_CallComposer*>(exp_node->get_payload());
    auto recv = static_cast<ims_CallComposer*>(data->payload);

    if (compare_call_composer_info(*exp, *recv)) {
        exp_node->set_expect_status(met);
    } else {
        exp_node->set_expect_status(not_met);
    }
...
void ril_utf_test_t::validate_ims_unsol_auto_call_rejection_with_call_composer(
        ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data)
{
    auto exp = static_cast<ims_CallComposerAutoRejected*>(exp_node->get_payload());
    auto recv = static_cast<ims_CallComposerAutoRejected*>(data->payload);

    if (compare_call_rejection_info(exp->rej, recv->rej) &&
            compare_call_composer_info(exp->composer, recv->composer)) {
        exp_node->set_expect_status(met);
    } else {
        exp_node->set_expect_status(not_met);
    }
