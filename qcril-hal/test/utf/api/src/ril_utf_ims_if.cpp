/******************************************************************************
#  Copyright (c) 2013, 2019-2020 Qualcomm Technologies, Inc.
#  All Rights Reserved.
    ...
    ims_VerstatVerificationStatus verificationStatus)
{
  ...
  ptr->verificationStatus = verificationStatus;
}
...
/*========================================================
  ims_MsgId_UNSOL_RETRIEVING_GEOLOCATION_DATA_STATUS
========================================================*/
void ril_utf_ims_if::update_default_ims_unsol_retrieving_geolocation_data_status(
    enum ims_GeoLocationDataStatus geoLocationDataStatus)
{
  payload = malloc(sizeof(ims_GeoLocationDataStatusInfo));
  ims_GeoLocationDataStatusInfo *info = (ims_GeoLocationDataStatusInfo *)payload;
  payload_len = sizeof(ims_GeoLocationDataStatusInfo);
  memset(payload, 0x00, payload_len);
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_RETRIEVING_GEOLOCATION_DATA_STATUS;
  info->geoLocationDataStatus = geoLocationDataStatus;
}
...
/*
   ims_MsgId_UNSOL_CALL_COMPOSER_INFO
*/
ims_CallComposer* ril_utf_ims_if::update_default_ims_unsol_call_composer_info()
{
    payload_type = ril_utf_ims_indication;
    msg_id = ims_MsgId_UNSOL_CALL_COMPOSER_INFO;

    payload = malloc(sizeof(ims_CallComposer));
    assert(payload != nullptr);
    payload_len = sizeof(ims_CallComposer);
    memset(payload, 0x00, payload_len);

    return static_cast<ims_CallComposer*>(payload);
}

/*
    ims_MsgId_UNSOL_CALL_COMPOSER_AUTO_REJECTED
*/
ims_CallComposerAutoRejected*
ril_utf_ims_if::update_default_ims_unsol_call_composer_auto_rejection_ind()
{
    payload_type = ril_utf_ims_indication;
    msg_id = ims_MsgId_UNSOL_CALL_COMPOSER_AUTO_REJECTED;

    payload = malloc(sizeof(ims_CallComposerAutoRejected));
    assert(payload != nullptr);
    payload_len = sizeof(ims_CallComposerAutoRejected);
    memset(payload, 0x00, payload_len);

    return static_cast<ims_CallComposerAutoRejected*>(payload);
}
