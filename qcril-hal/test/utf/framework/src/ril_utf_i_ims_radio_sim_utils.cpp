...
ims_VerstatVerificationStatus convertHidlToProtoVerificationStatus(
        V1_2::VerificationStatus verificationStatus) {
    switch(verificationStatus) {
        case V1_2::VerificationStatus::STATUS_VALIDATION_PASS:
            return ims_VerstatVerificationStatus_PASS;
        case V1_2::VerificationStatus::STATUS_VALIDATION_FAIL:
            return ims_VerstatVerificationStatus_FAIL;
        default:
            return ims_VerstatVerificationStatus_NONE;
    }
}
...
bool convertHidlToProtoAutoCallRejectionInfo(const V1_6::AutoCallRejectionInfo& in,
                                             ims_AutoCallRejectionInfo& out) {
  if (in.callType != V1_6::CallType::CALL_TYPE_INVALID) {
    out.callType = V1_6::utils::convertHidlToProtoCallType(in.callType);
  }
  if (in.autoRejectionCause != V1_5::CallFailCause::CALL_FAIL_INVALID) {
    out.autoRejectionCause = V1_5::utils::convertHidlToProtoCallFailCause(in.autoRejectionCause);
  }
  ...
  out.verificationStatus = V1_0::utils::convertHidlToProtoVerificationStatus(in.verificationStatus);
  return true;
}

bool convertFromHidl(ims_CallComposerPriority& out, const V1_6::CallPriority& in)
{
  switch (in) {
  case V1_6::CallPriority::URGENT:
    out = ims_CallComposerPriority_URGENT;
    break;
  case V1_6::CallPriority::NORMAL:
  default:
...
bool convertFromHidl(ims_CallComposerLocation& out, const V1_6::CallLocation& in)
{
...
bool convertFromHidl(ims_CallComposer& out, const V1_6::CallComposerInfo& in)
{
  out.call_id = in.callId;
  (void)convertFromHidl(out.priority, in.priority);
  out.subject_len = in.subject.size();
  if (out.subject_len > 0) {
    std::copy(in.subject.data(), in.subject.data() + in.subject.size(), out.subject);
  }
  (void)convertFromHidl(out.location, in.location);
