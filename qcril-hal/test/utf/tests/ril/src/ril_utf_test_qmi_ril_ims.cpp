...
/**==
@testname: ril_utf_ims_unsol_retrieving_geolocation_data_status_001
@description: onVoiceInfoChanged
==**/
test_result ril_utf_ims_unsol_retrieving_geolocation_data_status_001(user_specified_attrib_t attr) {
  Testcase t;
  QMIRILTestnode QMI_node;
  IMSTestnode IMS_node;
  ...
  // Inject: QMI_IMSA_GPS_INFO_IND_V01
  QMI_node.update_default_qmi_imsa_gps_info_ind(IMSA_GPS_ERROR_REASON_TIMEOUT_V01);
  t.inject(QMI_node);

  // Expect: ims_MsgId_UNSOL_RETRIEVING_GEOLOCATION_DATA_STATUS
  IMS_node.update_default_ims_unsol_retrieving_geolocation_data_status(
      ims_GeoLocationDataStatus_TIMEOUT);
  t.expect(IMS_node);

  // Inject: QMI_IMSA_GPS_INFO_IND_V01
  QMI_node.update_default_qmi_imsa_gps_info_ind(IMSA_GPS_ERROR_REASON_NO_CIVIC_ADDRESS_V01);
  t.inject(QMI_node);

  // Expect: ims_MsgId_UNSOL_RETRIEVING_GEOLOCATION_DATA_STATUS
  IMS_node.update_default_ims_unsol_retrieving_geolocation_data_status(
      ims_GeoLocationDataStatus_NO_CIVIC_ADDRESS);
  t.expect(IMS_node);

  // Inject: QMI_IMSA_GPS_INFO_IND_V01
  QMI_node.update_default_qmi_imsa_gps_info_ind(IMSA_GPS_ERROR_REASON_ENGINE_LOCK_V01);
  t.inject(QMI_node);

  // Expect: ims_MsgId_UNSOL_RETRIEVING_GEOLOCATION_DATA_STATUS
  IMS_node.update_default_ims_unsol_retrieving_geolocation_data_status(
      ims_GeoLocationDataStatus_ENGINE_LOCK);
  t.expect(IMS_node);

  // Inject: QMI_IMSA_GPS_INFO_IND_V01
  QMI_node.update_default_qmi_imsa_gps_info_ind(IMSA_GPS_ERROR_RESOLVED_V01);
  t.inject(QMI_node);

  // Expect: ims_MsgId_UNSOL_RETRIEVING_GEOLOCATION_DATA_STATUS
  IMS_node.update_default_ims_unsol_retrieving_geolocation_data_status(
      ims_GeoLocationDataStatus_RESOLVED);
  t.expect(IMS_node);
