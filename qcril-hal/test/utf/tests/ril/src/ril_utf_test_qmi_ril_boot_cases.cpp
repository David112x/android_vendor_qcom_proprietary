...
test_result qmi_ril_test_bootup_001()
{
  ...
  // Expect: QMI_IMSA_IND_REG_REQ_V01
  imsa_ind_reg_req_msg_v01 imsa_ind_msg;
  memset(&imsa_ind_msg, 0x00, sizeof(imsa_ind_msg));
  imsa_ind_msg.geo_location_status_config_valid = TRUE;
  imsa_ind_msg.geo_location_status_config = 1; // enable
  QMI_node.update_default_qmi_imsa_ind_reg_req_v01(&imsa_ind_msg);
  t.expect(QMI_node);
