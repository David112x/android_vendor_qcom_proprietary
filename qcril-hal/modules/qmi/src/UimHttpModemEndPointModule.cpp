...
)
{
  ...
  mMessageHandler = {
       ...
       HANDLER(LpaUimHttpTxnFailIndMsg, UimHttpModemEndPointModule::sendUimLpaHttpTxnFailureToQmi),
...
)
{
  ...
  Log::getInstance().d("qcril_uim_http_send_error_response tokenId:  " + std::to_string(token_id));
...
)
{
  ...
  uim_http_transaction_resp_msg_v01      * http_rsp_ptr    = nullptr;
  ...
  {
    case QMI_UIM_HTTP_TRANSACTION_RESP_V01:
      http_rsp_ptr = (uim_http_transaction_resp_msg_v01 *)http_transaction_rsp_msg_ptr->get_message(&msg_len);
      ...
      if (http_rsp_ptr != nullptr)
      {
        resp_data_ptr->qmi_error_code = http_rsp_ptr->resp.error;
      }
      break;
...
void UimHttpModemEndPointModule::sendUimLpaHttpTxnFailureToQmi(std::shared_ptr<Message> msg_ptr)
{

  std::shared_ptr<LpaUimHttpTxnFailIndMsg> httpTxnFailIndPtr(
    std::static_pointer_cast<LpaUimHttpTxnFailIndMsg>(msg_ptr));

  uint32_t tokenId;

  if (httpTxnFailIndPtr == nullptr)
  {
    Log::getInstance().d("httpTxnFailIndPtr is NULL");
    return;
  }

  tokenId = httpTxnFailIndPtr->getToken();
  qcril_uim_http_send_error_response(tokenId);
}
