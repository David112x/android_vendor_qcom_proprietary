...
{
  QCRIL_LOG_INFO("qmi_uim_get_profile_info_resp");
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    {
      rsp_data->rsp_data.profile_info_rsp.iccid_ptr = (uint8_t*)qcril_malloc(qmi_response->profile_iccid_len + 1);

      if(rsp_data->rsp_data.profile_info_rsp.iccid_ptr != NULL)
      {
        memset(rsp_data->rsp_data.profile_info_rsp.iccid_ptr, 0, (qmi_response->profile_iccid_len + 1));

        memcpy(rsp_data->rsp_data.profile_info_rsp.iccid_ptr,
               &qmi_response->profile_iccid[0],
               qmi_response->profile_iccid_len);
    ...
    {
      rsp_data->rsp_data.profile_info_rsp.nickname = (char*)qcril_malloc(qmi_response->profile_nickname_len + 1);

      if(rsp_data->rsp_data.profile_info_rsp.nickname != NULL)
      {
        memset(rsp_data->rsp_data.profile_info_rsp.nickname, 0, (qmi_response->profile_nickname_len + 1));
        memcpy(rsp_data->rsp_data.profile_info_rsp.nickname,
               &qmi_response->profile_nickname[0],
               qmi_response->profile_nickname_len);
      }
    }
    ...
    {
      rsp_data->rsp_data.profile_info_rsp.spn = (char*)qcril_malloc(qmi_response->profile_spn_len + 1);

      if(rsp_data->rsp_data.profile_info_rsp.spn != NULL)
      {
        memset(rsp_data->rsp_data.profile_info_rsp.spn, 0, (qmi_response->profile_spn_len + 1));
        memcpy(rsp_data->rsp_data.profile_info_rsp.spn,
               &qmi_response->profile_spn[0],
               qmi_response->profile_spn_len);
      }
    }
    ...
    {
      rsp_data->rsp_data.profile_info_rsp.name = (char*)qcril_malloc(qmi_response->profile_name_len + 1);

      if(rsp_data->rsp_data.profile_info_rsp.name != NULL)
      {
        memset(rsp_data->rsp_data.profile_info_rsp.name, 0, (qmi_response->profile_name_len + 1));
        memcpy(rsp_data->rsp_data.profile_info_rsp.name,
               &qmi_response->profile_name[0],
               qmi_response->profile_name_len);
      }
    }
    ...
    {
      rsp_data->rsp_data.profile_info_rsp.icon_data_ptr = (uint8_t*)qcril_malloc(qmi_response->profile_icon_data_len + 1);

      if(rsp_data->rsp_data.profile_info_rsp.icon_data_ptr != NULL)
      {
        memset(rsp_data->rsp_data.profile_info_rsp.icon_data_ptr, 0, (qmi_response->profile_icon_data_len + 1));
        memcpy(rsp_data->rsp_data.profile_info_rsp.icon_data_ptr,
               &qmi_response->profile_icon_data[0],
               qmi_response->profile_icon_data_len);
...
/*===========================================================================
  FUNCTION  qmi_uim_conf_code_resp
===========================================================================*/
/*!
@brief
  translates response data coming from QCCI into response data
...
*/
/*=========================================================================*/
static void qmi_uim_conf_code_resp
(
  const uim_profile_confirmation_code_resp_msg_v01         * qmi_response,
  qmi_uim_rsp_data_type                                    * rsp_data
)
{
  if(qmi_response == NULL || rsp_data == NULL)
  ...
  rsp_data->rsp_id = QMI_UIM_SRVC_CONF_CODE_RSP_MSG;
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    rsp_data->qmi_err_code = QMI_NO_ERR;
  }
...
)
{
  ...
  if (qcci_data_ptr->profile_name_valid && qcci_data_ptr->profile_name_len > 0 \
        && qcci_data_ptr->profile_name_len <= QMI_UIM_EUICC_PROFILE_NAME_MAX_V01)
  {
      qmi_data_ptr->lpa_add_profile_ind.profile_name = (char *)&qcci_data_ptr->profile_name[0];
  }
...
)
{
  ...
  QCRIL_LOG_INFO("qcril_uim_qmi_free_data, msg_id: 0x%x", msg_id);
  ...
  switch (msg_id)
  {
    ...
    case QMI_UIM_GET_PROFILE_INFO_RESP_V01:
      if (rsp_data->rsp_data.profile_info_rsp.iccid_ptr)
      {
        qcril_free(rsp_data->rsp_data.profile_info_rsp.iccid_ptr);
        rsp_data->rsp_data.profile_info_rsp.iccid_ptr = NULL;
        rsp_data->rsp_data.profile_info_rsp.iccid_len = 0;
      }
      if (rsp_data->rsp_data.profile_info_rsp.nickname)
      {
        qcril_free(rsp_data->rsp_data.profile_info_rsp.nickname);
        rsp_data->rsp_data.profile_info_rsp.nickname = NULL;
      }
      if (rsp_data->rsp_data.profile_info_rsp.spn)
      {
        qcril_free(rsp_data->rsp_data.profile_info_rsp.spn);
        rsp_data->rsp_data.profile_info_rsp.spn = NULL;
      }
      if (rsp_data->rsp_data.profile_info_rsp.name)
      {
        qcril_free(rsp_data->rsp_data.profile_info_rsp.name);
        rsp_data->rsp_data.profile_info_rsp.name = NULL;
      }
      if (rsp_data->rsp_data.profile_info_rsp.icon_data_ptr)
      {
        qcril_free(rsp_data->rsp_data.profile_info_rsp.icon_data_ptr);
        rsp_data->rsp_data.profile_info_rsp.icon_data_ptr = NULL;
        rsp_data->rsp_data.profile_info_rsp.icon_len = 0;
      }
      break;
    default:
  ...
  else
  {
    ...
    switch (msg_id)
    {
      ...
      case QMI_UIM_EUICC_DEFAULT_SERVER_ADDRESS_RESP_V01:
        ...
        break;
      case QMI_UIM_PROFILE_CONFIRMATION_CODE_RESP_V01:
        qmi_uim_conf_code_resp((uim_profile_confirmation_code_resp_msg_v01 *)qmi_response,&rsp_data);
        break;
      default:
...
)
{
  ...
  if(!user_consent_req.user_ok)
  {
      user_consent_req.nok_reason_valid = 1;

      QCRIL_LOG_ERROR("Nok_reason valid");

      QCRIL_LOG_INFO("nok_reason is %d",params_ptr->nok_reason);

      QCRIL_LOG_INFO("size of user_consent_structure is %d",sizeof(qmi_uim_profile_user_consent_req_type));

      switch(params_ptr->nok_reason)
      {
          case 1:
                user_consent_req.nok_reason = UIM_PROFILE_END_USER_REJECTION_V01;
                QCRIL_LOG_ERROR("Nok_reason with reason as End user rejection");
                break;
          case 2:
               user_consent_req.nok_reason  = UIM_PROFILE_END_USER_POSTPONED_V01;
                QCRIL_LOG_ERROR("Nok_reason with reason as End user Postponed");
               break;
          default:
              QCRIL_LOG_ERROR("Invalid Nok_reason when user_consent is Failure");
              break;
      }
   }
...
/*===========================================================================
  FUNCTION qcril_qmi_uim_send_conf_code
===========================================================================*/
/*!
@brief
  LPA send confirmation code
...
*/
/*=========================================================================*/
EXTERN int qcril_qmi_uim_send_conf_code
(
  qmi_client_type                                   client_handle,
  const qmi_uim_conf_code_req_type                * params_ptr,
  qmi_uim_user_async_cb_type                        user_cb,
  void                                            * user_data,
  qmi_uim_rsp_data_type                           * rsp_data
)
{
  ...
  qmi_txn_handle                                       txn_handle;
  int                                                  rc                   = 0;
  uim_profile_confirmation_code_req_msg_v01            conf_code_req;
  uim_profile_confirmation_code_resp_msg_v01         * conf_code_resp_ptr   = NULL;
  qmi_uim_cb_struct_type                             * cb_data              = NULL;

  memset(&conf_code_req, 0x00, sizeof(conf_code_req));

  if (params_ptr == nullptr)
  {
    QCRIL_LOG_ERROR("Invalid params_ptr or confirmation code");
    return QMI_SERVICE_ERR;
  }

  conf_code_req.slot = (uim_slot_enum_v01) params_ptr->slot;

  if((params_ptr->confirmation_code_ptr != nullptr) &&  (strlen(params_ptr->confirmation_code_ptr)))
  {
      conf_code_req.confirmation_code_len = strlen(params_ptr->confirmation_code_ptr);
      memset(conf_code_req.confirmation_code, '\0' , conf_code_req.confirmation_code_len);
      strlcpy(conf_code_req.confirmation_code,
                params_ptr->confirmation_code_ptr,
                sizeof(conf_code_req.confirmation_code));
      conf_code_req.confirmation_code_valid = TRUE;
      QCRIL_LOG_ERROR("valid Confirmation Code Received from User");
  }
  else
  {
     conf_code_req.confirmation_code_valid = FALSE;
     QCRIL_LOG_ERROR("Confirmation code cancelled by the user");
  }

  /* Allocate response pointer since it is an async command */
  conf_code_resp_ptr = (uim_profile_confirmation_code_resp_msg_v01 *)
                             qcril_malloc(sizeof(uim_profile_confirmation_code_resp_msg_v01));
  if (conf_code_resp_ptr == NULL)
  {
    QCRIL_LOG_ERROR("error allocating memory for conf_code_resp_ptr");
    return QMI_SERVICE_ERR;
  }
  ...
  if (user_cb)
  {
    ...
    {
      QCRIL_LOG_ERROR("failed to allocate cb_data");
      qcril_free(conf_code_resp_ptr);
      return QMI_SERVICE_ERR;
    }
    QMI_UIM_MAKE_CB_STRUCT(cb_data, user_cb, user_data);

    rc = qmi_client_send_msg_async( client_handle,
                                    QMI_UIM_PROFILE_CONFIRMATION_CODE_REQ_V01,
                                    &conf_code_req,
                                    sizeof(uim_profile_confirmation_code_req_msg_v01),
                                    (void *) conf_code_resp_ptr,
                                    sizeof(uim_profile_confirmation_code_resp_msg_v01),
                                    qmi_uim_client_async_cb,
                                    cb_data,
                                    &txn_handle);
    if( rc != QMI_NO_ERR )
  ...
  else
  {
    ...
    if(!rc)
    {
      qmi_uim_conf_code_resp(conf_code_resp_ptr, rsp_data);
    }
    qcril_free(conf_code_resp_ptr);
  }
