...
qcril_qmi_ims_create_ServiceStatusInfoList(const imsa_service_status_ind_msg_v01 *qmi_data) {
  ...
  if (srvStatInfoList != nullptr) {
    ...
    if (qmi_data->callcomposer_service_status_valid) {
      srvStatInfoList->addToServiceStatusInfo(qcril_qmi_ims_create_ServiceStatusInfo(
          qcril::interfaces::CallType::CALLCOMPOSER, qmi_data->callcomposer_service_status,
          qmi_data->callcomposer_service_rat_valid, qmi_data->callcomposer_service_rat));
    }
  }
...
)
{
  ...
  switch(ims_config_item)
  {
    ...
    case qcril::interfaces::ConfigItem::CALL_COMPOSER_SETTING:
      ret = QCRIL_QMI_RADIO_CONFIG_MMTEL_CALL_COMPOSER_PREFERENCE;
      break;
...
)
{
  ...
  switch(radio_config_item)
  {
    ...
    case QCRIL_QMI_RADIO_CONFIG_MMTEL_CALL_COMPOSER_PREFERENCE:
      ret = qcril::interfaces::ConfigItem::CALL_COMPOSER_SETTING;
      break;
