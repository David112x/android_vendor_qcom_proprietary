...
using namespace vendor::qti::hardware::radio::lpa::V1_1;
...
(
  const sp<V1_0::IUimLpaResponse>& responseCallback,
...
#ifndef QMI_RIL_UTF
  mIndicationCb_1_1 = V1_1::IUimLpaIndication::castFrom(indicationCallback).withDefault(nullptr);
#endif
...
sp<V1_0::IUimLpaResponse> UimLpaImpl::getResponseCallback() {
  std::shared_lock<qtimutex::QtiSharedMutex> lock(mCallbackLock);
  return mResponseCb;
}

sp<V1_0::IUimLpaIndication> UimLpaImpl::getIndicationCallback() {
  std::shared_lock<qtimutex::QtiSharedMutex> lock(mCallbackLock);
  return mIndicationCb;
}
sp<V1_1::IUimLpaIndication> UimLpaImpl::getIndicationCallback_1_1() {
  std::shared_lock<qtimutex::QtiSharedMutex> lock(mCallbackLock);
...
)
{
  ...
  if (txnInd == NULL)
  {
    QCRIL_LOG_ERROR("uimLpaHttpTxnIndication txnInd NULL");
    return;
  }
  ...
  if (indCb == NULL)
  {
    ...
    if(txnInd != NULL)
    {
      mModule->sendUimLpaHttpTxnFailIndication(txnInd->tokenId);
    }
    return;
  }
...
)
{
  ...
  sp<V1_1::IUimLpaIndication> indCb_1_1 = getIndicationCallback_1_1();
  ...
  if(indCb_1_1 != NULL)
  {
      ...
      progress.base.status              = (UimLpaAddProfileStatus)progInd->status;
      progress.base.cause               = (UimLpaAddProfileFailureCause)progInd->cause;
      progress.base.progress            = progInd->progress;
      progress.base.policyMask          = (UimLpaProfilePolicyMask)progInd->policyMask;
      progress.profileName              = progInd->profile_name;
      progress.base.userConsentRequired = progInd->userConsentRequired;

      Return<void> ret = indCb_1_1->UimLpaAddProfileProgressIndication_1_1(progress);
      if (!ret.isOk()) {
        QCRIL_LOG_ERROR("Unable to send indication. Exception : %s", ret.description().c_str());
      }
  }
...
===========================================================================*/
Return<void> UimLpaImpl::UimLpaUserRequest_1_1
(
  int32_t              token,
...
(
  ...
  const hidl_vec<V1_0::UimLpaHttpCustomHeader>& customHeaders
)
{
