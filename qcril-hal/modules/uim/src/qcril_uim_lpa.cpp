...
)
{
  ...
  user_consent_req.nok_reason = (uint8_t)nok_reason;

  QCRIL_LOG_INFO( "nok_reason %d  \n", nok_reason);
  QCRIL_LOG_INFO( "nok_reason %d  \n", user_consent_req.nok_reason);
...
)
{
  ...
  /* Free memory allocated in Sync msg response handling for QMI_UIM_GET_PROFILE_INFO_RESP_V01 */

  if (profile_info_resp_ptr->rsp_data.profile_info_rsp.iccid_ptr)
  {
    qcril_free(profile_info_resp_ptr->rsp_data.profile_info_rsp.iccid_ptr);
    profile_info_resp_ptr->rsp_data.profile_info_rsp.iccid_ptr = NULL;
    profile_info_resp_ptr->rsp_data.profile_info_rsp.iccid_len = 0;
  }
  if (profile_info_resp_ptr->rsp_data.profile_info_rsp.nickname)
  {
    qcril_free(profile_info_resp_ptr->rsp_data.profile_info_rsp.nickname);
    profile_info_resp_ptr->rsp_data.profile_info_rsp.nickname = NULL;
  }
  if (profile_info_resp_ptr->rsp_data.profile_info_rsp.spn)
  {
    qcril_free(profile_info_resp_ptr->rsp_data.profile_info_rsp.spn);
    profile_info_resp_ptr->rsp_data.profile_info_rsp.spn = NULL;
  }
  if (profile_info_resp_ptr->rsp_data.profile_info_rsp.name)
  {
    qcril_free(profile_info_resp_ptr->rsp_data.profile_info_rsp.name);
    profile_info_resp_ptr->rsp_data.profile_info_rsp.name = NULL;
  }
  if (profile_info_resp_ptr->rsp_data.profile_info_rsp.icon_data_ptr)
  {
    qcril_free(profile_info_resp_ptr->rsp_data.profile_info_rsp.icon_data_ptr);
    profile_info_resp_ptr->rsp_data.profile_info_rsp.icon_data_ptr = NULL;
    profile_info_resp_ptr->rsp_data.profile_info_rsp.icon_len = 0;
  }
...
/*===========================================================================
  ...
  FUNCTION:  qcril_uim_lpa_send_confirmation_code_req

===========================================================================*/
static bool qcril_uim_lpa_send_confirmation_code_req
(
  std::shared_ptr<UimLpaReqMessage>   msg_ptr,
...
{
  int                                             qmi_err_code            = 0;
  qmi_uim_conf_code_req_type                      conf_code_req;
  uint8                                           slot                    = QMI_UIM_MAX_CARD_COUNT;

  memset(&conf_code_req, 0, sizeof(qmi_uim_conf_code_req_type));
  ...
  if (!qcril_uim_lpa_convert_slot_id_to_slot_type(slot, &conf_code_req.slot))
  {
    return FALSE;
  }

  conf_code_req.confirmation_code_ptr = (char *)confirmation_code;

  qmi_err_code = qcril_uim_send_qmi_async_msg(QCRIL_UIM_REQUEST_SEND_CONF_CODE,
                                              &conf_code_req,
                                              msg_ptr);
  ...
  return TRUE;
} /* qcril_uim_lpa_send_confirmation_code_req */
...
)
{
  ...
  switch(resp_ptr->qmi_rsp_data.rsp_id)
  {
    ...
    case QMI_UIM_SRVC_CONF_CODE_RSP_MSG:
      break;
...
)
{
  ...
  switch(ind_param_ptr->ind_data.lpa_add_profile_ind.profile_download_status)
  {
    ...
    case QMI_UIM_PROFILE_USER_CONSENT_REQUIRED:
      ...
      break;
    case QMI_UIM_PROFILE_CONFIRMATION_CODE_REQUIRED:
      add_profile_ind.status = QCRIL_UIM_LPA_ADD_PROFILE_STATUS_GET_USER_CONFIRMATION_CODE;
      break;
    default:
      return;
  }
  ...
  if(ind_param_ptr->ind_data.lpa_add_profile_ind.profile_name != nullptr)
  {
       prof_name_len = strlen(ind_param_ptr->ind_data.lpa_add_profile_ind.profile_name);
       add_profile_ind.profile_name = new char[prof_name_len + 1]{};
       if (add_profile_ind.profile_name != nullptr)
       {
         memcpy(add_profile_ind.profile_name,
                ind_param_ptr->ind_data.lpa_add_profile_ind.profile_name,
                prof_name_len);
       }
  }
...
)
{
  ...
  switch(in_ptr->event)
  {
    ...
    case QCRIL_UIM_LPA_USER_CONSENT:
      result = qcril_uim_lpa_user_consent(req_ptr, in_ptr->userOk , in_ptr->nok_reason);
      break;
    ...
    case QCRIL_UIM_LPA_CONFIRM_CODE:
      result =  qcril_uim_lpa_send_confirmation_code_req(req_ptr,in_ptr->confirmationCode);
      break;
