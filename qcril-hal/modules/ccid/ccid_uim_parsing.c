...
===========================================================================*/
extern const uint8 ccid_uim_slot;

extern uint8 bulkMsg_slot;
/*===========================================================================
...
)
{
  ...
  if (cmd_info_ptr)
  {
    ...
    cmd_info_ptr->slot         = ccid_uim_slot;
    cmd_info_ptr->sequence     = sequence;

    LOGI("Allocate req_msg_type: %d bulk slot: %d qmi slot: %d\n", req_msg_type, slot, ccid_uim_slot);
  }
...
)
{
  ...
  LOGI("ccid_uim_convert_ccid_slot_to_qmi_slot bulk slot: %d qmi slot: %d\n", ccid_slot, ccid_uim_slot);
  switch (ccid_uim_slot)
  {
    case 0x00:
...
)
{
  ...
  LOGI("\n");
  LOGI("P2R - BULK_OUT :: message_type: 0x%x, slot: 0x%x, seq_number: %d, length: %lu\n",
       bulkout_req_ptr->message_type,
       bulkout_req_ptr->slot,
       bulkout_req_ptr->seq_number,
  ...
  bulkout_req_ptr->slot = ccid_uim_slot;
  LOGE("ccid_uim_process_pc_to_rdr_bulk ccid_uim_slot: %d \n", ccid_uim_slot);

  /* Determine the actual request type */
  switch (bulkout_req_ptr->message_type)
  {
    case CCID_UIM_MSG_BULKOUT_ICC_POWERON:
//      bulkMsg_slot = bulkout_req_ptr->slot;
      bulkMsg_slot = 0; //harcode to 0 for now, later change it to get from comp
  ...
  {
    LOGI("\n");
    LOGI("P2R - BULK_IN  :: message_type: 0x%x, slot: 0x%x, seq_number: %d, ",
...
)
{
  ...
  LOGI("ccid_uim_process_rdr_to_pc_bulk for message_type : 0x%x qmi slot: %d  bulk slot: %d\n",
       cmd_info_ptr->req_msg_type, cmd_info_ptr->slot, bulkMsg_slot);

  /* Init response struct with common data  */
  memset(&bulkin_rsp, 0, sizeof(struct ccid_uim_bulk_in_header));
//  bulkin_rsp.slot   = cmd_info_ptr->slot;
  bulkin_rsp.slot   = bulkMsg_slot;
  ...
  LOGI("\n");
  LOGI("R2P -> BULK_IN  :: message_type: 0x%x, slot: 0x%x, seq_number: %d, ",
         bulkin_rsp.message_type,
         bulkin_rsp.slot,
         bulkin_rsp.seq_number);
  LOGI("status: 0x%x, error: 0x%x, length: 0x%x \n",
