...
/* Global to store slot id received in Bulk-OUT messages, to be sent out in Bulk-IN messages */
uint8 bulkMsg_slot = 0;
...
)
{
  ...
  LOGI("process_ind_sap_connection: slot: 0x%x, sap_state: 0x%x, bulkMsg_slot: 0x%x\n",
       sap_conn_ind_ptr->sap_connection_event.slot,
       sap_conn_ind_ptr->sap_connection_event.sap_state, bulkMsg_slot);
...
)
{
  ...
  */
    ...
    qmi_response_ptr->card_status.card_info[ccid_uim_slot].error_code == UIM_CARD_ERROR_CODE_SAP_CONNECTED_V01)
  {
    ...
    LOGI("SAP State CONNECTED for slot: %d \n", ccid_uim_slot);
  }
...
)
{
  ...
  for (i = 0; i<QMI_UIM_CARDS_MAX_V01; i++)
  {
    ...
    {
      LOGI("ccid_uim_cleanup, Issuing SAP disconnect ... slot: %d \n", i);
...
int main (int argc, char **argv)
{
  ...
  if(argc <= 1)
  {
    ...
    LOGE("Running updated ccid_daemon - 04 Apr ! \n");
    LOGE("Slot-id not provided for ccid_daemon ! \n");
    LOGE("ccid_daemon <slot_id> \n");
    LOGE("For e.g  ccid_daemon 0  or  ccid_daemon 1 \n");
    LOGE("Note: Run only 1 instance of ccid_daemon for either of the slot \n");
    exit(0);
  ...
  if (argc > 1)
  {
    if((atoi(argv[1]) == 0) || (atoi(argv[1]) == 1))
    {
      ...
      LOGI("Running ccid_daemon for slot: %d \n", ccid_uim_slot);
    }
    else
    {
      LOGE("Valid slot id values are 0 or 1 only - Entered slot: %d \n", atoi(argv[1]));
      exit(0);
    }
  }
