...
qmi_ril_sms_format_type qcril_qmi_sms_get_ims_sms_format()
{
    ...
    qcril_sms_struct_type* i_ptr = &qcril_sms;
...
)
{
  ...
  if(ril_err != RIL_E_SUCCESS)
  {
    ...
    auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(ril_err, nullptr);
...
)
{
  ...
  if (msg->get_message_id() == RilRequestSendSmsMessage::get_class_message_id())
  {
    ...
    auto respData = std::make_shared<RilSendSmsResult_t>(message_ref, error_code);
    auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(ril_err, respData);
  ...
  else if (msg->get_message_id() == RilRequestImsSendSmsMessage::get_class_message_id())
  {
    ...
    auto respData = std::make_shared<RilSendSmsResult_t>(
             message_ref, error_code, !!qcril_qmi_nas_is_lte_only_registered());
    auto resp = std::make_shared<QcRilRequestMessageCallbackPayload> (ril_err, respData);
...
)
{
  ...
  do
  {
    ...
    if (!i_ptr->sms_ack_info.gw_ack_needed)
    {
      ...
      auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(RIL_E_SUCCESS, nullptr);
      msg->sendResponse(msg, Message::Callback::Status::SUCCESS, resp);
      break;
    }
  ...
  if(res != RIL_E_SUCCESS)
  {
    ...
    auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(res, nullptr);
    msg->sendResponse(msg, Message::Callback::Status::SUCCESS, resp);
  }

  if ( need_reset_ack_expry_tmr )
  {
      if ( QMI_RIL_ZERO != qcril_sms.mt_pending_ack_expry_tmr )
      {
         QCRIL_LOG_INFO( "resetting MT SMS ack tmr per ack reception" );
         qcril_cancel_timed_callback( (void*)(uintptr_t)qcril_sms.mt_pending_ack_expry_tmr );
         qcril_sms.mt_pending_ack_expry_tmr = QMI_RIL_ZERO;
      }
  }
...
)
{
  ...
  auto respData = std::make_shared<RilWriteSmsToSimResult_t>(index_on_sim);
  auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(ril_err, respData);
  msg->sendResponse(msg, Message::Callback::Status::SUCCESS, resp);
...
)
{
  ...
  if (msg->get_message_id() == RilRequestGetSmscAddressMessage::get_class_message_id())
  {
    ...
    auto respData = std::make_shared<RilGetSmscAddrResult_t>(std::move(smsc_address));
    auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(ril_err, respData);
  ...
  do
  {
    if (transp_err == QMI_NO_ERR && bc_config_resp->resp.result == QMI_RESULT_SUCCESS_V01)
    {
      ...
      if (bc_config_resp->wms_3gpp_broadcast_info_valid)
      {
        ...
        {
            auto respData = std::make_shared<RilGetGsmBroadcastConfigResult_t>(std::move(config_list));
            auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(ril_err, respData);
      ...
      else if (bc_config_resp->wms_3gpp2_broadcast_info_valid)
      {
        ...
        {
            auto respData = std::make_shared<RilGetCdmaBroadcastConfigResult_t>(std::move(config_list));
            auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(RIL_E_SUCCESS, respData);
...
)
{
  ...
  auto respData = std::make_shared<RilGetImsRegistrationResult_t>();
  if (respData && (res == RIL_E_SUCCESS))
  {
      respData->isRegistered = !!ims_sms_state.is_ims_wms_reg;
      respData->ratFamily = (ims_sms_state.format == QMI_RIL_SMS_FORMAT_3GPP2) ?
            RADIO_TECH_3GPP2 : RADIO_TECH_3GPP;
  }
  auto resp = std::make_shared<QcRilRequestMessageCallbackPayload>(res, respData);
  msg->sendResponse(msg, Message::Callback::Status::SUCCESS, resp);
...
(
  const wms_transfer_route_mt_message_type_v01 * mt_sms,
  uint8_t& message_ref
)
{
    ...
    if (is_processing_ok)
    {
      ...
      if ( qmi_ril_sms_mt_expiry_window_len_set )
      {
        ...
        qcril_setup_timed_callback(   QCRIL_DEFAULT_INSTANCE_ID,
                                      ...
                                      &qcril_sms.mt_pending_ack_expry_tmr );
      }
...
void qcril_sms_mt_transaction_expired(void * param)
{
    ...
    QCRIL_MUTEX_LOCK( &qcril_sms.sms_ack_info_mutex, "sms_ack_info_mutex" );

    QCRIL_LOG_INFO( "current CDMA ack pending %d, needed %d",
                    qcril_sms.sms_ack_info.cdma_ack_pending,
                    qcril_sms.sms_ack_info.cdma_ack_needed  );

    QCRIL_LOG_INFO( "current GW ack pending %d, needed %d",
                    qcril_sms.sms_ack_info.gw_ack_pending,
                    qcril_sms.sms_ack_info.gw_ack_needed  );

    qcril_sms.sms_ack_info.cdma_ack_pending     = FALSE;
    qcril_sms.sms_ack_info.cdma_ack_needed      = FALSE;
    qcril_sms.sms_ack_info.cdma_send_ack_on_ims = FALSE;

    qcril_sms.sms_ack_info.gw_ack_pending     = FALSE;
    qcril_sms.sms_ack_info.gw_ack_needed      = FALSE;
    qcril_sms.sms_ack_info.gw_send_ack_on_ims = FALSE;

    qcril_sms.mt_pending_ack_expry_tmr     = QMI_RIL_ZERO;

    QCRIL_MUTEX_UNLOCK( &qcril_sms.sms_ack_info_mutex, "sms_ack_info_mutex" );
...
void qcril_qmi_sms_reset_mt_sms_ack_needed_power_opt_buffer()
{
    ...
    qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_needed.clear();
    qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_needed.shrink_to_fit();
    if ( QMI_RIL_ZERO != qcril_mt_sms_buffer_power_opt.mt_sms_ack_needed_expry_tmr )
    {
        qcril_cancel_timed_callback(
            (void*)(uintptr_t)qcril_mt_sms_buffer_power_opt.mt_sms_ack_needed_expry_tmr);
...
void qcril_qmi_sms_reset_mt_sms_ack_not_needed_power_opt_buffer()
{
    ...
    qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_not_needed.clear();
    qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_not_needed.shrink_to_fit();
}

void qcril_qmi_sms_update_mt_sms_with_ack_needed_power_opt_buffer(
    wms_event_report_ind_msg_v01& ind_data_ptr)
{
    QCRIL_LOG_FUNC_ENTRY();
    qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_needed.push_back(ind_data_ptr);
    QCRIL_LOG_INFO("MT SMS ACK NEEDED Power Opt buffer length %d",
                   qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_needed.size());
    QCRIL_LOG_FUNC_RETURN();
}
...
void qcril_qmi_sms_notify_mt_sms_with_ack_needed_power_opt_to_atel()
{
    QCRIL_LOG_INFO("MT SMS ACK NEEDED Power Opt buffer length %d",
                   qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_needed.size());
    if(qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_needed.size())
    {
        for(const auto &ind : qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_needed)
        {
            qcril_sms_process_event_report_ind(&ind, QCRIL_DEFAULT_INSTANCE_ID);
        }
        qcril_qmi_sms_reset_mt_sms_ack_needed_power_opt_buffer();
    }
}

void qcril_qmi_sms_update_mt_sms_with_ack_not_needed_power_opt_buffer(
    wms_event_report_ind_msg_v01& ind_data_ptr)
{
    QCRIL_LOG_FUNC_ENTRY();
    qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_not_needed.push_back(ind_data_ptr);
    QCRIL_LOG_INFO("MT SMS ACK NOT NEEDED Power Opt buffer length %d",
                   qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_not_needed.size());
    QCRIL_LOG_FUNC_RETURN();
}
...
void qcril_qmi_sms_notify_mt_sms_with_ack_not_needed_power_opt_to_atel()
{
    QCRIL_LOG_INFO("MT SMS ACK NOT NEEDED Power Opt buffer length %d",
                   qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_not_needed.size());
    if(qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_not_needed.size())
    {
        for(const auto &ind : qcril_mt_sms_buffer_power_opt.mt_sms_with_ack_not_needed)
        {
            qcril_sms_process_event_report_ind(&ind, QCRIL_DEFAULT_INSTANCE_ID);
        }
        qcril_qmi_sms_reset_mt_sms_ack_not_needed_power_opt_buffer();
