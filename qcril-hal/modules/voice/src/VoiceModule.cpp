...
VoiceModule::VoiceModule(string name) : AddPendingMessageList(name) {
  ...
  */
  ...
  mMessageHandler = {
      ...
      HANDLER(IpcAudioCallStateMessage, VoiceModule::handleIpcAudioCallStateMessage),
...
void VoiceModule::initFeatureMask()
{
  ...
  qmi_ril_get_property_value_from_boolean(QCRIL_QMI_VOICE_REPORT_SPEECH_CODEC, &enabled, FALSE);
  setFeature(VoiceFeatureType::REPORT_SPEECH_CODEC, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QMI_RIL_SYS_PROP_NAME_SUBADDRESS, &enabled, FALSE);
  setFeature(VoiceFeatureType::SUBADDRESS_SUPPORT, !!enabled);

  // Default value is TRUE
  enabled = TRUE;
  qmi_ril_get_property_value_from_boolean(QMI_RIL_SYS_PROP_NAME_SUBADDRESS_AMPERSAND, &enabled, TRUE);
  setFeature(VoiceFeatureType::SUBADDRESS_SUPPORT_AMP, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QCRIL_REJECT_CAUSE_21_SUPPORTED, &enabled, FALSE);
  setFeature(VoiceFeatureType::REJECT_CAUSE_21_SUPPORTED, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QMI_RIL_SYS_PROP_NAME_SUBADDRESS_IA5_IDENTIFIER, &enabled, FALSE);
  setFeature(VoiceFeatureType::SUBADDRESS_IA5_ID_SUPPORT, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QMI_RIL_SYS_PROP_NAME_REDIR_PARTY_NUM, &enabled, FALSE);
  setFeature(VoiceFeatureType::REDIR_PARTY_NUM_SUPPORT, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QCRIL_QMI_VOICE_CLIR_FACNOTSUP_AS_NONW, &enabled, FALSE);
  setFeature(VoiceFeatureType::FAC_NOT_SUPP_AS_NO_NW, !!enabled);

  // Default value is TRUE
  enabled = TRUE;
  qmi_ril_get_property_value_from_boolean(QCRIL_QMI_SUPPORT_ENCRYPTED_CALLS, &enabled, TRUE);
  setFeature(VoiceFeatureType::ENCRYPTED_CALLS_SUPPORT, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QCRIL_QMI_VOICE_FAKE_AUDIO_UP, &enabled, FALSE);
  ...
  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QCRIL_PROCESS_SUPS_IND, &enabled, FALSE);
  setFeature(VoiceFeatureType::PROCESS_SUPS_IND, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QCRIL_INFORCE_0X9E_TREAT_AS_TAG, &enabled, FALSE);
  setFeature(VoiceFeatureType::IS_0x9E_NOT_TREAT_AS_NAME, !!enabled);

  enabled = FALSE;
  qmi_ril_get_property_value_from_boolean(QCRIL_PROCESS_ORIGINAL_NUMBER_DISPLAY, &enabled, FALSE);
...
void VoiceModule::handleQcrilInit(std::shared_ptr<QcrilInitMessage> msg) {
  ...
  Messenger::get().registerForMessage(
      IpcAudioCallStateMessage::get_class_message_id(),
      [](IPCIStream &is) -> std::shared_ptr<IPCMessage> {
        auto msg = std::make_shared<IpcAudioCallStateMessage>();
        if (msg) {
          msg->deserialize(is);
        }
        return msg;
...
void VoiceModule::handleIpcAudioCallStateMessage(std::shared_ptr<IpcAudioCallStateMessage> msg) {
  Log::getInstance().d("[" + mName + "]: Handling msg = " + msg->dump());
  // Handle only the message from the other RILD
  if (msg->getIsRemote()) {
    qcril_am_inter_rild_msg_type ipcData = msg->getIpcData();
    qcril_am_handle_event(QCRIL_AM_EVENT_INTER_RIL_CALL_STATE,
                          (qcril_am_call_state_type *)&ipcData);
  }
}
      ...
      if (clipInfo) {
        if (qmiResp->clip_response_valid) {
          clipInfo->setClipStatus(
              mapProvisionStatusToClipStatus(qmiResp->clip_response.provision_status));
        }
