/******************************************************************************
#  Copyright (c) 2012-2013, 2017-2020 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
#include <interfaces/ims/QcRilUnsolImsCallComposerInfo.h>
...
{
    QCRIL_LOG_FUNC_ENTRY();
    if (!call_status_ind_ptr) return;

    qcril::interfaces::CallComposerInfo info{};
    info.setId(call_id);
    if (call_status_ind_ptr->call_composer_importance_valid)
    {
        for (int i = 0; i < call_status_ind_ptr->call_composer_importance_len;
                i++)
        {
    ...
    auto msg = std::make_shared<QcRilUnsolImsCallComposerInfo>(info);
    Dispatcher::getInstance().dispatchSync(msg);
}
...
)
{
  ...
  if( ind_data_ptr != NULL )
  {
      ...
      for (i = 0; (i < call_status_ind_ptr->call_info_len && i < QMI_VOICE_CALL_INFO_MAX_V02); i++)
      {
        ...
        if (iter_call_info->call_state == CALL_STATE_PRE_ALERTING_V02)
        {
            QCRIL_LOG_ESSENTIAL("call state PRE_ALERTING for conn id %d", iter_call_info->call_id);
            // This ind with new state is for Call Composer Info. It comes before INCOMING and
            // contains call_id. Since it goes through a different interface to notify client,
            // and it is OK for ATEL that it comes with call_id before RING, we will not create
            // call_info_entry here and cache the info, to minimize the impact on current flow.
            qcril_qmi_voice_all_call_status_ind_pre_alerting_hdlr(
