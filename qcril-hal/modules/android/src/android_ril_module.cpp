...
bool android_ril_module::dispatch_request(qcril_instance_id_e_type instance_id, int req_id, void* data, size_t size, RIL_Token t)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (qcril_legacy_functions[instance_id] &&
                qcril_legacy_functions[instance_id]->onRequest &&
                req_id == RIL_REQUEST_OEM_HOOK_RAW)
            {
              ...
              QCRIL_LOG_ERROR(
                  "Qcril legacy onRequest() is not available/qcril_legacy_functions is null for "
                  "instance_id:%d",
                  instance_id);
              {
                std::lock_guard<std::recursive_mutex> lock(outstandingReqsLock);
