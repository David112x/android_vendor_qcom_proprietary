...
AndroidImsRadioModule::AndroidImsRadioModule() {
  ...
  mMessageHandler = {
      ...
      HANDLER(QcRilUnsolImsGeoLocationDataStatus,
              AndroidImsRadioModule::handleQcRilUnsolImsGeoLocationDataStatus),
      HANDLER(QcRilUnsolImsCallComposerInfo,
              AndroidImsRadioModule::handleQcRilUnsolOnCallComposerInfoMessage),
  };
}
...
void AndroidImsRadioModule::handleQcRilUnsolImsGeoLocationDataStatus(
...
void AndroidImsRadioModule::handleQcRilUnsolOnCallComposerInfoMessage(
    std::shared_ptr<QcRilUnsolImsCallComposerInfo> msg) {
  Log::getInstance().d("[" + mName + "]: Handling msg = " + msg->dump());
  if (mImsRadio != nullptr) {
    mImsRadio->notifyOnCallComposerInfoAvailable(msg);
  }
}
