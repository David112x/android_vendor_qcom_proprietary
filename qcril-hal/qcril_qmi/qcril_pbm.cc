...
struct pbm_cache_info_type {
    ...
    std::vector<int> pbm_session_type;
    std::vector<qcril::interfaces::qcril_pbm_adn_count_info> pbm_capacity;

    //Store reg mask info
    pbm_reg_mask_type_v01 pbm_reg_mask;

    // Store alternate emergency number details
    std::shared_ptr<qcril_pbm_alt_emer_num_info> alt_emer_num_info;

    // PBM centralized ecc support
    uint8_t pbm_centralized_ecc_supported;
...
)
{
  QCRIL_LOG_FUNC_ENTRY();
  PBM_CACHE_LOCK();
  pbm_cache_info.pbm_session_type.clear();
  pbm_cache_info.pbm_session_type.shrink_to_fit();
  pbm_cache_info.pbm_capacity.clear();
  pbm_cache_info.pbm_capacity.shrink_to_fit();
  memset(&pbm_cache_info, 0, sizeof(pbm_cache_info));
  PBM_CACHE_UNLOCK();
  QCRIL_LOG_FUNC_RETURN();
}
...
uint8_t qcril_qmi_pbm_centralized_ecc_support_available()
{
    uint8_t res = FALSE;
    PBM_CACHE_LOCK();
...
RIL_Errno qcril_qmi_pbm_enable_emergency_number_indications(uint8_t enable)
{
...
//===========================================================================
uint8_t qmi_ril_phone_number_is_emergency(const char * number_to_check)
{
  uint8_t res = FALSE;
...
//===========================================================================
uint8_t qmi_ril_phone_number_is_non_std_otasp(const char * number_to_check)
{
  return strncmp(number_to_check, NON_STD_OTASP_NUMBER, strlen(NON_STD_OTASP_NUMBER) + 1) ? FALSE
                                                                                          : TRUE;
} // qmi_ril_phone_number_is_non_std_otasp
...
//===========================================================================
uint8_t qmi_ril_number_received_from_pbm(const char * number)
{
  uint8_t res = FALSE;
  if (number == nullptr) {
...
RIL_Errno qmi_ril_send_ecc_list_indication()
{
    ...
    PBM_CACHE_LOCK();
    for (const auto &elem : pbm_cache_info.pbm_ecc_map) {
    ...
    for (const auto &elem : ril_ecc_map) {
    ...
    PBM_CACHE_LOCK();
    // Add alt_emer_num
    if (pbm_cache_info.alt_emer_num_info)
    {
    ...
    for (const auto &val : final_ecc_map) {
...
RIL_Errno qcril_qmi_pbm_enable_phonebook_ready_indications(uint8_t enable)
{
    RIL_Errno ril_req_res = RIL_E_GENERIC_FAILURE;
    qmi_client_error_type qmi_transport_error;
...
RIL_Errno qcril_qmi_pbm_get_records_count(void)
{
    ...
    if ( RIL_E_SUCCESS == result )
    {
        ...
        {
          PBM_CACHE_LOCK();
          pbm_cache_info.pbm_session_type.reserve(pbm_info_resp.pb_set_info_array_len);
          pbm_cache_info.pbm_capacity.reserve(pbm_info_resp.pb_set_info_array_len);
          PBM_CACHE_UNLOCK();
          ...
          {
            PBM_CACHE_LOCK();
            pbm_cache_info.pbm_session_type.push_back(session_type);
            qcril::interfaces::qcril_pbm_adn_count_info capacity_item;
            capacity_item.max_adn_num= max_records;
            capacity_item.valid_adn_num= used_records;
            capacity_item.max_email_num= max_email;
            capacity_item.valid_email_num= used_email;
            capacity_item.max_ad_num= max_ad_num;
            capacity_item.valid_ad_num= used_ad_num;
            capacity_item.max_name_len= max_name_len;
            capacity_item.max_number_len= max_number_len;
            capacity_item.max_email_len= max_email_len;
            capacity_item.max_anr_len= max_anr_len;
            pbm_cache_info.pbm_capacity.push_back(capacity_item);
            pbm_cache_info.pbm_status = QCRIL_PBM_STATUS_GET_CAPACITY_COMPLETE;
            PBM_CACHE_UNLOCK();
          }
        }
...
)
{
    ...
    for ( i = 0; i < pbm_cache_info.pbm_capacity.size(); i++ )
    {
...
RIL_Errno qcril_qmi_pbm_get_all_of_records(int adn_count, int session_type)
{
    ...
    if ( RIL_E_SUCCESS == ril_req_res && pbm_read_resp.num_of_recs_valid )
    {
        ...
        else
        {
            ...
            for ( index = 0; index < pbm_cache_info.pbm_capacity.size(); index++ )
            {
...
)
{
    ...
    if ( ind_data_ptr != NULL )
    {
        ...
        for ( element_index = 0; element_index < pbm_cache_info.pbm_capacity.size(); element_index++ )
        {
...
)
{
    ...
    if(ret.second)
    {
        ...
        if(index < pbm_cache_info.pbm_capacity.size()) {
...
)
{
  ...
  else
  {
    ...
    //support multi-session
    for ( j = 0; j < pbm_cache_info.pbm_capacity.size(); j++ )
    {
...
void qmi_ril_set_alternate_emergency_call_info(const char *number, uint32_t emer_cat)
{
  QCRIL_LOG_FUNC_ENTRY();
  if (number && strlen(number) > 0)
  {
    PBM_CACHE_LOCK();
    pbm_cache_info.alt_emer_num_info = std::make_shared<qcril_pbm_alt_emer_num_info>();
    pbm_cache_info.alt_emer_num_info->number = number;
...
void qmi_ril_reset_alternate_emergency_call_info()
{
  ...
  {
    PBM_CACHE_LOCK();
    pbm_cache_info.alt_emer_num_info = nullptr;
    PBM_CACHE_UNLOCK();
#ifndef QMI_RIL_UTF
    qcril_event_queue(QCRIL_DEFAULT_INSTANCE_ID,
                      QCRIL_DEFAULT_MODEM_ID,
...
boolean qmi_ril_is_alternate_emergency_call(const char *number)
{
  boolean result = FALSE;
  QCRIL_LOG_FUNC_ENTRY();
  PBM_CACHE_LOCK();
  if (pbm_cache_info.alt_emer_num_info && number)
  {
    if (pbm_cache_info.alt_emer_num_info->number == number)
    {
      result = TRUE;
    }
  }
