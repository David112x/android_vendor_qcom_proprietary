...
ssize_t QtiBusSocketTransportServer::receiveMessage(int fd, char *recv_buf, size_t recv_buf_size) {
  ...
  do {
    recv_len = recv(fd, message_size_buf_ptr, bytes_remaining, 0);
    if (recv_len < 1) {
      message_size = recv_len;
      break;
    }
  ...
  if (recv_len > 0) {
    ...
    // Copy size to recv_buf
    memcpy(recv_buf, message_size_buf, sizeof(message_size_buf));
    recv_buf += sizeof(message_size_buf);

    // Reduce the 'overall_size' length
    bytes_remaining = message_size - sizeof(size_t);
    do {
      recv_len = recv(fd, recv_buf, bytes_remaining, 0);
      if (recv_len < 1) {
        // Receive failure;
        message_size = recv_len;
        break;
    ...
    for (int i = 0; i < ready_cnt; i++) {
      if (events[i].events & EPOLLIN) {
        ...
        if (events[i].data.fd == sock_fd) {
          ...
          recv_buf_len = receiveMessage(events[i].data.fd, recv_buf, sizeof(recv_buf));
