...
int32_t IsAcdbDeltaFileValid(const uint8_t *pFileBuf,const uint32_t nFileSize)
{
   ...
   if(!((fileMajor == 0 && fileMinor == 0) || (fileMajor == 0 && fileMinor == 1)|| (fileMajor == 1 && fileMinor == 0))) // could only parse (0 , 0), (0,1) and (1,0) file versions, add additional versions here in future.
   {
      return ACDB_PARSE_INVALID_FILE;
   }
   if(fileMajor == 0 && fileMinor == 0) // maj, min, rev
   {
      offset += 3 * (uint32_t)sizeof(uint32_t);
   }
   else
   {
      offset += 4 * (uint32_t)sizeof(uint32_t); // maj, min, rev and cplInfo.
   }
...
int32_t AcdbDeltaFileGetSWVersion(const uint8_t *pFileBuf,const uint32_t nFileSize,uint32_t *pMajVer,uint32_t *pMinVer,uint32_t *pRevVer, uint32_t *pVerCplInfo)
{
   ...
   else if((fileMajor == 0 && fileMinor == 1) || (fileMajor == 1 && fileMinor == 0))
   {
