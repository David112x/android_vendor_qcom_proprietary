...
int32_t AcdbDataGetDataPtr(AcdbCmdFileInfo fileInfo,uint32_t dataoffset,AcdbDataInfo *pDataInfo)
{
	...
	if((dataoffset + nDataLen + (uint32_t)(sizeof(nDataLen)) > nChkBufSize))
	{
...
int32_t AcdbDataGetDevPropData(AcdbDevPropInfo *dpInfo)
{
	...
	AcdbDevPropLut devPropLut = { 0, NULL };
	uint8_t *pChkBuf = NULL;
	uint32_t nChkBufSize=0;
	uint32_t i = 0,idx=0;
	if(dpInfo == NULL)
...
{
	int32_t result = ACDB_ERROR;
	AcdbGlbPropLut glbPropLut = { 0, NULL };
	uint8_t *pChkBuf = NULL;
	uint32_t nChkBufSize=0;
	uint32_t i = 0,idx=0;
	if(glbInfo == NULL)
...
int32_t AcdbDataGetGlobalTblsInfo(AcdbTableCmd *tblCmd,AcdbTableInfo *tblInfo)
{
	...
	case AUDSTRM_INST_TBL_ID:
		lutId=ACDB_CHUNKID_AUDSTREAMINSTCLUTCHUNKID;
		cdefId=ACDB_CHUNKID_AUDSTREAMINSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_AUDSTREAMINSTCDOTCHUNKID;
		break;
	case VOCSTRM_INST_TBL_ID:
		lutId=ACDB_CHUNKID_VOCSTREAM2INSTCLUTCHUNKID;
		cdefId=ACDB_CHUNKID_VOCSTREAM2INSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_VOCSTREAM2INSTCDOTCHUNKID;
		break;
	default:
		tblFound = 0;
	}
			...
			else
			{
				if(lutId)
				{
					if(ACDB_PARSE_SUCCESS != AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,
						gDataFileInfo.fInfo[i].nFileSize,lutId,(uint8_t**)&tblInfo->tblLutChnk.pData,
						(uint32_t *)&tblInfo->tblLutChnk.nDataLen))
					{
				...
				{
					if(ACDB_PARSE_SUCCESS != AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,
						gDataFileInfo.fInfo[i].nFileSize,cdefId,(uint8_t**)&tblInfo->tblCdftChnk.pData,
						(uint32_t *)&tblInfo->tblCdftChnk.nDataLen))
					{
				...
				{
					if(ACDB_PARSE_SUCCESS != AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,
						gDataFileInfo.fInfo[i].nFileSize,cdotId,(uint8_t**)&tblInfo->tblCdotChnk.pData,
						(uint32_t *)&tblInfo->tblCdotChnk.nDataLen))
					{
				...
				{
					if(ACDB_PARSE_SUCCESS != AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,
						gDataFileInfo.fInfo[i].nFileSize,dataPoolId,(uint8_t**)&tblInfo->dataPoolChnk.pData,
						(uint32_t *)&tblInfo->dataPoolChnk.nDataLen))
					{
...
int32_t AcdbDataGetAVTblsInfoCVD(AcdbTableCmd *tblCmd,AcdbTableInfoCVD *tblInfo)
{
	...
	case VOCPROC_DYN_INST_TBL_ID:
		lutId=ACDB_CHUNKID_VOCPROCDYNINSTCLUTCHUNKID;
		cdefId=ACDB_CHUNKID_VOCPROCDYNINSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_VOCPROCDYNINSTCDOTCHUNKID;
		lutCVDId=ACDB_CHUNKID_VOCPROCDYNINSTCVDCHUNKID;
		lutOffsetId=ACDB_CHUNKID_VOCPROCDYNINSTOFSTCHUNKID;
		break;
	case VOCPROC_STAT_INST_TBL_ID:
		lutId=ACDB_CHUNKID_VOCPROCSTATINSTCLUTCHUNKID;
		cdefId=ACDB_CHUNKID_VOCPROCSTATINSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_VOCPROCSTATINSTCDOTCHUNKID;
		lutCVDId=ACDB_CHUNKID_VOCPROCSTATINSTCVDCHUNKID;
		lutOffsetId=ACDB_CHUNKID_VOCPROCSTATINSTOFSTCHUNKID;
		break;
	default:
		tblFound = 0;
	}
	...
	if(tblFound == 1)
	{
		...
		for(i=0;i<gDataFileInfo.noOfFiles;i++)
		{
			...
			else
			{
				...
				{
					if(ACDB_PARSE_SUCCESS != AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,
						gDataFileInfo.fInfo[i].nFileSize,lutCVDId,(uint8_t**)&tblInfo->tblLutCVDChnk.pData,
						(uint32_t *)&tblInfo->tblLutCVDChnk.nDataLen))
					{
						ACDB_DEBUG_LOG("Invalid AV file. It doesnt contain LUT for tblid %08X \n",tblCmd->tblId);
						return ACDB_ERROR;
					}
				...
				{
					if(ACDB_PARSE_SUCCESS != AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,
						gDataFileInfo.fInfo[i].nFileSize,lutOffsetId,(uint8_t**)&tblInfo->tblLutOffsetChnk.pData,
						(uint32_t *)&tblInfo->tblLutOffsetChnk.nDataLen))
					{
						ACDB_DEBUG_LOG("Invalid AV file. It doesnt contain LUT for tblid %08X \n",tblCmd->tblId);
						return ACDB_ERROR;
					}
...
int32_t AcdbDataGetAVTblsInfo(AcdbTableCmd *tblCmd,AcdbTableInfo *tblInfo)
{
	...
	case LSM_INST_TBL_ID:
		lutId=ACDB_CHUNKID_LSMINSTCLUTCHUNKID;
		cdefId=ACDB_CHUNKID_LSMINSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_LSMINSTCDOTCHUNKID;
		break;
	case AFECMN_INST_TBL_ID:
		lutId=ACDB_CHUNKID_AFECMNINSTCLUTCHUNKID;
		cdefId=ACDB_CHUNKID_AFECMNINSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_AFECMNINSTCDOTCHUNKID;
		break;
	case AUDPROC_INST_TBL_ID:
		lutId=ACDB_CHUNKID_AUDPROCINSTLUTCHUNKID;
		cdefId=ACDB_CHUNKID_AUDPROCINSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_AUDPROCINSTCDOTCHUNKID;
		break;
	case AUDPROCVOL_INST_TBL_ID:
		lutId=ACDB_CHUNKID_AUDVOLINSTCLUTCHUNKID;
		cdefId=ACDB_CHUNKID_AUDVOLINSTCDFTCHUNKID;
		cdotId=ACDB_CHUNKID_AUDVOLINSTCDOTCHUNKID;
		break;
	case CODEC_COMMON_TBL:
		lutId=ACDB_CHUNKID_CODECCOMMONLUT;
		cdefId=ACDB_CHUNKID_CODECCOMMONCDFT;
		cdotId=ACDB_CHUNKID_CODECCOMMONCDOT;
		break;
	case CODEC_GAIN_TBL:
		lutId=ACDB_CHUNKID_CODECGAINDEPLUT;
		cdefId=ACDB_CHUNKID_CODECGAINDEPCDFT;
		cdotId=ACDB_CHUNKID_CODECGAINDEPCDOT;
		break;
	default:
		tblFound = 0;
	}
				...
				if(usecaseID == dpInfo->usecaseId)
				{
					...
					for(idx = 0; idx < sgtCount ;idx ++)
					{
						...
						if(subgraphID == dpInfo->subgraphId)
						{
							...
							dpInfo->dataInfo.nDataLen = (midpidCount * 2 * (uint32_t)sizeof(uint32_t))+(uint32_t)sizeof(uint32_t);
							subgraphFound = 1;
							break;
						}
						else
						{
							midpidCount = READ_UInt32(buffer + offset);
							offset += (uint32_t)sizeof(uint32_t);
							offset += (uint32_t)(midpidCount*sizeof(uint32_t) * 2); //Payload contains array of MID and IID of length midpidCount
						}
					}
					if(subgraphFound == 1)
						break;
...
{
	int32_t result = ACDB_ERROR;
	AcdbGlbPropLut glblPropLut = { 0, NULL };
	uint8_t *pChkBuf = NULL;
	uint32_t nChkBufSize=0;
	uint32_t i = 0,idx=0;
	if(dpInfo == NULL)
	...
	case ACDBDATACMD_GET_LOADED_FILES_INFO:
		{
			...
			else
			{
				...
				int32_t offset =0;
				//<no of files:4 bytes>
				nBytesRequired += (uint32_t)sizeof(gDataFileInfo.noOfFiles);

				for(i=0;i<gDataFileInfo.noOfFiles;i++)
				{
					//<file len:4 bytes>
					nBytesRequired += (uint32_t)sizeof(gDataFileInfo.fInfo[i].nFileSize);
					//<file name len:4 bytes>
					nBytesRequired += (uint32_t)sizeof(gDataFileInfo.fInfo[i].nFileNameLen);
					//<file data: file name len bytes>
				...
				offset+=(int32_t)sizeof(gDataFileInfo.noOfFiles);
				for(i=0;i<gDataFileInfo.noOfFiles;i++)
				{
					//copy <file len>
					...
					offset += (int32_t)sizeof(uint32_t);
					//copy <file name len>
					...
					offset += (int32_t)sizeof(gDataFileInfo.fInfo[i].nFileNameLen);
					//copy <file data>
