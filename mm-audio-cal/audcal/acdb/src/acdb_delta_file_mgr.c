...
int32_t AcdbDeltaDataCmdGetDataCountForFile(uint32_t fileIndex, uint32_t *delta_data_count)
{
	...
	if(gDeltaDataFileInfo.fInfo[idx].deltaFileExists == TRUE && gDeltaDataFileInfo.fInfo[idx].pFileBuf != NULL)
	{
		...
		if(gDeltaDataFileInfo.fInfo[idx].nFileSize > sizeof(AcdbDeltaFileHeader) + sizeof(AcdbDeltaFileHeaderInfoV1) + sizeof(uint32_t))
		{
			...
			offset += (uint32_t)sizeof(uint32_t); // curDeltafilemajor
			ACDB_MEM_CPY(&curDeltafileminor,sizeof(uint32_t),gDeltaDataFileInfo.fInfo[idx].pFileBuf + offset,sizeof(uint32_t));
			offset += (uint32_t)sizeof(uint32_t); // curDeltafileminor
			if(curDeltafilemajor==0 && curDeltafileminor == 0)
			{
				offset += (uint32_t)sizeof(AcdbDeltaFileHeaderInfoV1);
			}
			else
			{
				offset += (uint32_t)sizeof(AcdbDeltaFileHeaderInfoV2);
			}
			offset += (uint32_t)sizeof(uint32_t); // data_len
	...
	if(gDeltaDataFileInfo.fInfo[idx].deltaFileExists == TRUE && gDeltaDataFileInfo.fInfo[idx].pFileBuf != NULL)
	{
		...
		if(gDeltaDataFileInfo.fInfo[idx].nFileSize > ( sizeof(AcdbDeltaFileHeader)  + sizeof(AcdbDeltaFileHeaderInfoV1) + sizeof(uint32_t) + sizeof(uint32_t)))
		{
			...
			*/
			...
			offset += (uint32_t)sizeof(uint32_t); // filemajor
			ACDB_MEM_CPY(&fileminor,sizeof(uint32_t),gDeltaDataFileInfo.fInfo[idx].pFileBuf + offset,sizeof(uint32_t));
			offset += (uint32_t)sizeof(uint32_t); // fileminor
			if(filemajor == 0 && fileminor == 0)
			{
				offset +=(uint32_t) sizeof(AcdbDeltaFileHeaderInfoV1);  // major, minor and revision.
			}
			else
			{
				offset += (uint32_t)sizeof(AcdbDeltaFileHeaderInfoV2);   // major, minor, revision and cplInfo.
			}
...
int32_t AcdbDeltaDataCmdSaveData(void)
{
	int32_t result = ACDB_SUCCESS;
	...
	int32_t offset = 0;
	AcdbDeltaFileHeader deltaHeaderInfo = { 0, 0 };
	AcdbDeltaFileHeaderInfoV2 deltaFileHeaderInfoV2 = { 0, 0, 0, 0 };
	AcdbCmdFileNameInfo fileNameInfo = { 0, "" };
	// expected format
	...
	for(idx=0;idx<gDeltaDataFileInfo.noOfFiles;idx++)
	{
		...
		if(newFileBuf != NULL)
		{
			...
			offset = 0;
				...
				offset += (int32_t)sizeof(deltaHeaderInfo);
				...
				offset += (int32_t)sizeof(deltaFileHeaderInfoV2);
...
int32_t AcdbDeltaDataCmdDeleteFiles(void)
{
...
int32_t AcdbDeltaDataFreeBuf(void)
{
	int32_t result = ACDB_SUCCESS;
	uint32_t idx =0;
	...
	case ACDBDELTADATACMD_GET_DELTA_ACDB_FILE_INDEX:
		{
			...
			{
				ACDB_MEM_CPY(pOutput,sizeof(uint32_t),&delta_file_index,sizeof(int32_t));
				result = ACDB_SUCCESS;
			}
		}
		break;
	...
	case ACDBDELTADATCMD_GET_DELTA_FILE_VERSION:
		{
			...
			uint32_t *pDeltaFileIndex = NULL;
			AcdbDeltaFileVersion *deltaVersion = NULL;
			if(pInput == NULL || nInputSize != sizeof(uint32_t) || pOutput == NULL || nOutputSize != sizeof(AcdbDeltaFileVersion) )
			{
			...
			{
				deltaVersion = (AcdbDeltaFileVersion *)(ACDB_MALLOC(sizeof(AcdbDeltaFileVersion)));
				...
				if(result != ACDB_SUCCESS)
				{
					ACDB_DEBUG_LOG("ACDB_DELTAFILE_MGR: call to AcdbDeltaDataCmdGetDeltaVersion() failed\n");
					return result;
				}
