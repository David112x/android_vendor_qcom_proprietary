...
int32_t AcdbFileGetChunkData(const uint8_t *pFileBuf,const uint32_t nFileBufLen,uint64_t chunkID,uint8_t **pChkBuf,uint32_t *pChkLen)
{
	...
	offset += (uint32_t)sizeof(AcdbFileProperties);
	while(offset < nFileBufLen && ((nFileBufLen-offset)>= (uint32_t)sizeof(AcdbChunkHeader)))
	{
		AcdbChunkHeader ChkHdr = { 0, 0 };
...
{
	AcdbFileProperties FileProp = { 0, 0, 0, 0, 0, 0, 0};
