   ...
   )
{
   ...
   AcdbQueryNoOfTblEntriesCmdType cmd = { 0 };
   ...
   )
{
   ...
   AcdbQueryTblEntriesCmdType cmd = { 0, 0, NULL, 0 };
   AcdbQueryResponseType rsp = { 0 };
   if (NULL == resp_buf_ptr)
   {
      /**not initilized*/
...
{
   //uint32_t acph_buf_len = ACPH_BUFFER_LENGTH - ACPH_ACDB_BUFFER_POSITION;
	AcdbCmdGetFileDataReq req = { 0, 0, 0, NULL };
   AcdbCmdResp resp = { 0, 0, NULL };
   uint32_t offset = 0;
   int32_t result = ACPH_SUCCESS;
   UNUSED(req_buf_len);
   if (NULL == resp_buf_ptr)
   ...
   )
{
   ...
   {
      expectedBufSize = ((uint32_t)sizeof(tblId) + (noOfTableIndices*(uint32_t)sizeof(uint32_t)) + (uint32_t)sizeof(mid) + (uint32_t)sizeof(iid) + (uint32_t)sizeof(pid));
   }
   else
   {
      expectedBufSize = ((uint32_t)sizeof(tblId) + (noOfTableIndices*(uint32_t)sizeof(uint32_t)) + (uint32_t)sizeof(mid) + (uint32_t)sizeof(pid));
   }
   ...
   offset += (noOfTableIndices*(uint32_t)sizeof(uint32_t));
   if (nonModuleTblFound == 0)
   {
   ...
   )
{
   ...
   if (nonModuleTblFound == 0)
   {
      if (noOfCdftIndices == INSTANCE_CDFT_INDICES_COUNT)
      {
         nInDataBufLen = req_buf_len - ((noOfTableIndices*(uint32_t)sizeof(uint32_t)) + (uint32_t)sizeof(tblId) + (uint32_t)sizeof(mid) + (uint32_t)sizeof(iid) + (uint32_t)sizeof(pid));
      }
      else
      {
         nInDataBufLen = req_buf_len - ((noOfTableIndices*(uint32_t)sizeof(uint32_t)) + (uint32_t)sizeof(tblId) + (uint32_t)sizeof(mid) + (uint32_t)sizeof(pid));
      }
   }
   else
   {
      nInDataBufLen = req_buf_len - ((noOfTableIndices*(uint32_t)sizeof(uint32_t)) + (uint32_t)sizeof(tblId));
   }

   buffer = (uint8_t *)malloc(((size_t)noOfTableIndices + (size_t)noOfCdftIndices)* sizeof(uint32_t));
   if(buffer == NULL)
   {
   ...
   )
{
   ...
   pInBuff = req_buf_ptr + offset;
   offset += (nInDataBufLen*(uint32_t)sizeof(uint8_t));
   ...
   {
      pIndices = (uint8_t *)(req_buf_ptr + offset);
      offset += ((noOfTableIndices + noOfCdftIndices)*(uint32_t)sizeof(uint32_t));
      ...
      if (result != 0)
      {
         ...
         fail_indices_offset += (noOfTableIndices * (uint32_t)sizeof(uint32_t));
         failed_indices_len += noOfTableIndices;
         if (nonModuleTblFound == 0)
         {
            ACDB_MEM_CPY((void *)(resp_buf_ptr + fail_indices_offset), sizeof(mid), &mid, sizeof(mid));
            fail_indices_offset += (uint32_t)sizeof(mid);

			if(noOfCdftIndices == INSTANCE_CDFT_INDICES_COUNT)
			{
				ACDB_MEM_CPY((void *)(resp_buf_ptr + fail_indices_offset), sizeof(iid), &iid, sizeof(iid));
				fail_indices_offset += (uint32_t)sizeof(iid);
				failed_indices_len += 1;
			}

            ACDB_MEM_CPY((void *)(resp_buf_ptr + fail_indices_offset), sizeof(pid), &pid, sizeof(pid));
            fail_indices_offset += (uint32_t)sizeof(pid);
   ...
   if (failed_indices_len > 0)
   {
      ...
      *resp_buf_bytes_filled = failed_indices_len*(uint32_t)sizeof(uint32_t) + (uint32_t)sizeof(uint32_t);
   }
   ...
   )
{
   ...
   {
	   ACPH_CMD_QUERY_ONLINE_VERSION_rsp rsp = { 0, 0 };
