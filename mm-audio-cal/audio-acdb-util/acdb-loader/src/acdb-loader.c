...
#define VSS_MAX_CHANNELS 32

#define ACDB_SW_MAJOR_VER_WITH_INSTANCE_ID 10

#define ACDB_MID_VAD 0x000102B9
#define ACDB_PID_VAD_CORE_CFG 0x000102BB
...
struct cal_list		cal_data[MAX_CAL_TYPES] = {
	...
	{0, 1},			/* ADM_LSM_AUDPROC_PERSISTENT_CAL_TYPE */
#endif
};
...
struct cal_block	mem_data[MAX_CAL_TYPES] = {
	...
	{16384, true, 0, -1, 0, VERSION_0_0},		/* ADM_LSM_AUDPROC_PERSISTENT_CAL_TYPE */
#endif
	{4096, true, 0, -1, 0, VERSION_0_0},		/* ADM_AUDPROC_PERSISTENT_CAL_TYPE */
};
...
static bool		instance_id_supported;
...
static bool get_instance_id_support();
...
static bool cvd_version_supported(char *cvd_version, int version_id)
{
	...
	if (cvd_version == NULL) {
		...
		LOGD("ACDB -> %s: CVD version %d not supported!", __func__, version_id);
		goto done;
	}
...
static int allocate_cal_type(int cal_type_idx, int size, int buffer_number)
{
        ...
        /* Allocate cal block */
        ret = allocate_cal_block(cal_type_idx, cal_node->buffer_number,
                                 &audio_alloc, cal_node->cal_block);
        if (ret < 0)
...
static int get_acdb_sw_major_version(void)
{
	AcdbModuleVersionTypeV3		response;
	int				ret = 0;

	ret = acdb_ioctl(ACDB_CMD_GET_ACDB_SW_VERSION_V3, NULL, 0,
			 (uint8_t *)&response, sizeof(response));
	if (ret) {
		LOGE("%s:Error in ACDB_CMD_GET_ACDB_SW_VERSION_V3:%d\n",
		     __func__, ret);
		ret = -EINVAL;
		goto done;
	}
	ret = response.major;
	ALOGD("%s: ACDB SW Major version: %d\n", __func__, ret);
done:
	return ret;
}

static bool get_instance_id_support(void)
{
	bool ret = false;
	int result;
	int acdb_sw_major_ver;
	uint32_t acdb_file_instance_id_support = 0;
	AcdbQueryCmdType	instance_support_info;
	AcdbQueryResponseType	response;

	acdb_sw_major_ver = get_acdb_sw_major_version();
	if (acdb_sw_major_ver < 0) {
		LOGE("%s: Error in getting ACDB SW major version %d\n",
		     __func__, acdb_sw_major_ver);
		goto done;
	}

	instance_support_info.nBufferLength = sizeof(uint32_t);
	instance_support_info.pBufferPointer = (uint8_t *)&acdb_file_instance_id_support;

	result = acdb_ioctl(ACDB_CMD_GET_ADSP_INSTANCE_ID_SUPPORT_INFO,
			 (const uint8_t *)&instance_support_info,
			 sizeof(instance_support_info),
			 (uint8_t *)&response, sizeof(response));
	if (result) {
		LOGE("%s:Error in ACDB_CMD_GET_ADSP_INSTANCE_ID_SUPPORT_INFO:%d\n",
		     __func__, ret);
		goto done;
	}
	ALOGD("%s: ACDB file instance ID support info:%d\n",
	      __func__, acdb_file_instance_id_support);

	if (acdb_sw_major_ver >= ACDB_SW_MAJOR_VER_WITH_INSTANCE_ID && acdb_file_instance_id_support)
		ret = true;
done:
	return ret;
}
...
int acdb_loader_init_v4(void* acdb_init_data, int acdb_loader_init_version)
{
	...
	struct acdb_init_data_v4*	init_data = NULL;
	...
	if(acdb_loader_init_version == ACDB_LOADER_INIT_V4)
		init_data =  (struct acdb_init_data_v4*)acdb_init_data;

	if(NULL == init_data) {
		LOGE("ACDB -> invalid init version!\n");
		ret = -EINVAL;
		goto done;
	}

	if (acdb_load_files(&acdb_init_cmd, init_data->snd_card_name) <= 0) {
	...
	instance_id_supported = get_instance_id_support();
	if (init_data->is_instance_id_supported)
		*init_data->is_instance_id_supported = instance_id_supported;

	ALOGD("%s: snd_card_name:%s, cvd_version:%s, instance_id_supported:%d",
	      __func__, init_data->snd_card_name, init_data->cvd_version,
	      instance_id_supported);
	...
	LOGD("ACDB -> RTAC INIT\n");
	acdb_rtac_init(instance_id_supported);

#ifdef _ANDROID_
	LOGD("ACDB -> MCS, FTS INIT\n");
	acdb_mcs_init(init_data->snd_card_name);
	acdb_fts_init();
#endif /* _ANDROID_ */
	...
	if (cvd_version_supported(init_data->cvd_version, PER_VOCODER_VERSION)) {
	...
	/*meta info*/
	if (send_meta_info_list(init_data->meta_key_list) < 0)
		LOGD("ACDB -> send_meta_info_list failed");

	current_feature_set = ACDB_VOCVOL_FID_DEFAULT;
	is_initialized = true;

	parse_codec_type(init_data->snd_card_name);
...
int acdb_loader_init_v3(char *snd_card_name,
			...
			struct listnode *meta_key_list)
{
	struct acdb_init_data_v4 init_data;

	init_data.snd_card_name = snd_card_name;
	init_data.cvd_version = cvd_version;
	init_data.meta_key_list = meta_key_list;
	init_data.is_instance_id_supported = NULL;
	return acdb_loader_init_v4(&init_data, ACDB_LOADER_INIT_V4);
}
...
static int get_afe_topology(struct audio_cal_afe_top *afe_top)
{
	...
	if (result) {
		LOGD("ACDB get afe topology for acdb id = %d, err = %d\n",
...
static int get_adm_topology(struct audio_cal_adm_top *adm_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_AUDPROC_COMMON_TOPOLOGY_ID,
		...
		(uint8_t *)&audio_top, sizeof(audio_top));
	if (result) {
		LOGD("Error: ACDB get adm topology for acdb id = %d, returned = %d\n",
...
static int get_asm_topology(struct audio_cal_asm_top *audstrm_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_AUDPROC_STREAM_TOPOLOGY_ID,
		...
		(uint8_t *)&audio_top, sizeof(audio_top));
	if (result) {
		LOGD("Error: ACDB get asm topology returned = %d\n",
		     result);
				...
				struct audio_cal_audstrm *audstrm_cal)
{
	...
	AcdbQueryResponseType		response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AUDPROC_STREAM_TABLE_V2,
	...
	result = acdb_ioctl(acdb_cmd[instance_id_supported],
		(const uint8_t *)&audstrmtable, sizeof(audstrmtable),
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB get audstream table returned = %d\n", result);
				...
				struct audio_cal_basic *common_top)
{
	...
	AcdbQueryResponseType		response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AVCS_CUSTOM_TOPO_INFO,
	...
	result = acdb_ioctl(acdb_cmd[instance_id_supported],
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB get common custom topologies returned = %d\n",
		     result);
				...
				struct audio_cal_basic *afe_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_AFE_TOPOLOGIES,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB get afe custom topologies returned = %d\n",
		     result);
				...
				struct audio_cal_basic *adm_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_AUDIO_COPP_TOPOLOGIES,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB get adm topologies returned = %d\n",
		     result);
				...
				struct audio_cal_basic *asm_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_AUDIO_POPP_TOPOLOGIES,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB get asm topologies returned = %d\n",
		     result);
			...
			struct audio_cal_audproc *audproc_cal)
{
	...
	AcdbQueryResponseType		response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AUDPROC_COMMON_TABLE,
	...
	result = acdb_ioctl(acdb_cmd[instance_id_supported],
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB audproc returned = %d\n", result);
			...
			struct audio_cal_audvol *audvol_cal)
{
	...
	AcdbQueryResponseType			response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AUDPROC_GAIN_DEP_STEP_TABLE,
	...
	LOGD("ACDB -> %s, vol index %d\n", instance_id_supported ?
	     "ACDB_CMD_GET_AUDPROC_INSTANCE_GAIN_DEP_STEP_TABLE":
	     "ACDB_CMD_GET_AUDPROC_GAIN_DEP_STEP_TABLE", audvol_cal->cal_type.cal_info.vol_index);

	result = acdb_ioctl(acdb_cmd[instance_id_supported],
		(const uint8_t *)&audvoltable, sizeof(audvoltable),
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB AudProc vol returned = %d\n", result);
			...
			struct audio_cal_hw_delay *hw_delay_cal)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_DEVICE_PROPERTY,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB ACDB_CMD_GET_DEVICE_PROPERTY error = %d\n",
		     result);
			...
			struct audio_cal_afe *afe_cal)
{
	...
	AcdbQueryResponseType			response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AFE_COMMON_TABLE,
	...
	result = acdb_ioctl(acdb_cmd[instance_id_supported],
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB AFE returned = %d\n", result);
				...
				struct audio_cal_basic *lsm_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_LSM_TOPOLOGIES,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB get lsm custom topologies returned = %d\n",
		     result);
...
static int get_lsm_topology(struct audio_cal_lsm_top *lsm_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_LSM_TOPOLOGY_ID,
		...
		(uint8_t *)&audio_top, sizeof(audio_top));
	if (result) {
		LOGD("Error: ACDB get lsm topology returned = %d\n",
		     result);
				...
				struct audio_cal_lsm *lsm_cal)
{
	...
	AcdbQueryResponseType			response;
	int acdb_cmd[2] = {ACDB_CMD_GET_LSM_TABLE,
	...
	result = acdb_ioctl(acdb_cmd[instance_id_supported],
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB LSM returned = %d\n", result);
...
int acdb_loader_get_ecrx_device(int acdb_id)
{
	...
	if (result) {
		LOGD("Error: ACDB EC_REF_RX returned = %d\n", result);
		goto done;
	}
...
static int send_afe_topology(int acdb_id, int path, int buff_num, int cal_type)
{
	...
	if (block == NULL) {
		LOGD("%s: no cal block for AFE_TOPOLOGY!\n", __func__);
		goto done;
	}
	...
	if (result) {
		LOGD("Error: get ACDB afe topology result = %d\n", result);
		goto done;
	}
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&afe_top);
	if (result) {
		LOGD("Error: Sending ACDB afe topology result = %d\n", result);
		goto done;
	}
...
static int get_meta_info_size(int metaInfoKey)
{
	...
	if (result < 0) {
		LOGD("Error: ACDB_CMD_GET_META_INFO_SIZE returned %d\n",
			result);
		goto done;
	}
	...
	if (metaInfoSize.nSize == 0) {
		LOGD("Error: ACDB_CMD_GET_META_INFO_SIZE returned 0 bytes\n");
		goto done;
	}
done:
        return result;
...
int send_meta_info(int metaInfoKey, int key_idx)
{
	...
	if (block == NULL) {
		LOGD("%s: ACDB -> Error:Could not get cal block!\n", __func__);
		goto done;
	}
	...
	if (result) {
		LOGD("ACDB -> get_meta_info returned error (%d), err:\n",
			result);
		goto done;
	}
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
			&metaInfo);
	if (result) {
		LOGD("Error in sending ACDB Meta Info, result = %d\n", result);
		goto done;
	}
			     ...
			     int cal_type)
{
	...
	block = get_cal_block(cal_type, buff_num);
	if (block == NULL) {
		LOGD("%s: Error: Could not get cal block!\n", __func__);
		goto done;
	}
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&adm_top);
	if (result) {
		LOGD("Error: Sending ACDB adm topology result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_asm_topology(int app_id, int buff_num)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&asm_top);
	if (result) {
		LOGD("Error: Sending ACDB asm topology result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_audstrmtable(int app_id, int buff_num)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&audstrm_cal);
	if (result) {
		LOGD("Error: Sending ACDB aud strm table result = %d\n", result);
		goto done;
		}
done:
	return result;
...
static int send_common_custom_topology(void)
{
	...
	if (block == NULL) {
		LOGD("%s: no cal block CORE_CUSTOM_TOPOLOGIES!\n", __func__);
		result = -ENODEV;
		goto done;
	}
	...
	result2 = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&common_top);
	if (result2) {
		LOGD("AUDIO_SET_CALIBRATION, Error = %d\n", result2);
		result = -EINVAL;
		goto done;
	}
done:
...
static int send_afe_custom_topology(void)
{
	...
	if (block == NULL) {
		LOGD("%s: no cal block AFE_CUST_TOPOLOGY!\n", __func__);
		goto done;
	}
	afe_top.hdr.data_size = sizeof(afe_top);
	afe_top.hdr.version = VERSION_0_0;
...
static int get_audproc_persist_table_size(int app_id, int acdb_id, int sample_rate)
{
	int result = 0;
	AcdbAudProcTableSizeCmdType get_audproc_persist_size_cmd;
	AcdbSizeResponseType response;

	get_audproc_persist_size_cmd.nDeviceId = acdb_id;
	get_audproc_persist_size_cmd.nDeviceSampleRateId = sample_rate;
	get_audproc_persist_size_cmd.nApplicationType = app_id;

	LOGD("ACDB -> ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE_SIZE");

	result = acdb_ioctl(ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE_SIZE,
			(const uint8_t *)&get_audproc_persist_size_cmd,
			sizeof(get_audproc_persist_size_cmd),
			(uint8_t *)&response, sizeof(response));

	if (result < 0) {
		LOGD("Error: ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE_SIZE returned %d",
			result);
		goto done;
	}
	...
	if (response.nSize == 0) {
		LOGD("Error: ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE_SIZE returned %d bytes",
			response.nSize);
		goto done;
	}
done:
...
static int get_audproc_persist_table(struct cal_block *block,
			struct audio_cal_audproc *audproc_cal)
{
	int				result = 0;
	AcdbAudProcTableCmdType		audtable;
	...
	LOGD("ACDB -> ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE");

	result = acdb_ioctl(ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE,
		(const uint8_t *)&audtable, sizeof(audtable),
		(uint8_t *)&response, sizeof(response));

	if (result) {
		LOGD("Error: ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE returned = %d\n",
			result);
		audproc_cal->cal_type.cal_data.cal_size = 0;
		goto done;
	}
...
static int send_audproc_persist_table(int acdb_id, int path, int app_id, int sample_rate,
			 int buff_num)
{
	int				result = 0;
	int				audproc_persist_size = 0;
	int				reallocate_size = 0;
	struct cal_block		*block;
	struct audio_cal_audproc	audproc_cal;

	block = get_cal_block(ADM_AUDPROC_PERSISTENT_CAL_TYPE, buff_num);
	if (block == NULL) {
		LOGD("%s: Error: Could not get cal block!\n", __func__);
		goto done;
	}

	/* Get Audproc persist table size from s/w and reallocate cal type in case of
	insufficient memory */
	audproc_persist_size = get_audproc_persist_table_size(app_id, acdb_id, sample_rate);
	LOGD("ACDB -> send_audproc_persist_table %d \n", audproc_persist_size);

	if (audproc_persist_size <= 0){
		LOGD("Error: Invalid Audproc persist size = %d\n", audproc_persist_size);
		goto done;
	}

	if (audproc_persist_size > block->size){
		if (audproc_persist_size < MIN_BUFF_SIZE)
			reallocate_size = MIN_BUFF_SIZE;
		else
			reallocate_size = audproc_persist_size;
		LOGD("Reallocate memory for Audproc persist table to size: %d\n", reallocate_size);
		deallocate_cal_type(ADM_AUDPROC_PERSISTENT_CAL_TYPE, buff_num);
		allocate_cal_type(ADM_AUDPROC_PERSISTENT_CAL_TYPE, reallocate_size, buff_num);
	}

	audproc_cal.hdr.data_size = sizeof(audproc_cal);
	audproc_cal.hdr.version = VERSION_0_0;
	audproc_cal.hdr.cal_type = ADM_AUDPROC_PERSISTENT_CAL_TYPE;
	...
	result = get_audproc_persist_table(block, &audproc_cal);

	if (result) {
		LOGD("Error: Sending ACDB Audproc persist result = %d\n", result);
		goto done;
	}

	LOGD("ACDB -> ADM_AUDPROC_PERSISTENT_CAL_TYPE acdb_id[%d] app_type[%d]\n",
		 acdb_id, app_id);
	...
	if (result) {
		LOGD("Error: Sending ACDB Audproc persist IOCTL result = %d\n", result);
		goto done;
	}
			 ...
			 int buff_num, int cal_type)
{
	...
	block = get_cal_block(cal_type, buff_num);
	if (block == NULL) {
		LOGD("%s: Error: Could not get cal block! cal_type[%d] buff_num[%d]\n",
			 __func__, cal_type, buff_num);
		goto done;
	}
	...
	LOGD("ACDB -> AUDIO_SET_AUDPROC_CAL cal_type[%d] acdb_id[%d] app_type[%d]\n",
		cal_type, acdb_id, app_id);

	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&audproc_cal);
	if (result) {
		LOGD("Error: Sending ACDB audproc result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_audvoltable(int acdb_id, int path, int app_id, int vol_index, int cal_type, int buff_num)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&audvol_cal);
	if (result) {
		LOGD("Error: Sending ACDB audproc vol result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_afe_cal(int acdb_id, int path, int sample_rate, int cal_type, int buff_num)
{
	...
	LOGD("ACDB -> AUDIO_SET_AFE_CAL cal_type[%d] acdb_id[%d]\n", cal_type, acdb_id);
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&afe_cal);
	if (result) {
		LOGD("Error: Sending ACDB AFE result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_hw_delay(int acdb_id, int path, int buff_num)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
				   &hw_delay_cal);
	if (result)
		LOGD("Error: Sending ACDB AFE result = %d errno=%d\n",
		      result, errno);
done:
	return result;
}
...
static int send_lsm_custom_topology(void)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&lsm_top);
	if (result) {
		LOGD("Error: Sending ACDB LSM topology result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_lsm_topology(int acdb_id, int app_id, int mode, int cal_type, int buff_idx)
{
	int				result = 0;
	int				result2 = 0;
	struct cal_block		*block;
	...
	block = get_cal_block(cal_type, buff_idx);
	if (block == NULL) {
		LOGD("%s: Error: Could not get cal block!\n", __func__);
		goto done;
	}
	...
	lsm_top.hdr.cal_type_size = sizeof(lsm_top.cal_type);
	lsm_top.cal_type.cal_hdr.buffer_number = buff_idx;
	lsm_top.cal_type.cal_hdr.version = block->version;
...
static int send_lsm_cal(int acdb_id, int app_id, int cal_type, int buff_idx)
{
	int				result = 0;
	int				lsmtable_size = 0;
	int				reallocate_size = 0;
	...
	if (lsmtable_size > block->size)
	{
		...
		allocate_cal_type(cal_type, reallocate_size, buff_idx);
	}
	...
	lsm_cal.cal_type.cal_hdr.buffer_number = buff_idx;
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&lsm_cal);
	if (result) {
		LOGD("Error: Sending ACDB LSM result = %d\n", result);
		goto done;
	}
done:
	return result;
...
int send_codec_cal(struct param_data *params)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_CODEC_CAL_DATA,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB CODEC CAL returned = %d\n", result);
	}
...
/* To send persistent calibration (ex: FFECNS) in audio usecases */
void acdb_loader_send_audio_cal_v5(int acdb_id, int capability, int app_id, int sample_rate,
					int use_case, int afe_sample_rate, int cal_mode)
{
	int				path;
	int				aud_vol_idx = 0;
	...
	LOGD("ACDB -> send_audio_cal, acdb_id = %d, path = %d, app id = 0x%x, sample rate = %d, afe_sample_rate = %d\n",
		acdb_id, path, app_id, sample_rate, afe_sample_rate);

	send_asm_topology(app_id, buffer_idx);
	send_adm_topology(acdb_id, path, app_id, buffer_idx_w_path,
			  ADM_TOPOLOGY_CAL_TYPE);
	cal_type = (cal_mode == CAL_MODE_PERSIST) ?
		ADM_LSM_AUDPROC_PERSISTENT_CAL_TYPE : ADM_AUDPROC_CAL_TYPE;
	send_audtable(acdb_id, path, app_id, sample_rate, buffer_idx_w_path,
		      cal_type);
	send_audproc_persist_table(acdb_id, path, app_id, sample_rate, buffer_idx_w_path);
	...
	send_afe_cal(acdb_id, path, afe_sample_rate, cal_type, buffer_idx);
...
void acdb_loader_send_audio_cal_v4(int acdb_id, int capability, int app_id,
				   int sample_rate, int use_case,
				   int afe_sample_rate)
{
	acdb_loader_send_audio_cal_v5(acdb_id, capability, app_id, sample_rate, use_case,
					afe_sample_rate, CAL_MODE_SEND);
	return;
}

void acdb_loader_send_audio_cal_v3(int acdb_id, int capability, int app_id, int sample_rate, int use_case)
{
	acdb_loader_send_audio_cal_v4(acdb_id, capability, app_id, sample_rate, use_case, sample_rate);
	return;
}
...
#endif
    send_afe_cal(acdb_id, TX_DEVICE, sample_rate, cal_type, BUFF_IDX_0);
}

void acdb_loader_send_listen_device_cal_v1(int acdb_id, int type, int app_id,
                                           int sample_rate, int cal_mode)
{
    int cal_type;
    int buffer_idx_w_path = TX_DEVICE + MAX_PATH_TYPE * BUFF_IDX_0;
    ...
    if (type != LISTEN_HW_TYPE_CPE) {
        ...
        cal_type = (cal_mode == LISTEN_CAL_MODE_PERSISTENT) ?
            ADM_LSM_AUDPROC_PERSISTENT_CAL_TYPE : ADM_LSM_AUDPROC_CAL_TYPE;
        send_audtable(acdb_id, TX_DEVICE, app_id, sample_rate,
                      buffer_idx_w_path, cal_type);
    }

    send_afe_topology(acdb_id, TX_DEVICE, buffer_idx_w_path,
...
int acdb_loader_send_listen_lsm_cal_v1(int acdb_id, int app_id, int mode, int type, int buff_idx)
{
	...
	result = send_lsm_topology(acdb_id, app_id, mode, cal_type, buff_idx);

	cal_type = (type == LISTEN_HW_TYPE_CPE) ?
			ULP_LSM_CAL_TYPE : LSM_CAL_TYPE;
	send_lsm_cal(acdb_id, app_id, cal_type, buff_idx);
...
{
    return acdb_loader_send_listen_lsm_cal_v1(acdb_id, app_id, mode, type, BUFF_IDX_0);
}
...
static int get_anc_table(int acdb_id, struct cal_block *block, int adie_pid)
{
	...
	result = acdb_ioctl(acdb_command_id,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB ANC returned = %d\n", result);
		goto done;
	}
done:
	return result;
...
void send_mbhc_data(struct param_data *params)
{
	...
	if (result)
	{
		LOGD("Error reading MBHC general config returned = %x\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC removal config returned = %x\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC plug type config returned = %x\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC button press config returned = %x\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC impedance config returned = %x\n", result);
		goto acdb_error;
	}
...
void send_vad_data(struct param_data *params)
{
	int result;
	AcdbGblTblCmdType global_cmd;
	AcdbQueryResponseType   response;
	...
	global_cmd.nModuleId = ACDB_MID_VAD;
	global_cmd.nParamId = ACDB_PID_VAD_CORE_CFG;
	global_cmd.nBufferLength = params->buff_size;
	global_cmd.nBufferPointer = (uint8_t *)params->buff;

	LOGD("ACDB -> VAD ACDB_PID_VAD_CORE_CFG\n");
	result = acdb_ioctl(ACDB_CMD_GET_GLBTBL_DATA,
		(const uint8_t *) &global_cmd, sizeof(global_cmd),
		(uint8_t *) &response, sizeof(response));
	...
	if (result)
	{
		LOGD("Error reading VAD core config returned = %x\n", result);
		goto acdb_error;
	}
...
static void send_vbat_data(struct param_data *params)
{
	...
	if (result)
	{
		LOGD("Error reading VBAT ADC CAL returned = %x\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading VBAT GAIN PROC returned = %x\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error converting vbat data to wcd9xxx data = %x\n", result);
	}
...
static uint32_t get_voice_topology(struct audio_cal_voc_top *voc_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_VOCPROC_COMMON_TOPOLOGY_ID,
		...
		(uint8_t *)&audio_top, sizeof(audio_top));
	if (result) {
		LOGD("Error: ACDB get voice rx topology for acdb id = %d, returned = %d\n",
...
static int get_voice_channel_config(struct audio_cal_voc_top *voc_top)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_TARGET_SPECIFIC_DEVICE_INFO,
			    ...
			    (uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB get target specific info for acdb id = %d, returned = %d\n",
		     voc_top->cal_type.cal_info.acdb_id, result);
		result = -EINVAL;
		goto done;
	}
...
static int get_voice_channel_mapping(struct audio_cal_voc_top *voc_top)
{
	...
	voice_ch_map.nPropID = ACDB_CHANNEL_MAPPING_CONFIG_V2;
	voice_ch_map.nBufferLength = sizeof(ch_map);
	voice_ch_map.pBufferPointer = (uint8_t *)&ch_map;

	LOGD("ACDB -> ACDB_CHANNEL_MAPPING_CONFIG_V2: GET_DEVICE_PROPERTY\n");
...
static int get_sidetone(struct audio_cal_sidetone *sidetone_cal)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_AFE_DATA,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB AFE DATA Returned = %d\n", result);
...
static int get_sidetone_iir_config(struct audio_cal_sidetone_iir *sidetone_iir_cal)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_AFE_DATA,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("%s Error: ACDB AFE DATA Returned = %d\n", __func__, result);
		goto done;
	}
				...
				struct audio_cal_voc_col *voc_col_cal)
{
	...
	if (block->version & PER_VOCODER_CAL_BIT_MASK) {
		...
		result = acdb_ioctl(ACDB_CMD_GET_VOC_COLUMNS_INFO_V2,
			...
			(uint8_t *)&response, sizeof(response));
		if (result) {
			LOGD("Error: ACDB VOC COL V2 Returned = %d\n", result);
	   ...
	   uint32_t txacdb_id, uint32_t table_id)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&voc_col);
	if (result) {
		LOGD("Error: Sending ACDB voice columns result = %d\n", result);
		goto done;
	}
done:
	return result;
				...
				struct audio_cal_vocdev_cfg *vocdev_cal)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_VOC_PROC_DEVICE_CFG,
		...
		(uint8_t *)&response, sizeof(response));
	if (result) {
		LOGD("Error: ACDB VocProc Dev Cfg Returned = %d\n",
			result);
...
static int send_vocproc_dev_cfg(int rxacdb_id, int txacdb_id)
{
	...
	if (result) {
		LOGD("Error: Sending ACDB VocProc Dev Cfg data result = %d\n", result);
		goto done;
	}
done:
	return result;
				...
				struct audio_cal_vocproc *vocproc_cal)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_VOC_PROC_STATIC_TABLE,
			(const uint8_t *)&voctable, sizeof(voctable),
			(uint8_t *)&response, sizeof(response));

	if (result) {
		LOGD("Error: %s Returned = %d\n",
		     "ACDB_CMD_GET_VOC_PROC_STATIC_TABLE",
		     result);
				...
				struct audio_cal_basic *vocstrm_cal)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_VOC_STREAM_STATIC_TABLE,
				...
				struct audio_cal_vocvol *vocvol_cal)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_VOC_PROC_DYNAMIC_TABLE,
...
static int send_voice_rx_topology(int acdb_id)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&voc_top);
	if (result) {
		LOGD("Error: Sending ACDB voice rx topology result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_voice_tx_topology(int acdb_id)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&voc_top);
	if (result) {
		LOGD("Error: Sending ACDB voice tx topology result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_sidetone(int rxacdb_id, int txacdb_id)
{
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
			&sidetone_cal);
	if (result) {
		LOGD("Error: Sending ACDB sidetone data result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_voctable(int rxacdb_id, int txacdb_id)
{
	...
	if (result) {
		LOGD("Error: Sending ACDB VocProc data result = %d\n", result);
		goto done;
	}
done:
	return result;
...
static int send_vocstrmtable(void) {
	...
	result = ioctl(cal_driver_handle, AUDIO_SET_CALIBRATION,
		&vocstrm_cal);
	if (result < 0) {
		LOGD("Error: Sending ACDB VOCPROC STREAM fail result %d\n",
			result);
		goto done;
	}
done:
...
static int send_vocvoltable(int rxacdb_id, int txacdb_id, int feature_set)
{
	...
	if (result < 0)
		LOGD("Error: getting VocVol Table = %d\n", result);
...
static int validate_voc_cal_dev_pair(int rxacdb_id, int txacdb_id)
{
	...
	if (result < 0) {
		LOGD("Error: failure to vaildate the device pair = %d\n",
			result);
		goto done;
	}
...
static void send_voice_cal(int rxacdb_id, int txacdb_id, int feature_set)
{
	int	acdb_table_id[3][3] = {
				...
				{ACDB_VOC_PROC_STAT_INSTANCE_TABLE,
				ACDB_VOC_PROC_DYN_ISNTANCE_TABLE,
				ACDB_VOC_PROC_STREAM_INSTANCE_TABLE}
				};
	...
	if (validate_voc_cal_dev_pair(rxacdb_id, txacdb_id) != 1) {
		LOGD("ACDB -> Error: invalid device pair!");
		goto done;
	}
	...
	if (!instance_id_supported)
		send_voice_columns(rxacdb_id, txacdb_id, acdb_table_id[per_vocoder_enabled][0]);
	else
		send_voice_columns(rxacdb_id, txacdb_id, acdb_table_id[2][0]);
	...
	if (!instance_id_supported)
		...
		send_voice_columns(rxacdb_id, txacdb_id, acdb_table_id[2][1]);
	...
	if((send_vocvoltable(rxacdb_id, txacdb_id, feature_set) < 0) &&
		...
		if (send_vocvoltable(rxacdb_id, txacdb_id, ACDB_VOCVOL_FID_DEFAULT) < 0)
			LOGD("ACDB -> Resend default vocvol unsuccessful!\n");
	}
	if (!instance_id_supported)
		send_voice_columns(rxacdb_id, txacdb_id, acdb_table_id[per_vocoder_enabled][2]);
	else
		send_voice_columns(rxacdb_id, txacdb_id, acdb_table_id[2][2]);
...
static int get_voctable_size(int rxacdb_id, int txacdb_id, int rxacdb_id_sample_rate, int txacdb_id_sample_rate)
{
        ...
        result = acdb_ioctl(ACDB_CMD_GET_VOC_PROC_STATIC_TABLE_SIZE,
...
static int get_vocstrmtable_size()
{
        ...
        result = acdb_ioctl(ACDB_CMD_GET_VOC_STREAM_STATIC_TABLE_SIZE, NULL, 0,
                             (uint8_t *)&response, sizeof(response));
...
static int get_audtable_size(int acdb_id, int app_id, int sample_rate)
{
	...
	AcdbSizeResponseType			response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AUDPROC_COMMON_TABLE_SIZE,
...
static int get_audstrmtable_size(int app_id)
{
	...
	AcdbSizeResponseType response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AUDPROC_STREAM_TABLE_SIZE,
...
static int get_audvoltable_size(int acdb_id, int app_type, int vol_index)
{
	...
	AcdbSizeResponseType response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AUDPROC_GAIN_DEP_STEP_TABLE_SIZE,
...
static int get_afetable_size(int acdb_id, int sample_rate)
{
	...
	AcdbSizeResponseType response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AFE_COMMON_TABLE_SIZE,
...
static int get_lsmtable_size(int acdb_id, int mad_app_type)
{
	...
	AcdbSizeResponseType response;
	int acdb_cmd[2] = {ACDB_CMD_GET_LSM_TABLE_SIZE,
...
static int get_custom_topology_size()
{
	int result = 0;
	AcdbSizeResponseType response;
	int acdb_cmd[2] = {ACDB_CMD_GET_AVCS_CUSTOM_TOPO_INFO_SIZE,
...
int deregister_vocvoltable(void)
{
	...
	if (result < 0) {
		LOGD("Error: Deregister vocproc vol returned = %d\n",
			result);
		goto done;
	}
done:
...
int register_vocvoltable(void)
{
	...
	if (result < 0) {
		LOGD("Error: Register vocproc vol returned = %d\n",
			result);
		goto done;
	}
done:
...
int acdb_loader_reload_vocvoltable(int feature_set)
{
	...
	if (result < 0) {
		LOGD("ACDB -> No vocvol table to reload!\n");
		goto done;
	}

	result = deregister_vocvoltable();
	if (result < 0) {
		LOGD("ACDB -> Deregister vocvol table unsuccessful!\n");
		goto done;
	}
	...
	if (result < 0) {
		...
		if (feature_set != ACDB_VOCVOL_FID_DEFAULT) {
			...
			LOGD("ACDB -> Resend default vocvol table!\n");
			if (send_vocvoltable(rxacdb_id, txacdb_id, ACDB_VOCVOL_FID_DEFAULT) < 0)
				LOGD("ACDB -> Resend default vocvol unsuccessful!\n");
		}
		...
		LOGD("ACDB -> Reregister default vocvol table!\n");
		if(register_vocvoltable() < 0) {
			LOGD("ACDB -> Reregister default volume unsuccessful!\n");
			goto done;
		}
		LOGD("ACDB -> Resend default volume successful!\n");
		goto done;
	}

	result = register_vocvoltable();
	if (result < 0) {
		LOGD("ACDB -> Register vocvol table unsuccessful!\n");
		goto done;
	}
...
int acdb_loader_get_remote_acdb_id(unsigned int native_acdb_id)
{
	...
	result = acdb_ioctl(ACDB_CMD_GET_COMPATIBLE_REMOTE_DEVICE_ID,
			...
			(uint8_t *)&response, sizeof(response));
	if (result < 0) {
		LOGD("Error: Remote ACDB ID lookup failed = %d\n",
			result);
		goto done;
	}
...
int process_attribute(char *attr, struct param_data *params)
{
	...
	} else if (strcmp("vad_cal", attr) == 0) {
		send_vad_data(params);
	}
done:
	return ret;
}
...
int acdb_loader_get_calibration(char *attr, int size, void *data)
{
	...
	if (attr == NULL) {
		LOGD("%s: Error: attr is NULL!\n", __func__);
		ret = -EINVAL;
		goto done;
	} else if (size != sizeof(*params)) {
		LOGD("%s: Error: Invalid size %d, expected %zd\n",
			__func__, size, sizeof(*params));
		ret = -EINVAL;
		goto done;
	} else if (data == NULL) {
		LOGD("%s: Error: data is NULL!\n", __func__);
		ret = -EINVAL;
		goto done;
	}
...
int acdb_loader_store_set_audio_cal(acdb_audio_cal_cfg_t *cal, void *data, uint32_t datalen)
{
    ...
    if(cal->cal_type == AUDIO_DEVICE_CAL_TYPE) {
        if(cal->module_id && cal->param_id) {
            AcdbAudProcInstanceCmdType cmd;
            ...
            cmd.nInstanceId = cal->instance_id;
            cmd.nReserved1 = 0;
            cmd.nParamId = cal->param_id;
            cmd.nBufferLength = datalen;
            cmd.nBufferPointer = (uint8_t *)data;
            result = acdb_ioctl(ACDB_CMD_SET_AUDPROC_INSTANCE_COMMON_DATA,
                                (const uint8_t *)&cmd, sizeof(cmd),
                                 NULL,0);
        } else {
        ...
        if(cal->module_id && cal->param_id) {
            ...
            scmd.nBufferLength = datalen;
            scmd.nBufferPointer = (uint8_t *)data;
            result = acdb_ioctl(ACDB_CMD_SET_AUDPROC_INSTANCE_STREAM_TABLE,
                                (const uint8_t *)&scmd, sizeof(scmd),
                                 NULL,0);
        }
    } else {
...
{
    ACPH_CMD_RTC_SET_INSTANCE_CAL_DATA_req* req = NULL;
    ...
    if(cal->module_id && cal->param_id) {
        req_len = sizeof(ACPH_CMD_RTC_SET_INSTANCE_CAL_DATA_req) + datalen;
    } else {
        req_len = offsetof(ACPH_CMD_RTC_SET_INSTANCE_CAL_DATA_req, module_id) + datalen;
    }
    req = (ACPH_CMD_RTC_SET_INSTANCE_CAL_DATA_req*)malloc(req_len);
    if(NULL == req) {
    ...
    if(cal->module_id && cal->param_id) {
        ...
        req->instance_id = cal->instance_id;
        req->parameter_id = cal->param_id;
        req->length = datalen;
        req->reserved1 = 0;
        tptr = (uint8_t*)req;
        tptr += sizeof(ACPH_CMD_RTC_SET_INSTANCE_CAL_DATA_req);
    } else {
    ...
    for(i = 1; i <= copp_id[0]; i++) {
        req->pp_id = copp_id[i];
        result = acdb_rtac_callback(ACPH_CMD_RTC_SET_INSTANCE_CAL_DATA, (uint8_t*) req, req_len,
...
int acdb_loader_store_get_audio_cal(acdb_audio_cal_cfg_t *cal, void *data, uint32_t *datalen)
{
    ...
    if(cal->cal_type == AUDIO_DEVICE_CAL_TYPE) {
        AcdbAudProcInstanceCmdType data_cmd;
        ...
        if(cal->module_id && cal->param_id) {
            ...
            data_cmd.nInstanceId = cal->instance_id;
            data_cmd.nReserved1 = 0;
            data_cmd.nParamId = cal->param_id;
            data_cmd.nBufferLength = *datalen;
            data_cmd.nBufferPointer = (uint8_t*)data;
            result = acdb_ioctl(ACDB_CMD_GET_AUDPROC_INSTANCE_COMMON_DATA,
...
{
        int result=0,i;
        ACPH_CMD_RTC_GET_INSTANCE_CAL_DATA_req req_buf;
	uint32_t copp_id[MAX_ACTIVE_COPP+1]={0};
        ...
        if(cal->module_id && cal->param_id) {
                ...
                req_buf.instance_id = cal->instance_id;
                req_buf.parameter_id = cal->param_id;
                result = acdb_rtac_callback(ACPH_CMD_RTC_GET_INSTANCE_CAL_DATA, (uint8_t*)&req_buf, req_buf_len,
                                            resp_buf, resp_buf_len, &resp_buf_filled);
...
static int get_volume_steps(int vol_step_type)
{
	...
	ret = acdb_ioctl(ACDB_CMD_GET_VOL_TABLE_STEP_SIZE, NULL, 0,
		(uint8_t *)&resp, sizeof(resp));
	if (ret) {
		LOGD("Error: ACDB get vol steps returned = %d\n", ret);
		goto err;
	}
...
int acdb_loader_send_gain_dep_cal(int acdb_id, int app_id, int capability, int mode, int vol_index)
{
	...
	path = get_audcal_path((uint32_t)capability);
	if (path == INVALID_DATA) {
		LOGD("ACDB -> Device is not RX or TX!"
			"acdb_id = %d\n", acdb_id);
		goto done;
	}
                                  ...
                                  char *cvd_version, struct listnode *meta_key_list)
{
	...
	if (ret) {
		LOGD("Error reset ACDB returned = %d\n", ret);
		ret = -ENODEV;
		goto done;
	}
