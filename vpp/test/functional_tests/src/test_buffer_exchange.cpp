...
static void set_default_hqv_controls()
{
    ...
    pstTestCtx->hqvCtrl.mode = HQV_MODE_AUTO;
}
...
static uint32_t bUsecase_Bypasses(struct test_ctx *pTestCtx)
{
    ...
    if ((pTestCtx->u32hqvCtrlSetFlag & ((1 << HQV_CONTROL_CUST) - 1)) ==
        (1 << HQV_CONTROL_FRC))
    {
        if ((pTestCtx->hqvCtrl.frc.num_segments > 0) && (pTestCtx->hqvCtrl.frc.segments != NULL))
            return VPP_TRUE;
    }
    return VPP_FALSE;
}
...
TEST(BufferExchange1_Testcase)
{
    ...
    struct vpp_port_param in_param, out_param;
    ...
    if((u32TestMask & FUNCTEST_ROTATE_BASIC) || (u32TestMask & FUNCTEST_ROTATE_ALL))
    {
        ...
        // Port parameters
        in_param = pstTestCtx->port_param;
        out_param = pstTestCtx->port_param_out;
        ...
        u32MulFactor = u32GetMulFactor(&in_param, &req);
        ...
        u32 = VPP_SET_PARAM(p, VPP_PORT_INPUT, in_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = VPP_SET_PARAM(p, VPP_PORT_OUTPUT, out_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
        ...
        if(u32 == VPP_OK)
        {
            ...
            for (i = 0; i < 2; i++)
            {
                for (j = 0; j < pFuncTestCtx->u32StreamCount; j++)
                {
                    ...
                    u32OutputFlushDoneFlag = 0;
                    u32InputFlushDoneFlag = 0;
                    in_param = pFuncTestCtx->reconfig_streams[j].port_param;
                    out_param = pFuncTestCtx->reconfig_streams[j].port_param_out;

                    if ((in_param.width > 1920) || (in_param.height > 1080))
                        continue;
                    u32 = VPP_RECONFIG(p, in_param, out_param);
                    wait_reconfig_done(p, u32MulFactor, u32FrameType);

                    u32MulFactor = u32GetMulFactor(&in_param, &req);
                    LOGD("Reconfigured new input stream: %dx%d", in_param.width, in_param.height);
                    u32 = VPP_FLUSH(p, VPP_PORT_INPUT);
                    DVP_ASSERT_EQUAL(u32, VPP_OK);
                    u32OutFlushFlag = OUT_FLUSH_REQUEST;
                    u32 = VPP_FLUSH(p, VPP_PORT_OUTPUT);
...
TEST(BufferExchangeReconfigError_Testcase)
{
    ...
    if(u32TestMask & FUNCTEST_RECONFIG_ALL)
    {
        ...
        if(u32 == VPP_OK)
        {
            ...
            {
                u32ReconfigDoneFlag = 0;
                u32OldBuffSize = in_param.width * in_param.height;
                in_param = pFuncTestCtx->reconfig_streams[i].port_param;
                out_param = pFuncTestCtx->reconfig_streams[i].port_param_out;
                if ((in_param.width > 1920) || (in_param.height > 1080))
                    continue;
                ...
                LOGD("Reconfigured %dx%d, still %d old output buffers queued. PURPOSELY NOT FLUSHED",
                     in_param.width, in_param.height, pstTestCtx->u32OutHoldCnt);
                if (in_param.height * in_param.width > u32OldBuffSize)
...
TEST(BufferExchangeExtraStreamBypass_Testcase)
{
    ...
    if(u32TestMask & FUNCTEST_BYPASS)
    {
        ...
        u32 = VPP_SET_PARAM(p2, VPP_PORT_INPUT, in_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = VPP_SET_PARAM(p2, VPP_PORT_OUTPUT, out_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = setTestVppHqvCtrl(p3, pstTestCtx, &req);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = VPP_SET_PARAM(p3, VPP_PORT_INPUT, in_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = VPP_SET_PARAM(p3, VPP_PORT_OUTPUT, out_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
    ...
    if((u32TestMask & FUNCTEST_BYPASS) || (u32TestMask & FUNCTEST_RECONFIG_ALL))
    {
        ...
        if(u32 == VPP_OK)
        {
            ...
            for (i = 0; i < pFuncTestCtx->u32StreamCount; i++)
            {
                in_param = pFuncTestCtx->reconfig_streams[i].port_param;
                out_param = pFuncTestCtx->reconfig_streams[i].port_param_out;
                if ((in_param.width > u32MaxW) || (in_param.height > u32MaxH))
                    break;
            }
            if (i >= pFuncTestCtx->u32StreamCount)
            {
                in_param.width = u32MaxW + 1;
                in_param.height = u32MaxH + 1;
                out_param.width = u32MaxW + 1;
                out_param.height = u32MaxH + 1;
            }
