...
TEST(ConcurrencyBasic_Testcase)
{
    ...
    if(u32TestMask & FUNCTEST_CONC_BASIC)
    {
        ...
        u32MulFactor1 = u32GetMulFactor(&in_param, &req1);

        u32 = VPP_SET_PARAM(p1, VPP_PORT_INPUT, in_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        u32 = VPP_SET_PARAM(p1, VPP_PORT_OUTPUT, out_param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        in_param = pFuncTestCtx->reconfig_streams[0].port_param;
        out_param = pFuncTestCtx->reconfig_streams[0].port_param_out;

        u32MulFactor2 = u32GetMulFactor(&in_param, &req2);
