...
uint32_t setTestVppHqvCtrl(void *pVppCtx, struct test_ctx *pTestCtx, struct vpp_requirements *pReq)
{
    ...
    if ((ctrl.mode == HQV_MODE_OFF) || (ctrl.mode == HQV_MODE_AUTO))
    {
        u32 = VPP_SET_CTRL(pVppCtx, ctrl, pReq);
        return u32;
    }

    // Send manual mode settings
    ...
    if (pTestCtx->u32hqvCtrlSetFlag & (1 << HQV_CONTROL_FRC))
    {
        ...
        ctrl.frc.num_segments = pTestCtx->hqvCtrl.frc.num_segments;
        ctrl.frc.segments = pTestCtx->hqvCtrl.frc.segments;
        u32 |= VPP_SET_CTRL(pVppCtx, ctrl, pReq);
    }
