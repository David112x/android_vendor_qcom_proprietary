...
#define VPP_TEST_PROPERTY_BUF_DUMP_DISABLE "vendor.media.vpp.test.bufdump_disable"
...
static struct {
    ...
    uint64_t u64TotalAllocated;
} stBufPoolStats;

static pthread_mutex_t bufPoolStatsMutex = PTHREAD_MUTEX_INITIALIZER;
...
static const struct buf_pool_params clip_registry[CLIP_REG_MAX] = {
    ...
    [CLIP_UBWC_RGBA8_SD] = {
        ...
        .cInputPath = INPUT_FILEPATH_TEMPLATE,
        .cInputName = "Canyon_UBWC30_RGBA8888_720x480i_short.yuv",
        .eInputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eInputFileType = FILE_TYPE_MULTI_FRAMES,
        .eInputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
        .eBufferType = eVppBufType_Progressive,

        .cOutputPath = OUTPUT_FILEPATH_TEMPLATE,
        .cOutputName = "out_Canyon_UBWC30_RGBA8888_720x480i_short.yuv",
        .eOutputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eOutputFileType = FILE_TYPE_MULTI_FRAMES,
        .eOutputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
    },
    [CLIP_UBWC_RGBA8_HD] = {
        .u32Width = 1280,
        .u32Height = 720,
        ...
        .cInputPath = INPUT_FILEPATH_TEMPLATE,
        .cInputName = "CLIP_UBWC_RGBA8_HD.yuv",
        .eInputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eInputFileType = FILE_TYPE_MULTI_FRAMES,
        .eInputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
        .eBufferType = eVppBufType_Progressive,
        ...
        .cOutputPath = OUTPUT_FILEPATH_TEMPLATE,
        .cOutputName = "out_CLIP_UBWC_RGBA8_HD.yuv",
        .eOutputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eOutputFileType = FILE_TYPE_MULTI_FRAMES,
        .eOutputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
    },
    [CLIP_UBWC_RGBA8_FHD] = {
        ...
        .cInputPath = INPUT_FILEPATH_TEMPLATE,
        .cInputName = "CLIP_UBWC_RGBA8_FHD.yuv",
        .eInputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eInputFileType = FILE_TYPE_MULTI_FRAMES,
        .eInputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
        .eBufferType = eVppBufType_Progressive,
        ...
        .cOutputPath = OUTPUT_FILEPATH_TEMPLATE,
        .cOutputName = "out_CLIP_UBWC_RGBA8_FHD.yuv",
        .eOutputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eOutputFileType = FILE_TYPE_MULTI_FRAMES,
        .eOutputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
    },
    [CLIP_UBWC_RGBA8_UHD] = {
        ...
        .cInputPath = INPUT_FILEPATH_TEMPLATE,
        .cInputName = "CLIP_UBWC_RGBA8_UHD.yuv",
        .eInputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eInputFileType = FILE_TYPE_MULTI_FRAMES,
        .eInputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
        .eBufferType = eVppBufType_Progressive,
        ...
        .cOutputPath = OUTPUT_FILEPATH_TEMPLATE,
        .cOutputName = "out_CLIP_UBWC_RGBA8_UHD.yuv",
        .eOutputFileFormat = FILE_FORMAT_RGBA8_UBWC,
        .eOutputFileType = FILE_TYPE_MULTI_FRAMES,
        .eOutputBufFmt = VPP_COLOR_FORMAT_UBWC_RGBA8,
    },
...
#define MINIMUM_EXTRADATA_SIZE_MSM (sizeof(struct msm_vidc_extradata_header) + \
                                    ...
                                    sizeof(struct msm_vidc_frame_qp_payload))
...
static void allocated_fd(int fd)
{
    if (fd < NUM_FD_MAX)
    {
        pthread_mutex_lock(&bufPoolStatsMutex);
        stBufPoolStats.au8FdAlloc[fd]++;
        pthread_mutex_unlock(&bufPoolStatsMutex);
    }
}
...
static void buf_pool_stats_increase_total_allocated(uint64_t addition)
{
    pthread_mutex_lock(&bufPoolStatsMutex);
    stBufPoolStats.u64TotalAllocated += addition;
    pthread_mutex_unlock(&bufPoolStatsMutex);
}

static void buf_pool_stats_increment_alloc_count()
{
    pthread_mutex_lock(&bufPoolStatsMutex);
...
static void buf_pool_stats_decrement_alloc_count()
{
    pthread_mutex_lock(&bufPoolStatsMutex);
    stBufPoolStats.u32AllocCnt--;
    pthread_mutex_unlock(&bufPoolStatsMutex);
...
static uint32_t buf_pool_alloc_ion_metadata(t_StVppCtx *pstCtx,
                                            ...
                                            t_StVppIonBuf *pstIonBuf)
{
    ...
    VPP_RET_IF_NULL(pstIonBuf, VPP_ERR_PARAM);
    ...
    LOGI("allocating gralloc %s ion mem fd=%d addr=%p sz=%u",
                                   ...
                                   t_StVppMemBuf *pstIntMemBuf)
{
    ...
    buf_pool_stats_increase_total_allocated(u32Sz);
    ...
    buf_pool_stats_increment_alloc_count();
...
static uint32_t fill_gralloc_data(struct bufnode *node)
{
    uint32_t bIsHdr, u32;
    ...
    u32 = u32VppTestMetaData_GetDynamicRange(&node->pPool->params.stMetaData, &bIsHdr);
    if (u32 != VPP_OK)
    {
        bIsHdr = VPP_FALSE;
        LOGI("u32VppTestSvc_GetDynamicRange failed u32=%u", u32);
    }

    u32 = u32VppTestMetaData_SetColorPrimaries(node->pIntBuf, bIsHdr);
    LOGE_IF(u32 != VPP_OK, "u32VppTestMetaData_SetColorPrimaries failed u32=%u", u32);
...
void dump_buf_stats()
{
    ...
    LOGD("- total allocated memory: %" PRIu64, stBufPoolStats.u64TotalAllocated);
                                 ...
                                 struct vpp_port_param *port_param)
{
    ...
    switch (port_param->fmt)
    {
        ...
        case VPP_COLOR_FORMAT_RGBA8:
            eBufFmt = FILE_FORMAT_RGBA8;
            break;
        case VPP_COLOR_FORMAT_UBWC_RGBA8:
            eBufFmt = FILE_FORMAT_RGBA8_UBWC;
            break;
        case VPP_COLOR_FORMAT_UBWC_RGB565:
            eBufFmt = FILE_FORMAT_RGB565_UBWC;
            break;
        default:
                       ...
                       uint32_t bInitIon)
{
    ...
    *ppool = pool;
    ...
    pool->pstIonMemMd = calloc(u32Cnt, sizeof(t_StVppIonBuf));

    if (!pool->pstBuf || !pool->nodes || !pool->vpp_buffers ||
        !pool->pstIonMemPx || !pool->pstIonMemEx || !pool->pstIonMemMd)
    {
        ...
        if (pool->pstIonMemMd) free(pool->pstIonMemMd);
        if (pool) free(pool);
        return VPP_ERR_NO_MEM;
    }
    ...
    uint32_t u32BufSz, u32ExBufSz, u32MbiBufSz, u32Tmp;
    u32BufSz = u32VppUtils_GetBufferSize(&pool->stInputPort);
    u32Tmp = u32VppUtils_GetBufferSize(&pool->stOutputPort);
    if (u32Tmp > u32BufSz)
        u32BufSz = u32Tmp;

    u32ExBufSz = u32VppUtils_GetExtraDataBufferSize(&pool->stInputPort);
    u32Tmp = u32VppUtils_GetExtraDataBufferSize(&pool->stOutputPort);
    if (u32Tmp > u32ExBufSz)
        u32ExBufSz = u32Tmp;

    u32MbiBufSz = (u32VppUtils_GetMbiSize(pool->stInputPort.width, pool->stInputPort.height) * 2) + 8192;
    u32Tmp = (u32VppUtils_GetMbiSize(pool->stOutputPort.width, pool->stOutputPort.height) * 2) + 8192;
    if (u32Tmp > u32MbiBufSz)
        u32MbiBufSz = u32Tmp;
    ...
    for (i = 0; i < u32Cnt; i++)
    {
        ...
        if (!pool->params.bDisableExtraData)
        {
            u32 = buf_pool_alloc_ion(pool->pstCtx,
        ...
        if (!pool->params.bDisableGralloc)
        {
            uint32_t u32MetadataSz = u32VppBuf_GrallocMetaDataSize();
            u32 = buf_pool_alloc_ion_metadata(pool->pstCtx,
                                              PROTECTION_ZONE_NONSECURE,
                                              u32MetadataSz,
                                              &pool->pstIonMemMd[i]);
            if (u32 == VPP_OK)
            {
                void *pvGr = vpVppTestGralloc_Alloc(pool->vpp_buffers[i].pixel.fd,
                                                    pool->pstIonMemMd[i].fd_ion_mem);
                if (!pvGr)
                {
                   LOGE("u32VppTestGralloc_Alloc failed u32=%u", u32);
                   u32VppIon_Free(pool->pstCtx, &pool->pstIonMemMd[i]);
                }
                else
                {
                    pool->vpp_buffers[i].pvGralloc = pvGr;
                    buf_pool_stats_increase_total_allocated(u32MetadataSz);
                    allocated_fd(pool->pstIonMemMd[i].fd_ion_mem);
                    buf_pool_stats_increment_alloc_count();
                }
            }
            else
            {
                LOGE("Error: unable to allocate gr memory, i=%u, u32=%u", i, u32);
            }
        }
    }
    ...
    u32 = u32VppTestMetaData_Init(&pool->params.stMetaData, pool->params.cInputPath);
    LOGE_IF(u32 != VPP_OK, "u32VppTestMetaData_Init failed u32=%u", u32);
...
void free_buf_pool(struct buf_pool *pool, uint32_t bTermIon)
{
    ...
    for (i = 0; i < pool->u32Cnt; i++)
    {
        ...
        buf_pool_stats_decrement_alloc_count();
        ...
        if (!pool->params.bDisableGralloc)
        {
            LOGI("freeing ion mem fd=%d", pool->pstIonMemMd[i].fd_ion_mem);
            freed_fd(pool->pstIonMemMd[i].fd_ion_mem);
            u32VppIon_Free(pool->pstCtx, &pool->pstIonMemMd[i]);
            buf_pool_stats_decrement_alloc_count();
            u32VppTestGralloc_Free(pool->vpp_buffers[i].pvGralloc);
            pool->vpp_buffers[i].pvGralloc = NULL;
        }
    }
    ...
    vVppTestMetaData_Term(&pool->params.stMetaData);
...
uint32_t fill_buf(struct bufnode *node)
{
    ...
    if (!((pool->params.eInputFileFormat == FILE_FORMAT_NV12 &&
          ...
          (pool->params.eInputFileFormat == FILE_FORMAT_RGBA8 &&
           pool->params.eInputBufFmt == VPP_COLOR_FORMAT_RGBA8) ||
          (pool->params.eInputFileFormat == FILE_FORMAT_BGRA8 &&
           pool->params.eInputBufFmt == VPP_COLOR_FORMAT_BGRA8) ||
          (pool->params.eInputFileFormat == FILE_FORMAT_RGBA8_UBWC &&
           pool->params.eInputBufFmt == VPP_COLOR_FORMAT_UBWC_RGBA8) ||
          (pool->params.eInputFileFormat == FILE_FORMAT_BGRA8_UBWC &&
           pool->params.eInputBufFmt == VPP_COLOR_FORMAT_UBWC_BGRA8) ||
          (pool->params.eInputFileFormat == FILE_FORMAT_RGB565_UBWC &&
           pool->params.eInputBufFmt == VPP_COLOR_FORMAT_UBWC_RGB565) ||
          (pool->params.eInputFileFormat == FILE_FORMAT_BGR565_UBWC &&
           pool->params.eInputBufFmt == VPP_COLOR_FORMAT_UBWC_BGR565)))
    {
         ...
         pool->params.eInputFileFormat == FILE_FORMAT_BGRA8))
    {
        ...
        if (pool->params.eInputBufFmt == VPP_COLOR_FORMAT_P010)
            u32Bpc = 2;
        if (pool->params.eInputBufFmt == VPP_COLOR_FORMAT_RGBA8 ||
            pool->params.eInputBufFmt == VPP_COLOR_FORMAT_BGRA8)
            u32Bpc = 4;

        // unpadded plane and frame sizes
        u32YSz = pool->params.u32Width * pool->params.u32Height * u32Bpc;
        if (u32VppUtils_IsFmtRgb(pool->params.eInputBufFmt))
            u32UVSz = 0;
        else
            u32UVSz = u32YSz / 2;
...
uint32_t dump_buf(struct bufnode *node)
{
    ...
    if (property_get_bool(VPP_TEST_PROPERTY_BUF_DUMP_DISABLE, VPP_FALSE))
    {
        LOGI("%s is true... not dumping buffer", VPP_TEST_PROPERTY_BUF_DUMP_DISABLE);
        return VPP_OK;
    }
    ...
    if (pool->params.eOutputFileType != FILE_TYPE_NONE)
    {
        ...
        if (!((pool->params.eOutputFileFormat == FILE_FORMAT_NV12 &&
              ...
              (pool->params.eOutputFileFormat == FILE_FORMAT_P010 &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_P010) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_NV12_UBWC &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_NV12) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_TP10_UBWC &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_TP10) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_RGBA8 &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_RGBA8) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_BGRA8 &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_BGRA8) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_RGBA8_UBWC &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_RGBA8) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_BGRA8_UBWC &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_BGRA8) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_RGB565_UBWC &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_RGB565) ||
              (pool->params.eOutputFileFormat == FILE_FORMAT_BGR565_UBWC &&
               pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_BGR565)))
            {
            LOGE("Error: Invalid colour formats for write, file_fmt=%u, buf_fmt=%u",
                 pool->params.eOutputFileFormat, pool->params.eOutputBufFmt);
        }
        ...
        switch (node->pIntBuf->eBufPxType)
        {
            ...
            {
                u32Stride = u32VppUtils_GetStride(&pool->stOutputPort);

                LOGI("writing YUV frame: %s --> base=%p, virt=%p", cName, base, virt);

                if ((pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_NV12_VENUS &&
                     pool->params.eOutputFileFormat == FILE_FORMAT_NV12) ||
                    (pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_P010 &&
                     pool->params.eOutputFileFormat == FILE_FORMAT_P010) ||
                    (pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_RGBA8 &&
                     pool->params.eOutputFileFormat == FILE_FORMAT_RGBA8) ||
                     ...
                     pool->params.eOutputFileFormat == FILE_FORMAT_BGRA8))
                {
                    ...
                    u32Bpc = 1;
                    if (pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_P010)
                        u32Bpc = 2;
                    else if (pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_RGBA8 ||
                             pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_BGRA8)
                        u32Bpc = 4;
                    ...
                    if (pool->params.u32Width == u32Stride)
                    {
                        ...
                        if (!u32VppUtils_IsFmtRgb(pool->params.eOutputBufFmt))
                        {
                            // Venus has alignment restrictions
                            virt = virt + u32VppUtils_GetUVOffset(&pool->stOutputPort);
...
#ifdef DUMP_PADDED
                    {
                        ...
                        if (!u32VppUtils_IsFmtRgb(pool->params.eOutputBufFmt))
                        {
                            virt = base + u32VppUtils_GetUVOffset(&pool->stOutputPort);
                            for (i = 0; i < (pool->params.u32Height) >> 1; i++)
                            {
                                u32 = fwrite(virt, sizeof(char), u32Stride, fd);
                                if (u32 != u32Stride)
...
#else
                    {
                        ...
                        LOGI("dump_buf: y=%p, uv=%p", base, base + u32VppUtils_GetUVOffset(&pool->stOutputPort));
                ...
                else if ((pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_NV12 &&
                          pool->params.eOutputFileFormat == FILE_FORMAT_NV12_UBWC) ||
                         (pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_TP10 &&
                          pool->params.eOutputFileFormat == FILE_FORMAT_TP10_UBWC) ||
                         (pool->params.eOutputBufFmt == VPP_COLOR_FORMAT_UBWC_RGBA8 &&
...
uint32_t write_extradata_buf_omx(void *pv, uint32_t u32Ft, uint32_t u32Marker,
                                 uint32_t u32Sz, t_StMetaData *pstMetadata)
{
    ...
    if (pstMetadata)
    {
        struct OMX_QCOM_EXTRADATA_QP stOmxQp;
        t_StMetaDataQp stQpData;

        pHdr = (struct OMX_OTHER_EXTRADATATYPE *)((char *)pHdr + pHdr->nSize);

        uint32_t u32 = u32VppTestMetaData_GetQpData(pstMetadata, &stQpData);
        if (u32 == VPP_OK)
        {
            stOmxQp.nQP = 0;
            stOmxQp.nQPSum = stQpData.u32QPSum;
            stOmxQp.nTotalNumBlocks = stQpData.u32QPSumCnt;
            stOmxQp.nSkipQPSum = stQpData.u32SkipQPSum;
            stOmxQp.nSkipNumBlocks = stQpData.u32SkipQPSumCnt;

            pHdr->nSize = sizeof(struct OMX_OTHER_EXTRADATATYPE) +
                sizeof(struct OMX_QCOM_EXTRADATA_QP);

            pHdr->eType = (OMX_EXTRADATATYPE)OMX_ExtraDataQP;

            pHdr->nDataSize = sizeof(struct OMX_QCOM_EXTRADATA_QP);
            memcpy(pHdr->data, &stOmxQp, sizeof(struct OMX_QCOM_EXTRADATA_QP));
...
uint32_t fill_extra_data_int_buf_omx(t_StVppBuf *pstIntBuf, uint32_t u32Ft,
                                     uint32_t u32Marker, t_StMetaData *pstMetadata)
{
    ...
    u32FillLen = write_extradata_buf_omx(pvDst, u32Ft, u32Marker, u32Valid, pstMetadata);
                                 ...
                                 uint32_t u32Sz, t_StMetaData *pstMetadata)
{
    ...
    if (pstMetadata)
    {
        struct msm_vidc_frame_qp_payload stMsmQp;
        t_StMetaDataQp stQpData;
        ...
        {
            stMsmQp.frame_qp = 0;
            stMsmQp.qp_sum = stQpData.u32QPSum;
            stMsmQp.total_num_blocks = stQpData.u32QPSumCnt;
            stMsmQp.skip_qp_sum = stQpData.u32SkipQPSum;
            stMsmQp.skip_num_blocks = stQpData.u32SkipQPSumCnt;

            pHdr->size = sizeof(struct msm_vidc_extradata_header) +
                sizeof(struct msm_vidc_frame_qp_payload);

            pHdr->type = MSM_VIDC_EXTRADATA_FRAME_QP;

            pHdr->data_size = sizeof(struct msm_vidc_frame_qp_payload);
            memcpy(pHdr->data, &stMsmQp, sizeof(struct msm_vidc_frame_qp_payload));
                                     ...
                                     uint32_t u32Marker, t_StMetaData *pstMetadata)
{
    ...
    void *pvDst = (char *)pstIntBuf->stExtra.pvBase;
    u32FillLen = write_extradata_buf_msm(pvDst, u32Ft, u32Marker, u32Valid, pstMetadata);
                                 ...
                                 uint32_t u32Marker, t_StMetaData *pstMetadata)
{
    ...
    if (eFormat == EXTRADATA_BUFFER_FORMAT_MSM)
        return fill_extra_data_int_buf_msm(pstIntBuf, u32Ft, u32Marker, pstMetadata);
#ifndef OMX_EXTRADATA_DOES_NOT_EXIST
    else if (eFormat == EXTRADATA_BUFFER_FORMAT_OMX)
        return fill_extra_data_int_buf_omx(pstIntBuf, u32Ft, u32Marker, pstMetadata);
#endif
    else
        return VPP_ERR;
}
...
{
    uint32_t u32 = VPP_OK;
    t_StMetaData *pstMetaData;

    if (!node)
        return VPP_ERR;

    pstMetaData = &node->pPool->params.stMetaData;
    u32 = u32VppTestMetaData_UpdateNextData(pstMetaData);
    LOGE_IF(u32 != VPP_OK, "u32VppTestSvc_ReadNextMetaData failed. u32=%u", u32);

    if (node->pIntBuf->pBuf->pvGralloc)
    {
        u32 = fill_gralloc_data(node);
        LOGE_IF(u32 != VPP_OK, "fill_gralloc_data failed. u32=%u", u32);
    }

    u32 = fill_extra_data_int_buf(node->pIntBuf, u32Ft, u32Marker, pstMetaData);
    if (u32 == VPP_OK)
                             ...
                             uint32_t u32AvailSz)
{
    ...
    if (eFormat == EXTRADATA_BUFFER_FORMAT_MSM)
        return write_extradata_buf_msm(pv, u32Ft, u32Marker, u32AvailSz, NULL);
#ifndef OMX_EXTRADATA_DOES_NOT_EXIST
    else if (eFormat == EXTRADATA_BUFFER_FORMAT_OMX)
        return write_extradata_buf_omx(pv, u32Ft, u32Marker, u32AvailSz, NULL);
#endif
    else
        return 0;
}
