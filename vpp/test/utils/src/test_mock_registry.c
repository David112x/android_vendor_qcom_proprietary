 ...
 * Local definitions
 ***********************************************************************/
#define MOCK_METADATA 0xABCDABCD
...
static t_EVppColorFmt color_fmt_gpu[] = {
    VPP_COLOR_FORMAT_NV12_VENUS,
};
        ...
        for (i = 0; i < pstUc->algos.u32CtrlCnt; i++)
        {
            ...
            switch (ctrl_type)
            {
                ...
                case HQV_CONTROL_FRC:
                    if (pstCtrl->frc.segments == NULL)
                    {
                        DVP_ASSERT_FAIL();
                        break;
                    }
                    pstCtrl->frc.num_segments = 1;
                    pstCtrl->frc.segments->mode = HQV_FRC_MODE_SLOMO;
                    pstCtrl->frc.segments->level = HQV_FRC_LEVEL_HIGH;
                    pstCtrl->frc.segments->interp = HQV_FRC_INTERP_2X;
