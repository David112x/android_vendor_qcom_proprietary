...
{
    int32_t idx;
    idx = vpp_queue_peek_front(&pstPort->stQ);
...
static uint32_t u32VppIpMock_NeedToBypassBuffer(t_StVppIpMockCb *pstCb,
                                                t_StVppBuf *pstBufIn)
{
    return (pstCb->u32OutputMin == 0 ||
            ...
            (!VPP_FLAG_IS_SET(pstCb->stCfg.u32EnableMask, MOCK_ALGO_DI) &&
             pstBufIn->eBufType != eVppBufType_Progressive) ||
...
{
    uint32_t u32BufSz, u32AvailSz;
    uint32_t i;
    struct vpp_mem_buffer *pstMemBuf;
    t_StVppBuf *pstBufIn = NULL;
    ...
    if (!u32VppIpMock_NeedToBypassBuffer(pstCb, pstBufIn))
    {
