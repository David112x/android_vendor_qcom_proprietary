 ...
 *
 * @cr
 * Copyright (c) 2015-2017, 2019-2020 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
uint64_t ts_get_next(struct ts_mgr *pTsMgr)
{
    ...
    if (pTsMgr->bEnableAsserts)
    {
        ...
        {
            if (pTsMgr->u64CurrentTs > UINT64_MAX -
                ((pTsMgr->u64TsStep * i) / (pTsMgr->u32OuttoInRatio)))
            {
                pthread_mutex_unlock(&pTsMgr->mutex);
                LOGE("Additional timestamp for buffer %u would overflow", i);
                return ERR_TS;
            }
...
uint32_t ts_set_next(struct ts_mgr *pTsMgr, uint64_t u64Timestamp)
{
    int32_t s32Idx = -1;
    ...
    pTsMgr->u64CurrentTs = u64Timestamp;
    LOGI("setting timestamp: ts=%"PRIu64"", pTsMgr->u64CurrentTs);
...
uint32_t ts_put_ts(struct ts_mgr *pTsMgr, uint64_t u64Ts)
{
    ...
    if (bEqual != VPP_TRUE)
    {
        u64Expected2 = pTsMgr->u64TsLastDequeue;
        if (u64Expected2 < UINT64_MAX)
            u64Expected2++;
        bEqual = u64Expected2 == u64Ts ? VPP_TRUE : VPP_FALSE;
        if (bEqual != VPP_TRUE)
        {
            s32Idx = vpp_queue_peek_front(&pTsMgr->stQ);
            if (s32Idx >= 0)
            {
                u64Peek = pTsMgr->pu64Ts[s32Idx];
                u64Expected2 = (pTsMgr->u64TsLastDequeue / 2) + (u64Peek / 2);
                bEqual = u64Expected2 == u64Ts ? VPP_TRUE : VPP_FALSE;
            }
        }
    }
                          ...
                          uint32_t height, enum vpp_color_format fmt)
{
    ...
    set_port_params(&pCtx->port_param_out, width, height, fmt);
}
