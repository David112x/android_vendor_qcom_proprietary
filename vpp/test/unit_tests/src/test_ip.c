...
TEST(IpQueueFuzz)
{
    ...
    u32 = u32VppIp_CmdPeekBack(NULL, &stCmd);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
    u32 = u32VppIp_CmdPeekBack(&stQ, NULL);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
    u32 = u32VppIp_CmdPeekBack(NULL, NULL);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
...
TEST(IpQueuePeekBack)
{
    ...
    u32 = u32VppIp_CmdPeekBack(&stQ, &stCmd);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
    ...
    for (i = 0; i < u32Sz; i++)
    {
        ...
        u32 = u32VppIp_CmdPeekBack(&stQ, &stCmd);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
        DVP_ASSERT_EQUAL(aeCmd[i], stCmd.eCmd);
    }

    // After a rejected CmdPut, CmdPeekBack still works.
    stCmd.eCmd = aeCmd[0];
    u32 = u32VppIp_CmdPut(&stQ, stCmd);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
    ...
    u32 = u32VppIp_CmdPeekBack(&stQ, &stCmd);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
    DVP_ASSERT_EQUAL(aeCmd[u32Sz - 1], stCmd.eCmd);

    // Pull from queue. The CmdPeekBack() command should stay the same, since
    // Get() gets from the front of the queue.
    for (i = 0; i < u32Sz; i++)
    {
        u32 = u32VppIp_CmdPeekBack(&stQ, &stCmd);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
    ...
    // After dequeueing all commands, CmdPeekBack() should fail because nothing
    // left in queue.
    u32 = u32VppIp_CmdPeekBack(&stQ, &stCmd);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
...
TEST(IpPortCondSignal)
{
    ...
    u32 = u32VppIp_PortInit(&stCb.stPort);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
...
ERR_THREAD_CREATE:
    ...
    u32 = u32VppIp_PortTerm(&stCb.stPort);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
}
/************************************************************************
 * Global Functions
