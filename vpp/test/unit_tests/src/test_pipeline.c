                                  ...
                                  struct pipeline_test_ctx *pstPlCtx)
{
    ...
    pstPlCtx->stCtrl.frc.num_segments = 1;
    pstPlCtx->stCtrl.frc.segments = &pstPlCtx->stFrcCtrlSeg;
...
TEST(Pipeline_ErrorIpSubmodOpen)
{
    /*!
     * If there is an issue with initalizing the IP block, the open will fail
     * and pipeline will not accept any buffers.
     */
    uint32_t u32;
    struct test_ctx tctx, *pstCtx;
    struct pipeline_test_ctx pltctx, *pstPlCtx;
    ...
    if (pNode)
    {
        fill_buf(pNode);
        pNode->port_owner = VPP_PORT_INPUT;
        u32 = u32VppPipeline_QueueBuf(&pstCtx->stVppCtx, pNode->port_owner,
                                      pNode->pIntBuf);
...
void do_pipeline_error_ip_cnt(enum test_suite ts)
{
    uint32_t u32;
    struct test_ctx tctx, *pstCtx;
    struct pipeline_test_ctx pltctx, *pstPlCtx;
...
TEST(Pipeline_ErrorIpCntIsZero)
{
    do_pipeline_error_ip_cnt(TEST_SUITE_PIPELINE_UC_COMP_IS_ZERO);
}
...
TEST_CASES PipelineTests[] = {
    ...
    TEST_CASE(Pipeline_ErrorIpSubmodOpen),
    TEST_CASE(Pipeline_ErrorIpCntIsZero),
#endif
#if TEST_PL_FLUSH
