...
static enum vpp_color_format eVppTest_FindSupportedFormat(uint64_t u64ColorFmtMask)
{
    uint32_t i;
    enum vpp_color_format eFmt = VPP_COLOR_FORMAT_MAX;

    for (i = 0; i < VPP_COLOR_FORMAT_MAX; i++)
    {
        if (u64ColorFmtMask & (0x1 << i))
        {
            eFmt = i;
            break;
        }
    }
    LOGI("Returning color format eFmt=%d", eFmt);
    return eFmt;
}
...
TEST(Vpp_OpenCloseSingle)
{
    ...
    // Should fail, no valid use-case available
    u32 = vpp_open(p);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);

    // Set to auto
    ctrl.mode = HQV_MODE_AUTO;
    u32 = vpp_set_ctrl(p, ctrl);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
    // Set valid input & output params
    param.width = 1920;
    param.height = 1080;
    ...
    // Open the vpp session
    u32 = vpp_open(p);
    DVP_ASSERT_EQUAL(u32, VPP_OK);

    // Try to open the vpp session again, should fail
    u32 = vpp_open(p);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
...
TEST(Vpp_OpenCloseMultiple)
{
    ...
    for (i = 0; i < MAX_CONCURRENT_SESSIONS; i++)
    {
        ...
        // Set to auto
        ctrl.mode = HQV_MODE_AUTO;
        u32 = vpp_set_ctrl(apvCtx[i], ctrl);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
        // Set valid input & output params
        u32 = vpp_set_parameter(apvCtx[i], VPP_PORT_INPUT, param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
        u32 = vpp_set_parameter(apvCtx[i], VPP_PORT_OUTPUT, param);
        DVP_ASSERT_EQUAL(u32, VPP_OK);

        // Open the vpp session
        u32 = vpp_open(apvCtx[i]);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
    }
    ...
    {
        uint32_t j;
        for (j = i + 1; j < MAX_CONCURRENT_SESSIONS; j++)
            DVP_ASSERT_PTR_NEQUAL(apvCtx[i], apvCtx[j]);
    }

    for (i = 0; i < MAX_CONCURRENT_SESSIONS; i++)
    {
        // Close all the vpp sessions
        u32 = vpp_close(apvCtx[i]);
        DVP_ASSERT_EQUAL(u32, VPP_OK);
...
TEST(Vpp_SetCtrl)
{
    ...
    u32 = vpp_set_ctrl(p, ctrl);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
    u32 = vpp_get_buf_requirements(p, &req);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
...
TEST(Vpp_SetParameter)
{
    ...
    // No valid hqv controls are set here, should return error
    u32 = vpp_open(p);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
    ...
    // This call should fail as well, vpp not in active state
    u32 = vpp_close(p);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
    vpp_term(p);

    // Reset and try again, set valid hqv controls
    p = vpp_init(0, sCb);
    DVP_ASSERT_PTR_NNULL(p);
    // Set to auto mode
    ctrl.mode = HQV_MODE_AUTO;
...
TEST(Vpp_GetBufRequirements)
{
    ...
    u32 = vpp_get_buf_requirements(NULL, &req);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);

    u32 = vpp_get_buf_requirements(NULL, NULL);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);

    u32 = vpp_get_buf_requirements(p, NULL);
    DVP_ASSERT_NEQUAL(u32, VPP_OK);
    ...
    // Open vpp and try to get the buffer requirements, should return VPP_OK
    u32 = vpp_open(p);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
    u32 = vpp_get_buf_requirements(p, &req);
    DVP_ASSERT_EQUAL(u32, VPP_OK);

    // Close vpp and try to get the buffer requirements, should return VPP_OK
    u32 = vpp_close(p);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
    u32 = vpp_get_buf_requirements(p, &req);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
...
TEST(Vpp_Timestamp)
{
    ...
    param.fmt = eVppTest_FindSupportedFormat(req.in_color_fmt_mask);
    if (param.fmt >= VPP_COLOR_FORMAT_MAX)
    {
        LOGE("No valid color format found, in_color_fmt_mask=%"PRIu64, req.in_color_fmt_mask);
        DVP_ASSERT_FAIL();
    }
...
TEST(Vpp_ReconfigBasic)
{
    ...
    u32MaxRes = u32VppUsecase_GetMaxResForUc(pstUcAuto);

    struct vpp_port_param param = {
        .width = u32GetMaxWidthForRes(u32MaxRes),
        .height = u32GetMaxHeightForRes(u32MaxRes),
    };
    ...
    param.width = u32GetMaxWidthForRes(u32MaxRes ? u32MaxRes - 1 : u32MaxRes);
    param.height = u32GetMaxHeightForRes (u32MaxRes ? u32MaxRes - 1 : u32MaxRes);
    param.stride = u32VppUtils_CalculateStride(param.width, param.fmt);
    param.scanlines = u32VppUtils_CalculateScanlines(param.height, param.fmt);
    u32 = vpp_reconfigure(p, param, param);
