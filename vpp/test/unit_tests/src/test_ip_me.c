...
#define INP_DIR_PATH            "/data/vendor/vpp/input/"
#define OUT_DIR_PATH            "/data/vendor/vpp/output/"
                              ...
                              const char *pcOutFile)
{
    ...
    uint32_t u32Ret = VPP_OK;
    const uint32_t u32AdditionalInpBufs = 5;
    ...
    u32Ret = init_buf_pool(&stTctx.stVppCtx, &stTctx.buf_pool, &stTctx.params,
                           stInputBufReq.u32MinCnt + u32AdditionalInpBufs, VPP_TRUE);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
...
TEST(ME_TestApi)
{
    ...
    stHqvCtrl.frc.segments = &stFrcCtrlSeg;
    stHqvCtrl.frc.segments->mode = HQV_FRC_MODE_SLOMO;
    stCtxMe.pvME = NULL;
...
TEST(ME_QueueMaxAndBypass)
{
    ...
    // Queue only the input buffers so that the "TME input buffer queue" fills up
    // to its max limit, then queue an additional buffer with the bypass flag set.
    while (u32InFramesQueued < (stInputBufReq.u32MinCnt + 1))
    {
        uint32_t u32Flags = 0;
        if (u32InFramesQueued == stInputBufReq.u32MinCnt)
            u32Flags |= VPP_BUF_FLAG_BYPASS;
    ...
    // Next queue input and output buffers normally, expect to get all the buffers back from ME
    while ((u32InFramesQueued < u32InFramesToRun) || (stTctx.u32InRxCnt < u32InFramesToRun))
    {
        // Queue output buf
        if ((stTctx.buf_pool_ext->u32ListSz) && (stTctx.u32InRxCnt < u32InFramesToRun))
...
TEST(ME_ReverseTimestamps)
{
    uint32_t u32Ret = VPP_OK;
    uint32_t u32InFramesToRun, u32InFrameId, u32OutFramesQueued = 0;

    u32InFrameId = u32InFramesToRun = 20;
    ...
    while (stTctx.u32InRxCnt < u32InFramesToRun)
    {
        ...
        // Queue input buf
        if (stTctx.buf_pool->u32ListSz && (u32InFrameId > 0))
        {
            uint32_t u32Flags = 0;
            if (u32InFrameId == 1)
                u32Flags |= VPP_BUFFER_FLAG_EOS;

            u32Ret = u32QueueInputBuf(u32InFrameId, u32Flags);
            DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
...
TEST_CASES IpMeTests[] = {
    ...
    TEST_CASE(ME_QueueMaxAndBypass),
    TEST_CASE(ME_ReverseTimestamps),
    TEST_CASE_NULL(),
};
