...
static uint32_t bAieWouldBypass(struct hcp_knobs stKnobs) {
    return (stKnobs.u32AieMode == HQV_MODE_OFF ||
            (stKnobs.u32AieMode == HQV_MODE_MANUAL &&
...
static uint32_t getDriverAieMode(enum hqv_mode mode) {
    return (mode == HQV_MODE_AUTO) ? LTM_MODE_AUTO : LTM_MODE_MANUAL;
}
        ...
        case HCP_OP_MODE_PROC:
            if (stCtrl.mode == HQV_MODE_AUTO)
            {
                DVP_ASSERT_EQUAL(pstSessCb->stDrvCtx.u32EarMode,
                                 HCP_EAR_MODE_DFLT);
                DVP_ASSERT_EQUAL(pstSessCb->stDrvCtx.u32AieMode,
                                 HQV_MODE_AUTO);
            }
            else
            {
                                          ...
                                          uint32_t u32NumBufs)
{
    ...
    vHcpIpTest_ClearTestCtxBufCnt(pstTestCtx, &pstTestCtx->mutex);
}

static void vHcpIpTest_TestDrainBypassBuffers(struct test_ctx *pstTestCtx,
                                              uint32_t u32NumBypassBuffers,
                                              uint32_t u32NumInputBuffers)
{
    /*
     * This test queues input buffers, then some input buffers with its bypass
     * flag set, calls Drain(), then queues output buffers to match the input
     * buffers that are not bypassed. This is to verify that Drain works when
     * bypass buffers are in the buffer queue.
     */
    uint32_t u32Ret;
    uint32_t u32NumOutputBuffers = u32NumInputBuffers;
    uint32_t u32NumBuffers = u32NumInputBuffers + u32NumBypassBuffers +
                             u32NumOutputBuffers;
    t_StVppIpHcpSessionCb *pstSessCb;
    struct hcp_test_ctx *pstCtxHcp = HCPIPTEST_HCP_TEST_CTX(pstTestCtx);

    vHcpIpTest_TestSetup(pstTestCtx, u32NumBuffers);
    pstSessCb = pstCtxHcp->pstHcpCb;

    pstCtxHcp->u32NumExpectedBypassBufs = u32NumBypassBuffers;

    // These non-bypass input buffers will be queued first, which will cause all
    // of the next bypass buffers to stall since we haven't queued an output
    // buffer yet.
    for (uint32_t u32 = 0; u32 < u32NumInputBuffers; u32++)
    {
        u32Ret = u32HcpIpTest_QueueBuf(pstTestCtx, VPP_PORT_INPUT,
                                       eVppBufType_Progressive);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    }

    // Queue bypass input buffers.
    for (uint32_t u32 = 0; u32 < u32NumBypassBuffers; u32++)
    {
        u32Ret = u32HcpIpTest_QueueBufCustomFlag(pstTestCtx, VPP_PORT_INPUT,
                                                 eVppBufType_Progressive,
                                                 VPP_BUF_FLAG_BYPASS);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    }
    ...
    // Start Drain.
    u32Ret = u32VppIpHcp_Drain((void *)pstSessCb);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);

    // Queue output buffers, to clear the input queue of the stalling input
    // buffers.
    for (uint32_t u32 = 0; u32 < u32NumOutputBuffers; u32++)
    {
        u32Ret = u32HcpIpTest_QueueBuf(pstTestCtx, VPP_PORT_OUTPUT,
                                       eVppBufType_Progressive);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    }

    // Should NOT hang.
    vHcpIpTest_WaitForDrainDone(pstTestCtx);
...
static uint32_t bHcpIpTest_IsCustTuningEn(t_StVppIpHcpGlobalCb *pstGlobal,
                                                 ...
                                                 struct hcp_knobs stKnobs)
{
    ...
    DVP_ASSERT_EQUAL(getDriverAieMode(stKnobs.u32AieMode),
                     pstSessCb->stDrvCtx.u32AieMode);

    if (stKnobs.u32AieMode == HQV_MODE_MANUAL)
    {
...
TEST(HcpSetCtrlCustom)
{
    ...
    // Invalid len of custom data (greater than HQV_CUSTOM_PAYLOAD_SZ)
    stCtrl.mode = HQV_MODE_MANUAL;
    stCtrl.ctrl_type = HQV_CONTROL_CUST;
    stCtrl.custom.len = HQV_CUSTOM_PAYLOAD_SZ + 1;
    u32 = u32VppIpHcp_SetCtrl(pv, stCtrl);
    DVP_ASSERT_EQUAL(u32, VPP_ERR_PARAM);

    // Invalid len of custom data (not multiple of 4)
    memset(&stCtrl, 0, sizeof(stCtrl));
    stCtrl.mode = HQV_MODE_MANUAL;
    stCtrl.ctrl_type = HQV_CONTROL_CUST;
    stCtrl.custom.len = 6;
    ...
    // Valid custom data (actual data can be random)
    memset(&stCtrl, 0, sizeof(stCtrl));
    stCtrl.mode = HQV_MODE_MANUAL;
    stCtrl.ctrl_type = HQV_CONTROL_CUST;
    stCtrl.custom.len = 20;
    u32 = u32VppIpHcp_SetCtrl(pv, stCtrl);

    pstSessCb = (t_StVppIpHcpSessionCb *)pv;
    bCustSdrLutEn = bHcpIpTest_IsCustTuningEn(HCP_GLOB_SESS(pstSessCb),
                                              LTMTUNE_NCUST_SDR_LUT_EN);
    bCustHdrLutEn = bHcpIpTest_IsCustTuningEn(HCP_GLOB_SESS(pstSessCb),
                                              LTMTUNE_NCUST_HDR_LUT_EN);
    if (!bCustSdrLutEn && !bCustHdrLutEn)
    {
        // Error because the tunings are not set
        DVP_ASSERT_EQUAL(u32, VPP_ERR_PARAM);
    }
    else
    {
...
TEST(HcpValidateAieModes)
{
    struct hqv_control stCtrl;
    struct vpp_port_param port;
    uint32_t u32Cnt, u32, u32Looper;
    const uint32_t u32LoopCnt = 10;
    t_StVppIpHcpSessionCb *pstSessCb;
    struct test_ctx *pstTestCtx = &stTestCtx;
    t_StVppCtx stCtx;

    // AIE mode transition list based on indexing of stAieKnobs[]
    uint32_t au32KnobTestList[] = { 0, 1, 2, 1, 0, 2, 0 };

    struct hcp_knobs astAieKnobs[] = {
        // AIE auto mode
        {HQV_EAR_MODE_OFF, HQV_MODE_AUTO, 0, 0, 0, 0, 0, 0},
        // AIE manual mode
        {HQV_EAR_MODE_OFF, HQV_MODE_MANUAL, 30, 40, 60, 70, 80, 90},
        // AIE bypass mode
        {HQV_EAR_MODE_OFF, HQV_MODE_MANUAL, HCP_DE_GAIN_OFF, HCP_SAT_GAIN_OFF,
         HCP_SAT_OFFSET_OFF, HCP_ACE_STR_OFF, HCP_ACE_BRI_L_OFF, HCP_ACE_BRI_H_OFF}
    };
    ...
    vHcpIpTest_TestSetupSetParam(pstTestCtx, 2);
    pstSessCb = HCPIPTEST_HCP_TEST_CTX(pstTestCtx)->pstHcpCb;
    ...
    u32 = u32VppIpHcp_SetParam(pstSessCb, VPP_PORT_INPUT, port);
    DVP_ASSERT_EQUAL(u32, VPP_OK);

    u32 = u32VppIpHcp_SetParam(pstSessCb, VPP_PORT_OUTPUT, port);
    DVP_ASSERT_EQUAL(u32, VPP_OK);
    ...
    u32 = u32VppIpHcp_Open(pstSessCb);
    DVP_ASSERT_EQUAL(u32, VPP_OK);

    u32Cnt = sizeof(au32KnobTestList) / sizeof(au32KnobTestList[0]);

    for (u32Looper = 0; u32Looper < u32LoopCnt; u32Looper++)
    {
        for (uint32_t u32KnobCnt = 0; u32KnobCnt < u32Cnt; u32KnobCnt++)
        {
            uint32_t u32KnobIdx = au32KnobTestList[u32KnobCnt];

            memset(&stCtrl, 0, sizeof(stCtrl));
            stCtrl.mode = HQV_MODE_MANUAL;
            stCtrl.ctrl_type = HQV_CONTROL_AIE;
            stCtrl.aie.mode = astAieKnobs[u32KnobIdx].u32AieMode;
            stCtrl.aie.cade_level = astAieKnobs[u32KnobIdx].u32DeGain;
            stCtrl.aie.ltm_sat_gain = astAieKnobs[u32KnobIdx].u32SatGain;
            stCtrl.aie.ltm_sat_offset = astAieKnobs[u32KnobIdx].u32SatOff;
            stCtrl.aie.ltm_ace_str = astAieKnobs[u32KnobIdx].u32AceStrCon;
            stCtrl.aie.ltm_ace_bright_l = astAieKnobs[u32KnobIdx].u32AceBriL;
            stCtrl.aie.ltm_ace_bright_h = astAieKnobs[u32KnobIdx].u32AceBriH;
            u32 = u32VppIpHcp_SetCtrl(pstSessCb, stCtrl);
            DVP_ASSERT_EQUAL(u32, VPP_OK);

            HcpSetProcCtrlDuringActiveCheckKnobs(pstTestCtx, 1,
                                                 astAieKnobs[u32KnobIdx]);
        }
    }
...
TEST(HcpSetProcCtrlDuringActive)
{
    ...
    stCtrl.ctrl_type = HQV_CONTROL_AIE;
    stCtrl.aie.mode = stKnobs.u32AieMode;
...
TEST(HcpDrainWithBypass)
{
    /*
     * This test will send bypass buffers during a DRAIN. Because the input
     * buffers will be bypassed, they won't be sent to HCP FW and therefore
     * won't be returned from HCP FW via a message. This means the worker thread
     * needs some other means to wake up and clear the DRAIN state aside from
     * the message queue.
     */
    vHcpIpTest_TestDrainBypassBuffers(&stTestCtx, 4, 1);
}

TEST(HcpInternalDrainFullCmdQueue)
{
    /*
     * This test will create a scenario where INTERNAL_DRAIN commands will be
     * pushed into HCP IP's command queue multiple times, causing the command
     * queue to be full. Afterwards, any commands trying to be added to the
     * queue will fail until the command queue starts being emptied. The exact
     * scenario is to send some buffers to FW, then queue some bypass buffers.
     * These bypass buffers needs to wait until the FW is done with its current
     * buffers before the bypass buffers can be sent out. This waiting is done
     * via an INTERNAL_DRAIN command, which is queued to the command queue. If
     * the worker thread doesn't know that it already has queued an
     * INTERNAL_DRAIN command, it will keep queueing this command until the
     * command queue is full. Then, any command afterwards (ex. a Close())
     * should fail. Note that this issue is timing-related, so this test may not
     * catch the issue every time.
     */

    // Because this issue is timing-related, run the test multiple times.
    uint32_t u32NumIterations = 50;
    for (uint32_t u32 = 0; u32 < u32NumIterations; u32++)
    {
        // Setting u32NumBypassBuffers=1 because we only need 1 to queue
        // INTERNAL_DRAIN commands.
        // Setting u32NumInputBuffers=1 because we need at least 1 to stall the
        // bypass buffers and cause the worker thread to queue INTERNAL_DRAIN
        // commands.
        vHcpIpTest_TestDrainBypassBuffers(&stTestCtx, 1, 1);
    }
}
...
TEST(HcpBufferExchange)
{
    ...
    while (eOpMode != HCP_OP_MODE_MAX)
    {
        ...
        {
            if (u32VppUtils_IsFmtRgb(u32FmtCnt))
                continue;
...
TEST_CASES HcpTests[] = {
    ...
    TEST_CASE(HcpSetCtrlCustom),
    TEST_CASE(HcpValidateAieModes),
    ...
    TEST_CASE(HcpDrainWithBypass),
    TEST_CASE(HcpInternalDrainFullCmdQueue),
