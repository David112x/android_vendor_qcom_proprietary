...
// For enabling the debug logs
extern uint64_t u64LogLevel;
...
#define ENABLE_SUITE_C2D            1
#define ENABLE_SUITE_CONFIGSTORE    1
#define ENABLE_SUITE_TUNINGS        1

#if ENABLE_SUITE_QUEUE
TEST_SUITE_DECLARE(QueueSuite);
TEST_SUITE_DECLARE(DataQueueSuite);
#endif
...
#if ENABLE_SUITE_CONFIGSTORE
TEST_SUITE_DECLARE(ConfigStoreSuite);
#endif

#if ENABLE_SUITE_VPPUTILS
TEST_SUITE_DECLARE(TuningsSuite);
#endif
...
static void readProperties()
{
    ...
    u64LogLevel = strtoull(property_value, NULL, 0);

    // Log after setting global
    LOGD("%s=0x%llx", VPP_PROPERTY_LOG_CORE, (long long unsigned int)u64LogLevel);
}
...
int main(int argc, char **argv)
{
    ...
    TEST_SUITE_INSTALL(DataQueueSuite);
#endif
...
#if ENABLE_SUITE_ME
#ifdef VPP_TARGET_USES_FRC_ME
    TEST_SUITE_INSTALL(IpMeSuite);
#endif
#endif

#if ENABLE_SUITE_MC
#ifdef VPP_TARGET_USES_FRC_MC
...
#if ENABLE_SUITE_C2D
#ifdef VPP_TARGET_USES_C2D
    TEST_SUITE_INSTALL(IpC2DSuite);
#endif
#endif

#if ENABLE_SUITE_CONFIGSTORE
    TEST_SUITE_INSTALL(ConfigStoreSuite);
#endif

#if ENABLE_SUITE_TUNINGS
    TEST_SUITE_INSTALL(TuningsSuite);
#endif
