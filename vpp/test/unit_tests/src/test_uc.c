...
static void set_deinterlacer(t_StVppHqvCtrl *pstCtrl, uint32_t u32En)
{
    if (u32En)
    {
...
static void set_tnr(t_StVppHqvCtrl *pstCtrl, uint32_t u32En)
{
    if (u32En)
    {
        pstCtrl->mode = HQV_MODE_MANUAL;
...
static void set_aie(t_StVppHqvCtrl *pstCtrl, uint32_t u32En)
{
    if (u32En)
    {
        pstCtrl->mode = HQV_MODE_MANUAL;
...
static void set_cnr(t_StVppHqvCtrl *pstCtrl, uint32_t u32En)
{
    if (u32En)
    {
        pstCtrl->mode = HQV_MODE_MANUAL;
...
static void set_frc(t_StVppHqvCtrl *pstCtrl, uint32_t u32En)
{
    if (u32En)
    {
        pstCtrl->mode = HQV_MODE_MANUAL;
        pstCtrl->frc.num_segments = 1;
        pstCtrl->frc.segments->mode = HQV_FRC_MODE_SMOOTH_MOTION;
        pstCtrl->frc.segments->level = HQV_FRC_LEVEL_HIGH;
        pstCtrl->frc.segments->ts_start = 0;
        pstCtrl->frc.segments->frame_copy_input = 0;
        pstCtrl->frc.segments->frame_copy_on_fallback = 0;
    }
    else
    {
...
static uint32_t usecase_test_init(t_StVppHqvCtrl **stppCtrl1, t_StVppHqvCtrl **stppCtrl2,
                                  t_StVppParam **ppstVppParam1, t_StVppParam **ppstVppParam2)
{
    struct vpp_ctrl_frc_segment *pstFrcCtrlSeg1, *pstFrcCtrlSeg2;
    ...
    *ppstVppParam2 = calloc(sizeof(t_StVppParam), 1);
    pstFrcCtrlSeg1 = calloc(sizeof(struct vpp_ctrl_frc_segment), 1);
    pstFrcCtrlSeg2 = calloc(sizeof(struct vpp_ctrl_frc_segment), 1);

    if(!(*stppCtrl1) || !(*stppCtrl2) || !(*ppstVppParam1) || !(*ppstVppParam2) ||
       !(pstFrcCtrlSeg1) || !(pstFrcCtrlSeg2))
    {
        ...
        if (pstFrcCtrlSeg1) free(pstFrcCtrlSeg1);
        if (pstFrcCtrlSeg2) free(pstFrcCtrlSeg2);
    ...
    (*stppCtrl1)->frc.segments = pstFrcCtrlSeg1;
    (*stppCtrl2)->frc.segments = pstFrcCtrlSeg2;
...
static void usecase_test_deinit(t_StVppHqvCtrl **stppCtrl1, t_StVppHqvCtrl **stppCtrl2,
                                t_StVppParam **ppstVppParam1, t_StVppParam **ppstVppParam2)
{
    if (*stppCtrl1)
    {
        if ((*stppCtrl1)->frc.segments)
        {
            free((*stppCtrl1)->frc.segments);
        }
        free(*stppCtrl1);
        *stppCtrl1 = NULL;
    }
    if (*stppCtrl2)
    {
        if ((*stppCtrl2)->frc.segments)
        {
            free((*stppCtrl2)->frc.segments);
        }
        free(*stppCtrl2);
        *stppCtrl2 = NULL;
    }
...
static void usecase_test_set_resolution(t_StVppParam *pstVppParam, uint32_t u32W, uint32_t u32H)
{
    VPP_RET_VOID_IF_NULL(pstVppParam);

    pstVppParam->input_port.width = u32W;
    pstVppParam->input_port.height = u32H;
    pstVppParam->output_port.width = u32W;
    pstVppParam->output_port.height = u32H;
}
...
TEST(Usecase_Reconfigure)
{
    ...
    usecase_test_set_resolution(pstVppParam2, 3840, 2160);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc2, stpCtrl2, pstVppParam2);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
...
TEST(Usecase_Resolution)
{
    ...
    usecase_test_set_resolution(pstVppParam1, 4096, 2160);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    ...
    usecase_test_set_resolution(pstVppParam1, 4097, 2160);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    usecase_test_set_resolution(pstVppParam1, 4096, 2161);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    usecase_test_set_resolution(pstVppParam1, 5000, 3000);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    usecase_test_set_resolution(pstVppParam1, 96, 64);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    usecase_test_set_resolution(pstVppParam1, 95, 64);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    usecase_test_set_resolution(pstVppParam1, 96, 63);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    usecase_test_set_resolution(pstVppParam1, 2, 2);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    // Rotated max resolutions, should be accepted for both
    usecase_test_set_resolution(pstVppParam1, 2160, 4096);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc1, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc1, stpCtrl1, pstVppParam1);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    usecase_test_set_resolution(pstVppParam1, 1080, 1920);
    u32Ret = u32VppUsecase_Validate(NULL, pstUc2, pstVppParam1, &pstUcFallBack);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppUsecase_Configure(pstCtx1, pstUc2, stpCtrl1, pstVppParam1);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
