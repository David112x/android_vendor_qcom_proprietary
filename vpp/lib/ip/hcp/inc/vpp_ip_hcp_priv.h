...
#define HCP_DUMP_META_DATA_NM               "dbg_meta_%p.txt"

#define HCP_PROPERTY_FWLOGFLAGS             "vendor.media.vpp.hcp.logflags"
#define HCP_PROPERTY_DUMP_META_DATA         "vendor.media.vpp.hcp.dumpmeta"
...
#define HCP_FLAG_ALGO_AIE       BIT(eHcpFlag_AlgoAie)

#define HCP_FLAG_ALGO_CUST      BIT(eHcpFlag_AlgoCustom)
...
#define HCP_HNDL_SESS(_ps)      ((_ps)->hndl)
#define HCP_GLOB_SESS(_ps)      ((_ps)->pstGlobal)
...
// Maximum number of continuous hardware errors that can be received by
// the global receiver before tripping a hardware reset.
#define HCP_RX_MAX_ERR_CNT      2

// Number of times to try to load firmware
#define HCP_FW_LOAD_MAX_RETRY   10
...
#define HCP_STATS(_E) \
    ...
    _E(HCP_STAT_HW_SET_PROP_CUST_KNOBS, 1, 1) \
...
typedef enum {
    HCP_EVT_ID_PD_CRASHED = EVT_ID_CUSTOM_BASE,
} t_EVppIpHcpEvtId;
...
typedef struct {
    ...
    struct hqv_ctrl_custom stCtrl;
} t_StVppIpHcpCustCtrl;
