...
typedef union {
    struct {
        ...
        uint32_t bFwCritErr         : 1; // if firmware is irrecoverable
                                 ...
                                 const char *pcPrefix)
{
    ...
    switch (pstHdr->eMsgId)
    {
        ...
        {
            t_StHcpHfiMsgEvt *p = &pstMsg->stMsgEvt;
            LOG_RSP_MSG_ARGS(bErr, pcPrefix, pstHdr, "eId=%u", p->eId);
            break;
        }
...
static void vVppIpHcp_ReadSessProps(t_StVppIpHcpSessionCb *pstSessCb)
{
    ...
    u32VppUtils_ReadPropertyU32(HCP_PROPERTY_DUMP_META_DATA,
                                &pstSessCb->stDbgCfg.bDumpMetaData, "0");

    LOGI("HCP debug properties: u32LogFlags=%u bDumpMetaData=%u",
         pstSessCb->stDbgCfg.u32LogFlags, pstSessCb->stDbgCfg.bDumpMetaData);
}
...
static uint32_t bVppIpHcp_IsCustTuningEn(t_StVppIpHcpGlobalCb *pstGlobal,
                                         uint32_t u32TuningId)
{
    uint32_t u32Ret = VPP_OK;
    uint32_t u32Count = u32VppIpHcp_GetTuningCount(pstGlobal, u32TuningId);
    if (u32Count)
    {
        t_UTuningValue auTuning[u32Count];
        u32Ret = u32VppIpHcp_GetTuning(pstGlobal, u32TuningId,
                                       auTuning, u32Count);
        if (u32Ret == VPP_OK && auTuning[0].U32)
        {
            LOGI("Custom LUT Tunings[id=%u]: Enabled", u32TuningId);
            return VPP_TRUE;
        }
    }
    return VPP_FALSE;
...
static uint32_t u32VppIpHcp_ValidateCtrl(t_StVppIpHcpSessionCb *pstSessCb,
                                         struct hqv_control ctrl)
{
    ...
    {
        uint32_t bCustSdrLutEn = VPP_FALSE, bCustHdrLutEn = VPP_FALSE;

        // Length needs to be multiple of 4
        if ((ctrl.custom.len > HQV_CUSTOM_PAYLOAD_SZ) || (ctrl.custom.len & 0x03))
        {
            LOGE("ValidateCtrl() failed: Invalid paylod size: %u", ctrl.custom.len);
            return VPP_ERR_PARAM;
        }

        bCustSdrLutEn = bVppIpHcp_IsCustTuningEn(HCP_GLOB_SESS(pstSessCb),
                                                 LTMTUNE_NCUST_SDR_LUT_EN);
        bCustHdrLutEn = bVppIpHcp_IsCustTuningEn(HCP_GLOB_SESS(pstSessCb),
                                                 LTMTUNE_NCUST_HDR_LUT_EN);
        if (!bCustSdrLutEn && !bCustHdrLutEn)
        {
            LOGE("ValidateCtrl() failed: No custom tunings set");
            return VPP_ERR_PARAM;
        }
    }
    return VPP_OK;
...
{
    // AIE Mode
    pstSessCb->stDrvCtx.u32AieMode = LTM_MODE_MANUAL;
...
static void vVppIpHcp_Compute(t_StVppIpHcpSessionCb *pstSessCb, uint32_t bLocked)
{
        ...
        VPP_FLAG_IS_SET(pstCfg->u32ComputeMask, HCP_FLAG_ALGO_MEAS))
    {
            ...
            !VPP_FLAG_IS_SET(pstCfg->u32EnableMask, HCP_FLAG_ALGO_MEAS))
        {
            ...
            {
                // AIE Mode
                pstSessCb->stDrvCtx.u32AieMode = LTM_MODE_AUTO;
            }
        ...
        if (!pstSessCb->stDrvCtx.u32KnobsLoaded ||
            stTmpCtx.u32AieMode != pstSessCb->stDrvCtx.u32AieMode)
            VPP_FLAG_SET(pstCfg->u32LoadMask, HCP_FLAG_ALGO_AIE);

        if (pstSessCb->stDrvCtx.u32AieMode == LTM_MODE_MANUAL)
        {
            if (!pstSessCb->stDrvCtx.u32KnobsLoaded ||
    ...
    if (VPP_FLAG_IS_SET(pstCfg->u32ComputeMask, HCP_FLAG_ALGO_CUST))
    {
        uint32_t u32, u32Ret = VPP_OK;
        uint32_t u32Cnt = u32VppDataQueue_Cnt(&pstCfg->stCustQ);

        for (u32 = 0; u32 < u32Cnt; u32++)
        {
            t_StVppDataQueueNode *pstNode = pstVppDataQueue_Pop(&pstCfg->stCustQ);
            if (!pstNode)
            {
                LOGE("pstVppDataQueue_Pop() failed, u32Ret=%u u32=%u",
                     u32Ret, u32);
                u32Ret = VPP_ERR;
                break;
            }
            ...
            {
                u32Ret = u32VppDataQueue_Push(&pstSessCb->stDrvCtx.stCustQ,
                                              pstNode);
                if (u32Ret)
                {
                    LOGE("u32VppDataQueue_Push(stDrvCtx) failed, u32Ret=%u u32=%u",
                         u32Ret, u32);

                    if (u32VppDataQueue_PutFront(&pstCfg->stCustQ, pstNode))
                    {
                        free(pstNode);
                        LOGE("u32VppDataQueue_PutFront() failed, Node lost!!");
                    }
                    break;
                }
            }
        ...
        if (u32Ret == VPP_OK)
        {
            VPP_FLAG_SET(pstCfg->u32LoadMask, HCP_FLAG_ALGO_CUST);
            VPP_FLAG_CLR(pstCfg->u32ComputeMask, HCP_FLAG_ALGO_CUST);
        }
    }
                                            ...
                                            t_StVppIpHcpSessionCb *pstSess)
{
    ...
    if (pstGlobal->u32RxHealth)
    {
        LOGE("RegisterSession: critical error, denying session=%p", pstSess);
...
static void vVppIpHcp_BroadcastFinishedLoadingTunings(t_StVppIpHcpGlobalCb *pstGlobal)
{
    if (!pstGlobal)
        return;

    LOGD("finished loading tunings");
    HCP_LOCK(pstGlobal);
    pstGlobal->u.bFinishTuningLoad = VPP_TRUE;
    HCP_BROADCAST(pstGlobal);
    HCP_UNLOCK(pstGlobal);
}
    ...
    *bNopReceived = VPP_FALSE;
    ...
    else if (pstMsg->stHdr.eMsgId == MSG_ID_BUFF_RELEASED)
    {
            ...
            (pstMsg->stMsgBuf.bufAttr.stGenBufAttr.eUsage == BUFF_USAGE_TUNE))
        {
            ...
            if (bVppIpHcp_IsTuningLoadComplete(pstGlobal))
            {
                LOGI("tunings load complete, wake up thread...");
                vVppIpHcp_BroadcastFinishedLoadingTunings(pstGlobal);
            }
        }
        else
        {
    ...
    if (pstSessCb->stCfg.eOpMode == HCP_OP_MODE_PROC &&
        (pstDrv->uSplitScreen.st.u32ProcessedPercent == 0 ||
         (pstDrv->u32EarMode == HCP_EAR_MODE_OFF &&
          pstDrv->u32AieMode == LTM_MODE_MANUAL &&
                                     ...
                                     t_UVppIpHcpCanProcRes *pstRes)
{
    ...
    pstRes->bOpModeByp = u32InSz && (eOpMode == HCP_OP_MODE_BYPASS);

    pstRes->bFwCritErr = u32InSz &&
        (pstSessCb->stDrvCtx.eHwState == HCP_HW_STATE_ERROR);
    ...
    pstRes->bCanBypassIn = (pstRes->bWakeBypass || pstRes->bOpModeByp ||
                            ...
                            pstRes->bFwWouldByp || pstRes->bFwCritErr);
...
static uint32_t u32VppIpHcp_WorkerShouldSleep(t_StVppIpHcpSessionCb *pstSessCb)
{
    ...
    if (VPP_IP_STATE_EQUAL(pstSessCb, VPP_IP_STATE_ACTIVE))
    {
        ...
        if (pstSessCb->eExecutingCmd == VPP_IP_CMD_DRAIN &&
            (u32FwInCnt == 0 && u32FwOutCnt == 0 && u32InSz == 0))
        {
            LOGI("wake because finished draining.");
            return VPP_FALSE;
        }
...
static uint32_t u32VppIpHcp_ProcCmdInit(t_StVppIpHcpSessionCb *pstSessCb)
{
    ...
    {
        // Handle the case where a critical error is notified after this
        // session was registered but before the init command was processed.
        u32 = VPP_ERR_STATE;
        LOGE("invalid state to open fw, eHwState=%u",
             pstSessCb->stDrvCtx.eHwState);
    }
    else
    {
        u32 = u32VppIpHcp_HwOpen(pstSessCb);
        LOGE_IF(u32 != VPP_OK, "HwOpen failed, u32=%u", u32);
    }
...
static uint32_t u32VppIpHcp_ProcCmdTerm(t_StVppIpHcpSessionCb *pstSessCb)
{
    ...
    if (pstSessCb->stDrvCtx.eHwState == HCP_HW_STATE_ERROR)
    {
        // Explicitly handle freeing the intermediate buffers and not
        // letting HandlePendingCommand() do it, since we are not
        // sending a CLOSE to firmware, and so we would not get a
        // CLOSE_DONE event to trigger that handling.
...
static uint32_t u32VppIpHcp_ProcessBuffer(t_StVppIpHcpSessionCb *pstSessCb)
{
    ...
    if (stProcRes.bCanBypassIn)
    {
        ...
        {
            u32 = u32VppIp_NtsCmdPeekBack(&pstSessCb->stCmdQ, &stCmd, NULL);
            if (u32 != VPP_OK || stCmd.eCmd != VPP_IP_CMD_INTERNAL_DRAIN)
            {
    ...
    LOGE_IF(u32 != VPP_OK, "HwSessSendBufs failed, u32=%u", u32);
...
static uint32_t u32VppIpHcp_ProcMsgEvent(t_StVppIpHcpSessionCb *pstSessCb,
                                         t_StHcpHfiMsgPkt *pstMsg)
{
    uint32_t u32;
    t_StVppBuf *pstBuf;
    ...
    if (pstMsg->stMsgEvt.eId == HCP_EVT_ID_PD_CRASHED)
    {
        // Receiving this means that we will not receive any more messages
        // from firmware for this session. Depending on the state, we need
        // to handle this message accordingly. If there are any buffers
        // currently queued to firmware, bypass them.

        LOGE("PD crash detected, flushing and bypassing... ");

        u32 = u32VppIpHcp_FlushPort(pstSessCb, VPP_PORT_OUTPUT, &pstSessCb->stFwOut);
        LOGE_IF(u32 != VPP_OK, "failed to flush output port, u32=%u", u32);

        while (u32VppIp_PortBufGet(&pstSessCb->stFwIn, &pstBuf, NULL) == VPP_OK)
        {
            if (!pstBuf)
                continue;
            LOGD("fatal error from fw, bypassing buffer=%p", pstBuf);
            u32VppIp_CbBufDone(&pstSessCb->stBase.stCb, VPP_PORT_OUTPUT, pstBuf);
        }

        // Set bypass flag for session
        vVppIpHcp_HwSetState(pstSessCb, HCP_HW_STATE_ERROR);

        pstSessCb->stDrvCtx.u32InBufCnt = 0;
        pstSessCb->stDrvCtx.u32OutBufCnt = 0;

        if (pstSessCb->stIntBufs.stIntBufPx.eBufState == HCP_BUF_OWNER_FW)
            pstSessCb->stIntBufs.stIntBufPx.eBufState = HCP_BUF_OWNER_IP;

        if (pstSessCb->stIntBufs.stIntBufMeas.eBufState == HCP_BUF_OWNER_FW)
            pstSessCb->stIntBufs.stIntBufMeas.eBufState = HCP_BUF_OWNER_IP;
    }
                                    ...
                                    t_StHcpHfiMsgPkt *pstMsg)
{
    ...
    switch (pstMsg->stHdr.eMsgId)
    {
        ...
        case MSG_ID_EVENT_NOTIFY:
        {
            u32Ret = u32VppIpHcp_ProcMsgEvent(pstSessCb, pstMsg);
            break;
        }
        case MSG_ID_NOOP:
...
static uint32_t u32VppIpHcp_BroadcastCriticalError(t_StVppIpHcpGlobalCb *pstGlobal)
{
    uint32_t i, u32;
    t_StVppIpHcpSessionCb *pstSess;
    ...
    for (i = 0; i < HCP_MAX_SESSIONS; i++)
    {
        ...
        pstMsgQNode = pstVppIpHcp_MsgQGet(&pstGlobal->stMsgQ, NULL);
        if (!pstMsgQNode)
        {
            LOGE("unable to get msg queue node... skipping session?");
            continue;
        }
        ...
        pstHdr->eMsgId = MSG_ID_EVENT_NOTIFY;
        pstHdr->s32SessId = HCP_SESS_GET_HW_ID(pstSess);
        pstHdr->u32Size = sizeof(*pstHdr) + sizeof(pstMsg->stMsgEvt);
        pstHdr->s32Err = ERR_ID_FAIL;

        pstMsg->stMsgEvt.eId = HCP_EVT_ID_PD_CRASHED;

        u32 = u32VppIpHcp_MsgQPut(&pstSess->stMsgQ, pstMsgQNode,
                                  &pstSess->mutex, &pstSess->cond);
        if (u32 != VPP_OK)
        {
            LOGE("unable to send critical error message to session=%p, "
                 "s32FwId=%d, local_sess_id=%u, u32=%u", pstSess,
                 pstHdr->s32SessId, pstSess->u32SessId, u32);

            u32 = u32VppIpHcp_MsgQPut(&pstGlobal->stMsgQ, pstMsgQNode,
                                      &pstGlobal->mutex, NULL);
            LOGE_IF(u32 != VPP_OK, "failed to return q node to pool, u32=%u", u32);
        }
    }
...
static uint32_t u32VppIpHcp_FirmwareLoad(t_StVppIpHcpGlobalCb *pstGlobal)
{
    uint32_t u32, i;

    for (i = 0; i < HCP_FW_LOAD_MAX_RETRY; i++)
    {
        u32 = u32VppIpHcp_HwGetHandle(pstGlobal);
        if (u32 != VPP_OK)
        {
            LOGE("unable to get hw handle, attempt=%u, u32=%u", i, u32);
            sleep(1);
        }
        else
        {
            LOGI("acquired new handle on attempt=%u", i);
            break;
        }
    }
    ...
    if (u32 != VPP_OK)
    {
        LOGE("failed to acquire handle after %u attempts", i);
        goto ERR_GET_HANDLE;
    }
    ...
    HCP_LOCK(pstGlobal);
    pstGlobal->u.bFinishTuningLoad = VPP_FALSE;
    HCP_UNLOCK(pstGlobal);
    ...
    if (bVppIpHcp_IsTuningLoadComplete(pstGlobal))
    {
        LOGI("tunings wasn't sent, so no need to wait for message");
        vVppIpHcp_BroadcastFinishedLoadingTunings(pstGlobal);
    }
...
static uint32_t u32VppIpHcp_FirmwareUnload(t_StVppIpHcpGlobalCb *pstGlobal)
{
    uint32_t u32;
    ...
    LOGE_IF(u32 != VPP_OK, "unable to get info w/ unlock, u32=%u", u32);
...
static uint32_t u32VppIpHcp_FirmwareReset(t_StVppIpHcpGlobalCb *pstGlobal)
{
    uint32_t u32;

    VPP_RET_IF_NULL(pstGlobal, VPP_ERR_PARAM);

    u32 = u32VppIpHcp_FirmwareUnload(pstGlobal);
    LOGE_IF(u32 != VPP_OK, "FirmwareUnload failed, u32=%u", u32);

    u32 = u32VppIpHcp_FirmwareLoad(pstGlobal);
    LOGE_IF(u32 != VPP_OK, "FirmwareLoad failed, u32=%u", u32);

    return u32;
}

static uint32_t u32VppIpHcp_HandleRecvPktErr(t_StVppIpHcpGlobalCb *pstGlobal,
                                             uint32_t u32Err)
{
    uint32_t u32, u32Ret = VPP_OK;
    ...
    if (u32Err == VPP_OK)
    {
        pstGlobal->u32RxHealth = 0;
    }
    else
    {
        if (u32Err == VPP_ERR_FATAL)
        {
            pstGlobal->u32RxHealth++;
        }
        LOGE("HwGlobalRecvPkt returned error, u32Err=%u, u32RxHealth=%u",
             u32Err, pstGlobal->u32RxHealth);
    }

    if (pstGlobal->u32RxHealth >= HCP_RX_MAX_ERR_CNT)
    {
        // trip error handling
        LOGE("Detected critical error, trying reset...");
        u32VppIpHcp_BroadcastCriticalError(pstGlobal);
        u32 = u32VppIpHcp_FirmwareReset(pstGlobal);

        if (u32 == VPP_OK)
        {
            LOGD("successfully reset firmware!");
            pstGlobal->u32RxHealth = 0;
            u32Ret = VPP_ERR_HW;
        }
        else
        {
            // game over
            LOGE("failed to reset firmware, u32=%u", u32);
            u32Ret = VPP_ERR_FATAL;
        }
    }
...
static void *vpVppIpHcp_ReceiverWorker(void *pv)
{
    ...
    vVppIpHcp_BroadcastGlobalRes(pstGlobal, u32);

    if (u32 != VPP_OK)
    {
        LOGE("FirmwareLoad failed, u32=%u", u32);
    ...
    while (1)
    {
        ...
        u32 = u32VppIpHcp_HandleRecvPktErr(pstGlobal, u32);
        if (u32 != VPP_OK)
        {
            LOGE("HandleRecvPktErr error, u32=%u", u32);
            if (u32 == VPP_ERR_FATAL)
            {
                HCP_LOCK(pstGlobal);
                pstGlobal->eState = HCP_GLOBAL_STATE_ERROR;
                HCP_UNLOCK(pstGlobal);
                u32Exit = VPP_TRUE;
                LOGE("HandleRecvPktErr fatal error, u32=%u", u32);
            }
        }
                                       ...
                                       uint32_t bBootLock)
{
    ...
    LOGD("waiting for tunings to load...");
    HCP_LOCK(pstGlobal);
    while (!pstGlobal->u.bFinishTuningLoad)
        HCP_WAIT(pstGlobal);
    HCP_UNLOCK(pstGlobal);
    LOGD("finished waiting for tunings to load");
                                           ...
                                           uint32_t bBootLock)
{
    ...
    bWasError = pstGlobal->eState == HCP_GLOBAL_STATE_ERROR;
    pstGlobal->eState = HCP_GLOBAL_STATE_TERMINATING;
...
void * vpVppIpHcp_Init(t_StVppCtx *pstCtx, uint32_t u32Flags, t_StVppCallback cbs)
{
    ...
    if (u32VppDataQueue_Init(&pstSessCb->stCfg.stCustQ) != VPP_OK)
    {
        LOGE("unable to u32VppDataQueue_Init(stCfg) custom data queue");
        goto ERROR_CFG_DATA_Q_INIT;
    }

    if (u32VppDataQueue_Init(&pstSessCb->stDrvCtx.stCustQ) != VPP_OK)
    {
        LOGE("unable to u32VppDataQueue_Init(stDrvCtx) custom data queue");
        goto ERROR_DRV_DATA_Q_INIT;
    }
...
ERROR_MUTEX_INIT:
    LOGI("destroying drv custom data queue");
    u32VppDataQueue_Term(&pstSessCb->stDrvCtx.stCustQ);

ERROR_DRV_DATA_Q_INIT:
    LOGI("destroying cfg custom data queue");
    u32VppDataQueue_Term(&pstSessCb->stCfg.stCustQ);

ERROR_CFG_DATA_Q_INIT:
...
void vVppIpHcp_Term(void *ctx)
{
    ...
    u32 = u32VppDataQueue_Term(&pstSessCb->stDrvCtx.stCustQ);
    LOGE_IF(u32 != VPP_OK, "Drv DataQueue_Term failed, u32=%u", u32);

    u32 = u32VppDataQueue_Term(&pstSessCb->stCfg.stCustQ);
    LOGE_IF(u32 != VPP_OK, "Cfg DataQueue_Term failed, u32=%u", u32);
...
uint32_t u32VppIpHcp_SetCtrl(void *ctx, struct hqv_control ctrl)
{
    ...
    u32 = u32VppIpHcp_ValidateCtrl(pstSessCb, ctrl);
    if (u32 != VPP_OK)
    {
    ...
    if (ctrl.ctrl_type == HQV_CONTROL_CUST)
    {
        t_StVppIpHcpCustCtrl *pstCtrl =
            (t_StVppIpHcpCustCtrl *)calloc(1, sizeof(t_StVppIpHcpCustCtrl));
        if (!pstCtrl)
        {
            LOGE("SetCtrl(): calloc() failed");
            HCP_UNLOCK(pstSessCb);
            LOG_EXIT_RET(VPP_ERR_NO_MEM);
        }
        pstCtrl->stCtrl = ctrl.custom;
        pstCtrl->stNode.pvData = (void *)pstCtrl;
        u32 = u32VppDataQueue_Push(&pstCfg->stCustQ, &pstCtrl->stNode);
        if (u32 != VPP_OK)
        {
            LOGE("SetCtrl(): u32VppDataQueue_Push() failed, u32=%u", u32);
            free(pstCtrl);
            HCP_UNLOCK(pstSessCb);
            LOG_EXIT_RET(VPP_ERR_PARAM);
        }
        VPP_FLAG_SET(pstCfg->u32ComputeMask, HCP_FLAG_ALGO_CUST);
    }
