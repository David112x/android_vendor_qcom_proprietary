...
static void vVppIpHcp_DumpVdecParam(uint32_t u32BufNum, int32_t fd,
                                    t_StVppIpHcpSessionCb *pstSessCb,
                                    t_StSessVdecParam *pstVdecParam)
{
    const uint32_t u32MaxSize = 256;
    char cData[u32MaxSize], cPath[HCP_DUMP_NM_LEN];

    VPP_RET_VOID_IF_NULL(pstSessCb);
    VPP_RET_VOID_IF_NULL(pstSessCb->stBase.pstCtx);
    VPP_RET_VOID_IF_NULL(pstVdecParam);

    snprintf(cPath, HCP_DUMP_NM_LEN, HCP_DUMP_META_DATA_NM, pstSessCb);

    snprintf(cData, u32MaxSize, "meta: num=%u, fd=%d, fr=%u, codec=%u, ftype=%u, "
             "qp={sum=%u, sumCnt=%u, skipSum=%u, skipCnt=%u}, dynRange=%u\n",
             u32BufNum,
             fd,
             pstVdecParam->nFrameRate,
             pstVdecParam->eCodec,
             pstVdecParam->ePicType,
             pstVdecParam->nQPSum,
             pstVdecParam->nQPSumCnt,
             pstVdecParam->nSkipQPSum,
             pstVdecParam->nSkipQPSumCnt,
             pstVdecParam->stDynRangeParam.eDynRange);

    u32VppBuf_DumpRawData(pstSessCb->stBase.pstCtx, cData, strlen(cData), cPath,
                          eVppDumpMode_Append);
}
...
static uint32_t u32VppIpHcp_HwSessSetKnobCustom(t_StVppIpHcpSessionCb *pstSessCb,
                                                t_StHcpHfiCmdPkt *pstCmdPkt)
{
    uint32_t u32;
    hcp_rpc_pkt_t stPkt;
    t_StHcpHfiCmdSetProp *pstProp = (t_StHcpHfiCmdSetProp *)pstCmdPkt->u32Bdy;
    t_StHcpHfiCmdPktHdr *pstHdr = &pstCmdPkt->stHdr;
    ...
    pstHdr->u32Size = sizeof(t_StHcpHfiCmdPktHdr);
    pstHdr->u32Size += sizeof(t_StHcpHfiCmdSetProp);
    pstHdr->u32Size += pstProp->stKnobs.stCustom.u32BdySizeBytes;

    LOGD("HwSessSetProp: propId=%u (%s)", pstProp->propId, pcGetPropIdStr(pstProp->propId));

    HCP_STAT(pstSessCb, FwTxSetProp);
    u32 = u32VppIpHcp_HwSendCmd(HCP_HNDL_SESS(pstSessCb), &stPkt, pstCmdPkt);
    LOGE_IF(u32 != VPP_OK, "Error sending custom knob u32=%u", u32);
...
static void vVppIpHcp_PopulateVideoMetadataMsm(const t_StVppBuf *pstBuf,
                                               t_StSessVdecParam *pstVdecParam)
{
    uint32_t u32Flags;
    struct msm_vidc_frame_qp_payload *pstQp;

    VPP_RET_VOID_IF_NULL(pstBuf);
    VPP_RET_VOID_IF_NULL(pstVdecParam);

    pstQp = (struct msm_vidc_frame_qp_payload *)
        vpVppBuf_FindExtradata(pstBuf,
                               MSM_VIDC_EXTRADATA_FRAME_QP,
                               EXTRADATA_BUFFER_FORMAT_MSM);

    if (pstQp)
    {
        pstVdecParam->nQPSum = pstQp->qp_sum;
        pstVdecParam->nQPSumCnt = pstQp->total_num_blocks;
        pstVdecParam->nSkipQPSum = pstQp->skip_qp_sum;
        pstVdecParam->nSkipQPSumCnt = pstQp->skip_num_blocks;
    }

    u32Flags = pstBuf->pBuf->flags;
    if (VPP_FLAG_IS_SET(u32Flags, VPP_BUFFER_FLAG_SYNCFRAME))
    {
        pstVdecParam->ePicType = PICTURETYPE_I;
    }
    else if (VPP_FLAG_IS_SET(u32Flags, VPP_BUFFER_FLAG_B_FRAME))
    {
        pstVdecParam->ePicType = PICTURETYPE_B;
    }
    else if (VPP_FLAG_IS_SET(u32Flags, VPP_BUFFER_FLAG_P_FRAME))
    {
...
#ifndef OMX_EXTRADATA_DOES_NOT_EXIST
static void vVppIpHcp_PopulateVideoMetadataOmx(const t_StVppBuf *pstBuf,
                                               t_StSessVdecParam *pstVdecParam)
{
    OMX_QCOM_EXTRADATA_QP *pstQp;
    OMX_QCOM_EXTRADATA_FRAMEINFO *pstInfo;

    VPP_RET_VOID_IF_NULL(pstBuf);
    VPP_RET_VOID_IF_NULL(pstVdecParam);

    pstQp = (OMX_QCOM_EXTRADATA_QP *)
        vpVppBuf_FindExtradata(pstBuf,
                               OMX_ExtraDataQP,
                               EXTRADATA_BUFFER_FORMAT_OMX);

    pstInfo = (OMX_QCOM_EXTRADATA_FRAMEINFO *)
        vpVppBuf_FindExtradata(pstBuf,
                               OMX_ExtraDataFrameInfo,
                               EXTRADATA_BUFFER_FORMAT_OMX);
                                            ...
                                            t_StSessVdecParam *pstVdecParam)
{
    ...
    pstVdecParam->stDynRangeParam.eDynRange = bVppBuf_IsHdr(pstBuf) ? DYNRANGE_HDR : DYNRANGE_SDR;

    if (VPP_EXTERNAL_EXTRADATA_TYPE == EXTRADATA_BUFFER_FORMAT_MSM)
    {
        vVppIpHcp_PopulateVideoMetadataMsm(pstBuf, pstVdecParam);
    }
#ifndef OMX_EXTRADATA_DOES_NOT_EXIST
    else if (VPP_EXTERNAL_EXTRADATA_TYPE == EXTRADATA_BUFFER_FORMAT_OMX)
    {
        vVppIpHcp_PopulateVideoMetadataOmx(pstBuf, pstVdecParam);
    }
#endif

    LOGI("meta: num=%u, fd=%d, fr=%u, codec=%u, ftype=%u, "
         "qp={sum=%u, sumCnt=%u, skipSum=%u, skipCnt=%u}",
         pstBuf->u32BufNum,
    ...
    if (pstSessCb->stDbgCfg.bDumpMetaData)
        vVppIpHcp_DumpVdecParam(pstBuf->u32BufNum, pstBuf->stPixel.fd, pstSessCb, pstVdecParam);
...
uint32_t u32VppIpHcp_HwGetHandle(t_StVppIpHcpGlobalCb *pstGlobal)
{
    ...
    {
        LOGE("hcp_rpc_open returned err, res=%d, hndl=0x%"PRIx64, res, hndl);
        LOG_EXIT_RET(VPP_ERR_HW);
    }

    LOGD("hcp_rpc_open returned hndl=0x%"PRIx64, hndl);
                                     ...
                                     t_StHcpHfiMsgPkt *pstMsg)
{
    ...
    if (res != AEE_SUCCESS)
    {
        ...
        if (res == -1 || res == AEE_ENOSUCH || res == AEE_ECONNRESET)
            u32 = VPP_ERR_FATAL;
    }
...
uint32_t u32VppIpHcp_HwSessSetPropKnobs(t_StVppIpHcpSessionCb *pstSessCb)
{
    ...
    // Always send the AIE/LTM mode after all the LTM/DE knobs are sent.
    if (VPP_FLAG_IS_SET(pstSessCb->stCfg.u32LoadMask, HCP_FLAG_ALGO_AIE))
    {
        pstProp->stKnobs.eKnobId = KNOB_ID_LTM_MODE;
        pstProp->stKnobs.u32LtmMode = pstSessCb->stDrvCtx.u32AieMode;
        LOG_KNOB(pstProp, u32LtmMode);
        VPP_IP_PROF_START(&pstSessCb->stBase, HCP_STAT_HW_SET_PROP_KNOBS);
        vVppIpHcp_HwSessSetKnob(pstSessCb, pstProp, &u32Ret);
        VPP_IP_PROF_STOP(&pstSessCb->stBase, HCP_STAT_HW_SET_PROP_KNOBS);

        VPP_FLAG_CLR(pstSessCb->stCfg.u32LoadMask, HCP_FLAG_ALGO_AIE);
    }
    ...
    if (VPP_FLAG_IS_SET(pstSessCb->stCfg.u32LoadMask, HCP_FLAG_ALGO_CUST))
    {
        uint32_t u32, u32Ret = VPP_OK;
        uint32_t u32Cnt = u32VppDataQueue_Cnt(&pstSessCb->stDrvCtx.stCustQ);
        LOGI("u32VppDataQueue_Cnt : %u", u32Cnt);

        for (u32 = 0; u32 < u32Cnt; u32++)
        {
            uint32_t u32Sz;
            t_StHcpHfiCmdPkt *pstCmdPkt;
            struct hqv_ctrl_custom stCustom;

            t_StVppDataQueueNode *pstNode =
                pstVppDataQueue_Pop(&pstSessCb->stDrvCtx.stCustQ);
            if (!pstNode)
            {
            ...
            stCustom = ((t_StVppIpHcpCustCtrl *)pstNode->pvData)->stCtrl;
            u32Sz = sizeof(t_StHcpHfiCmdPktHdr)
                    + sizeof(t_StHcpHfiCmdSetProp)
                    + stCustom.len;
            uint8_t customCmdPkt[u32Sz];

            pstCmdPkt = (t_StHcpHfiCmdPkt *)customCmdPkt;

            pstProp = (t_StHcpHfiCmdSetProp *)pstCmdPkt->u32Bdy;
            pstProp->propId = PROP_ID_SESS_KNOB;
            pstProp->stKnobs.eKnobId = KNOB_ID_CUSTOM;
            pstProp->stKnobs.stCustom.u32BdySizeBytes = stCustom.len;
            memcpy(pstProp->stKnobs.stCustom.u32Bdy, stCustom.data, stCustom.len);
            free(pstNode);

            LOG_KNOB(pstProp, stCustom.u32BdySizeBytes);
            VPP_IP_PROF_START(&pstSessCb->stBase, HCP_STAT_HW_SET_PROP_KNOBS);
            VPP_IP_PROF_START(&pstSessCb->stBase,
                              HCP_STAT_HW_SET_PROP_CUST_KNOBS);
            u32Ret = u32VppIpHcp_HwSessSetKnobCustom(pstSessCb, pstCmdPkt);
            VPP_IP_PROF_STOP(&pstSessCb->stBase,
                             HCP_STAT_HW_SET_PROP_CUST_KNOBS);
            VPP_IP_PROF_STOP(&pstSessCb->stBase, HCP_STAT_HW_SET_PROP_KNOBS);
        }

        if (u32Ret == VPP_OK)
            VPP_FLAG_CLR(pstSessCb->stCfg.u32LoadMask, HCP_FLAG_ALGO_CUST);
    }
                                ...
                                t_StHcpHfiPixBufAttr *pstPxAttr)
{
    ...
    {
        pstVppStat->eVersion = eVppUbwcStatVer_Unknown;
        pstVppStat->bValid = VPP_FALSE;
    }
