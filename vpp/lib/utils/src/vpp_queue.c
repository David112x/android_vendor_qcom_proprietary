...
{
    uint32_t result = i + del;
    return (result < mod) ? result : result - mod;
}
...
int32_t vpp_queue_peek_front(t_StVppQueue *q)
{
    int32_t next;
    ...
    VPP_QUEUE_LOG("peek_front: queue: %p", q);

    if (q->entries == 0)
    {
        VPP_QUEUE_LOG("trying to peek_front from empty q: %p", q);
        return -1;
    }

    next = DVP_MOD_PLUS(q->tail, 1, q->size);

    VPP_QUEUE_LOG("peek_front: returning: %d, entries=%u", next, q->entries);
    return next;
}

int32_t vpp_queue_peek_back(t_StVppQueue *q)
{
    if (!q)
        return -1;

    VPP_QUEUE_LOG("peek_back: queue: %p", q);

    if (q->entries == 0)
    {
        VPP_QUEUE_LOG("trying to peek_back from empty q: %p", q);
        return -1;
    }

    VPP_QUEUE_LOG("peek_back: returning: %d, entries=%u", q->head, q->entries);
    return q->head;
}
