...
static uint32_t isCtrlEnabled(t_StVppHqvCtrl *pstCtrl, enum hqv_control_type ctrl)
{
    ...
    // If there are any segments present then FRC needs to be turned ON
    // This is to allow the caller to change FRC controls during run-time
    if (ctrl == HQV_CONTROL_FRC &&
        pstCtrl->frc.num_segments > 0 &&
        pstCtrl->frc.segments != NULL)
        return VPP_TRUE;
...
{
    enum vpp_color_format eFmtIn, eFmtOut;
    uint32_t u32Supported = VPP_FALSE;
    uint32_t i;

    eFmtIn = pstParam->input_port.fmt;
    eFmtOut = pstParam->output_port.fmt;

    for (i = 0; i < (pstIp->stInputPort.u32FmtCnt); i++)
    {
        if (pstIp->stInputPort.peColorFmts[i] == eFmtIn)
        {
            u32Supported = VPP_TRUE;
            break;
        }
    }
    if (u32Supported == VPP_FALSE)
    {
        LOGE("eFmtIn=%d not supported for IP %s", eFmtIn, pstIp->name);
        return u32Supported;
    }

    u32Supported = VPP_FALSE;
    for (i = 0; i < (pstIp->stOutputPort.u32FmtCnt); i++)
    {
        if (pstIp->stOutputPort.peColorFmts[i] == eFmtOut)
        {
            u32Supported = VPP_TRUE;
            break;
        }
    }
    LOGE_IF(u32Supported == VPP_FALSE, "eFmtOut=%d not supported for IP %s", eFmtOut, pstIp->name);
...
{
    uint32_t u32WidthIn, u32WidthOut, u32HeightIn, u32HeightOut;

    u32WidthIn = pstParam->input_port.width;
    u32HeightIn = pstParam->input_port.height;
    if (pstParam->input_port.height > pstParam->input_port.width)
    {
        u32WidthIn = pstParam->input_port.height;
        u32HeightIn = pstParam->input_port.width;
    }

    u32WidthOut = pstParam->output_port.width;
    u32HeightOut = pstParam->output_port.height;
    if (pstParam->output_port.height > pstParam->output_port.width)
    {
        u32WidthOut = pstParam->output_port.height;
        u32HeightOut = pstParam->output_port.width;
    }

    if ((u32WidthIn >= pstIp->capabilities.min_res.u32Width) &&
        (u32WidthIn <= pstIp->capabilities.max_res.u32Width) &&
        (u32HeightIn >= pstIp->capabilities.min_res.u32Height) &&
        (u32HeightIn <= pstIp->capabilities.max_res.u32Height) &&
        (u32WidthOut >= pstIp->capabilities.min_res.u32Width) &&
        (u32WidthOut <= pstIp->capabilities.max_res.u32Width) &&
        (u32HeightOut >= pstIp->capabilities.min_res.u32Height) &&
        (u32HeightOut <= pstIp->capabilities.max_res.u32Height))
    {
        return VPP_TRUE;
    }

    LOGD("In:%ux%u Out:%ux%u NOT SUPPORTED", u32WidthIn, u32HeightIn, u32WidthOut, u32HeightOut);
    return VPP_FALSE;
}
...
static uint32_t u32GetUcMaxSessions(t_StVppUsecase *pstUc, uint32_t *o_pu32MaxSessions)
{
    ...
    VPP_RET_IF_NULL(o_pu32MaxSessions, VPP_ERR_PARAM);
                                 ...
                                 t_StVppHqvCtrl *pstCtrl, t_StVppParam *pstParam)
{
    ...
    if (pstUc && (u32GetUcMaxSessions(pstUc, au32UcMaxSessions) != VPP_OK))
        return VPP_ERR;
    ...
    if (pstUc == NULL)
    {
        LOGI("Setting session for context %p to unused", pstConfigureSession->pstUcCtx);
...
/*!
* @brief       Release a use case
*
* @description Release the resources and credits for a session
*
* @in          pstCtx The context being release
*
* @return      OK if use case released, ERR otherwise
*/
uint32_t u32VppUsecase_Release(t_StVppCtx *pstCtx)
{
    t_StVppConcSession *pstCurrentSession;
    ...
    while (pstCurrentSession != NULL)
    {
        ...
        {
            // Context already has a session open
            LOGD("Found context to release");
            pstCurrentSession->bInUse = 0;
            pthread_mutex_unlock(&concurrency_main.mutex);
            LOG_EXIT_RET(VPP_OK);
        }
        pstCurrentSession = pstCurrentSession->pNext;
    ...
    pthread_mutex_unlock(&concurrency_main.mutex);
    LOGE("No session found for context=%p", pstCtx);
    LOG_EXIT_RET(VPP_ERR_PARAM);
}
...
{
    uint32_t u32Resolution, u32CompIpIndex, u32Ret;
    uint32_t i;
    ...
    for (i = 0; i < VPP_IP_BLOCK_MAX; i++)
    {
        ...
        u32Ret = u32VppUsecase_GetIpPortParam(pstUc, pstParam, u32CompIpIndex, &stParamTmp);
        if (u32Ret != VPP_OK)
        {
            LOGE("Error getting port param for IP=%u, u32Ret=%u", u32CompIpIndex, u32Ret);
            return VPP_ERR_INVALID_CFG;
        }
        if ((!u32GetIpColorFormatSupported(&stParamTmp,
                                           pstUc->composition[u32CompIpIndex].ip)) ||
            (!u32GetSizeSupported(&stParamTmp,
                                  pstUc->composition[u32CompIpIndex].ip)))
        {
            LOGE("IN:%ux%u, fmt=%d OUT:%ux%u, fmt=%d not supported for IP=%u (%s)",
                 stParamTmp.input_port.width, stParamTmp.input_port.height,
                 stParamTmp.input_port.fmt, stParamTmp.output_port.width,
                 stParamTmp.output_port.height, stParamTmp.output_port.fmt,
                 pstUc->composition[u32CompIpIndex].ip->eIp,
                 pstUc->composition[u32CompIpIndex].ip->name);
            return VPP_ERR_INVALID_CFG;
        }
...
/*!
* @brief       Validate a use case for a specific chipset
*
* @description The registry of a chipset may optionally define its own additional validation
*              function that may not be covered by the standard registry definition
*
* @in          pstUc The use case being validated
* @in          pstSessCfg The configuration to validate
*
* @return      OK if use case validated, ERR otherwise
*/
uint32_t u32VppUsecase_ChipsetValidate(t_StVppUsecase *pstUc, t_StVppUcSessCfg *pstSessCfg)
{
    uint32_t u32Ret = VPP_OK;

    VPP_RET_IF_NULL(pstSessCfg, VPP_ERR_PARAM);

    // Return OK here since a NULL use case will bypass the chipset registry
    if (!pstUc)
        return VPP_OK;

    if (pstUc->pu32ChipsetValidate)
    {
        u32Ret = pstUc->pu32ChipsetValidate(pstSessCfg);
        LOGE_IF(u32Ret != VPP_OK, "Chipset usecase validation failed for UC=%s, u32Ret=%u",
                pstUc->name, u32Ret);
    }
    return u32Ret;
}
...
{
    uint32_t u32Resolution, u32MaxFpsIndex, u32MaxFps;
    uint32_t i, j;
    uint32_t au32RequiredCredit[VPP_IP_BLOCK_MAX];
    uint32_t au32UsedCredit[VPP_IP_BLOCK_MAX];
    t_StVppConcSession *pstCurrentSession;
                                      ...
                                      uint32_t u32IpIndex, t_StVppParam *o_pstIpParam)
{
    ...
    o_pstIpParam->input_port.stride =
        u32VppUtils_CalculateStride(o_pstIpParam->input_port.width,
                                    o_pstIpParam->input_port.fmt);
    o_pstIpParam->input_port.scanlines =
        u32VppUtils_CalculateScanlines(o_pstIpParam->input_port.height,
                                       o_pstIpParam->input_port.fmt);
    o_pstIpParam->output_port.fmt = stReq.eOutColorFmt;
    o_pstIpParam->output_port.stride =
        u32VppUtils_CalculateStride(o_pstIpParam->output_port.width,
                                    o_pstIpParam->output_port.fmt);
    o_pstIpParam->output_port.scanlines =
        u32VppUtils_CalculateScanlines(o_pstIpParam->output_port.height,
                                       o_pstIpParam->output_port.fmt);
