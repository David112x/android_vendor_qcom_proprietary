...
#define VPP_LOG_LVL_HVX_CORE     VPP_LOG_LVL_ERROR
...
#define VPP_LOG(which) ((uint64_t)VPP_LOG_LVL_##which << (uint64_t)(VPP_LOG_FLD_WID*VPP_LOG_MODULE_##which))
#define VPP_LOG_DEFAULT (VPP_LOG(PIPELINE) | VPP_LOG(GPU) | VPP_LOG(HVX) | \
                         ...
                         VPP_LOG(FRC_MC) | VPP_LOG(FRC_CORE) | VPP_LOG(HCP) | VPP_LOG(HVX_CORE))
...
void vVpp_DumpControl(struct hqv_control ctrl)
{
    ...
    LOG_AIE(I, &ctrl.aie);
    LOG_DI(I, &ctrl.di);
    if (ctrl.ctrl_type == HQV_CONTROL_FRC)
        LOG_FRC(I, &ctrl.frc);
    LOG_EAR(I, &ctrl.ear);
    LOG_QBR(I, &ctrl.qbr);
}
...
static uint32_t u32Vpp_HandleReconfigure(t_StVppCtx *pstCtx)
{
    ...
    stSessCfg.pstVppCtrl = &pstCb->stCtrl;
    stSessCfg.pstParam = &pstCb->stParam;
    u32Ret = u32VppUsecase_ChipsetValidate(pstCb->pstUc, &stSessCfg);
    if (u32Ret != VPP_OK)
    {
        LOGE("Usecase_ChipsetValidate failed, reconfigure to bypass, u32Ret=%u", u32Ret);
        pstNewUc = NULL;
    }
...
void vpp_term(void *ctx)
{
    ...
    {
        u32Ret = vpp_close(ctx);
        LOGE_IF(u32Ret != VPP_OK, "error vpp_close, u32Ret=%u", u32Ret);
    }
    ...
    if (pstCb->stCtrl.frc.segments)
        free(pstCb->stCtrl.frc.segments);
...
uint32_t vpp_set_ctrl(void *ctx, struct hqv_control ctrl)
{
    ...
    t_StVppCb *pstCb;
    t_StVppUcSessCfg stSessCfg;
    t_StVppHqvCtrl stVppCtrlOld;
    struct vpp_ctrl_frc_segment *pstFrcSegsOld = NULL;

    LOG_ENTER();

    if (!ctx || ctrl.mode >= HQV_MODE_MAX)
    {
             ...
             ctrl.ctrl_type < HQV_CONTROL_MAX))
        {
            switch (ctrl.ctrl_type)
            {
                ...
                case HQV_CONTROL_FRC:
                    // If there are new segments from the caller then allocate new segments
                    pstFrcSegsOld = pstCb->stCtrl.frc.segments;
                    if (ctrl.frc.num_segments > 0)
                    {
                        struct vpp_ctrl_frc_segment *pstFrcSegs, *pstFrcSegTmp;
                        uint32_t u32Idx;

                        LOGD("allocating %u frc segments", ctrl.frc.num_segments);
                        pstFrcSegs = (struct vpp_ctrl_frc_segment *)
                            calloc(ctrl.frc.num_segments, sizeof(struct vpp_ctrl_frc_segment));

                        if (!pstFrcSegs)
                        {
                            LOGE("failed to allocate %u segments", ctrl.frc.num_segments);
                            LOG_EXIT_RET(VPP_ERR_NO_MEM);
                        }

                        memcpy(pstFrcSegs, ctrl.frc.segments,
                               ctrl.frc.num_segments * sizeof(struct vpp_ctrl_frc_segment));

                        for (u32Idx = 0; u32Idx < ctrl.frc.num_segments; u32Idx++)
                        {
                            pstFrcSegTmp = &pstFrcSegs[u32Idx];
                            if (pstFrcSegTmp->mode >= HQV_FRC_MODE_MAX)
                            {
                                LOGE("FRC mode %d out of range idx=%u", pstFrcSegTmp->mode, u32Idx);
                                free(pstFrcSegs);
                                LOG_EXIT_RET(VPP_ERR_PARAM);
                            }
                            pstFrcSegTmp->level = RANGE_CHECK(pstFrcSegTmp->level,
                                                              HQV_FRC_LEVEL_OFF,
                                                              HQV_FRC_LEVEL_HIGH);
                            if (pstFrcSegTmp->mode == HQV_FRC_MODE_SLOMO &&
                                pstFrcSegTmp->interp >= HQV_FRC_INTERP_MAX)
                            {
                                LOGE("FRC INTERP OOB=%u, setting to default 2x idx=%u",
                                     pstFrcSegTmp->interp, u32Idx);
                                pstFrcSegTmp->interp = HQV_FRC_INTERP_2X;
                            }
                        }
                        pstCb->stCtrl.frc.segments = pstFrcSegs;
                        pstCb->stCtrl.frc.num_segments = ctrl.frc.num_segments;
                    }
                    else
                    {
                        pstCb->stCtrl.frc.segments = NULL;
                        pstCb->stCtrl.frc.num_segments = 0;
                    }
                    break;
    ...
    {
        t_StVppParam *pstParam = NULL;
        // If the port-params are already set, then find/validate the use-case
        // using the port-params
        if (pstCb->u32VppFlags & VPP_FLAG_INPUT_PORT_SET &&
            pstCb->u32VppFlags & VPP_FLAG_OUTPUT_PORT_SET)
            pstParam = &pstCb->stParam;

        pstCb->pstUc = pstVppUsecase_Find(&pstCb->stCtrl, pstParam);
    ...
    else if (VPP_STATE_EQUAL(pstCb, VPP_STATE_ACTIVE) || bIsCustom)
    {
        if (!bIsCustom)
        {
            // In active state, need to validate that the chipset can support the updated
            // control before sending it
            stSessCfg.pstVppCtrl = &pstCb->stCtrl;
            stSessCfg.pstParam = &pstCb->stParam;
            u32 = u32VppUsecase_ChipsetValidate(pstCb->pstUc, &stSessCfg);
            if (u32 != VPP_OK)
            {
                // Restore orignal controls, since new control will be ignored
                if (ctrl.mode == HQV_MODE_MANUAL && ctrl.ctrl_type == HQV_CONTROL_FRC &&
                    pstCb->stCtrl.frc.segments)
                {
                    // Free the new segment before restoring the old
                    free(pstCb->stCtrl.frc.segments);
                }
                pstCb->stCtrl = stVppCtrlOld;
                LOGE("Usecase_ChipsetValidate failed, ignore new control, u32=%u", u32);
                LOG_EXIT_RET(u32);
            }
        }
...
uint32_t vpp_set_parameter(void *ctx, enum vpp_port port, struct vpp_port_param param)
{
    ...
    if (pstCb->u32VppFlags & VPP_FLAG_INPUT_PORT_SET &&
        pstCb->u32VppFlags & VPP_FLAG_OUTPUT_PORT_SET)
    {
        pstUc = pstVppUsecase_Find(pstCtrl, pstParam);
        if (pstUc != pstCb->pstUc)
        {
            LOGD("Changing usecase from %s to %s", pstCb->pstUc ? pstCb->pstUc->name : "NULL",
                 pstUc ? pstUc->name : "NULL");
            pstCb->pstUc = pstUc;
        }
        ...
        if (pstUc == NULL)
        {
            ...
            LOGE("pstVppUsecase_Find returned NULL use-case");
        }

        stSessCfg.pstVppCtrl = pstCtrl;
        stSessCfg.pstParam = pstParam;
        u32Ret = u32VppUsecase_ChipsetValidate(pstCb->pstUc, &stSessCfg);
        if (u32Ret != VPP_OK)
        {
            pstCb->pstUc = NULL;
            if (u32Err == VPP_OK)
                u32Err = VPP_ERR_INVALID_CFG;
            LOGE("Usecase_ChipsetValidate failed, enabling bypass, u32Ret=%u", u32Ret);
        }
    }
    LOG_EXIT_RET(u32Err);
}
...
uint32_t vpp_get_buf_requirements(void *ctx, struct vpp_requirements *req)
{
    ...
    u32Ret = u32VppUsecase_GetRequirements(pstCb->pstUc, req);
    LOGE_IF(u32Ret != VPP_OK, "Usecase-GetRequirements returned error %u", u32Ret);
...
uint32_t vpp_open(void *ctx)
{
        ...
        pstCb->pstUc)
    {
        ...
        if (u32Ret != VPP_OK)
        {
            LOGE("Pipeline_Open failed, u32Ret=%u", u32Ret);
            LOG_EXIT_RET(u32Ret);
        }
    ...
    else
    {
        ...
        LOGI("Cannot open vpp: port params not sent or NULL usecase, flags=%u, pstUc=%p",
             pstCb->u32VppFlags, pstCb->pstUc);
    }
    LOG_EXIT_RET(u32Ret);

ERR_SET_USE_CASE:
ERR_SET_PORT_PARAM:
    u32 = u32VppPipeline_Close(VPP_CTX_GET(ctx));
    LOGE_IF(u32 != VPP_OK, "u32VppPipeline_Close failed, u32=%u", u32);
    LOG_EXIT_RET(u32Ret);
}
...
uint32_t vpp_close(void *ctx)
{
    ...
    u32Err = u32VppPipeline_Close(VPP_CTX_GET(ctx));
    if (u32Err != VPP_OK)
    {
        LOGE("error closing pipeline, u32Err=%u", u32Err);
        u32Ret = u32Err;
    }

    u32Err = u32VppUsecase_Release(VPP_CTX_GET(ctx));
    if (u32Err != VPP_OK)
    {
        LOGE("error releasing usecase, u32Err=%u", u32Err);
        u32Ret = u32Err;
    }
    vVpp_SetState(pstCb, VPP_STATE_INITED);
