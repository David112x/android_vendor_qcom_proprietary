...
#define copyMetaDataVaToHandle(a, b) VPP_ERR
#define copyMetaDataHandleToVa(a, b) VPP_ERR
                              ...
                              uint32_t u32MapFlags)
{
    ...
    uint32_t u32PaOff = 0, u32PaMapLen = 0, u32ExpOff = 0;
...
static t_EVppBufType eVppBuf_GetInternalFrameTypeFromMsm(uint32_t u32MsmType)
{
...
static uint32_t u32VppBuf_GetMsmFrameTypeFromInternal(t_EVppBufType eType)
{
    uint32_t u32Ret = MSM_VIDC_INTERLACE_FRAME_PROGRESSIVE;

    switch (eType)
    {
        VPP_BUF_MAP(u32Ret, eVppBufType_Interleaved_TFF, MSM_VIDC_INTERLACE_INTERLEAVE_FRAME_TOPFIELDFIRST);
        VPP_BUF_MAP(u32Ret, eVppBufType_Interleaved_BFF, MSM_VIDC_INTERLACE_INTERLEAVE_FRAME_BOTTOMFIELDFIRST);
        VPP_BUF_MAP(u32Ret, eVppBufType_Frame_TFF, MSM_VIDC_INTERLACE_FRAME_TOPFIELDFIRST);
        VPP_BUF_MAP(u32Ret, eVppBufType_Frame_BFF, MSM_VIDC_INTERLACE_FRAME_BOTTOMFIELDFIRST);
        VPP_BUF_MAP(u32Ret, eVppBufType_Progressive, MSM_VIDC_INTERLACE_FRAME_PROGRESSIVE);
        default:
            u32Ret = MSM_VIDC_INTERLACE_FRAME_PROGRESSIVE;
    }

    return u32Ret;
}
static void * vpVppBuf_FindExtradataHeaderMsm(const t_StVppBuf *pstBufSrc,
                                              uint32_t u32ExType)
{
    uint32_t u32CurLen = 0;
...
static void * vpVppBuf_FindExtradataMsm(const t_StVppBuf *pstBufSrc,
                                        uint32_t u32ExType)
{
    void *pv;
    struct msm_vidc_extradata_header *pExDataSrc;
                                           ...
                                           t_EVppBufType eDstBufType)
{
    ...
    while (u32CurLen < pstMemBufSrc->u32MappedLen)
    {
        ...
        if (pExDataDst->type == MSM_VIDC_EXTRADATA_INTERLACE_VIDEO)
        {
            ...
            pl->format = u32VppBuf_GetMsmFrameTypeFromInternal(eDstBufType);
        }
...
static void * vpVppBuf_FindExtradataHeaderOmx(const t_StVppBuf *pstBufSrc,
                                              uint32_t u32ExType)
{
    uint32_t u32CurLen = 0;
...
static void * vpVppBuf_FindExtradataOmx(const t_StVppBuf *pstBufSrc,
                                        uint32_t u32ExType)
{
    void *pv;
    OMX_OTHER_EXTRADATATYPE *pExDataSrc;
                                         ...
                                         t_StVppUbwcStats *pstVppStat)
{
    ...
    {
        pstGrallocStat->bDataValid = 0;
        pstGrallocStat->version = UBWC_UNUSED;
    }
}
#endif
...
void * vpVppBuf_FindExtradata(const t_StVppBuf *pstBufSrc, uint32_t u32ExType,
...
uint32_t u32VppBuf_GrallocMetaDataSize()
{
...
uint32_t u32VppBuf_GrallocMetadataCopy(t_StVppBuf *pstBufSrc, t_StVppBuf *pstBufDst)
{
    ...
    if ((pstBufSrc->u32InternalFlags & VPP_BUF_FLAG_INTERNAL) &&
        ...
        s32 = copyMetaDataVaToHandle(GRALLOC_PV(pstBufSrc), GRALLOC_HDL(pstBufDst));
    else if (pstBufDst->u32InternalFlags & VPP_BUF_FLAG_INTERNAL)
        s32 = copyMetaDataHandleToVa(GRALLOC_HDL(pstBufSrc), GRALLOC_PV(pstBufDst));
    else
...
uint32_t u32VppBuf_GrallocUbwcStatsGet(t_StVppBuf *pstBuf)
{
    ...
    if (s32)
    {
        LOGI("unable to get ubwc stats from gralloc, s32=%d", s32);
        return VPP_ERR;
    }
...
uint32_t u32VppBuf_GrallocMetadataColorFmtSet(t_StVppBuf *pstBuf,
                                              enum vpp_color_format eFmt)
{
    ...
    if (!pstBuf || eFmt >= VPP_COLOR_FORMAT_MAX)
        return VPP_ERR_PARAM;

    if (u32VppUtils_IsFmtRgb(eFmt))
    {
        // TODO: GraphicsMetadata struct is non-trivial. They should provide API for filling.
        LOGE("eFmt=%d is RGB. Setting RGB gralloc is not implemented!", eFmt);
        return VPP_ERR_PARAM;
    }
    else
    {
        // Explicit else since above condition may not return when fully implemented
        // Program color metadata for YUV BT.709
        memset(&stColorMetaData, 0, sizeof(ColorMetaData));
        stColorMetaData.colorPrimaries = ColorPrimaries_BT709_5;
        stColorMetaData.range = Range_Full;
        stColorMetaData.transfer = Transfer_SMPTE_170M;
        stColorMetaData.matrixCoefficients = MatrixCoEff_BT709_5;
        u32 = u32VppBuf_GrallocMetaDataSet(pstBuf, COLOR_METADATA, &stColorMetaData);
        if (u32 != VPP_OK)
        {
            LOGE("Error setting color metadata, u32=%u", u32);
            u32Ret = VPP_ERR;
        }

        // Invalidate graphics buffer
        u32 = u32VppBuf_GrallocMetaDataSet(pstBuf, SET_GRAPHICS_METADATA, NULL);
        if (u32 != VPP_OK)
        {
            LOGE("Error clearing graphics metadata, u32=%u", u32);
            u32Ret = VPP_ERR;
        }
    }
...
uint32_t bVppBuf_IsHdr(t_StVppBuf *pstBuf)
{
#ifdef VPP_GRALLOC_DOES_NOT_EXIST
    VPP_UNUSED(pstBuf);
    return VPP_FALSE;
#else
    uint32_t bIsHdr = VPP_FALSE;
    ColorMetaData stColorMetaData;
    int s32;

    VPP_RET_IF_NULL(pstBuf, VPP_FALSE);

    GET_GRALLOC_METADATA(pstBuf, GET_COLOR_METADATA, &stColorMetaData, s32);
    if (s32)
    {
        LOGE("unable to get color metadata from gralloc, s32=%d", s32);
        return VPP_FALSE;
    }

    if (stColorMetaData.colorPrimaries == ColorPrimaries_BT2020 &&
        (stColorMetaData.transfer == Transfer_SMPTE_ST2084 ||
         stColorMetaData.transfer == Transfer_HLG))
    {
        bIsHdr = VPP_TRUE;
    }

    LOGD("Dynamic Range bIsHdr=%u", bIsHdr);
    return bIsHdr;
#endif
}
                        ...
                        struct vpp_port_param stPort, const char *pcFn)
{
    ...
    {
        uint32_t u32Bpc = 1; // Bytes per component
        if (stPort.fmt == VPP_COLOR_FORMAT_P010)
            u32Bpc = 2;
        if (stPort.fmt == VPP_COLOR_FORMAT_RGBA8 || stPort.fmt == VPP_COLOR_FORMAT_BGRA8)
            u32Bpc = 4;
        ...
        if (!u32VppUtils_IsFmtRgb(stPort.fmt))
        {
...
uint32_t u32VppBuf_DumpRawData(t_StVppCtx *pstCtx, const char *pcAddr, uint32_t u32Size,
                               const char *pcFn, t_EVppDumpMode eMode)
{
    char cPath[VPP_DUMP_BUF_LEN];
    FILE *fp;
    uint32_t u32Ret;
...
uint32_t u32VppBuf_GrallocMetaDataGet(t_StVppBuf *pstBuf, uint32_t u32Param, void *pvData)
{
#ifdef VPP_GRALLOC_DOES_NOT_EXIST
    VPP_UNUSED(pstBuf);
    VPP_UNUSED(u32Param);
    ...
    VPP_RET_IF_NULL(pstBuf, VPP_ERR_PARAM);
    VPP_RET_IF_NULL(pstBuf->pBuf, VPP_ERR_PARAM);
    VPP_RET_IF_NULL(pstBuf->pBuf->pvGralloc, VPP_ERR_PARAM);
    VPP_RET_IF_NULL(pvData, VPP_ERR_PARAM);

    GET_GRALLOC_METADATA(pstBuf, u32Param, pvData, s32);
    if (s32)
    {
        LOGE("unable to get metadata u32Param=0x%x from gralloc, s32=%d", u32Param, s32);
        return VPP_ERR;
    }
...
uint32_t u32VppBuf_GrallocMetaDataSet(t_StVppBuf *pstBuf, uint32_t u32Param, void *pvData)
{
#ifdef VPP_GRALLOC_DOES_NOT_EXIST
    VPP_UNUSED(pstBuf);
    VPP_UNUSED(u32Param);
    ...
    // pvData NULL is allowed. NULL will unset the parameter
    VPP_RET_IF_NULL(pstBuf, VPP_ERR_PARAM);
    VPP_RET_IF_NULL(pstBuf->pBuf, VPP_ERR_PARAM);
    VPP_RET_IF_NULL(pstBuf->pBuf->pvGralloc, VPP_ERR_PARAM);

    SET_GRALLOC_METADATA(pstBuf, u32Param, pvData, s32);
    if (s32)
    {
        LOGE("unable to set metadata u32Param=0x%x from gralloc, s32=%d", u32Param, s32);
        return VPP_ERR;
    }
