...
uint32_t u32VppIp_CmdPeekBack(t_StVppIpCmdQueue *pstQ, t_StVppIpCmd *pstCmd)
{
    if (!pstQ || !pstCmd)
        return VPP_ERR_PARAM;
    ...
    if (!pstQ->bThreadSafe)
    {
        LOGE("non thread safe queue passed to peekback");
        return VPP_ERR_PARAM;
    }

    return u32VppIp_NtsCmdPeekBack(pstQ, pstCmd, &pstQ->mutex);
}
...
uint32_t u32VppIp_NtsCmdPeekBack(t_StVppIpCmdQueue *pstQ, t_StVppIpCmd *pstCmd,
                                 pthread_mutex_t *pstMutex)
{
    int32_t idx;
    uint32_t u32Ret = VPP_OK;
    ...
    idx = vpp_queue_peek_back(&pstQ->stCmdQ);
    if (idx < 0)
    {
        u32Ret = VPP_ERR;
    }
    ...
    {
        *pstCmd = pstQ->pstCmdNode[idx];
        LOG_CMD("PeekBackCmd", pstCmd->eCmd);
    }
