...
struct vpp_ctrl_frc_segment {
    ...
    /*! The buffer timestamp to start applying the parameters. The parameters will remain */
    /*! active until the buffer timestamps reach the next applicable segment */
    uint64_t ts_start;
    /*! Generate an output frame even if the algo chooses to fallback (no interpolation). */
    /*! The frame will be a copy of the input. This ensures a constant FPS output */
    uint32_t frame_copy_on_fallback;
    /*! Copy input to output buffers instead of directly bypassing input buffer to output port*/
    uint32_t frame_copy_input;
};

struct hqv_ctrl_frc {
    /*! Pointer to segment(s) with FRC control parameters. NULL is equivalent to disabling FRC */
    struct vpp_ctrl_frc_segment *segments;
    /*! The number of segments being sent. Sending 0 segments is equivalent to disabling FRC */
 ...
 *
 * @return      OK          if there is no error with accepting the controls
 ...
 *              When the VPP library has received a call for both ports, it shall
 *              validate the port params received.
 *
 * @input       ctx         Context returned from vpp_init
...
/*!
 * @brief       Synchronous. Populates the metadata and buffer requirements.
 *
 * @description Upon returning from this function, the library
 *              will indicate the buffering requirements and necessary metadata
 *              for the session (based on the controls and port parameters).
 *
 * @input       ctx         Context returned from vpp_init
 * @output      req         Requirements from VPP for this session
 *
 * @return      OK          if the metadata and buffer requirements populated
 *              ERROR       if there is an error
 */
 ...
 * @input       ctx         Context returned from vpp_init
 *
 * @return      OK          if vpp successfully transitions to active state.
 *              ERROR       if there is an error
 */
 ...
 * @input       ctx         Context returned from vpp_init
 *
 * @return      OK          if vpp successfully transitions to inited state.
 *              ERROR       if there is an error
 */
