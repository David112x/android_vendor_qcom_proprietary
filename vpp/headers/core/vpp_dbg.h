...
#define VPP_LOG_MODULE_C2D              13
#define VPP_LOG_MODULE_C2D_TAG          "vpp-c2d"
#define VPP_LOG_MODULE_PROFILE          14
#define VPP_LOG_MODULE_PROFILE_TAG      "vpp-prof"
#define VPP_LOG_MODULE_TUNINGS          15
#define VPP_LOG_MODULE_TUNINGS_TAG      "vpp-tunings"
#define VPP_LOG_MODULE_OTHER            16
...
#define VPP_LOG_UT_C2D                  13
#define VPP_LOG_UT_C2D_TAG              "vpp-test_c2d"
...
#define VPP_LOGA(fmt, a...)     printf("%20s - A/ " fmt "\n", LOG_TAG, ##a)
#else
...
#define VPP_LOGA(fmt, a...)     ({ vpp_warning_suppressor(0, fmt, ##a); })
#endif
...
#define LOG_FRC(L, pfrc)        do { \
                                    if ((pfrc)->segments) \
                                        for (uint32_t i = 0; i < (pfrc)->num_segments; i++) \
                                            LOG##L("->frc[%u]:{mode=%u, level=%u, interp=%u, ts_start=%" PRId64 ", frame_copy_on_fallback=%u, frame_copy_input=%u}", \
                                                   i, (pfrc)->segments[i].mode, (pfrc)->segments[i].level, (pfrc)->segments[i].interp, \
                                                   (pfrc)->segments[i].ts_start, (pfrc)->segments[i].frame_copy_on_fallback, (pfrc)->segments[i].frame_copy_input); \
                                } while (0)
