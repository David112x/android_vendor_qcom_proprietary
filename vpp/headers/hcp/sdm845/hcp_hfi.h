...
/////////////////////////////////////////////////////////////////
    ...
    //! LTM NUMBER OF BLENDED LUTS {1, 64, [DFLT=16]}
    LTMTUNE_NLUTCNTBLEND,

    //! LTM LUMA CLIPPIMG GAP: {0, 127, [DFLT={4,8,16,24,24,30,30,0}], 0 MEANS NO CLIPPING}. LEN=8
    LTMTUNE_ANCLPGAP,
    ...
    //! USE ROUNDING (INSTEAD OF CLIPPING) IN AVERAGE LUT GENERATION {0, 1, [DFLT=0]}
    LTMTUNE_NENLUTROUND,

    //! USE FILTERED HISTOGRAM TO UPDATE LUT (OTHERWISE USE CURRENT FRAME) {0=USE CURRENT, 1, [DFLT=0]}
    LTMTUNE_NUPDLUTWITHAVGHISTO,

    //!UPDATE DE GAIN IF NUMBER OF TILES WITH SCENE CHANGED IN THE FRAME IS HIGHER THAN THIS VALUE (PERCENTAGE) : {0, 128, [DFLT=0]}
    LTMTUNE_NHSTUPDDEL,

    //!SET DE GAIN TO ZERO IF NUMBER OF FLAT TILES IN THE FRAME IS GREATER THAN THIS VALUE (PERCENTAGE): {0, 128, [DFLT=0]}
    LTMTUNE_NLUTUPDDEL,

    //!IIR FILTER COEFFICIENT FOR HISTOGRAM ACCUMULATION: {0.0,1.0, [DFLT=0.25]}
    LTMTUNE_FHSTCOEFFIIR,

    //! DE IIR FILTER COEFFICIENT FOR CORING TABLE UPDATE: {0.0,1.0 [DFLT=1.0]}
    LTMTUNE_FDECOEFFIIR,

    //! TONE MAPPING COEFFICIENT  {0.0,10.0 [DFLT=1.0]}
    LTMTUNE_FTMCOEFF,

    //! TONE MAPPING FUNCTION ORDER. 0=AFTER HE, 1=BEFORE HE  {0,1 [DFLT=1]}
    LTMTUNE_NTMORDER,
