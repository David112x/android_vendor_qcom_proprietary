...
int LBSAdapter::injectWifiPosition(const Location* location)
{
    struct WifiPosMsg : public LocMsg {
        LocApiBase* mLocApi;
        const Location* mWifiPos;
        inline WifiPosMsg(LocApiBase* locApi, const Location *location) :
            LocMsg(), mLocApi(locApi), mWifiPos(location) {}
        inline ~WifiPosMsg() {delete mWifiPos;}
        inline virtual void proc() const {
            mLocApi->injectPosition(*mWifiPos, true);
        }
    };
    sendMsg(new WifiPosMsg(mLocApi, location));
    return true;
}
