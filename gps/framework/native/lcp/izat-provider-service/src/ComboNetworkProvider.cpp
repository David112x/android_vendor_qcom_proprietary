...
void ComboNetworkProvider::injectLocationInIdleMsg::proc() const
{
    ...
    ENTRY_LOG();
    do {
        LOC_LOGD("LocTech-Label :: Position Report In");
        string locationReport;
        mLocationReport.stringify(locationReport);
        LOC_LOGD("LocTech-Value :: Location Report: %s", locationReport.c_str());

        // drop the location if location not valid
        BREAK_IF_ZERO(1, mLocationReport.isValid());

        // drop the location if ProcessorSrc of location is not ProcessorSrc_AP
        BREAK_IF_ZERO(2, mLocationReport.mHasProcessorSource);
        BREAK_IF_ZERO(3, LocationReport::ProcessorSrc_AP == mLocationReport.mProcessorSource);

        if (COMBO_STATE_DISABLED != mCombo->mProviderState) {
            if (mCombo->mLbsAdapter != NULL) {
                LOC_LOGD("Inject Wifi position");
                mCombo->mLbsAdapter->injectWifiPosition(convertLocationReport(mLocationReport));
            }
        }
        result = 0;
    } while (0);
...
const Location* ComboNetworkProvider::convertLocationReport(const LocationReport& in) {
    ENTRY_LOG();
    Location* out = new Location();
    memset(out, 0, sizeof(Location));
    out->size = sizeof(Location);
    if (in.mHasUtcTimestampInMsec) {
        out->timestamp = in.mUtcTimestampInMsec;
    }
    if (in.mHasLatitude && in.mHasLongitude) {
        out->latitude = in.mLatitude;
        out->longitude = in.mLongitude;
        out->flags |= LOCATION_HAS_LAT_LONG_BIT;
    }
    if (in.mHasHorizontalAccuracy) {
        out->accuracy = in.mHorizontalAccuracy;
        out->flags |= LOCATION_HAS_ACCURACY_BIT;
    }
    if (in.mHasAltitudeWrtEllipsoid) {
        out->altitude = in.mAltitudeWrtEllipsoid;
        out->flags |= LOCATION_HAS_ALTITUDE_BIT;
    }
    if (in.mHasVertUnc) {
        out->verticalAccuracy = in.mVertUnc;
