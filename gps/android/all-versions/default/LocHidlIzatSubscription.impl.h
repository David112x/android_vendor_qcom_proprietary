...
static void setUserPref(bool userPref) {
    void *handle = nullptr;
    createXtAdapterInstance* createInstanceFn = (createXtAdapterInstance*)dlGetSymFromLib(handle,
            "libxtadapter.so", "createXtAdapterInstance");
    if (nullptr == createInstanceFn) {
        LOC_LOGe("createInstanceFn NULL!");
        return;
    }
    (*createInstanceFn)();
    setXtAdapterUserPref* setUserPrefFn = (setXtAdapterUserPref*)dlGetSymFromLib(handle,
            "libxtadapter.so", "setXtAdapterUserPref");
    ...
    for (int idx = 0; idx < (int)dataItemArray.size(); idx++) {
        switch (dataItemArray[idx].id)
        {
            case ENH_DATA_ITEM_ID:
                ...
                //update GTP WWAN state
                setUserPref(enh_data_item.mEnabled);
                break;
