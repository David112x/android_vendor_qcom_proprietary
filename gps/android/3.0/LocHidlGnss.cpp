...
Return<sp<V1_0::ILocHidlIzatProvider>> LocHidlGnss::getExtensionLocHidlIzatNetworkProvider() {
    ...
    if (mIzatNetworkProvider == nullptr) {
        ...
        //Register LocHidl ODCPI callback for NON-GMS case
        IIzatManager *pIzatManager = getIzatManager(OSFramework::getOSFramework());
        if (NULL != pIzatManager) {
            static_cast<IzatManager*>(pIzatManager)->registerIzatOdcpiCallback();
        } else {
            LOC_LOGE("Failed to get Izatmanager instance.");
        }
    }
