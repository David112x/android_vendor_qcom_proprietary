...
boolean ftm_bt_hci_hal_read_event (uint8 * event_buf_ptr)
{
  ...
  */
       ...
       is_slim_bus_test == 1)
  {
    ...
    // emulate HCI_VS_DEBUG_PIN_TEST_EVT event
    event_buf_ptr[1] = BT_HCI_VND_SPECIFIC_EVT;
    event_buf_ptr[2] = HCI_VS_DEBUG_PIN_TEST_EVT_LEN;
    event_buf_ptr[3] = HCI_VS_DEBUG_OPCODE;
    event_buf_ptr[4] = HCI_VS_DEBUG_PIN_TEST_CMD;
    event_buf_ptr[5] = INTERFACE_ID_SLIMBUS;
    if (ret_val < 0)
    {
      event_buf_ptr[6] = ret_val;
    } else
    {
      event_buf_ptr[6] = 0;
    }
  ...
  if ( event_buf_ptr[0] == BT_HCI_EVT_PKT &&
      event_buf_ptr[1] == BT_HCI_CONN_CMPLT_EVT &&
      event_buf_ptr[2] == HCI_CONN_CMPLT_EVT_LEN &&
      is_slim_bus_test == 1)
  {
    flag_send = FALSE;
  }
  ...
  {
      printf("skip sending cc event to diag interface\n");
  }
