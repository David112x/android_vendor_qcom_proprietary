...
#else
#  if defined(__LP64__)
#    if defined (USES_FULL_TREBLE) || defined (USES_MULTI_HAL)
...
#else
  #ifdef SNS_LE_QCS605
    #define HAL_LIB_NAME "/usr/lib64/libsensors.ssc.so.1"
  #else
    #define HAL_LIB_NAME HAL_LIB_PATH"sensors."BOARD_PLATFORM".so"
  #endif /* SNS_LE_APQ8053 */
#endif
...
#endif /* SENSORS_DEVICE_API_VERSION_1_1 */
      {
       ...
       if( data[i].type == SENSOR_TYPE_ADDITIONAL_INFO) {
        ...
        printf("%lu.%06lu, %s/%s, %ld.%06ld, %f, %f, %f, %f, %lld, latency(ms): %lu.%06lu\n",
                 (unsigned long)now_ns/NSEC_PER_MS, (unsigned long)now_ns%NSEC_PER_MS,
                 type_to_string( data[i].type ), handle_to_string(list, data[i].sensor),
                 (long int)data[i].timestamp/NSEC_PER_MS, (long int)data[i].timestamp%NSEC_PER_MS,
                 data[i].data[0], data[i].data[1], data[i].data[2], data[i].data[3],
                 (long long int)data[i].u64.step_counter,
                 (unsigned long)(now_ns - data[i].timestamp) / NSEC_PER_MS,
                 (unsigned long)(now_ns - data[i].timestamp) % NSEC_PER_MS);
        }
#endif
...
int main( int argc, char * const argv[] )
{
  ...
  fprintf(stderr,"HAL open\n");
  fprintf(stderr,"HAL module_api_version: 0x%d\n", HAL_MODULE_INFO_SYM.common.module_api_version );
  fprintf(stderr,"HAL hal_api_version   : 0x%d\n", HAL_MODULE_INFO_SYM.common.hal_api_version );
  fprintf(stderr,"HAL hal_id            : %s\n", HAL_MODULE_INFO_SYM.common.id );
  fprintf(stderr,"HAL hal_name          : %s\n", HAL_MODULE_INFO_SYM.common.name );
  fprintf(stderr,"HAL hal_author        : %s\n", HAL_MODULE_INFO_SYM.common.author );
#endif
...
#else
  sensor_count = HAL_MODULE_INFO_SYM.get_sensors_list( NULL, ((struct sensor_t const**)&list) );
#endif
