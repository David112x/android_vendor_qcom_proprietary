...
using android::hardware::sensors::V1_0::implementation::convertToSensor;
...
std::vector<sensor_t> _hal_sensors_;
...
int main( int argc, char * const argv[] )
{
  ...
  sensors->getSensorsList(
    ...
    for (auto it = list_.begin(); it != list_.end(); it++,i++) {
      sensor_t sensor;
      android::hardware::sensors::V1_0::implementation::convertToSensor(list_[i],&sensor);
      _hal_sensors_.push_back(sensor);
    }
  });

  list = &_hal_sensors_[0];
  sensor_count = _hal_sensors_.size();
  ...
  //De-activating already enabled sensors
  struct sensor_t const *list_p_temp = list;
  for(int i=0;i<sensor_count;i++){
    result  = sensors->activate(list_p_temp->handle, false );
    list_p_temp++;
  }
  ...
  if(!is_interactive)
  {
    ...
    result  = sensors->activate(sensor_to_use->handle, true );
    if( result != Result::OK) {
    ...
    result = sensors->batch(sensor_to_use->handle, sampling_rate_nsecs, report_rate_nsecs );
    ...
    //DEACTIVATE;
    result  = sensors->activate(sensor_to_use->handle, false );
    if( result != Result::OK) {
  ...
  do
  {
    ...
    do
    {
      fprintf(stderr,"Please choose a command (a,d,b,f,e)> ");
      fgets(input_buffer,80,stdin);
      fflush(stdin);
    ...
    switch(command)
    {
      ...
      case 'a':
        ...
        result = sensors->activate(list_p->handle, true );
        if( result != Result::OK) {
          fprintf(stderr,"Error %d activating sensor\n", error );
        }
      break;
      ...
      case 'd':
        ...
        result  = sensors->activate(list_p->handle, false );
        if( result != Result::OK) {
          fprintf(stderr,"Error %d deactivating sensor\n", error );
        }
      break;
      ...
      case 'b':
        ...
        result  = sensors->batch(list_p->handle, sampling_rate_nsecs, report_rate_nsecs );
        if( result != Result::OK) {
          fprintf(stderr,"Error %d in batch\n", error );
        } else {
          fprintf(stderr,"batch success\n");
      ...
      case 'f':
        ...
        result  = sensors->flush(list_p->handle );
        if( result != Result::OK) {
