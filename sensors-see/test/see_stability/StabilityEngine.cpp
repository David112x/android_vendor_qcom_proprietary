...
const std::string StabilityEngine::RIGID_BODY    = "rigid_body";
const std::string StabilityEngine::RANDOM_SENSOR = "random_sensor"; // not implemented
...
const std::string StabilityEngine::RAND_ODR      = "rand_odr";
const std::string StabilityEngine::SENSOR_ATTRIB_NAME = "sensor_attrib_name";
...
const std::string StabilityEngine::DELAY_BEFORE_REQ   = "delay_before_req";
...
const std::string StabilityEngine::STREAM_PRIORITY = "stream_priority";
const std::string StabilityEngine::TEST_TYPE       = "test_type";

const std::string StabilityEngine::DISABLE_PRIORITY = "disable_priority";
const std::string StabilityEngine::DELAY_BEFORE_NEXT_DISABLE_REQ =
                                               "delay_before_next_disable_req";

// Keys to populate test_param map
// '$' is used as a delimiter
const std::string StabilityEngine::RIGID_BOD         = "$rigid_body";
const std::string StabilityEngine::MSG_ID            = "$msg_id";
const std::string StabilityEngine::SENS_ATTRIB_NAME  = "$sens_attrib_name";
...
const std::string StabilityEngine::RAND_ODR_RT       = "$rand_odr_rt";
...
const std::string StabilityEngine::MIN_DEL_BEFORE_REQ  = "$min_delay_before_req";
const std::string StabilityEngine::MAX_DEL_BEFORE_REQ  = "$max_delay_before_req";
const std::string StabilityEngine::DEL_BEFORE_REQ      = "$delay_before_req";
...
const std::string StabilityEngine::PRIORITY          = "$priority";
const std::string StabilityEngine::TTYPE             = "$ttype";

const std::string StabilityEngine::DISABLE_PRI       = "$disable_pri";
const std::string StabilityEngine::DEL_BEF_NEXT_DISABLE_REQ =
                                                    "$del_bef_next_disable_req";

#define NUM_RIGID_BODIES 3
const std::string rigid_bodies[] = {"display", "keyboard", "external"};
...
/**
 * Convert from rigid_body string to rigid body's index
 * @param value - rigid_body string value
 * @return float - rigid_body's index or -1 == invalid value
 */
float rigid_body_2_index(std::string value)
{
   int i = 0;
   for ( ; i < NUM_RIGID_BODIES; i++) {
      if ( value == rigid_bodies[i]) {
         return i;
      }
   }
   std::string desc = "FAIL rigid_body=" + value + " not defined.";
   throw desc.c_str();
   return -1;
}

/**
 * Convert from rigid_body's index to rigid body string value
 * @param index  - rigid_body's index
 * @return string - rigid body string value or ""
 */
const std::string StabilityEngine::index_2_rigid_body(float index)
{
   if ( index >= 0 && index < NUM_RIGID_BODIES) {
      return rigid_bodies[ (int)index];
   }
   return "";
}
                                         ...
                                         )
{
    ...
    for(;sensor_it != sensor_configs.cend(); ++sensor_it) {
        ...
        float sen_attrib_name = 0.0f;

        float msg_id = 0.0f;
        float clnt = 0.0f;
        float db_len = 0.0f;
        float priority = 0.0f;
        float test_type = 0.0f;
        float disable_priority = 0.0f;
        float delay_before_disable_req = 0.0f;

        for(auto config_it = sensor_it->second.cbegin();
            config_it != sensor_it->second.cend(); ++config_it) {

            // Type of request
            if ( config_it->first == RIGID_BODY) {
               std::string value = config_it->second;
               float index = rigid_body_2_index(value);
               test_params.insert(it,std::pair<std::string, float>(
                                  sensor_type + RIGID_BOD, index));
               continue;
            }
            if ( config_it->first == "TC_SPEC") {
               std::string tc_spec = config_it->second;
               tc_specs.push_back( tc_spec);

               float index = tc_specs.size() - 1;
               test_params.insert(it,std::pair<std::string, float>(
                                     sensor_type + "$tc_spec_index", index));
               continue;
            }
...
//----------------------------------------------------------------------------
            // Process sensor attrib name
            if(config_it->first == SENSOR_ATTRIB_NAME) {
                if(config_it->second == "true") {
                   sen_attrib_name = 1.0f;
                   test_params.insert(it, std::pair<std::string,
                                       float>(sensor_type + SENS_ATTRIB_NAME,
                                               sen_attrib_name));
                }
                else {
                    sen_attrib_name = 0.0f;
                    test_params.insert(it, std::pair<std::string,
                                        float>(sensor_type + SENS_ATTRIB_NAME,
                                                sen_attrib_name));
                }
            } // end sensor attrib name
//----------------------------------------------------------------------------
            // Process sensor name. Insert a dummy value of 0.0f as value for
            // the key (sensor_name)
            if((config_it->first).rfind(sensor_type + "$") == 0) {
                test_params.insert(it, std::pair<std::string, float>(
                                                 config_it->first, 0.0f));
            }
//----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
            // Process stream priority
            if(config_it->first == STREAM_PRIORITY) {
                priority = stof(config_it->second);
                test_params.insert(it,std::pair<std::string, float>(
                                     sensor_type + PRIORITY, priority));
            } // end stream priority
//-----------------------------------------------------------------------------
            // Process disable priority
            if(config_it->first == DISABLE_PRIORITY) {
                disable_priority = stof(config_it->second);
                test_params.insert(it,std::pair<std::string, float>(
                                     sensor_type + DISABLE_PRI, disable_priority));
            } // end disable priority
//-----------------------------------------------------------------------------
            // Process delay before next disable request
            if(config_it->first == DELAY_BEFORE_NEXT_DISABLE_REQ) {
                delay_before_disable_req = stof(config_it->second);
                test_params.insert(it,std::pair<std::string, float>(
                                     sensor_type + DEL_BEF_NEXT_DISABLE_REQ,
                                                    delay_before_disable_req));
            } // end delay before next disable request
//-----------------------------------------------------------------------------
            // Process test type
            if(config_it->first == TEST_TYPE) {
                if(config_it->second == "continuous") {
                    test_type = 1.0f;
                    test_params.insert(it,std::pair<std::string, float>(
                                         sensor_type + TTYPE, test_type));
                }
                else {
                      if(config_it->second == "b2b") {
                         test_type = 0.0f;
                          test_params.insert(it,std::pair<std::string, float>(
                                               sensor_type + TTYPE, test_type));
                      }
...
//----------------------------------------------------------------------------
            // Process random ODR
            if(config_it->first == RAND_ODR) {
                rand_odr = stof(config_it->second);
                test_params.insert(it, std::pair<std::string, float>(
                                           sensor_type + RAND_ODR_RT, rand_odr));
            } // end random odr
//-----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
            ...
            if(config_it->first == STREAM_DURATION) {
               ...
               if((config_it->second.find_first_of(COLON_DELIMITER)) !=
                                                   std::string::npos) {
                   // Get min and max. e.g. stream_duration = 0.2:0.9
                   ...
                   test_params.insert(it, std::pair<std::string, float>(
                                               sensor_type + MIN_STREAM_DUR,
                                                             min_str_dur));
                   // Insert into map:e.g. accel_max_stream_duration=>0.9
                   test_params.insert(it, std::pair<std::string, float>(
...
//----------------------------------------------------------------------------
            // Process delay
            if(config_it->first == DELAY_BEFORE_REQ) {
               ...
               if((config_it->second.find_first_of(COLON_DELIMITER)) !=
                  ...
                  test_params.insert(it, std::pair<std::string, float>(
                                            sensor_type + MIN_DEL_BEFORE_REQ,
                                                                  min_delay));

                  test_params.insert(it, std::pair<std::string, float>(
                                            sensor_type + MAX_DEL_BEFORE_REQ,
                                                                  max_delay));
                }
