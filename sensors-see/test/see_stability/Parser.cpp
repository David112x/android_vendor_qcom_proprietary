        ...
        void parse_required_args(const std::string& required_args)
        {
            ...
            while (getline(file, line))
            {
                        ...
                        std::string::npos)
                {
                        ...
                        std::string tc_spec = "[" + till_bracket + "]";
                        std::cout << "tc_spec: " << tc_spec << std::endl;
                                                        ...
                                                        != std::string::npos)
                        {
                            ...
                            sensor_config_mapping.insert(std::make_pair(
                                                     ...
                                                     sensor_configs()));
                            /* insert TC_SPEC */
                            if (sensor_name_sensor_configs_it !=
                                sensor_config_mapping.end()) {
                               sensor_name_sensor_configs_it->second.insert(
                                  std::make_pair("TC_SPEC", tc_spec));
                            }
...
//-----------------------------------------------------------------------------
        ...
        void set_values_of_optional_args(const std::string& optional_args)
        {
          ...
          opt_args_mapping.insert(std::make_pair(SEED, &set_seed));
          opt_args_mapping.insert(std::make_pair(MAX_CAP, &set_max_capacity));
          ...
          opt_args_mapping.insert(std::make_pair(NUM_QMI_CLIENTS,
                                                   &set_num_qmi_clients));
          opt_args_mapping.insert(std::make_pair(NUM_QUEUED_SENSORS,
                                                   &set_num_queued_sensors));
          opt_args_mapping.insert(std::make_pair(TOT_REPORT_COUNT,
                                                   &set_tot_report_count));
          opt_args_mapping.insert(std::make_pair(MAX_REQUEST_DURATION,
                                                   &set_max_request_duration));
...
//----------------------------------------------------------------------------
        ...
        {
           // Convert from std::string to unsigned int
           random_seed = std::atoi(seed.c_str());
...
//----------------------------------------------------------------------------
        ...
        // Return the number of sensors queued
        int get_num_queued_sensors() {
            auto pos = optional_args_mapping.find(NUM_QUEUED_SENSORS);
            if (pos != optional_args_mapping.end()) {
                return (stoi(optional_args_mapping[NUM_QUEUED_SENSORS]));
            }
            return(default_queued_sensors);
        } // Parser::get_num_queued_sensors
//----------------------------------------------------------------------------
