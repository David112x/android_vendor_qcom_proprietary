...
namespace sensors_moast {
    ...
    T get_random_min_max(const T& min, const T& max, const size_t& seed_val,
        ...
        if(std::is_integral<T>::value) { // if integer
            // returns randomized value between min and max
...
//-----------------------------------------------------------------------------
    /*
     * Return randomized string(sensor or stream or batch) or odr rate
     *
     * @param V as const std::Vector reference
     * @param seed as const size_t reference
     * @return randomized string or odr rate of type V
     */
    template<typename V>
    V get_random_string_or_odr(const std::vector<V>& v,
