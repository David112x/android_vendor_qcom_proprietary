...
const int SNS_STD_SENSOR_ATTRID_ADDITIONAL_LOW_LATENCY_RATES = 25;
const int SNS_STD_SENSOR_ATTRID_PASSIVE_REQUEST = 26;

const int SNS_STD_SENSOR_ATTRID_POR = 27;
const int SNS_STD_SENSOR_ATTRID_MAX_RATE = 28;
#define RESERVE_SPACE_FOR_ATTRIBUTES 48 // 28 + space for new entries
...
void init_attr()
{
   if (done_once == false) {
      ...
      attr_num_to_name.reserve(RESERVE_SPACE_FOR_ATTRIBUTES);
      ...
      attr_num_to_name.push_back("SNS_STD_SENSOR_ATTRID_ADDITIONAL_LOW_LATENCY_RATES");
      attr_num_to_name.push_back("SNS_STD_SENSOR_ATTRID_PASSIVE_REQUEST");
...
int lookup_attr_id_num(std::string &attr_name)
{
   ...
   if ( it != attr_name_to_num.end()) {
      ...
      attr_name_to_num.insert(std::make_pair( attr_name, attr_id));
      attr_num_to_name.push_back(attr_name);
   }
   return attr_id;
}
