                          ...
                          bool delete_or_disconnect)
{
   ...
   if ( psalt) {
      SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
      if (sns_cxt_instance) {
         rc = sns_cxt_instance->stop_request( handle, delete_or_disconnect);
      }
   }
   return (salt_rc)rc;
}
...
/**
 * @brief enable and register event callback function, or disable callback
 * @param psalt
 * @param handle - handle for sensor
 * @param event_cb  - address of event callback function
 *
 * @return salt_rc
 */
salt_rc usta_register_event_cb(see_salt *psalt,
                               unsigned int handle,
                               event_cb_func event_cb)
{
   salt_rc rc = SALT_RC_NULL_ARG;
   if ( psalt) {
      SensorCxt *sns_cxt_instance = usta_get_instance( psalt);
      if (sns_cxt_instance) {
         if (event_cb) {
            sns_cxt_instance->enableStreamingStatus(handle);
            sns_cxt_instance->update_display_samples_cb((display_samples_cb)event_cb);
         }
         else
            sns_cxt_instance->disableStreamingStatus(handle);
