...
*       "DRI" :  true | false,
*       "STREAM_TYPE" : "streaming" | "on_change" | "single_output",
*       "HW_ID" : number,
...
bool does_candidate_match_por_spec(sensor *psensor, por_sensor &por_spec)
{
   ...
   for (size_t i = 0; i < por_spec.list.size(); i++) {
      ...
      else if ( "STREAM_TYPE" == entry->key) {
         if ( psensor->has_stream_type()) {
            if ( entry->value == string("streaming")) {
               if ( psensor->is_stream_type_streaming())
                  continue;
            }
            else if ( entry->value == string("on_change")) {
               if ( psensor->is_stream_type_on_change())
                  continue;
            }
            else if ( entry->value == string("single_output")) {
      ...
      else if ( "HW_ID" == entry->key) {
         if ( psensor->has_hw_id()) {
            int actual_value = psensor->get_hw_id();
            if ( actual_value == atoi( entry->value.c_str())) {
               continue;
            }
         }
         match = false;
