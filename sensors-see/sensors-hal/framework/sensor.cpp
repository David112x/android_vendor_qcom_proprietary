...
void sensor::submit_sensors_hal_event(sensors_event_t event)
{
  /*SENSOR_TYPE_META_DATA e.g Flush do not need to have TS*/
    if (SENSOR_TYPE_META_DATA != event.type &&
        SENSOR_TYPE_ADDITIONAL_INFO != event.type) {
      if (event.timestamp > _previous_ts) {
        _event_cb(event, _bwakeup);
        _previous_ts = event.timestamp;
      } else {
        sns_logi("(%s)dropping un-ordered ts samples _previous %lld and current ts %lld",
            _sensor_info.name, (long long)_previous_ts, (long long)event.timestamp);
      }
    } else {
        _event_cb(event, _bwakeup);
    }
