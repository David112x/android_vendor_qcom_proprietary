     ...
     client_channel_handle(0)
{
    ...
    register_buf(buffer_ptr, buffer_size, this->get_buffer_fd(), 0);
...
int direct_channel::init_fastRPC_symbols(void)
{
  ...
  set_system_parition(false);

  sns_remote_handles remote_handles;
  if (0 != init_rpc_symbols(&remote_handles)) {
   sns_loge("init_rpc_symbols failed");
   return -EINVAL;
  }
  update_rpcmem_handles(&remote_handles);
  ...
  sns_logd("%s: all fastRPC symbols load success.", __FUNCTION__);
  return 0;
}
