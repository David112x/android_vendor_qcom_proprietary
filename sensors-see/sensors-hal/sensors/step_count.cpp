...
step_count::step_count(sensor_uid suid, sensor_wakeup_type wakeup):
    ssc_sensor(suid, wakeup), first_step_count(-1), persist_step_count_update(0)
{
...
#ifdef SNS_LE_QCS605
        sns_logv("step_detect_event_config: ts=%" PRIu64 " count %i persist_count_current %i",
#else
