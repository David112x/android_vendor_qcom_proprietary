...
public:
     ...
     */
     ...
     int get_pairedcalsuid(const std::string& datatype,
                                   const sensor_uid &master_suid,
                                   sensor_uid &paired_suid);

     bool is_default_sensor(const std::string& datatype, const sensor_uid& suid);
     ...
     */
    static void request_datatype(const char *datatype, bool default_only = true)
    {
        try {
            datatypes().insert(std::make_pair(std::string(datatype), default_only));
        } catch (const std::exception& e) {
...
private:
    ...
    std::unordered_map<std::string, std::vector<sensor_uid>> _default_suids_container;

    /* map of non-default sensor suids for each data type */
    std::unordered_map<std::string, std::vector<sensor_uid>> _all_suids_container;
    ...
    static std::unordered_map<std::string, bool>& datatypes()
    {
        static std::unordered_map<std::string, bool> _datatypes;
        return _datatypes;
    }
    ...
    void default_suids_lookup_callback(const std::string& datatype,
                              const std::vector<sensor_uid>& suids);

    /* callback function to handle suid events for all sensors*/
    void all_suids_lookup_callback(const std::string& datatype,
                              const std::vector<sensor_uid>& suids);
    ...
    /**
     * @brief compares the past registry version in persist to the version
              present in /vendor/etc/sensors/sns_reg_config file
     *
     * @param config file path and variable to return the latest version
     * @return true if changes
     */
    bool check_registry_version_change(const char* config_file, int &ret_version);
    /**
     * @brief handle change in version
     *
     * @param version version to be compare
     */
    void handle_version_update(int version);

    /**
     * @brief checks if any registry upgrade is required
     *        if upgrade happens , it clears the existing registry metadata
               and sensors_list file to let them populate freshly
     * @param void
     */
    void handle_registry_version_change();

#endif
    /* callback function to handle registry suid event to check registry up */
    void registry_lookup_callback
            (const std::string& datatype, const std::vector<sensor_uid>& suids);

    /* to check registry sensor up in expected time */
    bool is_registry_up();

    /* variables to process non-default sensor discovery */
    std::condition_variable _cv_non_default_sensor;
    int _num_of_response_for_non_default_sensors;
    int _num_of_non_default_sensors;
    pthread_mutex_t _m_waitMutex;
    /*condition variable to unblock registry_up method once registry available*/
    pthread_cond_t _m_waitCond;
