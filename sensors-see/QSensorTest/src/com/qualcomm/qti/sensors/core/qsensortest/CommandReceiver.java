    ...
    if(intent.getAction().contentEquals("com.qualcomm.qti.sensors.qsensortest.intent.STREAM_ALL")) {
      ...
      for(SensorAdapter adapter : sensorAdapters) {
          ...
          if (rate == SensorManager.SENSOR_DELAY_FASTEST) {
              if (sensor.isWakeUpSensor() == true && (sensorType != Sensor.TYPE_ACCELEROMETER_UNCALIBRATED &&
                  sensorType != Sensor.TYPE_GYROSCOPE_UNCALIBRATED && sensorType != Sensor.TYPE_MAGNETIC_FIELD_UNCALIBRATED)) {
                  adapter.streamRateIs(rate, batchRate, true);
              }
          } else {
              adapter.streamRateIs(rate, batchRate, true);
