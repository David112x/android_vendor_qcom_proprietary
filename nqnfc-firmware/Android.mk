LOCAL_PATH := $(call my-dir)

## NQ1x
include $(CLEAR_VARS)
LOCAL_MODULE             := libpn547_fw.so
LOCAL_MODULE_TAGS        := optional
LOCAL_MODULE_CLASS       := ETC
LOCAL_MODULE_PATH        := $(TARGET_OUT_VENDOR)/lib
LOCAL_SRC_FILES_arm      := pn547c2/32-bit/$(LOCAL_MODULE)
LOCAL_SRC_FILES_arm64    := pn547c2/64-bit/$(LOCAL_MODULE)
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_OWNER       := nxp
LOCAL_MODULE_TARGET_ARCH := arm arm64
include $(BUILD_PREBUILT)

## NQ2x
include $(CLEAR_VARS)
LOCAL_MODULE             := libpn548ad_fw.so
LOCAL_MODULE_TAGS        := optional
LOCAL_MODULE_CLASS       := ETC
LOCAL_MODULE_PATH        := $(TARGET_OUT_VENDOR)/lib
LOCAL_SRC_FILES_arm      := pn548ad/32-bit/$(LOCAL_MODULE)
LOCAL_SRC_FILES_arm64    := pn548ad/64-bit/$(LOCAL_MODULE)
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_OWNER       := nxp
LOCAL_MODULE_TARGET_ARCH := arm arm64
include $(BUILD_PREBUILT)

## NQ31x
include $(CLEAR_VARS)
LOCAL_MODULE             := libpn551_fw.so
LOCAL_MODULE_TAGS        := optional
LOCAL_MODULE_CLASS       := ETC
LOCAL_MODULE_PATH        := $(TARGET_OUT_VENDOR)/lib
LOCAL_SRC_FILES_arm      := pn551/32-bit/$(LOCAL_MODULE)
LOCAL_SRC_FILES_arm64    := pn551/64-bit/$(LOCAL_MODULE)
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_OWNER       := nxp
LOCAL_MODULE_TARGET_ARCH := arm arm64
include $(BUILD_PREBUILT)

## NQ3xx
include $(CLEAR_VARS)
LOCAL_MODULE             := libpn553_fw.so
LOCAL_MODULE_TAGS        := optional
LOCAL_MODULE_CLASS       := ETC
LOCAL_MODULE_PATH        := $(TARGET_OUT_VENDOR)/lib
LOCAL_SRC_FILES_arm      := pn553/32-bit/$(LOCAL_MODULE)
LOCAL_SRC_FILES_arm64    := pn553/64-bit/$(LOCAL_MODULE)
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_OWNER       := nxp
LOCAL_MODULE_TARGET_ARCH := arm arm64
include $(BUILD_PREBUILT)

## NQ4xx
include $(CLEAR_VARS)
LOCAL_MODULE             := libpn557_fw.so
LOCAL_MODULE_TAGS        := optional
LOCAL_MODULE_CLASS       := ETC
LOCAL_MODULE_PATH        := $(TARGET_OUT_VENDOR)/lib
LOCAL_SRC_FILES_arm      := pn557/32-bit/$(LOCAL_MODULE)
LOCAL_SRC_FILES_arm64    := pn557/64-bit/$(LOCAL_MODULE)
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_OWNER       := nxp
LOCAL_MODULE_TARGET_ARCH := arm arm64
include $(BUILD_PREBUILT)

## SN100x
include $(CLEAR_VARS)
LOCAL_MODULE             := libsn100u_fw.so
LOCAL_MODULE_TAGS        := optional
LOCAL_MODULE_CLASS       := ETC
LOCAL_MODULE_PATH        := $(TARGET_OUT_VENDOR)/lib
LOCAL_SRC_FILES_arm      := sn100x/32-bit/$(LOCAL_MODULE)
LOCAL_SRC_FILES_arm64    := sn100x/64-bit/$(LOCAL_MODULE)
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_OWNER       := nxp
LOCAL_MODULE_TARGET_ARCH := arm arm64
include $(BUILD_PREBUILT)

include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-mtp_default.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#NQ220 MTP
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-mtp_rf1.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#NQ3XX MTP
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-mtp-NQ3XX.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#NQ4XX MTP
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-mtp-NQ4XX.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#SN100 MTP
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-mtp-SN100.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#SN100 MTP with CLK_FREQ 38.4 MHz
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-mtp-SN100_38_4MHZ.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-mtp_rf2.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-qrd_default.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-qrd_rf1.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#NQ220 QRD
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-qrd_rf2.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#NQ3XX QRD
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-qrd-NQ3XX.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#NQ4XX QRD
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-qrd-NQ4XX.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#SN100 QRD
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-qrd-SN100.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

#SN100 QRD with CLK_FREQ 38.4 MHz
include $(CLEAR_VARS)
LOCAL_MODULE       := libnfc-qrd-SN100_38_4MHZ.conf
LOCAL_MODULE_TAGS  := optional
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH  := $(TARGET_OUT_VENDOR_ETC)
LOCAL_SRC_FILES    := $(LOCAL_MODULE)
include $(BUILD_PREBUILT)

include $(call all-makefiles-under,$(LOCAL_PATH))
