...
OperationStatus PmcRegistersAccessor::WritePmcRegister(const std::string& deviceName, const std::string& registerName, uint32_t value) const
{
    ...
    if(!registerInfo.IsValid())
    {
        ...
        oss << "Failed retrieving register info for register " << registerName;
        return OperationStatus(false, oss.str());
    }
    uint32_t regValue = 0;
    OperationStatus statusRead =
        Host::GetHost().GetDeviceManager().Read(deviceName, registerInfo.GetRegisterAddress(), reinterpret_cast<DWORD&>(regValue));
    ...
    OperationStatus statusWrite =
        Host::GetHost().GetDeviceManager().Write(deviceName, registerInfo.GetRegisterAddress(), reinterpret_cast<DWORD&>(dataToWrite));
...
OperationStatus PmcRegistersAccessor::ReadPmcRegister(const std::string& deviceName, const std::string& registerName, uint32_t& registerValue) const
{
    ...
    uint32_t temp;
    OperationStatus res =
        Host::GetHost().GetDeviceManager().Read(deviceName, registerInfo.GetRegisterAddress(), reinterpret_cast<DWORD&>(temp));
    if(!res.IsSuccess())
    {
        LOG_ERROR << res;
        std::ostringstream oss;
