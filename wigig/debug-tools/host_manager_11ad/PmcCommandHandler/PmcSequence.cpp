...
{
    return Host::GetHost().GetDeviceManager().DeallocPmc(deviceName, bSuppressError);
}
...
OperationStatus PmcSequence::EnablePmcClock(const std::string& deviceName)
{
    ...
    const std::string pmcClockRegister = GetPmcClockRegister(deviceName);
...
{
    const auto descriptorSizeInBytes = DESCRIPTOR_SIZE_KB * 1024;

    // Allocate DMA through driver DebugFS
    OperationStatus res = Host::GetHost().GetDeviceManager().AllocPmc(deviceName, descriptorSizeInBytes, NUM_OF_DESCRIPTORS);
    if (!res)
    {
        return res;
    }

    const BasebandRevision revision = GetDeviceRevision(deviceName);
    if (revision >= BasebandRevision::TLN_M_B0)
    {
        uint32_t pmcRingIndex;
        auto st2 = FindPmcRingIndex(deviceName, pmcRingIndex);
        if (!st2.IsSuccess())
        {
            return st2;
        }
        ...
        return PmcService::GetInstance().GetPmcRegistersAccessor().WritePmcRegister(deviceName, pmcDescLengthMnemonic, descriptorSizeInBytes);
    }
    return OperationStatus(true);
}
