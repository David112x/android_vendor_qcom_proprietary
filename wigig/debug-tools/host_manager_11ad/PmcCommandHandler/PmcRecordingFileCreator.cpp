...
{
    OperationStatus os = Host::GetHost().GetDeviceManager().CreatePmcFiles(m_deviceName, m_refNum);
    if (!os)
    {
        return os;
    }
    std::string foundPmcFilePath;
    os = Host::GetHost().GetDeviceManager().FindPmcDataFile(m_deviceName, m_refNum, foundPmcFilePath);
    if (!os)
    {
        return os;
    }

    os = CreateFinalPmcFileLocation(foundPmcFilePath, pmcFilePath);
    if(!os)
    {
        return os;
    }
    ...
    os = Host::GetHost().GetDeviceManager().FindPmcRingFile(m_deviceName, m_refNum, foundPmcFilePath);
    if (!os)
    {
