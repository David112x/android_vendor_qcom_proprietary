...
string FlashControlCommandsHandler::GetDeviceNameInWiburnFormat(const string& deviceName) const
{
    ...
    return "wmp0l";
#else
    // device name is equivalent to interface name and thus it can be used directly
    return "wEP0L!SPARROW!" + deviceName;
#endif
}
...
bool FlashControlCommandsHandler::CommandExecutionHelper(const vector<string>& devices, const string& commandToRun, string& statusMessage) const
{
    ...
    for (auto& device : devices)
    {
        ...
        string deviceName = GetDeviceNameInWiburnFormat(device);
