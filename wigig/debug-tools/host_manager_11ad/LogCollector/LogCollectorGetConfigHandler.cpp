...
void LogCollectorGetConfigHandler::HandleRequest(const LogCollectorGetConfigRequest& jsonRequest, LogCollectorGetConfigResponse& jsonResponse)
{
    ...
    jsonResponse.SetTargetServer(config.PostLogCollection.TargetServer);
    jsonResponse.SetUserName(config.PostLogCollection.UserName);
    jsonResponse.SetRemotePath(config.PostLogCollection.RemotePath);
