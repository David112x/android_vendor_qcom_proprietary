...
LogCollectorService::LogCollectorService() : m_deferredRecordingEnabled(false),
                                             m_deferredCompress(false),
                                             m_deferredUpload(false),
                                             m_deferredLoggingType(XmlRecorder)
...
void LogCollectorService::OnDeviceDiscovered(const std::string& deviceName)
{
    ...
    if (deviceContext == m_DeviceLoggingContextMap.end())
    {
        ...
        if (m_deferredRecordingEnabled) // if deferred recording is active
        {
            ...
            os = newDeviceLoggingContext->GetFwLogCollector()->ActivateLogging(m_deferredLoggingType, m_deferredCompress, m_deferredUpload);
            if (!os)
            {
                LOG_ERROR << os.GetStatusMessage() << endl;
            }

            os = newDeviceLoggingContext->GetuCodeLogCollector()->ActivateLogging(m_deferredLoggingType, m_deferredCompress, m_deferredUpload);
            if (!os)
            {
                LOG_ERROR << os.GetStatusMessage() << endl;
            }
        }
        LOG_DEBUG << "in " << __FUNCTION__ << " insert new context: " << deviceName << endl;
        m_DeviceLoggingContextMap.insert(std::make_pair( deviceName, std::move(newDeviceLoggingContext)));

        return;
    }
    LOG_DEBUG << "in " << __FUNCTION__ << " context exists for: " << deviceName << endl;
    ...
    if (m_deferredRecordingEnabled) // if deferred recording is active
    {
        if (! (deviceContext->second->GetFwLogCollector()->IsLoggingValid(XmlRecorder) || deviceContext->second->GetFwLogCollector()->IsLoggingValid(TxtRecorder)))
        {
            ...
            os = deviceContext->second->GetFwLogCollector()->ActivateLogging(m_deferredLoggingType, m_deferredCompress, m_deferredUpload);
            if (!os)
            {
                LOG_ERROR << os.GetStatusMessage() << endl;
            }
        ...
        {
            // if no other recording exist
            os = deviceContext->second->GetuCodeLogCollector()->ActivateLogging(m_deferredLoggingType, m_deferredCompress, m_deferredUpload);
            if (!os)
            {
                LOG_ERROR << os.GetStatusMessage() << endl;
            }
...
{
    LOG_DEBUG << "device removal reported for device " << deviceName << std::endl;
...
OperationStatus LogCollectorService::StartLogging(const std::string& deviceName, CpuType cpuType,
                                                  LoggingType loggingType, bool compress, bool upload)
{
    // Post collection steps are not supported on Windows!
    if((compress || upload) && (strcmp(GetOperatingSystemName(), "Windows") == 0))
    {
    ...
    {
        os = DeviceLoggingContextIter->second->GetFwLogCollector()->ActivateLogging(loggingType, compress, upload);
    }
    if (cpuType == CPU_TYPE_UCODE)
    {
        os = DeviceLoggingContextIter->second->GetuCodeLogCollector()->ActivateLogging(loggingType, compress, upload);
    }
...
OperationStatus LogCollectorService::EnableDeferredLogging(LoggingType loggingType, bool compress, bool upload)
{
    ...
    m_deferredCompress = compress;
    m_deferredUpload = upload;
