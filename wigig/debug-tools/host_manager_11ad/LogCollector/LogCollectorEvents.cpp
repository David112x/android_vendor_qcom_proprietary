...
NewLogsEvent::NewLogsEvent(const std::string& deviceName, const log_collector::CpuType& cpuType, std::vector<log_collector::RawLogLine>& logLines) :
    ...
    m_cpuType(cpuType)
{
    ...
    {
        m_logLines.emplace_back(new SerializableLogLine(std::move(logLine)));
    }
}
