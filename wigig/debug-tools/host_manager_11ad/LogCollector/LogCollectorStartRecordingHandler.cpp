...
void LogCollectorStartRecordingHandler::HandleRequest(const LogCollectorStartRecordingRequest& jsonRequest, LogCollectorStartRecordingResponse& jsonResponse)
{
        ...
        (RecordingTypeBoxed.GetValue() == "raw" || RecordingTypeBoxed.GetValue() == "both"))
    {
        ...
        {
            os = Host::GetHost().GetLogCollectorService().StartLogging(deviceName, CPU_TYPE_FW, XmlRecorder, compress, upload);
            if (!os)
            {
                jsonResponse.Fail(os.GetStatusMessage());
                return;
        ...
        {
            os = Host::GetHost().GetLogCollectorService().StartLogging(deviceName, CPU_TYPE_UCODE, XmlRecorder, compress, upload);
            if (!os)
            {
                jsonResponse.Fail(os.GetStatusMessage());
                return;
        ...
        (RecordingTypeBoxed.GetValue() == "txt" || RecordingTypeBoxed.GetValue() == "both"))
    {
        if (CheckCpuType(cpuTypeBoxed, CPU_TYPE_FW))
        {
            os = Host::GetHost().GetLogCollectorService().StartLogging(deviceName, CPU_TYPE_FW, TxtRecorder, compress, upload);
            if (!os)
            {
                jsonResponse.Fail(os.GetStatusMessage());
                return;
        ...
        if (CheckCpuType(cpuTypeBoxed, CPU_TYPE_UCODE))
        {
            os = Host::GetHost().GetLogCollectorService().StartLogging(deviceName, CPU_TYPE_UCODE, TxtRecorder, compress, upload);
            if (!os)
            {
                jsonResponse.Fail(os.GetStatusMessage());
                return;
