...
namespace
{
    ...
    std::string GetInterfaceNameFromPciEndPoint(const std::string& pciEndPoint)
    {
        // const definitions
    ...
    std::string GetPciEndPointFromInterfaceName(const char* interfaceName)
    {
        ...
        {
            // error reported
            LOG_DEBUG << "Failed to read link " << interfaceContaingFolder << std::endl;
            return std::string();
        }
...
bool UnixNetworkInterfaceMonitor::ParseAndHandleSingleMessage(const struct nlmsghdr *nh, const char* messageName)
{
    ...
    for (struct rtattr* rta = IFLA_RTA(ifi); RTA_OK(rta, len); rta = RTA_NEXT(rta, len))
    {
        ...
        LOG_DEBUG << __FUNCTION__ << ": received " << messageName << " message for interface " << interfaceName
...
void UnixNetworkInterfaceMonitor::HandleSingleMessage(const char* interfaceName, bool isNewLink)
{
    ...
    if (isNewLink && !IsInterfaceRelevant(interfaceName, endPointName))
    {
    ...
    if (isNewLink)
    {
        ...
        m_currentEndPoints.emplace(std::move(endPointName), std::string(interfaceName));
    }
    else
    {
...
// - extract corresponding PCI End Point and check it's not part of already detected End Points
bool UnixNetworkInterfaceMonitor::IsInterfaceRelevant(const char* interfaceName, std::string& endPointName) const
{
    ...
    {
        // can happen upon rmmod when we get NEWLINK
        LOG_DEBUG << "Path '" << driverContaingFolder << "' does not exist (possible for RTM_NEWLINK during rmmod)" << std::endl;
        return false;
    }
    ...
    {
        // error reported
        LOG_DEBUG << "Failed to read link " << driverContaingFolder << std::endl;
        return false;
    }
    ...
    if (sc_driverName.compare(driverBaseName) != 0)
    {
        ...
        LOG_DEBUG << "Ignore irrelevant interface " << interfaceName << " associated with driver " << driverBaseName << std::endl;
        return false;
    }

    // check it's a physical interface
    ...
    if (m_currentEndPoints.find(endPointName) != m_currentEndPoints.cend())
    {
        LOG_DEBUG << "Ignored virtual/already processed interface " << interfaceName << std::endl;
        return false;
    }
...
void UnixNetworkInterfaceMonitor::RemoveEndPointIfOwner(const char* interfaceName)
{
    ...
    {
        if (iter->second == interfaceName)
        {
            LOG_DEBUG << "Removed endpoint record for interface " << interfaceName << std::endl;
...
std::set<std::string> UnixNetworkInterfaceMonitor::InitializeCurrentInterfaceNames()
{
    ...
    if (!dp)
    {
        LOG_VERBOSE << "Failed to open PCI End Point folder" << std::endl;
    ...
    while ((de = readdir(dp)) != nullptr) // iterate through directory content and search for End Point folders
    {
        ...
        if (potentialPciEndPoint.find(":") != std::string::npos) // only PCI end point has ":" in its name
        {
            ...
            std::string networkInterfaceName = GetInterfaceNameFromPciEndPoint(potentialPciEndPoint);
            if (!networkInterfaceName.empty())
            {
                m_currentInterfaceNames.insert(networkInterfaceName);
                m_currentEndPoints.emplace(std::move(potentialPciEndPoint), std::move(networkInterfaceName));
            }
            else
            {
