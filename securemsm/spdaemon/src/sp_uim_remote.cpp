...
#define IUICC_APP_NAMES_DEFAULT                "iuicc+iuicc1"
...
/* Timer Property for spcom sync messages */
const char iuicc_spcom_sync_call_timer_prop[] = "persist.vendor.iuicc_spcom_sync_call_timer_prop";
#define SPCOM_SYNC_MSG_MIN_TIMER_VALUE         4000

/* Default timer value */
#define DEFAULT_TIMER            30000
int32_t spcom_sync_call_timer_ms = 0;
...
void sp_uim_remote_register_with_iuicc(uint8_t  slot)
{
    ...
    char const            *pattern                    = "^(iuicc.*)"; /* string regular expression been used for any iuicc app that start with string "iuicc" */
    uint8_t               result                      = 0;
    uint8_t               i                           = 0;
    char                  separator[]                 = "+";
    regmatch_t            match_str[256]              = {};
    char                  *pch                        = NULL;
    char                  *p_slot1_ch                 = NULL;
    char                  *p_slot2_ch                 = NULL;
    char                  *context                    = NULL;
    regex_t               cmp_str;
    ...
    //get ch_name from get property
    property_get(IUICC_APP_NAME_PROP_KEY, ch_name, IUICC_APP_NAMES_DEFAULT);
    LOGD ("property_get ch_name  %s\n", ch_name);

    //parsing the ch_name for slot1 and slot2.
    pch = strtok_r (ch_name, separator, &context);
    while (pch != NULL){
        i++  ;
        if (i==1){
            p_slot1_ch = pch;
        ...
        pch = strtok_r (NULL, separator, &context);
    }

    //Compile the regex for channel name in slot1 and slot2
    result = regcomp(&cmp_str, pattern, REG_EXTENDED);
    if (result != 0){
        LOGD("regcomp result %d\n", result);
        return;
    }
    ...
    if (slot == SLOT_2) {
        if (!p_slot2_ch){
            ...
            if (!regexec(&cmp_str, p_slot2_ch, 256, match_str, 0)){
                client_info.ch_name = p_slot2_ch;
            } else{
                return;
            }
        }
        ...
        if (!p_slot1_ch){
            ...
            if (!regexec(&cmp_str, p_slot1_ch, 256, match_str, 0)){
                client_info.ch_name = p_slot1_ch;
            } else{
                return;
            }
        }
    ...
    if(is_loaded == false) {
        LOGE("sp app not loaded on slot %d ch_name %s", slot, client_info.ch_name);
        return;
    }
    ...
    if(is_connected == false) {
        LOGE("sp remote service not connected [%d]", slot);
        return;
    }
...
)
{
    ...
    if (apdu_ind_ptr == NULL || apdu_ind_ptr->slot >= MAX_SLOT) {
        LOGE("Either invalid slot value or null ind ptr");
        return;
    }

    LOGD("Processing APDU indication from UIM REMOTE slot [%d]: 0x%x 0x%x 0x%x 0x%x 0x%x",
          apdu_ind_ptr->slot,
          apdu_ind_ptr->ind_data.apdu_ind_data.data_buf[0],
          apdu_ind_ptr->ind_data.apdu_ind_data.data_buf[1],
          apdu_ind_ptr->ind_data.apdu_ind_data.data_buf[2],
          apdu_ind_ptr->ind_data.apdu_ind_data.data_buf[3],
          apdu_ind_ptr->ind_data.apdu_ind_data.data_buf[4]);

    slot = (slot_type)apdu_ind_ptr->slot;

    if (sizeof(apdu_ind_ptr->ind_data.apdu_ind_data.data_buf) <= UIM_REMOTE_IUICC_MAX_REQ_SIZE &&
        apdu_ind_ptr->ind_data.apdu_ind_data.data_len <= UIM_REMOTE_IUICC_MAX_REQ_SIZE) {

        if (g_uim_rmt_client[slot] == NULL) {
            LOGE("Either invalid slot value [%d] or null client\n", slot);
            return;
        }
  ...
  UimRemoteClientEventRsp eventResp)
{
    ...
    if (UimRemoteClientEventRsp::UIM_REMOTE_FAILURE == eventResp) {
        ...
        LOGE("Power down iUICC slot [%d] ", slot);
...
)
{
    ...
    if (!is_ind_queued) {
      ...
      LOGD("Sending response for APDU IND slot [%d]", slot);
      g_uim_rmt_client[slot]->UimRemoteServiceClientApduReq (1, apduStatus, apduResp);
    }
...
)
{
    ...
    LOGD("UIM REMOTE indication, power up the iUICC hasTimeOut [%d], timeOut [%d], hasVoltageClass [%d], powerUpVoltageClass [%d]", hasTimeOut, timeOut, hasVoltageClass, powerUpVoltageClass);
...
)
{
    ...
    LOGD("UIM REMOTE indication for powering down the iUICC hasPowerDownMode [%d], powerDownMode [%d]", hasPowerDownMode, powerDownMode);
...
static void *sp_uim_remote_start_func(void* data)
{
    ...
    /*Get the user configurable timer value,provides the defualt value if not configured.*/
    if(slot == SLOT_1) {
        spcom_sync_call_timer_ms = property_get_int32(iuicc_spcom_sync_call_timer_prop, DEFAULT_TIMER);
        if(spcom_sync_call_timer_ms  < SPCOM_SYNC_MSG_MIN_TIMER_VALUE){
            spcom_sync_call_timer_ms = DEFAULT_TIMER;
        }
    }
