...
ARCHINFO_FILENAME = 'archinfo_tmp.txt'
...
class UnsupportedKernelArch(Exception):
	pass
...
def get_sha256(bite_string_data)  :
	sha256=hashlib.sha256(bite_string_data).hexdigest()
	sha256_split_bytes=[sha256[i:i+2] for i in range(0, len(sha256), 2)]
	sha256=""
	for i in range(len(sha256_split_bytes)-1):
		sha256+=sha256_split_bytes[i]+":"
	sha256+=sha256_split_bytes[-1]
	return sha256
...
class Kernel:
	...
	def get_archinfo_filename(self) :
		return ARCHINFO_FILENAME
	...
	def get_arch_info(self, filename):
	...
	def get_kernel(self, vmlinux) :
		...
		if (outdated(KERNEL_IMAGE, vmlinux)) :
			...
			# mv(kernel_path, ".");
		if (outdated(ARCHINFO_FILENAME, KERNEL_IMAGE)) :
			print("ARCHINFO\n")
			mpgen_shell.dsystemf('%s -f %s > %s', OBJDUMP, KERNEL_IMAGE, ARCHINFO_FILENAME)
		...
		arch = self.get_arch_info(self.get_archinfo_filename())
		if(arch == "arm"):
	...
	def get_symbal_sha256(self, symbol_name, value = None):
		sha256=None
		if(value!=None):
			sha256=get_sha256(value)
		else:
