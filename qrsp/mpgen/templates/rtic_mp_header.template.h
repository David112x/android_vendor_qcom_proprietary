...
#define RTIC_MPGEN_INTERFACE_VERSION		(/*!!!*/ 30 /*!!!*/) /* Should be incremented every time this template is changed! Header structures should not change after version 30. */

#define RTIC_MPGEN_VERSION /*!!!*/"{{VERSION}}"/*!!!*/ /* MPGEN version string */
...
#define MP_SECTION_ATTRIBUTE_ALLOW_ONE         (1 << 12) /* used by QHEE - ignore first access to an asset (allow in enforce mode and not trigger in permissive). QHEE behavior: [no list->0 writes, flag->1 write, list but no flag-> inf]; RTIC: ignore */
...
/**
 * Generic structure for pointing to particular attestation feature item (feature discovery)
 */
typedef struct {
	char const name[32]; // feature name - string representation for a given type - this also is feature id
	unsigned int const offset; // offset from MP base
	unsigned int const size; // whole size of given structure block
}__attribute__((__packed__)) mp_feature_discovery_t;
...
typedef struct {
	...
	unsigned char const mpgen_ver[16]; // MPGen tool version.
	...
	mp_feature_discovery_t feature_discovery[0]; // feature discovery marker
