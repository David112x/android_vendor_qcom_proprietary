...
int prevDefaultProfileId = QDP_PROFILE_ID_INVALID;
int currDefaultProfileId = QDP_PROFILE_ID_INVALID;
...
void CallManager::handleSetupDataCallRequestMessage(std::shared_ptr<Message> msg) {
  ...
  if (m != nullptr) {
    ...
    if (true == pCallInstance->setProfileInfo(profile)) {
      ...
      if(params.apnTypeBitmask & (uint32_t)ApnTypes_t::DEFAULT && prevDefaultProfileId != QDP_PROFILE_ID_INVALID && currDefaultProfileId != prevDefaultProfileId)
      {
        if(qdp_profile_release(prevDefaultProfileId))
        {
          prevDefaultProfileId = QDP_PROFILE_ID_INVALID;
        }
      }
...
AllocatedBearerResult_t CallManager::handleGetAllBearerAllocations()
{
    ...
    AllocatedBearerResult_t result;
    std::map<int32_t, rildata::AllocatedBearer_t>::iterator it, it_save;
    for (it=bearerCollection.begin(); it!=bearerCollection.end();) {
        it_save = it;
        ++it;
        result.connections.push_back(it_save->second);
    }
    result.error = ResponseError_t::NO_ERROR;
...
{
  std::set<CallInfo *>::iterator it, it_save;
  for (it=pCallInfoList.begin(); it!=pCallInfoList.end();) {
    it_save = it;
    ++it;
    Log::getInstance().d("[CallManager]: cleanAllCalls, reset call instance");
    resetCallInfo(*it_save);
  }
}
