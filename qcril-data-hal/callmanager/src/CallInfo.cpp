...
extern int prevDefaultProfileId;
extern int currDefaultProfileId;
...
void CallInfo::resetCallInfo()
{
  ...
  if (pCallsm) {
    /* Set the StateMachine to IdleState before deleting the object.
       This ensures that any timers which are still running would be
       stopped. CallSm cleanup can happen without any DSI events for
       PhoneProcess restart which results in all active call cleanup  */
    pCallsm->setCurrentState(_eIdle);
    delete pCallsm;
    pCallsm = nullptr;
  }
...
bool CallInfo::isPartiallyConnected()
{
  return ((IsV4V6()) &&
    (!(IsV4Connected()) || !(IsV6Connected())));
}
...
bool CallInfo::handoverDeactivateDataCall(std::shared_ptr<std::function<void(CallInfo *, rildata::ResponseError_t)>> completeCb) {
  ...
  /* Always send response for handoverDeactivateDataCall  */
  v4HandedOver = false;
  v6HandedOver = false;
  deactivateDataCallCompleteCallback = completeCb;
