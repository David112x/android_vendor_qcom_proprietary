...
void AuthManager::updateModemWithCarrierImsiKeyCache()
{
  ...
  if (isKeyTypeAvailable) {
    ...
    if(readCarrierImsiKeyCache(&req)) {
      rc = ModemEndPointFactory<AuthModemEndPoint>::getInstance().buildEndPoint()->sendRawSync(
...
bool AuthManager::readCarrierImsiKeyCache(auth_send_imsi_public_key_req_msg_v01 *req)
{
  unsigned size;
...
bool AuthManager::readCarrierImsiKeyCache(auth_send_imsi_public_key_ex_req_msg_v01 *req)
{
  unsigned size;
  Log::getInstance().d("[AuthManager]: readCarrierImsiKeyCache = "+
