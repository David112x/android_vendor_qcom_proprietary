...
int ApTriggeredState::handleEvent(const PreferredDataEventType& e) {
    ...
    switch (e.event) {
        case CurrentDDSInd:
        {
            if (mStateInfo->isRilIpcNotifier && switchInfo) {
                mStateInfo->dds = switchInfo->subId;
                ddsTransition();
        ...
        case DDSSwitchInd:
        {
            if (switchInfo && switchInfo->switchAllowed == DDSSwitchRes::ALLOWED) {
                return HANDLED;
            }
            ddsTransition();

            if (switchInfo && switchInfo->switchAllowed == DDSSwitchRes::FAIL) {
