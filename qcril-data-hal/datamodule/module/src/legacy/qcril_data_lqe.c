...
*/
/*=========================================================================*/
RIL_Errno qcril_data_start_lqe
(
  int  ril_report_interval,
  int  ril_report_mode,
  RIL_LceStatusInfo  *lce_status_resp
)
{
  ...
  QCRIL_LOG_INFO( "%s", "qcril_data_start_lqe entered" );

  if(lce_status_resp == NULL)
  {
    QCRIL_LOG_ERROR ("%s", "Lce Status Info is NULL");
    return ril_req_res;
  }
...
)
{
  ...
  QCRIL_LOG_INFO( "%s", "qcril_data_stop_lqe entered" );
  ...
  do
  {
    if(!qcril_data_lqe_is_pull_supported)
    {
      QCRIL_LOG_ERROR ("%s", "Modem does not support PULL mode");
      ril_req_res = RIL_E_LCE_NOT_SUPPORTED;
      break;
...
/*===========================================================================
  ...
  FUNCTION:  qcril_data_get_lqe_info
...
*/
/*=========================================================================*/
RIL_Errno qcril_data_get_lqe_info
(
  RIL_LceDataInfo *lqe_data_resp
)
{
  ...
  QCRIL_LOG_INFO( "%s", "qcril_data_get_lqe_info entered" );

  if(lqe_data_resp == NULL)
  {
    QCRIL_LOG_ERROR ("%s", "Lce Data Info is NULL");
    return ril_req_res;
  }
