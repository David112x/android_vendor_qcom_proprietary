...
static const INT    MaxSensorRegisterControlSize = 32;     ///< Max size of the sensor control registers
static const UINT32 MaxTableKnees                = 50;     ///< Defines Maximum number of knees allowed for a exposure table
static const UINT32 MaxMeteringTableSize         = 256;    ///< Max size of the metering table
...
typedef struct
{
    ...
    BOOL   enableForceSeamlessIHDRSnapshot;     ///< Flag to enable/disable Force seamless IHDR snapshot mode
    BOOL   enable3ADebugData;                   ///< Flag to enable/disable 3A debug data
    BOOL   enableConcise3ADebugData;            ///< Flag to enable/disable COncise 3A debug data
    UINT32 profile3A;                           ///< 3A Profiling value
...
typedef struct
{
    ...
    FLOAT           gain;           ///< specify gain, have more priority then ISO
    FLOAT           FNumber;        ///< Specify the aperture value used for manual mode
...
/// @brief Defines the absolute Dual camera OTP calibration data (new format)
typedef struct
{
    FLOAT  gain;            ///< Current camera gain
    UINT16 averageLuma;     ///< Average Luma of current camera
    UINT16 exposureTimeUs;  ///< Current camera Time or linecount
    UINT32 CCT;             ///< Current camera(master) calibration light CCT
} AECAlgoAbsoluteDCCalibrationInfo;

/// @brief Defines the relative Dual camera OTP calibration data (old format)
typedef struct
{
    ...
    UINT32  CCT;                    ///< Current camera(master) calibration light CCT
} AECAlgoRelativeDCCalibrationInfo;
...
typedef struct
{
    ...
    union {
        AECAlgoRelativeDCCalibrationInfo relative; ///< Calibration data for old OTP format
        AECAlgoAbsoluteDCCalibrationInfo absolute; ///< Calibration data for new OTP format
    };
} AECAlgoDCCalibrationInfo;

/// @brief Input to AEC aperture control feature: calibration data
typedef enum
{
    AECAlgoApertureCalib1       = 0,            ///< Default aperture calibration
    AECAlgoApertureCalib2       = 1,            ///< Second aperture (typically: smaller aperture)
    AECAlgoApertureCalibCount   = 2,            ///< Count of possible apertures
    AECAlgoApertureCalibMax     = 0x7FFFFFFF    ///< Anchor to indicate the last item in the defines
} AECAlgoApertureCalibrationType;

/// @brief calibration of multiple f numbers. Principle similar to AECsync but here there is only one camera, multiple apertures
typedef struct
{
    UINT32  lineCount;             ///< exposure: sensor line count
    FLOAT   gain;                  ///< exposure: sensor gain
} AECAlgoApertureCalibrationInfo;

/// @brief Array of calibration data for aperture. Typically at least two.
typedef struct
{
    AECAlgoApertureCalibrationInfo  calibrations[AECAlgoApertureCalibCount];    ///< Aperture calibration data
    UINT32                          count;                                      ///< Number of actual calibration data
} AECAlgoMultiApertureCalibration;
...
typedef struct
{
    ...
    AECYHistStretchData         stretchControl;                 ///< The Histogram Stretch Control
    ...
    AECAlgoApertureData         apertureSwitchInfo;             ///< Aperture switch Info
    InSensorHDR3ExpTriggerInfo  inSensorHDR3ExpTriggerOutput;   ///< In-sensor HDR 3 exposure trigger output information
...
typedef struct
{
    ...
    AECAlgoROI              processedROI;                       ///< Normalized ROI used for luma calculation
} AECAlgoFrameInfo;
...
typedef struct
{
    ...
    BOOL            bIsTorchAEC;            ///< Is torch + capture enabled
...
typedef enum
{
    ...
    AECAlgoInputBayerGridROI,                   ///< The Bayer grid ROI
...
/// @brief Identifies the type of configuration to AEC algorithm
    AECAlgoSetParamInvalid,                             ///< Invalid type
    ...
    AECAlgoSetParamIsDualCamera,                        ///< Is dual camera or single
                                                        ///  Payload type: BOOL
    AECAlgoSetParamUnstabilizedFaceROI,                 ///< The unstabilized face ROI from face detect module
                                                        ///  Payload type: UnstabilizedROIInformation
    AECAlgoSetParamControlPostRawSensitivityBoost,      ///< Control Post  RawSensitivity Boost value
                                                        ///  Payload type: INT32
    AECAlgoSetParamCustomExposureTable,                 ///< Custom exposure table read from vendor tag
                                                        ///  Payload type: AECCustomExposureTable
    AECAlgoSetParamCustomMeteringTable,                 ///< Custom metering table read from vendor tag
                                                        ///  Payload type: AECCustomMeteringTable
    AECAlgoSetParamDisableADRC,                          ///< Flag to enable ADRC
                                                        ///  Payload type: BOOL
    AECAlgoSetParamFlashInlineCalibrationData           ///< Dynamic Inline Calibration Data
                                                        ///  Payload type: VOID *
    AECAlgoSetParamIsFlashInfluencedStats               ///< Flash info flag for the stats of capture frame influenced by main flash
                                                        ///  Payload type: BOOL
    AECAlgoSetParamCount,                               ///< Indicate the total number of set param to algorithm
    AECAlgoSetParamMax                                  ///< Anchor to indicate the last item in the defines
    AECAlgoSetParamTrackerROI,                          ///< The track ROI from touch to track module
                                                        ///  Payload type: AECAlgoROIList
*/

/* To generate enum from macros */
#define AEC_GENERATE_ENUM(ENUM) ENUM,
/* To generate string for enum from macros */
#define AEC_GENERATE_STRING(STRING) #STRING,

/** AECAlgoSetParamType:
*   type of parameters that can be set for AEC.
**/
#define AEC_SET_ENUM_LIST(ADD_ENTRY)                            \
    ADD_ENTRY(AECAlgoSetParamOperationMode)                     \
    ADD_ENTRY(AECAlgoSetParamChromatixData)                     \
    ADD_ENTRY(AECAlgoSetParamMeteringMode)                      \
    ADD_ENTRY(AECAlgoSetParamExposureCompensation)              \
    ADD_ENTRY(AECAlgoSetParamFlickerCompensation)               \
    ADD_ENTRY(AECAlgoSetParamManualSetting)                     \
    ADD_ENTRY(AECAlgoSetParamAECLock)                           \
    ADD_ENTRY(AECAlgoSetParamFPSRange)                          \
    ADD_ENTRY(AECAlgoSetParamLEDMode)                           \
    ADD_ENTRY(AECAlgoSetParamTouchROI)                          \
    ADD_ENTRY(AECAlgoSetParamFaceROI)                           \
    ADD_ENTRY(AECAlgoSetParamAWBState)                          \
    ADD_ENTRY(AECAlgoSetParamGyroInfo)                          \
    ADD_ENTRY(AECAlgoSetParamOverrideSetting)                   \
    ADD_ENTRY(AECAlgoSetParamFlashMeasurement)                  \
    ADD_ENTRY(AECAlgoSetParamFlashCalibrationData)              \
    ADD_ENTRY(AECAlgoSetParamDebugDataEnable)                   \
    ADD_ENTRY(AECAlgoSetParamSensorInfo)                        \
    ADD_ENTRY(AECAlgoSetParamCameraInfo)                        \
    ADD_ENTRY(AECAlgoSetParamFlashMaximumCurrent)               \
    ADD_ENTRY(AECAlgoSetParamSetDiagTestConfig)                 \
    ADD_ENTRY(AECAlgoSetParamStartUpExpSensitivity)             \
    ADD_ENTRY(AECAlgoSetParamStatsCapability)                   \
    ADD_ENTRY(AECAlgoSetParamCropWindow)                        \
    ADD_ENTRY(AECAlgoSetParamPipelineDelay)                     \
    ADD_ENTRY(AECAlgoSetParamPeerInfo)                          \
    ADD_ENTRY(AECAlgoSetParamDualcamCalibrationInfo)            \
    ADD_ENTRY(AECAlgoSetParamMultiExpSensorMode)                \
    ADD_ENTRY(AECAlgoSetParamIsDualCamera)                      \
    ADD_ENTRY(AECAlgoSetParamUnstabilizedFaceROI)               \
    ADD_ENTRY(AECAlgoSetParamControlPostRawSensitivityBoost)    \
    ADD_ENTRY(AECAlgoSetParamCustomExposureTable)               \
    ADD_ENTRY(AECAlgoSetParamCustomMeteringTable)               \
    ADD_ENTRY(AECAlgoSetParamDisableADRC)                       \
    ADD_ENTRY(AECAlgoSetParamConvergenceSpeed)                  \
    ADD_ENTRY(AECAlgoSetParamFlashInlineCalibrationData)        \
    ADD_ENTRY(AECAlgoSetParamIsFlashInfluencedStats)            \
    ADD_ENTRY(AECAlgoSetParamFlashInlineCalibrationMode)        \
    ADD_ENTRY(AECAlgoSetParamTrackerROI)                        \
    ADD_ENTRY(AECAlgoSetParamframeID)                           \
    ADD_ENTRY(AECAlgoSetParamApertureCalibrationInfo)           \
    ADD_ENTRY(AECAlgoSetParamAmbientRaw)                        \
    ADD_ENTRY(AECAlgoSetParamCount)                             \
    ADD_ENTRY(AECAlgoSetParamMax)                               \

typedef enum
{
    AEC_SET_ENUM_LIST(AEC_GENERATE_ENUM)
} AECAlgoSetParamType;
...
/// @brief Identifies the type of query to AEC algorithm
    AECAlgoGetParamInvalid             ,             ///< The invalid type
    ...
    AECAlgoGetParamDynamicCalibrationData           ///< Dynamic inline calibration data
                                                    ///  Input payload: NULL
                                                    ///  Output payload: InlineCalibrationData
    AECAlgoGetParamCount,                           ///< Indicate the total number of get param to algorithm
    AECAlgoGetParamMax                              ///< Anchor to indicate the last item in the defines
*/

/** AECAlgoGetParamType:
*   type of parameters that can be get for AEC.
**/
#define AEC_GET_ENUM_LIST(ADD_ENTRY)                        \
    ADD_ENTRY(AECAlgoGetParamInvalid)                       \
    ADD_ENTRY(AECAlgoGetParamStartExposure)                 \
    ADD_ENTRY(AECAlgoGetParamSnapshotExposure)              \
    ADD_ENTRY(AECAlgoGetParamGainFromISOPreview)            \
    ADD_ENTRY(AECAlgoGetParamGainFromISOSnapshot)           \
    ADD_ENTRY(AECAlgoGetParamEVCapabilities)                \
    ADD_ENTRY(AECAlgoGetParamDebugData)                     \
    ADD_ENTRY(AECAlgoGetParamSWVersion)                     \
    ADD_ENTRY(AECAlgoGetParamFlashMeasurementConfig)        \
    ADD_ENTRY(AECAlgoGetParamFlashMeasurementResult)        \
    ADD_ENTRY(AECAlgoGetParamPublishingVendorTagsInfo)      \
    ADD_ENTRY(AECAlgoGetParamDependentVendorTags)           \
    ADD_ENTRY(AECAlgoGetParamBGStatsConfig)                 \
    ADD_ENTRY(AECAlgoGetParamPeerInfo)                      \
    ADD_ENTRY(AECAlgoGetParamBHistStatsConfig)              \
    ADD_ENTRY(AECAlgoGetParamDynamicCalibrationData)        \
    ADD_ENTRY(AECAlgoGetParamSnapshotType)                  \
    ADD_ENTRY(AECAlgoGetParamCount)                         \
    ADD_ENTRY(AECAlgoGetParamMax)                           \

typedef enum
{
    AEC_GET_ENUM_LIST(AEC_GENERATE_ENUM)
} AECAlgoGetParamType;
...
typedef enum
{
    ...
    AECAlgoGetParamInputIsFixedFocus,                    ///< Indicate if the focus is fixed
...
typedef enum
{
    ...
    AECAlgoGetParamOutputBHistStatsConfig,              ///< Bayer Histogram Statistics configuration
                                                        ///  Payload StatsBayerGridBayerExposureConfig
    AECAlgoGetParamOutputDynamicCalibrationData,        ///< Dynamic Calibration Inline Data
                                                        ///  Payload InlineCalibrationData
    AECAlgoGetParamOutputAdditionalInfo,                ///< Additional output Info from algorithm
                                                        ///  Payload: AECAlgoAdditionalInfo
    AECAlgoGetParamOutputSnapshotType,                  ///< Snapshot indicator
                                                        ///  Payload:AECAlgoSnapshotType
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// @brief Structure containing custom metering table
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
typedef struct
{
    FLOAT                 meteringTable[MaxMeteringTableSize];  ///< each element specify weight of the pixel
    UINT32                meteringTableSize;                    ///< Size of metering table provided.
} AECCustomMeteringTable;
