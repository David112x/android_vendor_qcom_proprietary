...
static const UINT32 StatsCCMRowCount                    = AWBNumCCMRows; ///< number of rows in the color correction matrix (CCM)
static const UINT32 StatsCCMColCount                    = AWBNumCCMCols; ///< number of columns in the color correction matrix (CCM)s
static const UINT8  MaxCCM                              = MaxCCMs;

static const UINT32 MaxAECGyroSampleSize                = 32;   ///< Maximum number of gyro samples per frame
...
/// @brief: Structure representing the calculated CDF for the in-sensor HDR 3 exposure trigger final result
struct CDFSet
{
...
struct InSensorHDR3ExpTriggerInfo
{
    ...
    InSensorHDR3ExpTriggeResult finalTriggerResult;     ///< The in-sensor HDR 3 exposure trigger final result
...
typedef struct
{
    UINT64              requestID;                              ///< Sequence of the frame were the ROI were detected
    UINT32              roiCount;                               ///< Number of ROIs detected
    StatsROIData        roi[StatisticsMaxROI];                  ///< Data for each ROI
} UnstabilizedROIInformation;
...
typedef struct
{
    ...
    FLOAT   minGain;                ///< Minimum sensor analog gain
...
typedef struct
{
    ...
    UINT32  bitDepth;                       ///< RS Output - Input bits
} StatsRowSum;
...
typedef struct
{
    UINT16*                     pGridData[StatisticsBayerChannelsCount];    ///< Bayer grid channel data from sensor HDR stream
...
///< @brief Defines the IHistogram statistic data structure
struct StatsIHist
{
    UINT16            arraySize;                        ///< Number of elements in the channelData field
    UINT32            numBins;                          ///< Number of bins for which the histogram is collected
    ...
    UINT16*           channelData[IHistChannelCount];   ///< Channel data for each channel
    StatisticsStartUp startupMode;                      ///< Statistics start up mode
};
...
struct StatsAWBCCMList
{
    ...
    StatsAWBCCM     CCM[MaxCCMs];        ///< Color Correction Matrix Value
};
...
typedef struct
{
    ...
    FLOAT   pAngularVelocityX[MaxAECGyroSampleSize];    ///< angular velocity along x-axis
    FLOAT   pAngularVelocityY[MaxAECGyroSampleSize];    ///< angular velocity along y-axis
    FLOAT   pAngularVelocityZ[MaxAECGyroSampleSize];    ///< angular velocity along z-axis
    FLOAT   gyroSqr;                                    ///< Calculated gyro square value from gyro sample
    UINT64  timeStamp[MaxAECGyroSampleSize];            ///< timestamp for each sample
    UINT16  samplingTimeMillisec;                       ///< Gyro Sample rate =  1 /SamplingTime
} StatsGyroInfo;
