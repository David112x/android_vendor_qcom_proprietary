...
static const UINT8  ASDLandscape    = 1;    ///< Index for Landscape mode
static const UINT8  ASDCount        = 2;    ///< Maximum index for scene detection type

// Face Detection static declaration
static const UINT8  MaxFaceROIs     = 10;   ///< Maximum index for Face ROIs

// Object Detection static declaration
static const UINT8  MaxTrackerROIs = 1;   ///< Maximum index for Tracker ROIs

// PD static declaration
static const UINT8  PDMaxWindowSize         = 200;    ///< Maximum size of PD output index
static const UINT32 PDLibMaxMapGridCount    = 400;    ///< Maximum size of PD output index
...
typedef struct
{
    INT32                  horizontalNum;                              ///< Number of horizontal regions
    INT32                  verticalNum;                                ///< Number of vertical regions
    ...
    BOOL                   enableQuadSync;                             ///< Enable/Disable Quad Sync
} BGBEConfig;
...
/// @brief Image Histogram options to select the input of the green channel
...
/// @brief Describes possible states of aperture control feature
typedef enum ApertureSwitchStateType
{
    ApertureSwitchIdle          = 0,            ///< Aperture feature is idle
    ApertureSwitchStart         = 1,            ///< Start to switch: prepare tuning data
    ApertureSwitchInProgress    = 2,            ///< Switch in progress: hide (skip) frames
    ApertureSwitchCompleted     = 3,            ///< switch completed
    ApertureSwitchReserved      = 4,            ///< for future development
    ApertureSwitchMax           = 0x7FFFFFFF,   ///< Type max
} ApertureSwitchStateType;

/// @brief Defines AEC output wrt aperture control feature
typedef struct
{
    ApertureSwitchStateType apertureStatus;     ///< informs node to prepare for aperture switch
    FLOAT                   currentFNumber;     ///< new updated F number (aperture)
} AECAlgoApertureData;
...
typedef struct
{
    ...
    FLOAT               correlatedColorTemperature[AWBAlgoDecisionMax]; ///< Correlated Color Temperature: degrees Kelvin (K)
...
typedef struct
{
    ...
    UINT32              outputID;     ///< Output ID used in BF v2.5 index mode. SW must program to control the output order.
    BOOL                needMerge;    ///< Flag to indicate if this region needs to be merged due to a split in dual IFE
...
typedef struct
{
    ...
    BOOL                   isDepthFocus;               ///< Depth focus indicator
} AFFrameInformation;
...
struct PDDepthMapInfo
{
    ...
    INT32   DefocusMap[PDLibMaxMapGridCount];           ///< Defocus map
...
struct PDOutput
{
    ...
    PDDepthMapInfo      depthMapInfo;       ///< depth map information from PDLib
};
...
/// @brief Describes Tracker ROI
typedef struct
{
    UINT32                 id;           ///< Id used to track a object in the scene
    UINT32                 confidence;   ///< Confidence of this object track
    RectangleCoordinate    rect;         ///< Detected object rectangle
} TrackerROIData;

/// @brief Structure describing track ROI data
typedef struct
{
    UINT64         requestId;            ///< Request ID corresponding to this results
    UINT32         ROICount;             ///< Number of ROIs
    TrackerROIData ROI[MaxTrackerROIs];  ///< Tracker ROI data
