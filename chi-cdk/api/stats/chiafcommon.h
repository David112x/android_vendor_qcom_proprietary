...
static const UINT16 MaxAFLinearAcceSampleSize           = 1;    ///< Maximum number of linear accelerometer samples per frame
...
static const UINT16 MaxAFCalDistances                   = 10;   ///< Maximum number of AF calibration distance that
                                                                /// can be configured in EEPROM
static const UINT32 MaxAFFocusMapNum                    = 3072; ///< Maximum AF focus map grid count 64x48 at maximum
...
typedef struct
{
    ...
    UINT16  sampleCount;                                       ///< Number of Samples
    FLOAT   pLinearAccelerationX[MaxAFLinearAcceSampleSize];   ///< linear acceleration along X axis
    FLOAT   pLinearAccelerationY[MaxAFLinearAcceSampleSize];   ///< linear acceleration along Y axis
    FLOAT   pLinearAccelerationZ[MaxAFLinearAcceSampleSize];   ///< linear acceleration along Z axis
...
typedef struct
{
    ...
    BOOL    isFixedFocus;           ///< Indicate if sensor is fixed focus
} AFSensorInfo;

/// @brief AF Lens Data structure to be passed to AF algorithm
struct AFLensData
{
    FLOAT   targetDistance;     ///< target distance in cm
    FLOAT   lensPos;            ///< lens position corresponding to target distance
};

/// @brief Calibration OTP data from Sensor Module to be passed to AF algorithm
struct AFCalibrationOTPData
{
    UINT32      numberOfDistances;                   ///< Number of calibrated distances
    AFLensData  calibrationInfo[MaxAFCalDistances];  ///< contains chart distance and correpsonding DAC values
    INT16       gravityOffset0to90;                  ///< memory offset value of gravity offset from 0 to 90
    INT16       gravityOffset90to180;                ///< memory offset value of gravity offset from 90 to 180
};
...
/// @brief PDAF depth information
typedef struct
{
    FLOAT*  PDMap;                  ///< PD value map
    INT*    DefocusMap;             ///< Defocus map
    UINT*   ConfidenceMap;          ///< Confidence map
    INT16*  DefocusRangeNearMap;    ///< Defocus Range Near map
    INT16*  DefocusRangeFarMap;     ///< Defocus Range Far map
    UINT32  horiNumOfRegion;        ///< number of horizontal grid for 2D depth map
    UINT32  vertNumOfRegion;        ///< number of vertical grid for 2D depth map
    UINT32  widthOfRegion;          ///< width of 2D depth map coverage in camif size
    UINT32  heightOfRegion;         ///< height of 2D depth map coverage in camif size
    UINT32  horiRegionOffset;       ///< horizonal offset of 2D depth map coverage in camif size
    UINT32  vertRegionOffset;       ///< vertical offset of 2D depth map coverage in camif size
} AFPDAFDepthMapInfo;
...
typedef struct
{
    ...
    AFPDAFDepthMapInfo  pDepthMap;          ///< Depth Map Information
} AFPDAFData;

/// @brief AF focus map information
typedef struct
{
    UINT*   ConfidenceMap;          ///< Confidence map
    ...
    UINT32  horiNumOfRegion;        ///< number of horizontal grid for 2D depth map
    UINT32  vertNumOfRegion;        ///< number of vertical grid for 2D depth map
    UINT32  widthOfRegion;          ///< width of 2D focus map coverage in camif size
    UINT32  heightOfRegion;         ///< height of 2D focus map coverage in camif size
    UINT32  horiRegionOffset;       ///< horizonal offset of 2D depth focus coverage in camif size
    UINT32  vertRegionOffset;       ///< vertical offset of 2D depth focus coverage in camif size
} AFFocusMapInfo;
...
{
    UINT64  requestId;                      ///< Current request id
    UINT16  width;                          ///< Width of the depth data, must be even per Android API
    UINT16  height;                         ///< Heigh of the depth data, must be even per Android API
    UINT16  stride;                         ///< Horizontal stride multiple of 16 pixels, this is per Android API
    UINT16* pDepthBuffer;                   ///< Pointer to depth buffer with format of Depth16
} AFDepth16StatsInfo;
...
typedef struct
{
    ...
    BOOL      BAFGammaEnable;   ///< Flag to enable BAF gamma
} AFBAFGammaLUTConfig;
...
typedef struct
{
    ...
    UINT    recordMode;             ///< Settings to enable/disable AF dump
    UINT    overrideHAFAlgoMask;    ///< Mask for override HAF algorithm
    BOOL    spotlightFallback;      ///< Setting to fallback to contrast AF in case of spotlight
    BOOL    enableLensSagComp;      ///< Setting to enable/disable lens sag compensation
    CHAR    activeNodeMask[MaxStringLength]; ///< Settings to debug probe based on active node
    UINT    apiMask;                ///< Settings to set the API Mask
    UINT32  profile3A;              ///< Settings to 3A Profiling
    BOOL   enable3ADebugData;       ///< Settings to enable/disable 3A debug data
    BOOL   enableConcise3ADebugData;///< Settings to enable/disable COncise 3A debug data
} AFAlgoSettingInfo;
...
typedef enum
{
    ...
    AFCreateParamTypeHAFALgoUtil,                               ///< HAF algorithm util interface, this is filled by HAF
    AFCreateParamTypeCreateQtiHAFFunctionPtr,                   ///< Function pointer to create qti haf interface, this is filled by HAF
    AFCreateParamAFCalibrationOTPData,                          ///< AF Calibration OTP Data
