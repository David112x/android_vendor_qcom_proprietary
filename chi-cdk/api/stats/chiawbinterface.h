...
/// Number of AWB external input size
static const UINT32 AWBAlgoExternalInputSize        = 12;

/// Number of AWB customized SA output size
static const UINT32 AWBAlgoCustomizedSAOutputSize   = 20;
...
/// @brief AWB Depth Information
typedef struct
{
    UINT*   ConfidenceMap;          ///< Confidence map
    UINT*   DistanceMap;            ///< Distance map
    BOOL*   AFSelectionMap;         ///< AF Selection map
    ...
    UINT32  horiRegionOffset;       ///< horitonal offset for grid
    UINT32  vertRegionOffset;       ///< vertical offset for grid
    UINT32  isDepthMapAvaiable;     ///< Depth map availability
}AWBAlogDepthInformation;
...
typedef enum
{
    ...
    AWBOutputTypeCurrentDecision,                   ///< Current frame AWB decision point
                                                    ///  Payload: AWBAlgoDecisionInformation
...
typedef struct
{
    ...
    BOOL                    bIsTorch;           ///< Is torch + capture enabled
...
typedef struct
{
    ...
    AWBDecisionPoint    curFrameDecisionAfterTC;      ///< current frame decision after temporal convergence
} AWBAlgoWarmstartInformation;

/// @brief Defines the external information input into the AWB algorithm
typedef struct
{
    FLOAT           inputData[AWBAlgoExternalInputSize];   ///< AWB external input data
} AWBAlgoExternalInformation;

/// @brief Indicates the type of e-spectral sensor info
typedef enum
{
    AWBAlgoEspectralSensorTypeInvalid   = -1,       ///< E-spectral Sensor type is invalid.
    AWBAlgoEspectralSensorSingleIR,                 ///< single IR sensor (no flicker detection & no clear)
    AWBAlgoEspectralSensorSingleUV,                 ///< single uv sensor(no flicker detection & no clear)
    AWBAlgoEspectralSensorDualIRMode1,              ///< dual IR sensor 1(no flicker detection & no clear)
    AWBAlgoEspectralSensorDualIRMode2,              ///< dual IR sensor 2(no flicker detection & clear)
    AWBAlgoEspectralSensorDualIRMode3,              ///< dual IR sensor 3(flicker detection & clear)
    AWBAlgoEspectralSensorIRAndUVMode1,             ///< IR/UV sensor 1(no flicker detection & no clear)
    AWBAlgoEspectralSensorIRAndUVMode2,             ///< IR/UV sensor 2(no flicker detection & clear)
    AWBAlgoEspectralSensorIRAndUVMode3,             ///< IR/UV sensor 3(flicker detection & clear)
    AWBAlgoEspectralSensorTypeMax     = 0x7FFFFFFF, ///< Max type
} AWBAlgoEspectralSensorInfoType;

/// @brief Defines the e-spectral sensor info input into the AWB algorithm
typedef struct
{
    BOOL                            eSensorInfoAvailable; ///< the e-spectral sensor info input to AWB algorithm are avaiable
    AWBAlgoEspectralSensorInfoType  eSpectralSensorMode;  ///< 0, single IR sensor (no flicker detection & no clear)
                                                          ///< 1, single uv sensor(no flicker detection & no clear)
                                                          ///< 2, dual IR sensor 1(no flicker detection & no clear)
                                                          ///< 3, dual IR sensor 2(no flicker detection & clear)
                                                          ///< 4, dual IR sensor 3(flicker detection & clear)
                                                          ///< 5, IR/UV sensor 1(no flicker detection & no clear)
                                                          ///< 6, IR/UV sensor 2(no flicker detection & clear)
                                                          ///< 7, IR/UV sensor 3(flicker detection & no clear)
    FLOAT                           dataIR;               ///< ir1(dual IR sensor, long wavelength IR) or ir(ir_uv sensor) value
    FLOAT                           dataIR2;              ///< ir2(dual IR sensor, Short wavelength IR) value
    FLOAT                           dataUV;               ///< uv(ir_uv sensor) value
    FLOAT                           dataR;                ///< R value
    FLOAT                           dataG;                ///< G value
    FLOAT                           dataB;                ///< B value
    FLOAT                           dataClear;            ///< no color filter sensor output
    FLOAT                           flickerFrequency;     ///< e-spectral sensor detect the ambient light's flicker frequency
    FLOAT                           CCT;                  ///< CCT compute by R/G/B
    FLOAT                           luxIndex;             ///< lux computed by e-spectral sensor
} AWBAlgoEspectralSensorInformation;

/// @brief Defines the external information input into the AWB algorithm
typedef struct
{
    UINT32          customizedSACount;                              ///< Numbers of customized SA used
    AWBAlgoPoint    decisionPoint[AWBAlgoCustomizedSAOutputSize];   ///< Decision points for customized SAs
    FLOAT           confidence[AWBAlgoCustomizedSAOutputSize];      ///< Confidence levels for customized SAs
    UINT32          SAID[AWBAlgoCustomizedSAOutputSize];            ///< Customized SA IDs
    UINT32          SASubID[AWBAlgoCustomizedSAOutputSize];         ///< Customized SA subModule IDs
    FLOAT           weight[AWBAlgoCustomizedSAOutputSize];          ///< Weights for customized SAs
} AWBAlgoCustomizedSAInformation;

/* To generate enum from macros */
#define AWB_GENERATE_ENUM(ENUM) ENUM,

/* To generate string for enum from macros */
#define AWB_GENERATE_STRING(STRING) #STRING,
...
/** AWBAlgoGetParamType:
*   Indicates the type of AWB Get/Query of parameters
**/
#define AWB_GET_ENUM_LIST(ADD_ENTRY)                     \
    ADD_ENTRY(AWBGetParamTypeInvalid)                    \
    ADD_ENTRY(AWBGetParamTypeDebugData)                  \
    ADD_ENTRY(AWBGetParamTypeLastOutput)                 \
    ADD_ENTRY(AWBGetParamTypeFlashOutput)                \
    ADD_ENTRY(AWBGetParamTypeSWVersion)                  \
    ADD_ENTRY(AWBGetParamTypeIsStatsDataRequired)        \
    ADD_ENTRY(AWBGetParamTypeFlashEstimationState)       \
    ADD_ENTRY(AWBGetParamTypePublishingVendorTagsInfo)   \
    ADD_ENTRY(AWBGetParamTypeDependentVendorTags)        \
    ADD_ENTRY(AWBGetParamTypePeerInfo)                   \
    ADD_ENTRY(AWBGetParamTypeCctToGain)                  \
    ADD_ENTRY(AWBGetParamTypeGainToCCT)                  \
    ADD_ENTRY(AWBGetParamTypeBGConfig)                   \
    ADD_ENTRY(AWBGetParamTypeLastIndex)                  \
    ADD_ENTRY(AWBGetParamTypeMax)                        \

typedef enum
{
    AWB_GET_ENUM_LIST(AWB_GENERATE_ENUM)
} AWBAlgoGetParamType;
...
typedef enum
{
    ...
    AWBSetParamTypeFaceROI,                                         ///< Face detection algorithm provides face region of
                                                                    ///  interest. Using FD stabilized ROI data
    ...
    AWBSetParamTypePeerInfo,                                        ///< Multi camera peer information
                                                                    ///  Payload type: VOID*
    AWBSetParamTypeUnstabilizedFaceROI,                             ///< Uniform Face ROI interface
                                                                    ///  Payload type: StatsFaceInformation
    AWBSetParamTypeWarmstart,                                       ///< Warmstart gains and CCT
                                                                    ///  Payload type: AWBAlgoWarmstartInformation
    AWBSetParamTypeExternalInfo,                                    ///< Information about the external input
                                                                    ///  Payload type: AWBAlgoExternalInformation
    AWBSetParamTypeCustomizedSAInfo,                                ///< Information about AWBAlgoCustomizedSAInformation
                                                                    ///  Payload type: AWBAlgoCustomizedSAInformation
    AWBSetParamTypeIsFixedFocus,                                    ///< Flag indicating whether sensor is fixed focus
                                                                    ///  Payload type: BOOL
    AWBSetParamTypeDynamicConvergenceSpeed,                         ///< Dynamic convergence speed
                                                                    ///  Payload type: FLOAT
    AWBSetParamTypeEspectralSensorInfo,                             ///< Information about the e-spectral sensor input info: IR/UV/R/G/B/Clear/flicker frequency
                                                                    ///  Payload type: AWBAlgoEspectralSensorInformation
    AWBSetParamTypePipelineDelay,                                   ///< Information about the PipelineDelay
                                                                    ///  Payload type: UINT8
...
/** AWBAlgoSetParamType:
*   Identifies the type of input parameter to AWB algorithm
**/
#define AWB_SET_ENUM_LIST(ADD_ENTRY)                            \
    ADD_ENTRY(AWBSetParamTypeInvalid)                           \
    ADD_ENTRY(AWBSetParamTypeDebugDataMode)                     \
    ADD_ENTRY(AWBSetParamTypeSensorInfo)                        \
    ADD_ENTRY(AWBSetParamTypeStatsWindow)                       \
    ADD_ENTRY(AWBSetParamTypeChromatixData)                     \
    ADD_ENTRY(AWBSetParamTypeIlluminantsCalibrationFactor)      \
    ADD_ENTRY(AWBSetParamTypeGeometricalDisparityCalibration)   \
    ADD_ENTRY(AWBSetParamTypeLEDCalibrationData)                \
    ADD_ENTRY(AWBSetParamTypeManualSettings)                    \
    ADD_ENTRY(AWBSetParamTypeWhiteBalanceMode)                  \
    ADD_ENTRY(AWBSetParamTypeLock)                              \
    ADD_ENTRY(AWBSetParamTypeSceneMode)                         \
    ADD_ENTRY(AWBSetParamTypeFaceROI)                           \
    ADD_ENTRY(AWBSetParamTypeTouchROI)                          \
    ADD_ENTRY(AWBSetParamTypeFlashType)                         \
    ADD_ENTRY(AWBSetParamTypeFlashData)                         \
    ADD_ENTRY(AWBSetParamTypeDualcamSync)                       \
    ADD_ENTRY(AWBSetParamTypeCameraInfo)                        \
    ADD_ENTRY(AWBSetParamTypeOperationMode)                     \
    ADD_ENTRY(AWBSetParamTypePeerInfo)                          \
    ADD_ENTRY(AWBSetParamTypeUnstabilizedFaceROI)               \
    ADD_ENTRY(AWBSetParamTypeWarmstart)                         \
    ADD_ENTRY(AWBSetParamTypeExternalInfo)                      \
    ADD_ENTRY(AWBSetParamTypeCustomizedSAInfo)                  \
    ADD_ENTRY(AWBSetParamTypeIsFixedFocus)                      \
    ADD_ENTRY(AWBSetParamTypeDynamicConvergenceSpeed)           \
    ADD_ENTRY(AWBSetParamTypeOverrideSettings)                  \
    ADD_ENTRY(AWBSetParamTypeFrameId)                           \
    ADD_ENTRY(AWBSetParamTypeROIBasedAWBData)                   \
    ADD_ENTRY(AWBSetParamTypeEspectralSensorInfo)               \
    ADD_ENTRY(AWBSetParamTypePipelineDelay)                     \
    ADD_ENTRY(AWBSetParamTypeLastIndex)                         \
    ADD_ENTRY(AWBSetParamTypeMax)                               \

typedef enum
{
    AWB_SET_ENUM_LIST(AWB_GENERATE_ENUM)
} AWBAlgoSetParamType;
