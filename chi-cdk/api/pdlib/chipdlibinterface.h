...
typedef struct PDLibCreateParams
{
    ...
    StatsTuningData tuningStats;            ///< Stats Tuning
} PDLibCreateParams;
...
typedef enum
{
    PDLibGetParamInvalid    = -1,           ///< The invalid type
    PDLibGetParamHWConfig,                  ///< Get hardware configuration for sparse/dual HW PD.
    PDLibGetParamLCRenable,                 ///< Get LCR enable from tuning
    PDLibGetParamMax        = 0x7FFFFFFF,   ///< Anchor to indicate the last item in the defines
} PDLibGetParamType;

/// @brief Defines the base structure for the get param output query
typedef struct
{
    PDHwConfig config;      ///< DualPD harware config information
                            ///< Type: PDLibGetParamHWConfig
    BOOL isLCREnable;       ///< LCR enable from tuning
                            ///< Type: PDLibGetParamLCRenable
} PDLibGetParamOutputData;

/// @brief Represents the structure to query information from algorithm
typedef struct
{
    PDLibGetParamType       type;       ///< The type of information to get from algorithm
    PDLibGetParamOutputData outputData; ///< The list of output for queried information
} PDLibGetParams;
...
typedef struct CHIPDLib
{
    ...
    /// PDLibGetParam
    ///
    /// @brief  This method retrieves the most up-to-date PD library data. The output
    ///         is generally updated after calling PDLibProcess() or PDLibInit()
    ///
    /// @param  pPDLib      Pointer to CHIPDLib instance
