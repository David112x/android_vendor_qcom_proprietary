...
static const INT32 MaxAWBBGStatsNum         = 3888;    ///< Max ROIs for AWB BG is 72x54,
...
typedef enum
{
    ...
    ISPStatsTypeDualPDHWPDAF,  ///< Dual PD HW PDAF
    ...
    ISPStatsTypeRDIRaw,        ///< RDI Raw buffer for advance sparse PD
    ISPStatsTypeRDIStats,      ///< RDI stats
    ISPStatsTypeLCRHWStats,    ///< LCR HW stats
    ISPStatsTypeBPSRegYUV,     ///< YUV frame to be processed in StatsParser
    ISPStatsTypeMax,           ///< Max type of stats available
} ISPStatsType;
...
typedef struct
{
    ...
    BOOL            uniform;                            ///< Select bin uniformity
...
typedef struct
{
    ...
    inline VOID SetChannelData(AWBBGChannelData* pAWBBGChannelData, UINT32 index, UINT32 hasSatInfo)
    {
        if (hasSatInfo)
        {
            m_channelDataWithSat[index] = *(static_cast<AWBBGChannelDataWithSat*>(pAWBBGChannelData));
        }
        else
        {
            m_channelData[index] = *pAWBBGChannelData;
        }
    }
...
typedef struct
{
    ...
    inline VOID SetChannelData(HDRBEChannelData* pHDRBEChannelData, UINT32 index, UINT32 hasSatInfo)
    {
        if (hasSatInfo)
        {
            m_channelDataWithSat[index] = *(static_cast<HDRBEChannelDataWithSat*>(pHDRBEChannelData));
        }
        else
        {
            m_channelData[index] = *pHDRBEChannelData;
        }
    }
