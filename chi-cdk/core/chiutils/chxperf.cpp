...
static INT32 perfLockParamsPowerHintVideoEncodeHFR_480FPS[] =
{
    POWER_HINT_ID_VIDEO_ENCODE_HFR_480FPS
};


static INT32 perfLockParamsSnapshotCapture[] =
{
...
PerfLockInfo PerfLockManager::mPerfLockInfo[PERF_LOCK_COUNT] = {
    ...
    { // PERF_LOCK_SNAPSHOT_CAPTURE
        perfLockParamsSnapshotCapture,
        sizeof(perfLockParamsSnapshotCapture) / sizeof(INT32)
    },
    ...
    { // PERF_LOCK_POWER_HINT_ID_VIDEO_ENCODE_60FPS
        perfLockParamsPowerHintVideoEncode60FPS,
        sizeof(perfLockParamsPowerHintVideoEncode60FPS) / sizeof(INT32)
    },
    { // PERF_LOCK_POWER_HINT_ID_VIDEO_ENCODE_HFR
        perfLockParamsPowerHintVideoEncodeHFR,
        sizeof(perfLockParamsPowerHintVideoEncodeHFR) / sizeof(INT32)
    },
    { // PERF_LOCK_POWER_HINT_ID_VIDEO_ENCODE_HFR_480FPS
        perfLockParamsPowerHintVideoEncodeHFR_480FPS,
        sizeof(perfLockParamsPowerHintVideoEncodeHFR_480FPS) / sizeof(INT32)
    },
    ...
    UINT32 timer)
{
        ...
        IsValidPerfLock(lock))
    {
        ...
        {
            CHX_LOG_WARN("Failed to acquire perflock type: %d", lock);
        }
    }
    return result;
}
    ...
    UINT32       duration)
{
    ...
    if (mRefCount == 0)
    {
        ...
        {
            CHX_LOG_WARN("Acquire perfhint failed type %d", hintId);
            result = CDKResultEFailed;
        }
    }
    else
...
const char * PerfLock::LockTypeToString(PerfLockType type)
{
    ...
    switch (type)
    {
        ...
        case PERF_LOCK_SNAPSHOT_CAPTURE:
            pString = "PERF_LOCK_SNAPSHOT_CAPTURE";
            break;
                                    ...
                                    UINT32       timer)
{
             ...
             forceReacquire == TRUE)
    {
        ...
        {
            CHX_LOG_WARN("Acquire perflock failed");
            result = CDKResultEFailed;
        }
    }
    else
...
VOID PerfLock::Destroy()
{
    if (mRefCount != 0)
    {
        CHX_LOG_WARN("Non zero references to this perflock");
    }
    m_pMutex->Destroy();
