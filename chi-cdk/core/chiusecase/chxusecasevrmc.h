...
private:
    ...
    /// Funtion to release reference to input buffers of a request
    CDKResult ReleaseReferenceToInputBuffers(CHIPRIVDATA* pPrivData);

    /// @brief Realtime Buffer Queue entry
    struct RealTimeBufferQueue
    {
            UINT32           frameNumber;                               ///< Frame number
            ChiMetadata*     pMetadata[MAX_REALTIME_PIPELINE];          ///< Array of metadata
            CHISTREAMBUFFER  buffer[MAX_REALTIME_PIPELINE];             ///< RT buffer
            BOOL             valid[MAX_REALTIME_PIPELINE];              ///< valid flag
            UINT32           resultMap;                                 ///< Flag to check if all result of this
                                                                        ///  request is ready
    };

    INT64                   m_shutterFrameNum;                              ///< last framenum processed.
    RealTimeBufferQueue     rtBuffer[MaxOutstandingRequests];               ///< realtime pipeline request buffer queue
    RealTimeBufferQueue     rtVamBuffer[MaxOutstandingRequests];            ///< realtime pipeline request buffer queue
    RealTimeBufferQueue     rtSnapshotBuffer[MaxOutstandingRequests];       ///< offline snapshot pipeline request buffer queue
    RealTimeBufferQueue     rtRawSnapshotBuffer[MaxOutstandingRequests];    ///< offline raw snapshot pipeline request buffer queue
    ...
    camera3_stream_buffer_t m_appSnapshotBuffer[MaxOutstandingRequests];    ///< application request buffer handle for snapshot
    camera3_stream_buffer_t m_appPreviewBuffer[MaxOutstandingRequests];     ///< application request buffer handle for preview
    camera3_stream_buffer_t m_appPostviewBuffer[MaxOutstandingRequests];    ///< application request buffer handle for postview
    camera3_stream_buffer_t m_appVideoBuffer[MaxOutstandingRequests];       ///< application request buffer handle for video
    camera3_stream_buffer_t m_appRawSnapshotBuffer[MaxOutstandingRequests]; ///< application request buffer handle for video
    camera3_stream_buffer_t m_appVamBuffer[MaxOutstandingRequests];         ///< application request buffer handle for vam

    CHIBufferManager*       m_pPreviewBufferManager[MAX_REALTIME_PIPELINE];     ///< buffer manager for realtime pipeline
                                                                                ///  preview output
    CHIBufferManager*       m_pVamBufferManager[MAX_REALTIME_PIPELINE];         ///< buffer manager for realtime pipeline
                                                                                ///  VAM output
    CHIBufferManager*       m_pRDIBufferManager[MAX_REALTIME_PIPELINE];         ///< buffer manager for realtime pipeline
                                                                                ///  RDI output
    CHIBufferManager*       m_pRDISnapshotBufferManager[MAX_REALTIME_PIPELINE]; ///< buffer manager for realtime pipeline
                                                                                ///  RDI Raw output
    CHIBufferManager*       m_pYUVBufferManager[MAX_REALTIME_PIPELINE];         ///< buffer manager for offline pipeline
                                                                                ///  YUV output
