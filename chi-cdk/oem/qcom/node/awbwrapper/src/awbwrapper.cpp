...
AWBWrapperAlgorithmHandler  g_AWBLibAlgorithmIntf = { 0 };
    ...
    AWBAlgoGetParam*    pGetParams)
{
    ...
    if (NULL != g_AWBLibAlgorithmIntf.pAWBAlgo->AWBGetParam)
    {
        result = g_AWBLibAlgorithmIntf.pAWBAlgo->AWBGetParam(g_AWBLibAlgorithmIntf.pAWBAlgo, pGetParams);
    }
    ...
    const AWBAlgoSetParamList*  pSetParams)
{
    ...
    if (NULL != g_AWBLibAlgorithmIntf.pAWBAlgo->AWBSetParam)
    {
        result = g_AWBLibAlgorithmIntf.pAWBAlgo->AWBSetParam(g_AWBLibAlgorithmIntf.pAWBAlgo, pSetParams);
    }
    ...
    const AWBAlgoDestroyParamList*   pDestroyParams)
{
    ...
    {
        if (g_AWBLibAlgorithmIntf.instanceCount > 0)
        {
            g_AWBLibAlgorithmIntf.instanceCount--;
        }

        if (0 == g_AWBLibAlgorithmIntf.instanceCount)
        {
            if (NULL != g_AWBLibAlgorithmIntf.pAWBAlgo->AWBDestroy)
            {
                g_AWBLibAlgorithmIntf.pAWBAlgo->AWBDestroy(g_AWBLibAlgorithmIntf.pAWBAlgo, pDestroyParams);
                g_AWBLibAlgorithmIntf.pAWBAlgo = NULL;
                AWB_MSG_INFO("AWB Wrapper instance count 0, AWB Algo destroyed");
            }
        }
        else
        {
            AWB_MSG_INFO("AWB Wrapper instance count %d skip destroy", g_AWBLibAlgorithmIntf.instanceCount);
        }
    }
}
    ...
    AWBAlgoOutputList*      pOutputs)
{
    ...
    if (NULL != g_AWBLibAlgorithmIntf.pAWBAlgo->AWBProcess)
    {
        result = g_AWBLibAlgorithmIntf.pAWBAlgo->AWBProcess(g_AWBLibAlgorithmIntf.pAWBAlgo, pInputs, pOutputs);
    }
    ...
    CHIAWBAlgorithm**               ppAWBAlgorithm)
{
    ...
    if (pCreateParams->pCreateParams != NULL)
    {
        ...
        if (result == CDKResultSuccess)
        {
            ...
            g_AWBLibAlgorithmIntf.pAWBAlgo = *ppAWBAlgorithm;
            g_AWBLibAlgorithmIntf.instanceCount++;
            AWB_MSG_INFO("AWB Wrapper: new instance of AWB Algo, instance count:%d", g_AWBLibAlgorithmIntf.instanceCount);
...
/// AWBGetCapabilities
///
/// @brief  This method creates an instance to the AWBAlgorithm.
///
    ...
    CHIQUERYVENDORTAG* pQueryVendorTag)
{
    ...
    if (CDKResultSuccess == result)
    {
        // AWB wrapper direct calling AWB lib QueryVendorTag
        if (g_AWBLibCallbacks.pfnQueryVendorTag != NULL)
        {
            result = g_AWBLibCallbacks.pfnQueryVendorTag(pQueryVendorTag);
        }
    }
    ...
    ChiAlgorithmInterface* pAlgoInterface)
{
    ...
    if (CDKResultSuccess == result)
    {
        // AWB wrapper direct calling AWB lib SetAlgoInterface
        if (g_AWBLibCallbacks.pfnSetAlgoInterface != NULL)
        {
            g_AWBLibCallbacks.pfnSetAlgoInterface(pAlgoInterface);
        }
    }
}
...
CAMX_VISIBILITY_PUBLIC VOID ChiAWBAlgorithmEntry(CHIAWBALGORITHMCALLBACKS* pCallBacks)
{
    ...
    // AWB wrapper direct calling AWB lib ChiAWBAlgorithmEntry
    ...
    {
        AWB_MSG_ERROR("Failed loaf AWB library - %s", libFilename);
    }
}
