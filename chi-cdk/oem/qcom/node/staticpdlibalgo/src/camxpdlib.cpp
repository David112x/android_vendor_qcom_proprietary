    ...
    const PDLibDestroyParamList*    pDestroyParams)
{
    ...
    for (UINT32 i = 0; i < pDestroyParams->paramCount; i++)
    {
        if (PDLibDestroyParamTypeCameraCloseIndicator == pDestroyParams->pParamList[i].destroyParamType)
        {
            ...
            {
                pOverridePDLibClose = static_cast<UINT*>(pDestroyParams->pParamList[i].pParam);
                CAMX_LOG_INFO(CamxLogGroupStats, "PDLibDestroy overrideCameraClose is %d pPDLibData %p",
                    *pOverridePDLibClose, pPDLibData);
                break;
            }
        }
    }

    if ((NULL != pOverridePDLibClose) && (1 == *pOverridePDLibClose))
    {
        if (NULL != pPDLibData)
        {
...
/// PDLibGetParam
///
/// @brief  Get metadata from CHIPDLib.
///
/// @param  pPDLib      Pointer to CHIPDLib instance
/// @param  pConfig     metadata parameters from the Library.
///
/// @return CDKResultSuccess upon success.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult PDLibGetParam(
    CHIPDLib*       pPDLib,
    PDLibGetParams* pGetParam)
{
    CDKResult           result = CDKResultSuccess;
    PDLibInternalData*  pPDLibDataLocal = (PDLibInternalData*)pPDLib;

    if (NULL == pPDLibDataLocal || NULL == pGetParam)
    {
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        case PDLibGetParamHWConfig:
            pGetParam->outputData.config.enablePDHw = FALSE;
            break;
    ...
    const PDLibCreateParamList* pCreateParams)
{
    ...
    for (UINT32 i = 0; i < pCreateParams->paramCount; i++)
    {
        if (PDLibCreateParamTypeCameraOpenIndicator == pCreateParams->pParamList[i].createParamType)
        {
            ...
            {
                pOverridePDLibOpen = static_cast<UINT*>(pCreateParams->pParamList[i].pParam);
                CAMX_LOG_INFO(CamxLogGroupStats, "CreatePDLib static create: overrideCameraOpen is %d", *pOverridePDLibOpen);
                break;
            }
        }
    }

    if ((NULL != pOverridePDLibOpen) && (1 == (*pOverridePDLibOpen)))
    {
        if(NULL == pPDLibData)
        {
            ...
            else
            {
                ...
                pPDLibData->algoOps.PDLibGetParam   = CamX::PDLibGetParam;
    ...
    {
        CAMX_LOG_INFO(CamxLogGroupStats, "PDLib:Not actual open ...ignore:pOverridePDLibOpen %p", pOverridePDLibOpen);
    }
