    ...
    const AFAlgoDestroyParamList*   pDestroyParams)
{
    ...
    AFAlgoDestroyParam* pCameraInfo = &pDestroyParams->pParamList[AFDestroyParamTypeCameraInfo];
    ...
    CHIAFAlgorithm**                ppAfAlgorithm)
{
    ...
    AFAlgoCreateParam* pCameraInfo = &pCreateParams->pParamList[AFCreateParamTypeCameraInfo];
    UINT32 cameraId                = static_cast<StatsCameraInfo *>(pCameraInfo->pParam)->cameraId;
    BOOL isCameraOpening           = FALSE;
    if ((NULL != pOverrideCameraOpen) && (0 != (*pOverrideCameraOpen)))
    {
        isCameraOpening = CamX::Utils::IsBitMaskSet((*pOverrideCameraOpen), (1 << cameraId));
    }
    if (TRUE == isCameraOpening)
    {
        if (NULL == pAFInternalData)
        {
