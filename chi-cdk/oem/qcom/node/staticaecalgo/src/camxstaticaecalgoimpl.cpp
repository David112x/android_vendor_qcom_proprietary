...
static const UINT8  StatsDefaultOutputBitDepth   = 14;                   ///< Output bit depth
static const UINT32 StatsDefaultChannelThreshold = ((255 - 16) << 6);    ///< Default channel threshold
static const UINT32 StatsDefaultHorizontalCount  = 64;                   ///< Horizontal count
static const UINT32 StatsDefaultVerticalCount    = 48;                   ///< Vertical count
    ...
    AECAlgoOutputList*          pOutput)
{
    ...
    StatsBayerGridBayerExposureConfig*  pStatsConfig      = NULL;
    StatsBayerHistogramConfig*          pStatsBHISTConfig = NULL;
    ...
    for (UINT32 i = 0; i < pOutput->numberOfAECOutputs; i++)
    {
        ...
        switch (pCurrentParam->outputType)
        {
            ...
            case AECAlgoOutputBGConfig:
            {
                pStatsConfig = reinterpret_cast<StatsBayerGridBayerExposureConfig*>(pCurrentParam->pAECOutput);
                pCurrentParam->sizeOfWrittenAECOutput = pCurrentParam->sizeOfAECOutput;
                break;

            }
            case AECAlgoOutputBHistConfig:
            {
                pStatsBHISTConfig = reinterpret_cast<StatsBayerHistogramConfig*>(pCurrentParam->pAECOutput);
                pCurrentParam->sizeOfWrittenAECOutput = pCurrentParam->sizeOfAECOutput;
                break;
            }
            default:
    ...
    if (NULL != pFrameInfo)
    {
        ...
        if (NULL != pFrameControl)
        {
            ...
            pFrameControl->luxIndex          = 240.0f;
            pFrameControl->ISOValue          = 100;
            pFrameControl->predictiveGain    = 1.0f;
        }
    ...
    if (NULL != pStatsConfig)
    {
        for (UINT32 i = 0; i < StatisticsBayerChannelsCount; i++)
        {
            pStatsConfig->channelGainThreshold[i] = StatsDefaultChannelThreshold;
        }
        for (UINT32 i = 0; i < RGBChannelCount; i++)
        {
            pStatsConfig->YStatsWeights[i] = 0;
        }
        pStatsConfig->horizontalRegionCount = StatsDefaultHorizontalCount;
        pStatsConfig->verticalRegionCount   = StatsDefaultVerticalCount;
        pStatsConfig->outputBitDepth        = StatsDefaultOutputBitDepth;
        pStatsConfig->ROISelection          = StatsROISelectionFullFOV;
    }

    if (NULL != pStatsBHISTConfig)
    {
        pStatsBHISTConfig->channel      = ColorChannelY;
        pStatsBHISTConfig->uniform      = TRUE;
        pStatsBHISTConfig->ROISelection = StatsROISelectionFullFOV;
    ...
    AECAlgoGetParam* pGetParam)
{
    ...
    for (UINT32 i = 0; i < pGetParam->outputList.numberOfAECOutputs; i++)
    {
        ...
        switch (pCurrentParam->outputType)
        {
            ...
            case AECAlgoGetParamOutputBGStatsConfig:
            {
                pStatsConfig = reinterpret_cast<StatsBayerGridBayerExposureConfig*>(pCurrentParam->pOutputData);
                pCurrentParam->sizeOfWrittenOutputData = pCurrentParam->sizeOfOutputData;
                break;
            }
            case AECAlgoGetParamOutputBHistStatsConfig:
            {
                pStatsBHISTConfig = reinterpret_cast<StatsBayerHistogramConfig*>(pCurrentParam->pOutputData);
                pCurrentParam->sizeOfWrittenOutputData = pCurrentParam->sizeOfOutputData;
                break;
            }
            default:
    ...
    AECAlgoGetParam*   pGetParam)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        switch (pGetParam->type)
        {
            ...
            case AECAlgoGetParamBGStatsConfig:
            case AECAlgoGetParamBHistStatsConfig:
            {
    ...
    const AECAlgoDestroyParamList*   pDestroyParams)
{
    ...
    AECAlgoDestroyParam* pCameraInfo = &pDestroyParams->pParamList[AECDestroyParamTypeCameraInfo];
    UINT32 cameraId                  = static_cast<StatsCameraInfo *>(pCameraInfo->pParam)->cameraId;
    ...
    for (UINT32 j = 0; j < pDestroyParams->paramCount; j++)
    {
        ...
        switch (pParamList->destroyParamType)
        {
            case AECDestroyParamTypeCameraCloseIndicator:
            {
                ...
                BOOL isCameraClosing     = Utils::IsBitMaskSet(overrideCameraClose, (1 << cameraId));
        ...
        switch (pParamList->createParamType)
        {
            case AECAlgoCreateParamTypeSessionHandle:
            {
                ...
                if ((pParamList->pCreateParam != NULL) && (pParamList->sizeOfCreateParam >= sizeof(CHISTATSHANDLE)))
                {
                    ...
                    if (NULL != g_ChiAlgoInterface.pGetMetadata)
                    {
                        result = g_ChiAlgoInterface.pGetMetadata(&StaticVendortagInfo);
                        if ((CDKResultSuccess != result) || (NULL == staticTagData[0].pData))
                        {
