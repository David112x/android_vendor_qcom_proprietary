...
// Support only RAW_DEPTH, DEPTH16 and POINTCLOUD
...
static const CHAR   SharedObjectName[]       = "libdepthmapwrapper";            ///< The name of the shared object to be loaded
static const CHAR   SecureSharedObjectName[] = "libdepthmapwrapper_secure";     ///< The name of the secure shared object to be loaded
    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    m_isSecureMode = pCreateInfo->nodeFlags.isSecureMode;
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    if (NULL != pMetaData)
    {
        ...
        config.in_depth_map_fps_range.min_fps       = static_cast<float>(_fps[0]);
        config.in_depth_map_fps_range.video_min_fps = static_cast<float>(_fps[0]);
        fps[0]                                      = _fps[0];
        config.in_depth_map_fps_range.max_fps       = static_cast<float>(_fps[1]);
        config.in_depth_map_fps_range.video_max_fps = static_cast<float>(_fps[1]);
        fps[1]                                      = _fps[1];
    }

    LOG_VERBOSE(CamxLogGroupChi,
                "Input: inputNum=%d, imageCount=%d, numPlanes=%d, wxh=%dx%d, format=%d,"
                " pAddr=%p, fd=%d size=%zu, exposure=%" PRIu64 ", timestamp=%" PRIu64 ", frameCount=%" PRIu64 ", autoExposure=%s,"
                " fps[%d:%d]",
                ...
                pProcessRequestInfo->phInputBuffer[0]->pImageList[0].fd[0],
    ...
    for (UINT i = 0; i < numOutputs; ++i)
    {
        LOG_VERBOSE(CamxLogGroupChi, "Output: outputNum=%u, imageCount=%u, numPlanes=%u, wxh=%ux%u, format=%d, pAddr=%p,"
                    ...
                    pProcessRequestInfo->phOutputBuffer[i]->pImageList[0].fd[0],
        ...
        case RawPlain16:
            ...
            outputs[i].out_image_length =
                static_cast<uint32_t>(pProcessRequestInfo->phOutputBuffer[i]->planeSize[0]);
            if (m_isSecureMode)
            {
                outputs[i].out_image_fd =
                    pProcessRequestInfo->phOutputBuffer[i]->pImageList[0].fd[0];
            }
            else
            {
    ...
    if (m_isSecureMode)
    {
        depthInput.in_image_fd = pProcessRequestInfo->phInputBuffer[0]->pImageList[0].fd[0];
    }
    else
    {
        depthInput.in_image = (const INT8*)pProcessRequestInfo->phInputBuffer[0]->pImageList[0].pAddr[0];
    }
...
ChiDepthNode::~ChiDepthNode()
{
    ...
    if (NULL != m_hDepthLib)
    {
        ...
        if (CDKResultSuccess != result)
        {
            LOG_ERROR(CamxLogGroupChi, "Uninitialize Failed to unmap lib %s: %d",
                (m_isSecureMode) ? SecureSharedObjectName : SharedObjectName,
                result);
        }
        m_hDepthLib = NULL;
    }
...
CDKResult ChiDepthNode::InitConversionLib()
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        ChiNodeUtils::SNPrintF(libFullName,
                               FILENAME_MAX,
                               "%s%s%s.%s",
        ...
        m_hDepthLib = ChiNodeUtils::LibMapFullName(libFullName);
        if (NULL == m_hDepthLib)
        {
            LOG_ERROR(CamxLogGroupChi, "Error loading lib %s", libFullName);
            result = CDKResultEFailed;
        }
        else
        {
            ...
            {
                LOG_ERROR(CamxLogGroupChi, "Error getting function address(es) from library: %s", libFullName);
                result = CDKResultEFailed;
            }
            else
            {
