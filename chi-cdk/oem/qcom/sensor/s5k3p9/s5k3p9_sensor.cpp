...
static const UINT   S5K3P9MinRegisterGain        = 32;                                    ///< Minimum analog register gain
static const UINT   S5K3P9MaxRegisterGain        = 512;                                   ///< Maximum analog register gain
static const DOUBLE S5K3P9MinRealGain            = (S5K3P9MinRegisterGain/32);            ///< Minimum analog real gain (1X)
static const DOUBLE S5K3P9MaxRealGain            = (S5K3P9MaxRegisterGain/32);            ///< Maximum analog real gain (16X)
static const UINT   S5K3P9MinDigitalRegisterGain = 256;                                   ///< Minimum digital register gain
static const UINT   S5K3P9MaxDigitalRegisterGain = 256;                                   ///< Maximum digital register gain
static const UINT   S5K3P9DigitalGainDecimator   = 256;                                   /// < Digital gain decimator factor
static const DOUBLE S5K3P9MinDigitalRealGain     = (S5K3P9MinDigitalRegisterGain / S5K3P9DigitalGainDecimator);
                                                                                          ///< Minimum digital real gain (1X)
static const DOUBLE S5K3P9MaxDigitalRealGain     = (S5K3P9MaxDigitalRegisterGain/ S5K3P9DigitalGainDecimator);
...
static DOUBLE RegisterToRealGain(
        UINT registerGain)
{
    if (S5K3P9MaxRegisterGain < registerGain)
    {
        registerGain = S5K3P9MaxRegisterGain;
    }

    return ((float)registerGain / 0x20);
}
...
static UINT RealToRegisterGain(
        DOUBLE realGain)
{
    if (S5K3P9MinRealGain > realGain)
    {
        realGain = S5K3P9MinRealGain;
    }
    else if (S5K3P9MaxRealGain < realGain)
    {
        realGain = S5K3P9MaxRealGain;
    }

    return static_cast<UINT>(realGain*0x20);
}
...
{
    float digitalRealGain = S5K3P9MinDigitalRealGain;

    if (S5K3P9MaxRealGain < totalRealGain)
    {
        digitalRealGain = totalRealGain / sensorRealGain;
    }
    else
    {
        digitalRealGain = S5K3P9MinDigitalRealGain;
    }

    if (S5K3P9MaxDigitalRealGain < digitalRealGain)
    {
        digitalRealGain = S5K3P9MaxDigitalRealGain;
    }
    return static_cast<UINT>(digitalRealGain * S5K3P9DigitalGainDecimator);
}
