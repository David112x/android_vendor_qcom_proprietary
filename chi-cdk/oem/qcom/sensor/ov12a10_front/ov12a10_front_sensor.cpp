...
static const UINT  OV12A10FrontMinRegisterGain        = 0x0080;                              ///< Minimum analog register gain
static const UINT  OV12A10FrontMaxRegisterGain        = 0x07F0;                              ///< Maximum analog register gain
static const FLOAT OV12A10FrontMinRealGain            = 1.0;                                 ///< Minimum analog real gain(1X)
static const FLOAT OV12A10FrontMaxRealGain            = 15.5;                                ///< Maximum analog real gain(15.5X)
static const UINT  OV12A10FrontRegisterGainDecimator  = 128;                                 ///< Register gain decimator factor
static const UINT  OV12A10FrontMinDigitalGain         = 1.0;                                 ///< Minimum digital gain
static const UINT  OV12A10FrontMaxDigitalGain         = 4.0;                                 ///< Maximum digital gain
static const UINT  OV12A10FrontDigitalGainDecimator   = 1024;                                ///< Digital gain decimator factor
    ...
    UINT registerGain)
{
    ...
    if (OV12A10FrontMaxRegisterGain < tempRegisterGain)
    {
        tempRegisterGain = OV12A10FrontRegisterGainDecimator * OV12A10FrontMaxRealGain;
    }
    else if (OV12A10FrontMinRegisterGain > tempRegisterGain)
    {
        tempRegisterGain = OV12A10FrontRegisterGainDecimator * OV12A10FrontMinRealGain;
    }

    return  static_cast<FLOAT>(tempRegisterGain / OV12A10FrontRegisterGainDecimator);
}
    ...
    FLOAT realGain)
{
    ...
    UINT  registerGain = OV12A10FrontMinRegisterGain;

    if (OV12A10FrontMinRealGain > realGain)
    {
        tempRealGain = OV12A10FrontMinRealGain;
    }
    if (2.0f >= realGain)
    {
    ...
    if (OV12A10FrontMinRealGain > tempRealGain)
    {
        tempRealGain = OV12A10FrontMinRealGain;
    }
    else if (OV12A10FrontMaxRealGain < tempRealGain)
    {
        tempRealGain = OV12A10FrontMaxRealGain;
    }

    registerGain = static_cast<UINT>(tempRealGain * OV12A10FrontRegisterGainDecimator);
    return registerGain;
}
    ...
    FLOAT analogRealGain)
{
    ...
    if (OV12A10FrontMinDigitalGain > digitalRealGain)
    {
        digitalRealGain = OV12A10FrontMinDigitalGain;
    }
    else if (OV12A10FrontMaxDigitalGain < digitalRealGain)
    {
        digitalRealGain = OV12A10FrontMaxDigitalGain;
    }

    return static_cast<UINT>(digitalRealGain * OV12A10FrontDigitalGainDecimator);
}
    ...
    SensorCalculateExposureData* pCalculateExposureData)
{
    ...
    pExposureInfo->digitalRealGain      = static_cast<FLOAT>(pExposureInfo->digitalRegisterGain) /
        static_cast<FLOAT>(OV12A10FrontDigitalGainDecimator);
