    ...
    UINT registerGain)
{
  ...
  if (tempRegisterGain < OV8856MinRegisterGain)
  {
    tempRegisterGain = OV8856MinRegisterGain;
  }
  else if (tempRegisterGain > OV8856MaxRegisterGain)
  {
    tempRegisterGain = OV8856MaxRegisterGain;
  }

  return static_cast<FLOAT>(tempRegisterGain/ OV8856RegisterGainDecimator);
}
    ...
    FLOAT realGain)
{
    ...
    UINT  registerGain = OV8856MinRegisterGain;

    if (OV8856MinRealGain > realGain)
    {
        tempRealGain = OV8856MinRealGain;
    }
    ...
    if (OV8856MinRealGain > tempRealGain)
    {
        tempRealGain = OV8856MinRealGain;
    }
    else if (OV8856MaxRealGain < tempRealGain)
    {
        tempRealGain = OV8856MaxRealGain;
    }

    registerGain = static_cast<UINT>(tempRealGain * OV8856RegisterGainDecimator);
    return registerGain;
}
    ...
    SensorCalculateExposureData* pCalculateExposureData)
{
    ...
    pExposureInfo->digitalRealGain      = static_cast<FLOAT>(pExposureInfo->digitalRegisterGain/
        static_cast<FLOAT>(OV8856DigitalGainDecimator));
    ...
    SensorFillExposureData* pExposureData)
{
    ...
    pRegSettingsInfo->regSetting[regCount].registerData  = (pExposureData->lineCount >> 12) & 0xF;
    regCount++;
    ...
    pRegSettingsInfo->regSetting[regCount].registerData  = (pExposureData->analogRegisterGain >> 8) & 0x7;
    regCount++;
