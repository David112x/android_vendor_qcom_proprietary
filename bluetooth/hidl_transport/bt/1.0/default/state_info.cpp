...
void BtState::SetPowerResourcesState(char *buf) {
  memset(stat_info_obj.power_resources_state, 0, MAX_PWR_RSRC_INFO_SIZE);
  strlcpy(stat_info_obj.power_resources_state, buf, sizeof(stat_info_obj.power_resources_state));
  ALOGV("%s:is %s", __func__, stat_info_obj.power_resources_state);
}
