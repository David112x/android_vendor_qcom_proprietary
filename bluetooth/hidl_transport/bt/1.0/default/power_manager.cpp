...
void PowerManager :: GetAndLogPwrRsrcStates()
{
  int ret = 0;
  int fd = -1;
  unsigned long val = 0;
  ...
  ALOGI("%s", __func__);
  if (bt_soc_type_ == BT_SOC_HASTINGS || bt_soc_type_ == BT_SOC_MOSELLE) {
    ...
    ret = ioctl(fd, get_bt_reset_gpio_cmd_, (unsigned long)val);
    if (ret < 0) {
      ALOGE(" ioctl failed: get_bt_reset_gpio_cmd ret(%d) error(%s)",
        ret, strerror(errno));
    } else if ((pos >= 0) && (pos < MAX_PWR_RSRC_INFO_SIZE)) {
      pos += snprintf(power_state_buff + pos, MAX_PWR_RSRC_INFO_SIZE - pos,
        "    BtResetGpioValue: %d\n", ret);
    }

    ret = ioctl(fd, get_bt_sw_ctrl_gpio_cmd_, (unsigned long)val);
    if (ret < 0) {
      ALOGE(" ioctl failed: get_bt_sw_ctrl_gpio_cmd ret(%d) error(%s)",
        ret, strerror(errno));
    } else if ((pos >= 0) && (pos < MAX_PWR_RSRC_INFO_SIZE)) {
      pos += snprintf(power_state_buff + pos, MAX_PWR_RSRC_INFO_SIZE - pos,
        "    BtSwCtrlGpioValue: %d\n", ret);
    ...
    ret = ioctl(fd, get_bt_vdd_aon_ldo_cmd_, (unsigned long)val);
    if (ret < 0) {
      ALOGE(" ioctl failed: get_bt_vdd_aon_ldo_cmd ret(%d) error(%s)",
        ret, strerror(errno));
    } else if ((pos >= 0) && (pos < MAX_PWR_RSRC_INFO_SIZE)) {
      pos += snprintf(power_state_buff + pos, MAX_PWR_RSRC_INFO_SIZE - pos,
        "    BtVddAonLdoValue: %d\n", ret);
    ...
    ret = ioctl(fd, get_bt_vdd_dig_ldo_cmd_, (unsigned long)val);
    if (ret < 0) {
      ALOGE(" ioctl failed: get_bt_vdd_dig_ldo_cmd ret(%d) error(%s)",
        ret, strerror(errno));
    } else if ((pos >= 0) && (pos < MAX_PWR_RSRC_INFO_SIZE)) {
      pos += snprintf(power_state_buff + pos, MAX_PWR_RSRC_INFO_SIZE - pos,
        "    BtVddDigLdoValue: %d\n", ret);
    ...
    ret = ioctl(fd, get_bt_vdd_rfa1_ldo_cmd_, (unsigned long)val);
    if (ret < 0) {
      ALOGE(" ioctl failed: get_bt_vdd_rfa1_ldo_cmd ret(%d) error(%s)",
        ret, strerror(errno));
    } else if ((pos >= 0) && (pos < MAX_PWR_RSRC_INFO_SIZE)) {
      pos += snprintf(power_state_buff + pos, MAX_PWR_RSRC_INFO_SIZE - pos,
        "    BtVddRfa1LdoValue: %d\n", ret);
    ...
    ret = ioctl(fd, get_bt_vdd_rfa2_ldo_cmd_, (unsigned long)val);
    if (ret < 0) {
      ALOGE(" ioctl failed: get_bt_vdd_rfa2_ldo_cmd ret(%d) error(%s)",
        ret, strerror(errno));
    } else if ((pos >= 0) && (pos < MAX_PWR_RSRC_INFO_SIZE)) {
      pos += snprintf(power_state_buff + pos, MAX_PWR_RSRC_INFO_SIZE - pos,
        "    BtVddRfa2LdoValue: %d\n", ret);
    ...
    ALOGI("%s: %s", __func__, power_state_buff);
