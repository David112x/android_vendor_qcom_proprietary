  ...
  if (!protocol_info_.size()) {
    ...
    if (init_status_ > INIT_STATUS_IDLE ) {
      if ( INIT_STATUS_INITIALIZING == init_status_) {
        ...
        bool getVerReqStatus = true;
        if (time_diff >= HIDL_INIT_TIMEOUT)
          getVerReqStatus = logger_->GetVerReqStatus();

        if (getVerReqStatus == false)
          controller_->GetAndLogPwrRsrcStates();
        ...
        Wakelock :: CleanUp();
#endif
        ALOGE("%s: GetVerStatus is %d", __func__, getVerReqStatus);
        if (getVerReqStatus == false) {
          char value[PROPERTY_VALUE_MAX] = {'\0'};
          property_get("persist.vendor.service.bdroid.trigger_crash", value, "false");
          // call kernel panic so that all dumps are collected
          if (strcmp(value, "true") == 0) {
            ALOGE("%s: No GetVer Response from soc", __func__);
            char panic_set ='c';
            ...
            if (fd_sysrq < 0) {
              ...
              ALOGE("%s: write (%s) fail - %s (%d)", __func__,
                PROC_PANIC_PATH, strerror(errno), errno);
            }
            ALOGE("%s: Returning after kernel panic", __func__);
            return true;
          }
        }
#ifdef USER_DEBUG
