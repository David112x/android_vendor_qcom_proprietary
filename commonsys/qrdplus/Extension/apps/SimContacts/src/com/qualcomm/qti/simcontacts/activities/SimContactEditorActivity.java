...
public class SimContactEditorActivity extends AppCompatActivity
    ...
    private void updateViewsVisibility(){
        final boolean anrsVisible = mEditContact
                ? !mAnrsEditor.isInitialValueEmpty()
                || ContactUtils.getSpareAnrCount(mSubscriptionId) > 0
                : ContactUtils.getSpareAnrCount(mSubscriptionId) > 0;
        mAnrsEditor.setVisibility(anrsVisible ? View.VISIBLE : View.GONE);
        final boolean emailsVisible = mEditContact
                ? !mEmailsEditor.isInitialValueEmpty()
                || ContactUtils.getSpareEmailCount(mSubscriptionId) > 0
                : ContactUtils.getSpareEmailCount(mSubscriptionId) > 0;
        mEmailsEditor.setVisibility(emailsVisible ? View.VISIBLE : View.GONE);
    }
