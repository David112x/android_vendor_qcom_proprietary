...
import com.qualcomm.qti.usta.ui.SensorEventDisplayFragment;
...
public class SensorContext {
  ...
  private boolean saveAsPredefinedScript = false;
  private SensorScriptUtility scriptUtil;
  private String preDefinedScriptName = "";
  public static boolean isScriptGenerationEnabled = false;
  private long lastRegUpdateTimeStamp = 0;
  ...
  private void attributeUpdate(String inputJsonString, Sensor sensorInstance) {
    try {
      JSONObject inputEvent = new JSONObject(inputJsonString);
      JSONArray eventsArray =   inputEvent.getJSONArray("events");
      for(int eventsArrayIndex = 0; eventsArrayIndex < eventsArray.length() ; eventsArrayIndex++) {
        JSONObject evnetsArrayElemet = eventsArray.getJSONObject(eventsArrayIndex);
        JSONObject payLoad = evnetsArrayElemet.getJSONObject("payload");
        JSONArray attributeInfoArray = payLoad.getJSONArray("attributes");
        for(int attributeIndex = 0 ; attributeIndex < attributeInfoArray.length() ; attributeIndex++) {
          JSONObject singleAttributeObj = attributeInfoArray.getJSONObject(attributeIndex);
          String AttributeType = singleAttributeObj.getString("attr_id");
          JSONObject valueObj = singleAttributeObj.getJSONObject("value");
          JSONArray valueArray = valueObj.getJSONArray("values");
          for(int valueArrayIndex = 0; valueArrayIndex < valueArray.length() ; valueArrayIndex++) {
            if(AttributeType.equals("SNS_STD_SENSOR_ATTRID_STREAM_TYPE")) {
              JSONObject valuesObj = valueArray.getJSONObject(valueArrayIndex);
              int value = valuesObj.getInt("sint");
              if(value == 0)
                sensorInstance.setOnChangeSensor(false);
              else
                sensorInstance.setOnChangeSensor(true);
            }
            if(AttributeType.equals("SNS_STD_SENSOR_ATTRID_HW_ID")) {
              JSONObject valuesObj = valueArray.getJSONObject(valueArrayIndex);
              int value = valuesObj.getInt("sint");
              sensorInstance.setHwID(value);
            }
            if(AttributeType.equals("SNS_STD_SENSOR_ATTRID_RIGID_BODY")) {
              JSONObject valuesObj = valueArray.getJSONObject(valueArrayIndex);
              int value = valuesObj.getInt("sint");
              sensorInstance.setRigidBodyType(value);
            }
          }
        }
      }
  ...
  private void updateSensorAttributeInformation() {
    for(int sensorIndex = 0; sensorIndex < sensors.size(); sensorIndex++) {
      String AttributeInfoFromNative = SensorContextJNI.getSensorAttributesNativeWrapper(USTAModeType,sensorIndex);
      String attributeInfoInJson = AttributeInfoFromNative.subSequence(25,AttributeInfoFromNative.length()).toString();
      Sensor sensorInstance = sensors.get(sensorIndex);
      attributeUpdate(attributeInfoInJson, sensorInstance);
    }
  }
  ...
  private SensorContext(ModeType inModeType) {
    ...
    updateSensorAttributeInformation();
    scriptUtil = new SensorScriptUtility();
  }
  ...
  public int sendRequest(int sensorHandle, int sensorReqHandle, Context context, String logFileName) {
    ...
    boolean isPassive   = stdFields.isPassive();
    ...
    if(isPassive == true){
      ...
      sendReqStdFieldObj.setPassive(sP);
    }
    ...
    if(isSaveAsPredefinedScript() == true) {
      generatePredefinedScript(sensorHandle, reqMsg, sendReqStdFieldObj);
      USTALog.i("script is generated  ");
    }else {
      USTALog.i("script Generation is disabled ");
    }
    boolean isWakeup = false;
    if(sendReqStdFieldObj.getWakeupType().compareTo("SNS_CLIENT_DELIVERY_WAKEUP") == 0) {
      isWakeup = true;
    }
    SensorEventDisplayFragment.updateCurrentWakeUpInfo(sensorHandle, isWakeup);
  ...
  public void generatePredefinedScript(int sensorHandle, ReqMsgPayload reqMsg, SendReqStdFields sendReqStdFieldObj){
    scriptUtil.createScriptFile(getPreDefinedScriptName(), getSensors().get(sensorHandle).getSensorName(), getSensors().get(sensorHandle).getSensorSUIDLow() , getSensors().get(sensorHandle).getSensorSUIDHigh(), reqMsg,sendReqStdFieldObj);
  }
  ...
  public void RegistryReadSendRequest(ModeType USTAModeType,  int sensorHandle, String regReadGroupName , Context context){
    ...
    //give room for 2secs before sending read request
    if((System.currentTimeMillis() - lastRegUpdateTimeStamp)/1000 < 2){
      throwErrorDialog("Registry Read Fail: wait for 2sec for any registry update requests" , context);
      USTALog.e("Registry Read Fail: wait for 2sec for any registry update requests");
      return;
    }
    lastRegUpdateTimeStamp = System.currentTimeMillis();
    ReqMsgPayload reqMsg = new ReqMsgPayload();
  ...
  public void sendFlushRequest(ModeType inModeType , int sensorHandle, int sensorReqHandle, Context context) {
    ...
    SensorPayloadField[] payLoadFiledArray = {};
    reqMsg.setFields(payLoadFiledArray);

    NativeErrorCodes errorCodeInfo = SensorContextJNI.sendRequestNativeWrapper(inModeType,sensorHandle, reqMsg,sendReqStdFieldObj, "");
  ...
  public void RegistryWriteSendRequest(ModeType USTAModeType, int sensorHandle, String registryWriteMainGroupName , String registryWriteGroupName, String registryWriteVersionNumber, int position, String oneOfString, Context context ) {
    ...
    //give room for 2secs before sending read request
    if((System.currentTimeMillis() - lastRegUpdateTimeStamp)/1000 < 2){
      throwErrorDialog("Registry Write Fail: wait for 2sec for any registry update requests" , context);
      USTALog.e("Registry write Fail: wait for 2sec for any registry update requests");
      return;
    }
    lastRegUpdateTimeStamp = System.currentTimeMillis();
    ReqMsgPayload reqMsg = new ReqMsgPayload();
  ...
  public void stopRequest(int sensorHandle, int sensorReqHandle) {
    SensorEventDisplayFragment.updateCurrentWakeUpInfo(sensorHandle, false);
  ...
  public boolean isSaveAsPredefinedScript() {
    return saveAsPredefinedScript;
  }

  public void setSaveAsPredefinedScript(boolean saveAsPredefinedScript) {
    this.saveAsPredefinedScript = saveAsPredefinedScript;
  }

  public void setPreDefinedScriptName(String fileName){
   preDefinedScriptName = fileName;
  }

  public String getPreDefinedScriptName(){
    return preDefinedScriptName;
  }

  public SensorScriptUtility getScriptUtil() {
    return scriptUtil;
  }
}
