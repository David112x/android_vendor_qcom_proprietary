...
import com.qualcomm.qti.usta.core.USTAServiceUtil;
...
public class UnifiedSensorTestActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    USTAServiceUtil.ServiceInstanceInit(getApplicationContext());
    ...
    tab = actionbar.newTab().setText("DeviceMode-Tab").setTabListener(new TabListener<deviceModeTab>(this, "DeviceMode", deviceModeTab.class));
    actionbar.addTab(tab);

    tab = actionbar.newTab().setText("On-Device-Logging").setTabListener(new TabListener<OnDeviceLoggingTab>(this, "ODL", OnDeviceLoggingTab.class));
    actionbar.addTab(tab);
  }
  ...
  protected void onDestroy() {
    super.onDestroy();
    SensorContextJNI.removeSensorsWrapper(ModeType.USTA_MODE_TYPE_UI);
  }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    if(item.getItemId() == R.id.disable_logging) {
      ...
      Toast.makeText(getApplicationContext(), (item.isChecked() ? "USTA logging is disabled":"USTA logging is enabled"), Toast.LENGTH_LONG).show();
      SensorContextJNI.updateLoggingFlagWrapper(item.isChecked());
      return true;
    }
    ...
    else if(item.getItemId() == R.id.script_id){
      item.setChecked(!item.isChecked());
      SensorContext.isScriptGenerationEnabled = item.isChecked();
      Toast.makeText(getApplicationContext(), (item.isChecked() ? "Script Generation is enabled":"Script Generation is disabled"), Toast.LENGTH_SHORT).show();
      return true;
    }
    else if(item.getItemId() == R.id.wakelock_id) {
      item.setChecked(!item.isChecked());
      SensorEventDisplayFragment.isWakeLockEnabled = item.isChecked();
      Toast.makeText(getApplicationContext(), (item.isChecked() ? "wakeLock is enabled":"wakeLock is disabled"), Toast.LENGTH_SHORT).show();
      return true;
    }
