...
public class SensorPayloadField {
  ...
  public void addNestedMsgs() {
    NestedMsgPayload currentNestedMsg = new NestedMsgPayload();
    currentNestedMsg.setNestedMsgName(nestedMsgs[0].getNestedMsgName());
    for(int fieldIndex = 0 ; fieldIndex < nestedMsgs[0].getFieldCount() ; fieldIndex++) {
      currentNestedMsg.addField(nestedMsgs[0].getFiledAt(fieldIndex));
    }
    final int newLength = this.nestedMsgs.length + 1;
    this.nestedMsgs = Arrays.copyOf(this.nestedMsgs,newLength );
    this.nestedMsgs[newLength - 1] = currentNestedMsg;
  }
}
