...
public class WFDSession extends IWfdSession.Stub implements UIBCManager.HIDCEventListener, UIBCManager.EventListener {
    ...
    private WfdOverlay mOverlay = null;
     ...
     */
    private native int nativeInit(int isSource);
    ...
    private native String[] nativeGetConfigItems(long nativePtr);
    ...
    private native long[] nativeGetNegotiatedRes(int[] result, long nativePtr);
                ...
                synchronized (mLock) {
                    if (action.equals(WindowManagerPolicyConstants.ACTION_HDMI_PLUGGED)) {
                        ...
                        if (AudioSystem.isStreamActive(AudioSystem.STREAM_MUSIC, 0)) {
                            getConfigItems();
                             if (mConfigItems!=null && 1 == mConfigItems[WfdEnums.ConfigKeys.AUDIO_IN_SUSPEND.ordinal()]) {
    ...
    public int init(IWfdActionListener listener, WfdDevice thisDevice) {
        Log.d(mTAG, "init()");
        synchronized (mLock) {
            if(nativeInit(mDeviceType == WFDDeviceType.SOURCE ? 1:0) == -1) {
    ...
    private int getConfigItems(){
       String[] res = nativeGetConfigItems(mNativePtr);
       if(res != null){
         for(int i = 0; i < res.length - 1; i+=2){
             if(res[i].equalsIgnoreCase("AudioStreamInSuspend"))
                 mConfigItems[WfdEnums.ConfigKeys.AUDIO_IN_SUSPEND.ordinal()] =
                 Integer.parseInt(res[i+1]);
             else if(res[i].equalsIgnoreCase("M14Support"))
                 mConfigItems[WfdEnums.ConfigKeys.UIBC_M14.ordinal()] =
                 Integer.parseInt(res[i+1]);
             else if(res[i].equalsIgnoreCase("UIBCValid"))
                 mConfigItems[WfdEnums.ConfigKeys.UIBC_VALID.ordinal()] =
                 Integer.parseInt(res[i+1]);
         }
         Log.d(mTAG, "Available Configs: " + Arrays.toString(res) +
                 " mConfigItems: " + Arrays.toString(mConfigItems));
       }
       return 0;
    }
    ...
    public int setUIBC() {
        synchronized (mLock) {
            Log.d(mTAG, "setUibc");
            if (nativeSetUIBC(mSessionId, mNativePtr) == 0) {
                return 0;
            } else {
    ...
    public int getCommonResolution(Bundle comRes) {
        synchronized (mLock) {
            ...
            Log.d(mTAG, "Dumping Common Capability bitmaps");
            for(int i = 0; i < ret.length/4; ++i) {
                Log.d(mTAG, ret[i*4] + " " + ret[i*4 + 1] + " " + ret[i*4 + 2] + " " + ret[i*4 + 3]);
            }
    ...
    public int getNegotiatedResolution(Bundle negRes) {
        synchronized (mLock) {
            ...
            int[] result = new int[1];
            long[] ret = nativeGetNegotiatedRes(result, mNativePtr);
            if (result[0] == -1) {
                Log.e(mTAG, "Failed to retrieve negotiated resolution");
                return ErrorType.UNKNOWN.getCode();
            }
            ...
            Log.d(mTAG, "Dumping Negotiated Capability bitmaps");
            Log.d(mTAG, ret[0] + " " + ret[1] + " " + ret[2] + " " + ret[3]);

            negRes.putLongArray("negRes", ret);
        }
        return 0;
    }
    ...
    public int setSurfaceProp(int width, int height, int orientation) {
        synchronized(mLock) {
            if (mDeviceType == WFDDeviceType.SOURCE) {
                ...
                int t[] = {0,0,width,height,orientation};
    ...
    public int execRuntimeCommand(int command) throws RemoteException {
        ...
        return 0 == ret ? 0 : ErrorType.UNKNOWN.getCode();
    }
    ...
    private void eventCallback(String eventName, Object[] objectArray) {
        ...
        if ("Error".equalsIgnoreCase(eventName)) {
            ...
            if (objectArray.length >= 2) {
                ...
                if ("PLAY".equalsIgnoreCase(state)) {
                    ...
                    Bundle b = new Bundle();
                    getCommonResolution(b);
                    getNegotiatedResolution(b);
        ...
        } else if ("OVEvent".equalsIgnoreCase(eventName)) {
            String event = (String) objectArray[0];
            Log.d(mTAG, "OV event: " + event);
            if ("CONFIGURE".equalsIgnoreCase(event)) {
                if (mOverlay != null) {
                    Log.e(mTAG, "previous overlay is not released");
                    mOverlay.release();
                    mOverlay = null;
                }
                int width = mSurfProp.surfaceCoord[2] - mSurfProp.surfaceCoord[0];
                int height = mSurfProp.surfaceCoord[3] - mSurfProp.surfaceCoord[1];
                mOverlay = new WfdOverlay(mContext, width, height);
            } else if ("RELEASE".equalsIgnoreCase(event)) {
                if (mOverlay != null) {
                    mOverlay.release();
                    mOverlay = null;
                }
            ...
            if (mOverlay != null)
                mOverlay.updateEvent(objectArray);
        } else {
...
//                                            | DisplayManager.VIRTUAL_DISPLAY_FLAG_SUPPORTS_PROTECTED_BUFFERS;
    ...
    private void createNotification(int width, int height, int secure) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            String channelName = mContext.getResources().getString(
                    R.string.channel_name);
            NotificationChannel channel = new NotificationChannel(mTAG,
                channelName, NotificationManager.IMPORTANCE_LOW);
            mNotiMgr.createNotificationChannel(channel);
            notiBldr.setChannelId(channel.getId());
            notiBldr.setGroup(mTAG);
        }
        Notification noti = notiBldr.build();
        mNotiMsgId = NOTI_MSG_ID.getAndIncrement();
        mNotiMgr.notify(mNotiMsgId, noti);
        Log.e(mTAG, "mNotiMsgId = " + mNotiMsgId);
    }
