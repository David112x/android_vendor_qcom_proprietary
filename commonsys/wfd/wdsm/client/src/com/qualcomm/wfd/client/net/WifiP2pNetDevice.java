...
public class WifiP2pNetDevice implements NetDeviceIF {
    ...
    public String getWfdInfoString() {
        ...
        if (mDevice.wfdInfo != null) {
            if (mDevice.wfdInfo.isWfdR2Supported()) {
                return new P2pWfdDeviceInfo(mDevice.wfdInfo.getDeviceInfoHex()+
                           mDevice.wfdInfo.getR2DeviceInfoHex()).toString();
            }
            else
                return new P2pWfdDeviceInfo(mDevice.wfdInfo.getDeviceInfoHex()).toString();
        }
        return ("");
    }
