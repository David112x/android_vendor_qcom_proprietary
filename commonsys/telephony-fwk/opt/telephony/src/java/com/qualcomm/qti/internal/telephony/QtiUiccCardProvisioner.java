...
public class QtiUiccCardProvisioner extends Handler {
    ...
    private void updateIccAvailability(int phoneId) {
        ...
        if (mCardState[phoneId] == CardState.CARDSTATE_PRESENT &&
                (mSimIccId[phoneId] == null ||
                !mSimIccId[phoneId].equals(iccId) ||
    ...
    public boolean isAllCardProvisionInfoReceived() {
        ...
        for(int index = 0; index < mNumPhones; index++) {
            if (isEuiccHasEmptyProfiles(index)) {
                received = true;
            } else {
                ...
                if ((provPref == UiccProvisionStatus.INVALID_STATE) ||
                    ...
                    logd("isAllCardProvisionInfoReceived, prov pref[" +
                            index + "] = " + provPref);
                    received = false;
                    break;
                }
    ...
    /**
     * To check the eUICC has empty profiles
     * @return TRUE if it's eUICC and has empty profiles else it returns FALSE.
     */
    private boolean isEuiccHasEmptyProfiles(int phoneId) {
        boolean retVal = false;
        UiccCard uiccCard = mUiccController.getUiccCard(phoneId);
        UiccSlot uiccSlot = mUiccController.getUiccSlotForPhone(phoneId);
        if (uiccCard != null && uiccSlot != null ) {
            UiccProfile uiccProfile = uiccCard.getUiccProfile();
            if (uiccProfile != null) {
                boolean eUiccSupported = uiccSlot.isEuicc();
                int numOfApps = uiccProfile.getNumApplications();
