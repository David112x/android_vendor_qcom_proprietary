...
public class QtiSubscriptionController extends SubscriptionController {
    ...
    void updateUserPreferences() {
        ...
        if (!isSubProvisioned(defaultSmsSubId)) {
            setDefaultSmsSubId(mNextActivatedSub.getSubscriptionId());
        }
        // While default voice sub ID is invalid, the outgoing call account user selected might
        // be null as well, meaning user expects the option "Ask every time".
        // Even if here attempts to initialize a valid default voice SUB ID, the outgoing call
        // account can't be set because phone accounts are not registered yet, leading to mismatch
        // issue then.
