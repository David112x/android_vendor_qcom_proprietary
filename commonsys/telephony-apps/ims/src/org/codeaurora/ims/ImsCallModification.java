...
public class ImsCallModification {
    ...
    private boolean validateOutgoingModifyConnectionType(Message msg, int newCallType) {
        ...
        final boolean isRttCallModificationAllowed = canModifyRttCallType(newCallType);

        boolean isValid = isIndexValid && !modifyToCurrCallType && isRttCallModificationAllowed;
        ...
        Log.i(this, "validateOutgoingModifyConnectionType modifyToCurrCallType = " +
                ...
                " isLowBattery = " + isLowBattery + " isRttCallModificationAllowed = " +
                isRttCallModificationAllowed);
    ...
    // Returns false if RTT VT upgrade is requested and RTT VT is not supported for this carrier.
    // Otherwise, it returns true
    private boolean canModifyRttCallType(int toCallType) {
        int fromCallType = mImsCallSessionImpl.getInternalCallType();
        boolean isVideoRttSupported = QtiImsExtUtils.isRttSupportedOnVtCalls(
                mImsCallSessionImpl.getPhoneId(), mContext);
        boolean isRttCall = mImsCallSessionImpl.getCallProfile().getMediaProfile().isRttCall();
        boolean isVideoUpgradeRequest = !VideoProfile.isVideo(fromCallType) &&
                VideoProfile.isVideo(toCallType);
        Log.d(this, "fromCallType: " + fromCallType + ", isRttCall: " + isRttCall +
                ", isVideoRttSupported: " + isVideoRttSupported + ", isVideoUpgradeRequest" +
                isVideoUpgradeRequest);
        return !(isVideoUpgradeRequest && isRttCall && !isVideoRttSupported);
    }
