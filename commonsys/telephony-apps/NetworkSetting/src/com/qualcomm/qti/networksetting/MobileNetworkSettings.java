...
public class MobileNetworkSettings extends Activity  {
    ...
    public static class MobileNetworkFragment extends PreferenceFragment implements
        ...
        private boolean mIs5gRadioSupported;
        ...
        public void onResume() {
            ...
            mIs5gRadioSupported =
                    ((long)mPhone.getRadioAccessFamily() & TelephonyManager.NETWORK_TYPE_BITMASK_NR)
                    > 0;
            ...
            if (carrierConfig.getBoolean(
                ...
                if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                    ...
                    if (isSupportTdscdma()) {
                        ...
                        log("mIs5gRadioSupported = " + mIs5gRadioSupported);
                        int select = (mShow4GForLTE == true) ? R.array.enabled_networks_4g_choices
                                : R.array.enabled_networks_choices;
                        CharSequence[] networkChoices = getResources().getTextArray(select);
                        CharSequence[] networkValues =
                                getResources().getTextArray(R.array.enabled_networks_values);
                        if (mIs5gRadioSupported) {
                            ArrayList<CharSequence> list = new ArrayList<>();
                            list.add(getResources().getString(mShow4GForLTE ?
                                    R.string.network_5G : R.string.network_5G_lte));
                            list.addAll(Arrays.asList(networkChoices));
                            networkChoices = list.toArray(new CharSequence[0]);
                            list.clear();
                            list.add(Integer.toString(
                                    TelephonyManager.NETWORK_MODE_NR_LTE_GSM_WCDMA));
                            list.addAll(Arrays.asList(networkValues));
                            networkValues = list.toArray(new CharSequence[0]);
                        }
                        mButtonEnabledNetworks.setEntries(networkChoices);
                        mButtonEnabledNetworks.setEntryValues(networkValues);
                    }
                    updateGsmUmtsOptions(this, prefSet, phoneSubId, mNetworkQueryService);
                } else {
                    throw new IllegalStateException("Unexpected phone type: " + phoneType);
        ...
        private void UpdateEnabledNetworksValueAndSummary(int NetworkMode) {
            switch (NetworkMode) {
                ...
                case TelephonyManager.NETWORK_MODE_NR_LTE_GSM_WCDMA:
                    ...
                    mButtonEnabledNetworks.setSummary(mShow4GForLTE
                            ? R.string.network_5G : R.string.network_5G_lte);
                    break;
                default:
