...
/*===========================================================================
  ...
  Copyright (c) 2010-2015, 2017-2020 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
char* int_to_char_arr (int n) {
    int c = int_length(n);
    int it = n;
    char* st = (char*) calloc(c + 1, sizeof(char));
    int j = 0;
    if (st) {
        while( it > 0) {
            st[c - 1 - j] = (char)((it % 10) + 48);
            it /= 10;
            j++;
        }
        st[c] = '\0';
  ...
  do
  {
    ...
    for(i=0; i<QDP_RIL_PARAM_MAX; i++)
    {
      ...
      case QDP_RIL_APN_TYPE_BITMASK:
        ...
        break;
      case QDP_RIL_APN_CLASS:
        if (params[i].len > 0)
        {
            if (params[i].buf) {
                req.apn_class_valid = TRUE;
                req.apn_class = strtoul(params[i].buf, NULL, 0);
            }
        }
        break;
      default:
    ...
    *tech_type = QDP_NOTECH;
        ...
        profile_id_epc == QDP_INVALID_PROFILE)
    {
      if(QDP_INVALID_PROFILE == profile_id_3gpp_list[0] )
      {
        ...
        {
          /* create a profile */
          params_3gpp[QDP_RIL_APN_CLASS].len = int_length(param_value) + 1;
          params_3gpp[QDP_RIL_APN_CLASS].buf = int_to_char_arr(param_value);
          *profile_pdn_type_3gpp  = QDP_PROFILE_PDN_TYPE_INVALID;
          rc = qdp_3gpp_profile_create(params_3gpp,
                                       profile_id_3gpp_list ,
                                       profile_pdn_type_3gpp,
...
boolean qdp_allow_ip_update()
{
  return (qdp_allow_update_modem_profile || carrier_config_flag);
}
