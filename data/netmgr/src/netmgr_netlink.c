...
/*===========================================================================
  ...
  Copyright (c) 2010, 2013-2016, 2020 Qualcomm Technologies, Inc.
  All Rights Reserved.
  ...
  while( RTA_OK(rtah, buflen ) ) {
    ...
    switch( rtah->rta_type ) {
      ...
      case NETMGR_RTATYPE_FLOWINFO:
        ds_memscpy( &event_info->flow_info, sizeof(netmgr_nl_flowinfo_t), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
      ...
      case NETMGR_RTATYPE_ADDRINFO:
        ds_memscpy( &event_info->addr_info, sizeof(event_info->addr_info), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
      ...
      case NETMGR_RTATYPE_GTWYINFO:
        ds_memscpy( &event_info->gtwy_info, sizeof(event_info->gtwy_info), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
      ...
      case NETMGR_RTATYPE_DNSPADDR:
        ds_memscpy( &event_info->dnsp_addr, sizeof(event_info->dnsp_addr), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
      ...
      case NETMGR_RTATYPE_DNSSADDR:
        ds_memscpy( &event_info->dnss_addr, sizeof(event_info->dnss_addr), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
      ...
      case NETMGR_RTATYPE_DEVNAME:
        ds_memscpy( &event_info->dev_name, sizeof(event_info->dev_name), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
      ...
      case NETMGR_RTATYPE_MTU:
        ds_memscpy( &event_info->mtu, sizeof(event_info->mtu), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
        event_info->param_mask |= NETMGR_EVT_PARAM_MTU;
        netmgr_log_med( "Attribute: MTU=%d", event_info->mtu );
        break;

      case NETMGR_RTATYPE_USER_CMD:
        ds_memscpy( &event_info->user_cmd, sizeof(event_info->user_cmd), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
      ...
      case NETMGR_RTATYPE_CMD_DATA:
        ds_memscpy( &event_info->cmd_data, sizeof(event_info->cmd_data), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
    ...
    if( buflen >= RTA_ALIGN((rtah)->rta_len) )
    {
