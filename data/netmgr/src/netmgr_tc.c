...
)
{
  ...
  if( (((netmgr_tc_filter_data *)first)->link == ((netmgr_tc_filter_data *)second)->link) &&
      ...
      (((netmgr_tc_filter_data *)first)->tcpudp_bundle == ((netmgr_tc_filter_data *)second)->tcpudp_bundle) )
...
)
{
  ...
  for (i = 0; i < flow_info->qos_flow.num_filter; i++)
  {
    ...
    filter_data.tcpudp_bundle = NETMGR_TC_TCPUDP_BUNDLE_NONE;

	netmgr_log_high( "delete flow chain ref of rule_id %d\n", filter_data.rule_id );

    if (flow_info->qos_flow.filter_list[i].qos_filter.filter_desc.protocol
        == NETMGR_QOS_TRANS_PROT_TCP_UDP)
    {
      /* Delete TCP rule from bundle first */
      netmgr_log_high( "Delete tcpdudp bundle rule, first for tcp\n" );
      filter_data.tcpudp_bundle = NETMGR_TC_TCPUDP_BUNDLE_TCP;
    }
...
REDO:
    ...
    while (NULL != node)
    {
      ...
      if (netmgr_tc_match_filter_data(&filter_data, dummy) == 0) {
        netmgr_log_high( "Delete v%d rule id: %d\n", filter_data.ip_version, idx );
    ...
    netmgr_log_high( "Finally filter_data.tcpudp_bundle %d\n", filter_data.tcpudp_bundle );
    if (filter_data.tcpudp_bundle == NETMGR_TC_TCPUDP_BUNDLE_TCP)
    {
      /* After TCP rule is removed, delete UDP rule from the bundle */
      netmgr_log_high( "Delete tcpdudp bundle rule, second for udp\n" );
      filter_data.tcpudp_bundle = NETMGR_TC_TCPUDP_BUNDLE_UDP;
      goto REDO;
    }
...
(
  ...
  uint32 rule_id,
  const netmgr_qos_granted_filter_data_type * qos_filter,
  const netmgr_tc_tcpudp_bundle_t tcpudp_bundle
)
{
  NETMGR_LOG_FUNC_ENTRY;
  ...
  {
    if (tcpudp_bundle != NETMGR_TC_TCPUDP_BUNDLE_NONE)
    {
      int protocol = ((tcpudp_bundle == NETMGR_TC_TCPUDP_BUNDLE_TCP) ?
                       NETMGR_TC_TCP_PROTO_IANA_NUM :
                       NETMGR_TC_UDP_PROTO_IANA_NUM);

      netmgr_log_high( "Create rule for tcpudp_bundle %d \n", protocol );

      if (netmgr_tc_iptables_v4_rule_helper( link,
                                             flow_id,
                                             rule_id,
                                             qos_filter,
                                             protocol) != NETMGR_SUCCESS)
      {
        netmgr_log_err("create_iptables_rule_v4: Error creating a V4 filter");
        goto error;
      }
    }
    else if (netmgr_tc_iptables_v4_rule_helper( link,
                                                flow_id,
                                                rule_id,
                                                qos_filter,
...
)
{
  ...
  else
  {
    ...
    else if (netmgr_tc_iptables_v6_rule_helper( link,
                                                flow_id,
                                                rule_id,
                                                qos_filter,
                                                qos_filter->qos_filter.filter_desc.protocol) != NETMGR_SUCCESS)
...
void netmgr_tc_delete_node_from_tc_list
(
  netmgr_tc_filter_data *filter_data
)
{
  ...
  if (filter_data == NULL)
  {
    netmgr_log_err( "NULL filter_data pointer, ignoring\n" );
    return;
  }
...
)
{
  ...
  int result = NETMGR_FAILURE;
  char * ipt_tool = NULL;
  int filter_index, list_index, filter_index_bundle, list_index_bundle;
  netmgr_tc_filter_data *filter_data, *filter_data_bundle;
  ds_dll_el_t* tail = NULL;
  ds_dll_el_t* node = NULL;
  const void* dummy = NULL;
  NETMGR_LOG_FUNC_ENTRY;
  ...
  for (i = 0; i < flow_info->qos_flow.num_filter; i++)
  {
    ...
    /* Check if it's tcpudp bundle rule */
    if (flow_info->qos_flow.filter_list[i].qos_filter.filter_desc.protocol
          == NETMGR_QOS_TRANS_PROT_TCP_UDP)
    {
      netmgr_log_high( "Process tcpudp bundle\n" );
      filter_data->tcpudp_bundle = NETMGR_TC_TCPUDP_BUNDLE_TCP;
    }
    ...
    */
    filter_index = netmgr_tc_get_filter_index(
                                  ...
                                  filter_data->ip_version,
                                  &list_index);
    netmgr_log_high( "Get filter_index:%d, list_index:%d for rule %d\n", filter_index, list_index, filter_data->rule_id );

    if ( filter_index < 0 )
    {
      netmgr_log_high( "Fail to get filter_index, free filter_data \n" );
      netmgr_free(filter_data);
      goto error;
    }
    /* Get filter_index - end */

    /* Insert the currnet filter object in the list */
    netmgr_log_high( "Insert filter_data to list \n" );
    if ( ( ds_dll_insert(netmgr_tc_cfg.filter_list,
                         NULL,
                         (void *)filter_data,
                         list_index) ) == NULL )
    {
      netmgr_log_high( "Fail to insert filter_data to list \n" );
      netmgr_free(filter_data);
      goto error;
    }
    /* Insert the currnet filter object in the list - end */

    /* Process the 2nd rule of tcpudp bundle - start*/
    if (filter_data->tcpudp_bundle != NETMGR_TC_TCPUDP_BUNDLE_NONE)
    {
      /* Allocate filter_data_bundle */
      filter_data_bundle = netmgr_tc_filter_alloc();

      if (filter_data_bundle == NULL)
      {
        netmgr_log_err( "No Memory\n" );
        /* Delete filter_data from list, and then free it */
        netmgr_tc_delete_node_from_tc_list(filter_data);
        netmgr_free(filter_data);
        goto error;
      }
      /* Allocate filter_data_bundle - end */

      *filter_data_bundle = *filter_data;
      filter_data_bundle->tcpudp_bundle = NETMGR_TC_TCPUDP_BUNDLE_UDP;

      /* Select index for another rule since it's tcpdup bundle */
      filter_index_bundle = netmgr_tc_get_filter_index(
                                           link,
                                           filter_data_bundle->flow_id,
                                           filter_data_bundle->rule_id,
                                           filter_data_bundle->precedence,
                                           filter_data_bundle->ip_version,
                                           &list_index_bundle);
	  netmgr_log_high( "Get filter_index_bundle:%d, list_index_bundle:%d for rule %d\n",
                       filter_index_bundle, list_index_bundle, filter_data_bundle->rule_id );

      if ( filter_index_bundle < 0 )
      {
        netmgr_log_high( "Fail to get filter_index_bundle \n" );
        netmgr_free(filter_data_bundle); /* Frist free filter_data_bundle */
      ...
      /* Select index for another rule since it's tcpdup bundle - end */

      /* Insert filter_data_bundle to list */
      netmgr_log_high( "Insert filter_data_bundle to list \n" );
      if ( (ds_dll_insert(netmgr_tc_cfg.filter_list,
                         NULL,
                         (void *)filter_data_bundle,
                         list_index)) == NULL )

      {
        netmgr_log_high( "Fail to insert filter_data_bundle to list \n" );
        netmgr_free(filter_data_bundle);
      ...
      /* Insert filter_data_bundle to list - end */
    }
    /* Process the 2nd rule of tcpudp bundle end */
    ...
    switch( filter_data->ip_version )
    {
      case NETMGR_QOS_IP_VERSION_4:
        netmgr_log_high( "Create v4 rule to with index %d \n", filter_index + 1 );
        ...
        if (filter_data->tcpudp_bundle != NETMGR_TC_TCPUDP_BUNDLE_NONE)
        {
          netmgr_log_high( "Create v4 bundle rule to with index %d \n", filter_index_bundle + 1 );
          result = netmgr_tc_create_iptables_rule_v4( link,flow_info->qos_flow.flow_id,
                                             filter_index_bundle + 1,
                                             &flow_info->qos_flow.filter_list[i],
                                             filter_data_bundle->tcpudp_bundle );
        }

        break;
      case NETMGR_QOS_IP_VERSION_6:
        netmgr_log_high( "Create v6 rule to with index %d \n", filter_index + 1 );
        ...
        if (filter_data->tcpudp_bundle != NETMGR_TC_TCPUDP_BUNDLE_NONE)
        {
          netmgr_log_high( "Create v6 bundle rule to with index %d \n", filter_index_bundle + 1 );
          result = netmgr_tc_create_iptables_rule_v6( link,flow_info->qos_flow.flow_id,
                                             filter_index_bundle + 1,
                                             &flow_info->qos_flow.filter_list[i],
                                             filter_data_bundle->tcpudp_bundle );
    ...
    {
      netmgr_log_high( "Fail to create rule, rollback... \n" );
      netmgr_tc_delete_node_from_tc_list(filter_data);
      netmgr_free(filter_data);

      if (filter_data->tcpudp_bundle != NETMGR_TC_TCPUDP_BUNDLE_NONE)
      {
        netmgr_log_high( "Rollback bundle rule \n" );
        netmgr_tc_delete_node_from_tc_list(filter_data_bundle);
        netmgr_free(filter_data_bundle);
      }
