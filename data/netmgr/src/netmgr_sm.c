...
)
{
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link) )
  {
    ...
    {
      if(NETMGR_IS_REV_IP_LOCAL_BRK_OUT_LINK(cmd_buf->data.link))
      {
            /* Need to update repost_count as below check for repost_count will
             * make sm move to wrong state and then follow wrong handling of
             * scenario.*/
            cmd_buf->repost_count = cmd_buf->repost_count + 1;
            netmgr_log_high("Reposting connect event for LBO on link[%d] repost_count[%d]\n",
                             cmd_buf->data.link,
            ...
            new_cmd->repost_count = cmd_buf->repost_count;
            ...
            /*Since we have passed reference of addr_info_ptr its better to
            * let it handle in SM and get freed rather than below.*/
            goto out;
      }
