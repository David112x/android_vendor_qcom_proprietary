...
)
{
  ...
  type = ds_get_image_type();

  if ((netmgr_main_cfg.netdev_budget > 0) ||
      (type == DS_IMAGE_TYPE_DEBUG && netmgr_main_cfg.debug_netdev_budget > 0))
  {
   /* If debug is the image type and debug limit has been specified  use debug, fallback to perf limit*/
   temp = (type == DS_IMAGE_TYPE_DEBUG && netmgr_main_cfg.debug_netdev_budget > 0)?
                                netmgr_main_cfg.debug_netdev_budget: netmgr_main_cfg.netdev_budget;

    netmgr_log_low("%s(): Setting netdev_budget to %d\n",
                   __func__, temp);

    if (netmgr_kif_set_netdev_budget(temp) != NETMGR_SUCCESS)
    {
  ...
  if ((netmgr_main_cfg.netdev_max_backlog > 0) ||
      (type == DS_IMAGE_TYPE_DEBUG && netmgr_main_cfg.debug_netdev_max_backlog > 0))
  {
   /* If debug is the image type and debug limit has been specified  use debug, fallback to perf limit*/
   temp = (type == DS_IMAGE_TYPE_DEBUG && netmgr_main_cfg.debug_netdev_max_backlog > 0)?
                                netmgr_main_cfg.debug_netdev_max_backlog: netmgr_main_cfg.netdev_max_backlog;

    netmgr_log_low("%s(): Setting netdev_max_backlog to %d\n",
                   __func__, temp);


    if (netmgr_kif_set_netdev_max_backlog(temp) != NETMGR_SUCCESS)
    {
  ...
  for (link = 0; link < netmgr_main_cfg.max_links_in_total; link++)
  {
    ...
    if (flags & IFF_UP) {
      ...
      else
      {
        ...
        event_info->cmd_data.ignore_subs = 1;
        event_info->param_mask |= NETMGR_EVT_PARAM_CMD_DATA;
