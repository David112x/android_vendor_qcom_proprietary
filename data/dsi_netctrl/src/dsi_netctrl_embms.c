...
/*===========================================================================
  FUNCTION:  dsi_embms_get_tuneaway_info
===========================================================================*/
/*!
    @brief
...
*/
/*=========================================================================*/
int dsi_embms_get_tuneaway_info
(
  dsi_hndl_t                   hndl,
  unsigned int                 txn_id
)
{
  ...
  do
  {
    ...
    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR("%s: invalid arg, st_hndl [%p]" ,
                     __func__,
                     (unsigned int*)st_hndl);
      break;
    }

    DSI_LOG_DEBUG( "%s: st_hndl [%p] txn_id:[%d]", (unsigned int*)st_hndl, txn_id );

    ret = dsi_mni_vtbl.mni_embms_get_tuneaway_info_f(st_hndl, txn_id);

    if (DSI_SUCCESS != ret)
    {
      DSI_LOG_ERROR("dsi_mni_vtbl.mni_embms_get_tuneaway_info [%p] returned err",
                    (unsigned int*)dsi_mni_vtbl.mni_embms_get_tuneaway_info_f);
      break;
    }
    ret = DSI_SUCCESS;
  } while(0);
