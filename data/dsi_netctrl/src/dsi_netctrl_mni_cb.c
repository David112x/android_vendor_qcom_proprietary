...
/*===========================================================================
  FUNCTION:  dsi_handle_embms_tuneaway_info_ind
===========================================================================*/
/*!
    @brief
    handles embms tuneaway info indications
...
*/
/*=========================================================================*/
static int dsi_handle_embms_tuneaway_info_ind
(
  qmi_client_type                wds_hndl,
  qmi_service_id_type            sid,
  void                          *user_data,
...
)
{
  ...
  int                count;
  boolean            handle_found = FALSE;
  DSI_LOG_VERBOSE("%s: ENTRY", __func__);

  memset(&payload, 0, sizeof(payload));

  do
  {
    event = DSI_NET_TUNEAWAY_INFO;

    DSI_LOG_DEBUG("received tuneaway_info_ind with wds_hndl [%p] "
                  "sid [%d] user_data [%p] ind_data [%p]",
                  wds_hndl, sid, user_data, ind_data);

    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("%s", "NULL ind data received in tuneaway_info_ind");
      break;
    }
    ...
    {
      DSI_LOG_ERROR("invalid user data (dsi iface id) "\
                    "%d received in tuneaway_info_ind", i);
      break;
    }
    ...
    if(ind_data->tuneaway_info.tx_id)
    {
      DSI_LOG_DEBUG("tuneaway_info_ind tx_id:[%d]",
                     ind_data->tuneaway_info.tx_id);
      DSI_L2S_FORMAT_MSG(DSI_L2S_INVALID_IFACE,"TUNEAWAY_INFO_IND tx_id=%d",
                         ind_data->tuneaway_info.tx_id);
    }

    memcpy(&payload.embms_info.embms_tuneaway_info,
           &ind_data->tuneaway_info,
           sizeof(payload.embms_info.embms_tuneaway_info));
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("%s: EXIT with suc", __func__);
  }
  else
  {
    DSI_LOG_VERBOSE("%s: EXIT with err", __func__);
  }
  return ret;
}
/*===========================================================================
...
)
{
  ...
  do
  {
    ...
    case QMI_WDS_EMBMS_TUNEAWAY_INFO_IND_V01:
      reti = dsi_handle_embms_tuneaway_info_ind(wds_hndl,
                                          sid,
                                          user_data,
                                          ind_data);
      break;
