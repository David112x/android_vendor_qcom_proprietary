    ...
    do
    {
      ...
      else
      {
        ...
        switch((int)soc_id)
        {
          ...
          case DS_TARGET_SOC_ID_TRINKET_IOT:
          case DS_TARGET_SOC_ID_BENGAL_IOT:
          case DS_TARGET_SOC_ID_SCUBA_IOT:
...
ds_image_type_t ds_get_image_type
(
  void
)
{
  struct utsname n;
  ds_image_type_t image_s = DS_IMAGE_TYPE_INVALID;

  memset(&n, 0, sizeof(n));
  if (uname(&n))
  {
    ds_log_err("%s(): failed to get uname\n", __func__);
    return DS_IMAGE_TYPE_INVALID;
  }

  ds_log_high("%s() %s %s %s %s %s\n", __func__, n.sysname, n.nodename,
                  n.release, n.version, n.machine);

  if (strstr(n.release, "perf+") != NULL)
  {
    return DS_IMAGE_TYPE_PERF;
  }

  return DS_IMAGE_TYPE_DEBUG;
};
