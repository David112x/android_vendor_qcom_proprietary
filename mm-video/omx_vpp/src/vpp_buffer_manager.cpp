...
bool VppBufferManager:: ResetBufferRequirements(OMX_PARAM_PORTDEFINITIONTYPE* portDef) {
    ...
    if (mOmxActBufCnt > mVppFtbCount) {
        portDef->nBufferCountActual = mOmxActBufCnt - mVppFtbCount;
    } else {
        portDef->nBufferCountActual = mOmxActBufCnt;
    }

    if (portDef->nBufferCountActual < mOmxMinBufCnt) {
        portDef->nBufferCountActual = mOmxMinBufCnt;
    }

    DEBUG_PRINT_LOW("Reset buffer count min:%u, actual:%u",
...
static uint32_t getVppResolution(uint32_t width, uint32_t height)
{
    ...
    uint32_t u32Width = width;
    uint32_t u32Height = height;

    if (height > width)
    {
        // Rotated video, use larger dimension as width.
        u32Width = height;
        u32Height = width;
    }

    if (u32Width <= RES_SD_MAX_W && u32Height <= RES_SD_MAX_H)
        u32VppRes = VPP_RESOLUTION_SD;

    else if (u32Width <= RES_HD_MAX_W && u32Height <= RES_HD_MAX_H)
        u32VppRes = VPP_RESOLUTION_HD;

    else if (u32Width <= RES_FHD_MAX_W && u32Height <= RES_FHD_MAX_H)
        u32VppRes = VPP_RESOLUTION_FHD;

    else if (u32Width <= RES_UHD_MAX_W && u32Height <= RES_UHD_MAX_H)
                                                ...
                                                bool isClientSet)
{
    ...
    if (isClientSet) {
        ...
        if (mOmxActBufCnt >= portDef->nBufferCountMin) {
            mOmxClientBufCnt = mOmxActBufCnt - portDef->nBufferCountMin;
        }
        DEBUG_PRINT_LOW("client set counts={min: %d, act: %d} client buf=%d",
                        portDef->nBufferCountMin, portDef->nBufferCountActual, mOmxClientBufCnt);
    ...
    DEBUG_PRINT_LOW("counts: original={min: %d, actual: %d} new={min: %d, actual: %d} client buf=%d",
...
void VppBufferManager::AssignBufferToPool(OmxVppBuffer &buf) {
    ...
    if (buf.pool != OMX_POOL_DECODER && buf.pool != OMX_POOL_VPP) {
        ...
        // feed the decoder pool to meet the minimum decoder required buffers at
        // the beginning. Once it hits the minimum number, bump up the buffer count
        // with the client's owned buffers. This logic is to cover dynamic bypass
        // mode and SSM use case.
        uint32_t decMinCnt = mOmxMinBufCnt;
        if (mDecoderPoolCnt >= mOmxMinBufCnt) {
            decMinCnt = mOmxMinBufCnt + mOmxClientBufCnt;
        }

        bool isVppFull = mVppPoolCnt >= mVppFtbCount;
        bool isDecoderFull = mDecoderPoolCnt >= decMinCnt;
        ...
        if (mComponent.mFlushing) {
            ...
            // split between them
            uint32_t dec_ex = mDecoderPoolCnt - decMinCnt;
            ...
            if (decMinCnt < 7) {
                u32Num = 1;
                u32Den = 2;
            } else if (decMinCnt < 10) {
                u32Num = 2;
                u32Den = 3;
            } else if (decMinCnt < 21) {
                u32Num = 5;
                u32Den = 6;
            } else {
                u32Num = 9;
            ...
            } else if (mDecoderPoolCnt < ((decMinCnt * u32Num) / u32Den)) {
            ...
            } else if (!cnts.vppIn && (mDecoderPoolCnt < (decMinCnt / 2))) {
        ...
        DEBUG_PRINT_LOW("assigned buf=%p to %s, pools={vdec=%d/%d, vpp=%d/%d} (%s)",
                        buf.omxHeader, buf.getPoolStr(), mDecoderPoolCnt, decMinCnt,
...
void VppBufferManager::PrepareBufferForVdecOut(OmxVppBuffer &buf)
{
    ...
    buf.owner = OMX_VDEC;
    hdr->nAllocLen = mMetaBufMode ?
        sizeof(struct VideoDecoderOutputMetaData) : hdr->nAllocLen;
    hdr->nFilledLen = 0;
    hdr->nTimeStamp = 0;
}
                                              ...
                                              bool translateParams)
{
    ...
    omx->nFilledLen = 0;
    omx->nTimeStamp = 0;
    omx->nAllocLen = mMetaBufMode ?
        sizeof(struct VideoDecoderOutputMetaData) : omx->nAllocLen;
                                                                        ...
                                                                        bool bypass)
{
    ...
    if (bypass || mComponent.mFlushing || mComponent.mDecoderReconfig) {
        omxVppBuffer->owner = OMX_CLIENT;
        DEBUG_PRINT_LOW("FbdDest: buffer: %p, to: client, flushing=%u, reconfig=%u, bypass=%u",
                        buffer, mComponent.mFlushing, mComponent.mDecoderReconfig, bypass);
...
VppBufferManager::OmxVppBuffer* VppBufferManager::GetVppObdDestination(OMX_BUFFERHEADERTYPE* omxHdr)
{
    ...
    if (mComponent.mDeferredFlush) {
        BufferCounts cnts;
        GetBufferCounts(cnts);
        if (cnts.vppIn > 0) {
            DEBUG_PRINT_LOW("resend the buffer to vpp out");
            PrepareBufferForVppOut(*omxVppBuffer);
        } else {
            DEBUG_PRINT_LOW("clear buffer and send to client");
            PrepareBufferForClient(*omxVppBuffer, false);
        }
        return omxVppBuffer;
    }
    ...
    if (omxVppBuffer->pool == OMX_POOL_VPP &&
        mDecoderPoolCnt < (mOmxMinBufCnt + mOmxClientBufCnt))
        UnassignBufferFromPool(*omxVppBuffer);
#endif
...
void VppBufferManager::SetBufferMode(bool mode) {
    DEBUG_PRINT_INFO("set meta buffer mode: %s", mode ? "True" : "False");
    mMetaBufMode = mode;
}
...
void VppBufferManager:: ConvertOmxVppFlags(uint32_t inflags, uint32_t& outflags,bool fromOmxToVpp) {
    ...
    uint32_t ctzOmxBFrame = __builtin_ctz(QOMX_VIDEO_BUFFERFLAG_BFRAME);
    ...
    uint32_t ctzVppBFrame = __builtin_ctz(VPP_BUFFER_FLAG_B_FRAME);
    ...
    if (fromOmxToVpp) {
        outflags = (((inflags & OMX_BUFFERFLAG_EOS) >> ctzOmxEosFlag) << ctzVppEosFlag ) |
                ...
                (((inflags & QOMX_VIDEO_BUFFERFLAG_BFRAME) >> ctzOmxBFrame) << ctzVppBFrame);

        if ((inflags & (OMX_BUFFERFLAG_SYNCFRAME | QOMX_VIDEO_BUFFERFLAG_BFRAME)) == 0) {
            outflags |= VPP_BUFFER_FLAG_P_FRAME;
        }
        ...
        outflags = (((inflags & VPP_BUFFER_FLAG_EOS) >> ctzVppEosFlag) << ctzOmxEosFlag) |
                ...
                (((inflags & VPP_BUFFER_FLAG_B_FRAME) >> ctzVppBFrame) << ctzOmxBFrame);

        if (inflags & VPP_BUFFER_FLAG_P_FRAME) {
            outflags &= ~(OMX_BUFFERFLAG_SYNCFRAME | QOMX_VIDEO_BUFFERFLAG_BFRAME);
        }
    }
    ...
    if (mType == EXTRADATA_INTERNAL) {
        ...
        for (uint32_t i = 0; i < mCount; i++) {
            ...
            OMX_ERRORTYPE err = mCmp->use_buffer(mCmp->mOmxHandle, &header,
                    VppOmxComponent::port_indexes::OMX_CORE_OUTPUT_EXTRADATA_INDEX,
                    mCmp->mAppData, mSize, buf);
...
void VppBufferManager::OmxVppExtraData::clear() {
    if (mType == EXTRADATA_INTERNAL) {
        ...
        for (uint32_t i = 0; i < mCacheBuffers.size(); i++) {
            OMX_ERRORTYPE err = mCmp->free_buffer(mCmp->mOmxHandle,
                    VppOmxComponent::port_indexes::OMX_CORE_OUTPUT_EXTRADATA_INDEX,
