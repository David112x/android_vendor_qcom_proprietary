...
private:
    ...
    bool getConfigStoreBool(const char *pcArea,
                            const char *pcConfig,
                            bool defaultValue);
    bool isFRCSmoothMode(uint64_t bufTs);
    void enableFRCBypassMode();
private:
    ...
    struct vid_prop_non_realtime mNonRealtime;

    bool mIsQCSoftwareDecoder;
    uint32_t mPixelSize;
    bool mIsDiscardFrameMode;
    uint32_t mConsecutiveRenderFrameNum;
    uint32_t mBypassFrameNum;
    uint32_t mLastAccessedFrcSegmentsIndex;
    std::atomic<bool> mDeferredFlush;
    ...
    sp<ICapabilityConfigStore> mConfigStore;
};
