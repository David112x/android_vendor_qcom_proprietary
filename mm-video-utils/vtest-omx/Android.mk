...
TARGETS_THAT_DONT_NEED_SW_VENC :=
TAEGWTS_THAT_DONT_NEED_SW_VDEC :=
...
ifneq ($(LINUX_OMX_TEST_ONLY_ENCODE), true)
vtest-inc        += $(TOP)/system/core/libion/include
vtest-inc        += $(TOP)/system/core/libion/kernel-headers
...
ifeq ($(call is-board-platform-in-list, $(TARGETS_THAT_DONT_NEED_SW_VENC)),true)
vtest-def        += -DDISABLE_SW_VENC
endif

ifeq ($(call is-board-platform-in-list, $(TARGETS_THAT_DONT_NEED_SW_VDEC)),true)
vtest-def        += -DDISABLE_SW_VDEC
endif
...
#Use the below two macros to disable native window sink
vtest-def        += -DNATIVE_WINDOW_SINK_DISABLE
NATIVE_WINDOW_SINK_DISABLE := true
...
ifeq ($(TARGET_ENABLE_VIDC_TESTAPP_INTSAN), true)
...
LOCAL_SHARED_LIBRARIES  := libOmxCore libbinder libcutils libfastcrc liblog libvideoutils
LOCAL_SHARED_LIBRARIES  += libdl libhidltransport libgui_vendor libui libutils libqdMetaData libxml2 libstreamparser libion
LOCAL_HEADER_LIBRARIES  := libmmvideo_proprietary_headers
...
ifneq ($(NATIVE_WINDOW_SINK_DISABLE), true)
LOCAL_SRC_FILES         += \
