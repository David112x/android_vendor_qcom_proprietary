...
OMX_ERRORTYPE XmlParser::ProcessMasterConfig(VideoStaticProperties* pGlobalVideoProp)
{
    ...
    while (pElem)
    {
        ...
        if (!STRCMP((OMX_STRING)pElem->name, "text")) {
            pElem = pElem->next;
            continue;
        } else if (!pElem->children || !pElem->children->content) {
            VTEST_MSG_ERROR("MasterConfig tag %s cannot be empty",(OMX_STRING)pElem->name);
            return OMX_ErrorBadParameter;
        } else if (!STRCMP((OMX_STRING)pElem->name, "InputFileRoot")) {
        ...
        } else if (!STRCMP((OMX_STRING)pElem->name, "TestAppLogMask")) {
            sscanf((OMX_STRING)pElem->children->content, "0x%x", &pGlobalVideoProp->logMask );
            VTEST_MSG_LOW("MasterXml: TestAppLogMask: 0x%x", pGlobalVideoProp->logMask);
