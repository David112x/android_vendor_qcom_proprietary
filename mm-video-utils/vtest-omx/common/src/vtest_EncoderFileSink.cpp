...
EncoderFileSink::~EncoderFileSink() {
    ...
    if (m_pStreamWriter != NULL) {
        m_pStreamWriter->Close(m_pStreamWriter);
...
OMX_ERRORTYPE EncoderFileSink::Configure(CodecConfigType *pConfig,
    ...
    //for rotation, swap the non-rotated width and height to rotated values
    //so as to write correct resolution in ivf header
    if(pConfig->nRotation == 90 || pConfig->nRotation == 270)
    {
        m_nWidth = pConfig->nOutputFrameHeight;
        m_nHeight = pConfig->nOutputFrameWidth;
        VTEST_MSG_HIGH("EncoderFileSink: Output width and height as %dx%d", m_nWidth, m_nHeight);
    }
    m_nFrames = pConfig->nFrames;
    pConfig->bProfileMode = m_bIsProfileMode;
    m_bSecureSession = pConfig->bSecureSession;

    //using start codes by defualt
    //for HEVC/AVC VBR cases using 4-byte NAL length by defualt
    m_nNalHeaderLength = 0;
    if(pConfig->eCodec == OMX_VIDEO_CodingHEVC || pConfig->eCodec == OMX_VIDEO_CodingAVC)
    {
        if(pConfig->eControlRate == OMX_Video_ControlRateVariable || pConfig->eControlRate == OMX_Video_ControlRateVariableSkipFrames)
        {
            m_nNalHeaderLength = 4;
        }
    }
    //for any specific config, using the set value
    if(pConfig->nNALHeaderLength != DEADVALUE && pConfig->nNALHeaderLength >= 0)
        m_nNalHeaderLength = pConfig->nNALHeaderLength;
    ...
    if (sFileName[0] != '\0') {
        ...
        if (m_pFile != NULL) {
            ...
            //Using the shared raw stream writer with beat
            //The new writer Will replace NAL length back to start codes when 4-byte NAL Header Length set
            m_pStreamWriter = (VideoStreamWriter*)malloc(sizeof(VideoStreamWriter));
            if (!m_pStreamWriter)
            {
                VTEST_MSG_ERROR("Memory allocation failed for VideoStreamWriter!! ");
                return OMX_ErrorUndefined;
            }
            ret = VideoStreamWriterInit(m_pStreamWriter, m_nNalHeaderLength, m_bSecureSession);
            if (ret)
            {
                VTEST_MSG_ERROR("VideoStreamWriterInit failed with %d!!",ret);
                return OMX_ErrorUndefined;
            }
        } else {
...
OMX_ERRORTYPE EncoderFileSink::writeFrame(OMX_BUFFERHEADERTYPE *pHeader) {
    ...
    OMX_ERRORTYPE result = OMX_ErrorNone;
    VidcStatus ret = VidcStatusSuccess;
    ParserFrameInfoType frm = { .pBuffer = NULL };
    OMX_S32 nBytes(0);
...
#ifdef USE_GBM
        struct gbm_bo *handle = buffer->meta_handle;
#else
...
#ifdef USE_GBM
        frm.nAllocLen = handle->size;
        frm.nIonfd = handle->ion_fd;
        frm.nFilledLen = handle->size;
#else
        frm.nAllocLen = handle->data[3];
        frm.nIonfd = handle->data[0];
        frm.nFilledLen = handle->data[2];
#endif
    }
    else
    {
        frm.pBuffer = pHeader->pBuffer + pHeader->nOffset;
        frm.nFilledLen = pHeader->nFilledLen;
        frm.nAllocLen = pHeader->nAllocLen;
    }

    VTEST_MSG_HIGH("StreamWriter write to file!");
    ret = m_pStreamWriter->bitstreamWriter(m_pStreamWriter, m_pFile->GetFile(), &frm);
    if (ret)
    {
