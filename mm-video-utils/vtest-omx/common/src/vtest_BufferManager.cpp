...
BufferManager::~BufferManager() {
    ...
    for (int i = m_pBufferPools.size(); i > 0; i--) {

        BufferPool *pBufferPool = m_pBufferPools[i - 1];
...
OMX_ERRORTYPE BufferManager::SetupBufferPool(ISource *pNodeSrc, ISource *pNodeSink) {
    ...
    //make the buffer count a even number so as to avoid back-to-back cases in the skip reading frame mode
    if(nBufferCount % 2 != 0) {
        nBufferCount = nBufferCount + 1;
        VTEST_MSG_HIGH("Add buffer count to an even number %d to avoid back-to-back cases in the skip reading frame mode", nBufferCount);
    }
