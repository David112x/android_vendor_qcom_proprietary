...
OMX_ERRORTYPE MdpOverlaySink::OverlayEnqueueBuffer(struct OMX_BUFFERHEADERTYPE *pBufHdr) {
    ...
    if (m_bRotate) {
        ...
        s_OvFront.dst_data.memory_id = m_nDataFd;
...
OMX_ERRORTYPE MdpOverlaySink::AllocateRotatorBuffer() {
    ...
    ionAllocData.heap_id_mask = ION_HEAP(ION_SYSTEM_HEAP_ID);
    ionAllocData.flags = 0;
    if (m_bSecureSession) {
        ionAllocData.flags |= ION_FLAG_SECURE;
        ionAllocData.heap_id_mask = ION_HEAP(ION_CP_MM_HEAP_ID);
    }

    if (ion_alloc_fd(m_nIonFd, ionAllocData.len, 0,
                     ionAllocData.heap_id_mask,
                     ionAllocData.flags, &m_nDataFd)) {
