...
NativeWindow::NativeWindow(OMX_BOOL bRotate,
    ...
    sp<IBinder> primaryDisplay = m_pSurfaceComposer->getInternalDisplayToken();
    ...
    if (m_bShowVideo) {
        ...
        t.setLayer(m_pControl,100000 + 1);
    } else {
        //Don't show
        t.setLayer(m_pControl, -1);
    }
    //show-up background below video
    t.setLayer(m_pBackgroundControl, 100000);
    t.show(m_pBackgroundControl).show(m_pControl).apply();
...
OMX_ERRORTYPE NativeWindow::Configure(CodecConfigType *pConfig,
    ...
    {
#if defined (PRE_SDM845) || ! defined (DISABLE_SW_VDEC)
...
OMX_ERRORTYPE NativeWindow::AllocateBuffers(BufferInfo **pBuffers,
    ...
    SurfaceComposerClient::Transaction t;
    t.setPosition(m_pControl,x, y);
    t.setSize(m_pControl, w, h);
    t.setLayer(m_pControl, 0x7FFFFFF).show(m_pControl).apply();
    ...
    for (OMX_U32 i = 0; i < iterator; i++) {
        ...
        if (err != 0) {
            VTEST_MSG_ERROR("dequeueBuffer failed: %s (%d)",
                    strerror(-err), -err);
            return OMX_ErrorBadParameter; //TBD: cancel allocated ones
        }
