...
struct CodecConfigType {
    ...
    char cCvpMetaFileName[VTEST_MAX_STRING];    // Config File Key: CVPMetaFileName
    ...
    OMX_U32 cancel_flag;
    //Config File Key: VUITimingInfo
    OMX_S32 nVUITimingInfo;
    ...
    //======== HEIF GridEnable
    OMX_BOOL bGridEnable;

    //======== ConcealColor
    OMX_U32 nConcealColor8bit;                     // Config File Key: ConcealColor8bit
    ...
    OMX_S32 nNALHeaderLength;
    char cMetadataFile[VTEST_MAX_STRING];          //HDR10+ metadata file

    uint32   nColorSpace;                          // Encoder color info configs
    uint32   nVideoFullRange;
    uint32   nTransferChar;
    uint32   nMatrixCoeff;
    uint32   nCSC;
};
...
#ifdef USE_GBM
typedef struct gbm_bo* BufferHandle;
#else
...
static ConfigEnum g_pXmlOmxCodecMap[] =
{
    ...
    { (OMX_STRING)"VIDEO_CodingVC1RCV",   OMX_VIDEO_CodingWMV },
    ...
    { (OMX_STRING)"VIDEO_CodingImageHEIC",  QOMX_VIDEO_CodingImageHeic },
    { 0, 0 }
};
...
static ConfigEnum g_pColorFormat[] =
{
    ...
    { (OMX_STRING)"NV12_512",              QOMX_COLOR_FORMATYUV420PackedSemiPlanar32m },
    { 0, 0 }
};
...
static ConfigEnum g_pHeaderMode[] =
{
    ...
    { (OMX_STRING)"Separate",          V4L2_MPEG_MSM_VIDC_DISABLE },
    { (OMX_STRING)"EverySyncFame",     V4L2_MPEG_MSM_VIDC_ENABLE },
    { 0, 0 }
};
static inline OMX_U32 GetFrameSize(OMX_U32 nFormat,
    ...
    switch (nFormat) {
        ...
        case QOMX_COLOR_FORMATYUV420PackedSemiPlanar32mCompressed:
            return VENUS_BUFFER_SIZE(COLOR_FMT_NV12_UBWC, nWidth, nHeight);
