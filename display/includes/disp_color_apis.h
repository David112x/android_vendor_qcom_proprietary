...
enum DISP_MULTI_FEATURE_ID {
  MULTI_FEATURE_INVALID = 0,
  MULTI_FEATURE_PCC_CONFIG = 1,
  MULTI_FEATURE_MAX = 0xFF
};
...
typedef enum {
  COLOR_LEFT_CONFIG = 0,
  COLOR_RIGHT_CONFIG = 1,
  COLOR_MAX_CONFIG = 2
} multi_disp_color_config;
...
/*=========================================================================
FUNCTION
  disp_api_set_multi_feature_config()

DESCRIPTION
  This common API sets the multiple DSPP features with respective config data.

  hctx       -- Context handle.
  disp_id    -- display ID type
  feature_id -- feature ID type
  enable     -- Dtate of mode : 0 = disable, 1 = enable
  data_size  -- feature config data size
  data       -- feature config data

RETURN VALUE
  ZERO (SUCCESS)- success.
  Negative Value - error\failure
=========================================================================*/
int32_t disp_api_set_multi_feature_config(DISPAPI_HANDLE hctx, uint32_t disp_id,
