...
static int open_db()
{
    ...
    if( db_conn == NULL )
    {
        QLOGE("unable to open database at this path=\"%s\" with sql err_msg=%s"
                     , _dbpath, sqlite3_errmsg(db_conn));
    ...
    if ( rc != SQLITE_OK ) {
        if (db_conn != NULL) {
            sqlite3_close_v2(db_conn);// Close the DB in case if its open with False return code.
            db_conn = NULL;
        }
        QLOGE("unable to open database at this path=\"%s\" with sql ret code=%d.", _dbpath, rc);
        return 0;
    }
...
static int close_database()
{
  ...
  int rc = sqlite3_close_v2(db_conn);

  db_conn = NULL;// _v2() will make sure and the deallocate the DB later on if its BUSY. So not checking for its return code.
