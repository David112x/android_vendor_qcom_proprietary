...
private:
    ...
    public:
        explicit BoostConfigInfo(int idnum, int type, bool enable, int timeout, int fps, char *tname, int res, char *resourcesPtr);
        int mId;
        int mType;
        bool mEnable;
        int mTimeout;
...
public:
    ...
    int GetBoostConfig(int hintId, int type, int *mapArray, int *timeout,
                       char *tName = NULL, int res = MAP_RES_TYPE_ANY, int fps = 0);
