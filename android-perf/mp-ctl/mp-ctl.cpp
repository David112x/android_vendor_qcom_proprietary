...
float getCurrentFps() {
    ...
    FILE *fpsVal;
    char buf[NODE_MAX];
    IDisplayConfig::DisplayAttributes dpy_attr = {};
    sp<IDisplayConfig> client = IDisplayConfig::getService();

    if (client != NULL) {
        client->getActiveConfig(IDisplayConfig::DisplayType::DISPLAY_PRIMARY,
        ...
        client->getDisplayAttributes(config, IDisplayConfig::DisplayType::DISPLAY_PRIMARY,
            [&](const auto &tmpError, const auto &tmpAttributes) {
            if (tmpError) {
                err = tmpError;
                return;
            }
            dpy_attr.vsyncPeriod = tmpAttributes.vsyncPeriod;
            });
        if(dpy_attr.vsyncPeriod) {
            QLOGI("VSYNC : %d", dpy_attr.vsyncPeriod);
            fps = NSEC_TO_SEC/dpy_attr.vsyncPeriod;
        }
    }
    QLOGI("Panel Attributes vsyncPeriod : %d Fps : %f",
            dpy_attr.vsyncPeriod, fps);
...
static void *mpctl_server(void *data) {
    ...
    Init();
    getCurrentFps();
    //set the property to 1 to indicate mp-ctl Init() is complete
    property_set(TARGET_INIT_COMPLETE,"1");
...
void perfmodule_sync_request_ext(int cmd, char **debugInfo) {
    switch (cmd) {
                ...
                if (debuggable) {
                    ...
                    if(debugInfo != NULL) {
                        *debugInfo = (char *)malloc((size+1) * sizeof(char));
                        if (!(*debugInfo)) {
                           QLOGE("malloc failed for debugInfo.");
                            return;
                        }
