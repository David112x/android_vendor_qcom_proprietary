...
void LearningModule::lmMain() {
    int ret_code = 0;
    char targetInit_completed[PROPERTY_VALUE_MAX];
    int value = 0;

    DEBUGV(LOG_TAG_LM,"Checking boot complete ");
    do {
        if (property_get(TARGET_INIT_COMPLETE, targetInit_completed, "0")) {
            value = atoi(targetInit_completed);
        }
        if(!value)
            usleep(100000);
    } while(!value);
    DEBUGV(LOG_TAG_LM,"Target Init by mp-ctl completed");
