...
FeatureInfo FeatureXML::getFeatureInfo() {
    //fetching target name from mp-ctl
    ...
    if(mXMLRoot != NULL) {
        ...
        if(profiles != NULL) {
            for(XMLElement* currProfile = profiles->FirstChildElement("Profile"); currProfile != NULL; currProfile = currProfile->NextSiblingElement("Profile")) {
                ...
                if(tname and xmltarget)
                    if((strlen(tname) != strlen(xmltarget)) || (strncmp(tname, xmltarget, strlen(xmltarget))))
                        valid_target = false;
         ...
         *    </Configs>
         */
        for(XMLElement* configs = mXMLRoot->FirstChildElement("Configs"); configs != NULL; configs = configs->NextSiblingElement("Configs")) {

            bool valid_target = true;
            tinyxml2::XMLElement *e = configs->ToElement();
            const char *xmltarget = e->Attribute("target");
            if(tname and xmltarget)
                if((strlen(tname) != strlen(xmltarget)) || (strncmp(tname, xmltarget, strlen(xmltarget))))
                    valid_target = false;
