...
AHalValues_t* AudioHalExtLocal::getAHalConfigs() {
    ...
    if (inst->mConfigMap.find("auto_hal_enabled") != inst->mConfigMap.end()) {
        ahal_configs.auto_hal_enabled =
            toBool(inst->mConfigMap["auto_hal_enabled"].c_str(), false);
    }
