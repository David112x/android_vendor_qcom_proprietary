                                 ...
                                 char *mixer_path_xml)
{
    ...
    */
    if ((strstr(snd_card_name, "msm8939") ||
        ...
        strstr(snd_card_name, "sa6155-adp-star-snd-card") ||
        strstr(snd_card_name, "sa8155-adp-star-snd-card")) &&
...
__attribute__((unused)) static int platform_stdev_get_device_out_channels
(
   struct platform_data *my_data,
   st_profile_type_t profile_type
)
{
    ...
    int out_channels = SOUND_TRIGGER_CHANNEL_MODE_MONO;

    list_for_each_safe(p_node, temp_node, &stdev->adm_cfg_list) {
        cfg_info = node_to_item(p_node, struct adm_cfg_info, list_node);
        if (cfg_info->profile_type == profile_type) {
            out_channels = cfg_info->out_channels;
            break;
        }
    }
    ALOGD("%s: out channels %d", __func__, out_channels);

    return out_channels;
}

__attribute__((unused)) static int platform_stdev_get_device_bit_width
(
   struct platform_data *my_data,
   st_profile_type_t profile_type
)
{
    ...
    struct adm_cfg_info *cfg_info = NULL;
    /* default bit width */
    int bit_width = SOUND_TRIGGER_BIT_WIDTH;

    list_for_each_safe(p_node, temp_node, &stdev->adm_cfg_list) {
        cfg_info = node_to_item(p_node, struct adm_cfg_info, list_node);
        if (cfg_info->profile_type == profile_type) {
            bit_width = cfg_info->bit_width;
            break;
        }
    }
    ALOGD("%s: bit width %d", __func__, bit_width);
...
#ifdef EXT_HW_PLUGIN_ENABLED
int platform_stdev_ext_hw_plugin_usecase_start
(
    void *platform,
    void *plugin,
    struct st_vendor_info *v_info
)
{
    ...
    int sample_rate, bit_width, out_channels;
    int status = 0;

    sample_rate = platform_stdev_get_device_sample_rate(my_data, v_info->profile_type);
    bit_width = platform_stdev_get_device_bit_width(my_data, v_info->profile_type);
    out_channels = platform_stdev_get_device_out_channels(my_data, v_info->profile_type);

    if (my_plugin->audio_hal_plugin_send_msg) {
        audio_hal_plugin_codec_enable_t codec_enable;
        memset(&codec_enable, 0, sizeof(codec_enable));

        codec_enable.usecase = AUDIO_HAL_PLUGIN_USECASE_EC_CAPTURE;
        codec_enable.sample_rate = sample_rate;
        codec_enable.bit_width = bit_width;
        codec_enable.num_chs = out_channels;
        status = my_plugin->audio_hal_plugin_send_msg(AUDIO_HAL_PLUGIN_MSG_CODEC_ENABLE,
                                                      (void *)&codec_enable, 
                                                      sizeof(codec_enable));
        ALOGD("%s: enable audio hal plugin input, usecase(%d), sample_rate(%d), bit_width(%d), num_chs(%d)",
    ...
    if (status)
        ALOGD("%s: enable failed, status = %d",
              __func__, status);
    return status;
}

int platform_stdev_ext_hw_plugin_usecase_stop
(
    void *plugin
)
{
  ...
  if (my_plugin->audio_hal_plugin_send_msg) {
      ...
      memset(&codec_disable, 0, sizeof(codec_disable));

      codec_disable.usecase = AUDIO_HAL_PLUGIN_USECASE_EC_CAPTURE;
      status = my_plugin->audio_hal_plugin_send_msg(AUDIO_HAL_PLUGIN_MSG_CODEC_DISABLE,
                                                    (void *)&codec_disable,
                                                    sizeof(codec_disable));
      ALOGD("%s: disable usecase(%d)",
            __func__, (int)codec_disable.usecase);
    }

    if (status)
      ALOGD("%s: disable failed, status = %d",
            __func__, status);

    return status;
}

void* platform_stdev_ext_hw_plugin_init(struct sound_trigger_device *stdev)
{
    int32_t status = 0;
    ...
    if (my_plugin->plugin_handle == NULL) {
        ...
        DLSYM(my_plugin->plugin_handle, my_plugin->audio_hal_plugin_init, 
              audio_hal_plugin_init, status);
        if (my_plugin->audio_hal_plugin_init == NULL) {
            ALOGE("%s: Could not find the symbol audio_hal_plugin_init from %s",
                  __func__, LIB_PLUGIN_DRIVER);
            goto plugin_init_fail;
        }

        DLSYM(my_plugin->plugin_handle, my_plugin->audio_hal_plugin_deinit,
              audio_hal_plugin_deinit, status);
        if (my_plugin->audio_hal_plugin_deinit == NULL) {
            ALOGE("%s: Could not find the symbol audio_hal_plugin_deinit from %s",
                  __func__, LIB_PLUGIN_DRIVER);
            goto plugin_init_fail;
        }

        DLSYM(my_plugin->plugin_handle, my_plugin->audio_hal_plugin_send_msg,
              audio_hal_plugin_send_msg, status);
        if (my_plugin->audio_hal_plugin_send_msg == NULL) {
            ALOGE("%s: Could not find the symbol audio_hal_plugin_send_msg from %s",
                  __func__, LIB_PLUGIN_DRIVER);
            goto plugin_init_fail;
        }

        status = my_plugin->audio_hal_plugin_init();
        if (status) {
            ALOGE("%s: audio_hal_plugin_init failed with status = %d",
                  __func__, status);
            goto plugin_init_fail;
        }
    }
...
int platform_stdev_ext_hw_plugin_deinit(void *plugin)
{
    int status = 0;
    struct ext_hw_plugin_data *my_plugin = (struct ext_hw_plugin_data *)plugin;
    ...
    if (my_plugin->audio_hal_plugin_deinit) {
        status = my_plugin->audio_hal_plugin_deinit();
        if (status) {
            ALOGE("%s: audio_hal_plugin_deinit failed with status = %d",
                  __func__, status);
        }
    }
    if (my_plugin->plugin_handle != NULL) {
