    ...
    if (enable) {
        ...
        if (!status) {
            if (1 == ref_cnt) {
                ...
                ALOGV("%s: calling external audio_hal_plugin to enable Codec path", __func__);
                status = platform_stdev_ext_hw_plugin_usecase_start(p_ses->stdev->platform,
                                                                    p_ses->stdev->ext_hw_plugin,
                                                                    p_ses->vendor_uuid_info);

                if (status) {
                    ALOGE("%s: ERROR. enabling Codec path failed status %d",
                          __func__, status);
                }
            }
        } else {
        ...
        if (0 == ref_cnt) {
            if (p_ses->stdev->reset_backend || is_hwmad_device) {
                ...
                ALOGV("%s: calling external audio_hal_plugin to disable Codec path", __func__);
                status = platform_stdev_ext_hw_plugin_usecase_stop(p_ses->stdev->ext_hw_plugin);

                if (status) {
                    ALOGE("%s: ERROR. disabling Codec path failed status %d",
                          __func__, status);
                }
