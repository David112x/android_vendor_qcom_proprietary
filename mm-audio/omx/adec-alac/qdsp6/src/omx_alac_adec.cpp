...
bool COmxAlacDec::audio_register_ion(struct mmap_info *ion_buf)
{
    ...
    audio_ion_buf.fd = ion_buf->map_fd;
#else
...
bool COmxAlacDec::audio_deregister_ion(struct mmap_info *ion_buf)
{
    ...
    if(0 > ioctl(m_drv_fd, AUDIO_DEREGISTER_ION, &audio_ion_buf))
    {
        DEBUG_PRINT_ERROR("%s[%p]W-D, Buf, DEREG-ION,fd[%d] \
                ion-buf[%p]", __FUNCTION__, this,
#if TARGET_ION_ABI_VERSION >= 2
                ion_buf->map_fd,
#else
                ion_buf->ion_fd_data.fd,
#endif
                ion_buf->pBuffer);
        rc = false;
    }
...
void COmxAlacDec::process_out_port_msg(void *client_data, unsigned char id)
{
    ...
    unsigned char ident;
    unsigned     qsize  = 0;
    unsigned long long tot_qsize = 0;
    COmxAlacDec *pThis = (COmxAlacDec *) client_data;
    OMX_STATETYPE state;
...
OMX_ERRORTYPE COmxAlacDec::component_init(OMX_STRING role)
{
    ...
    if(ionfd < 0)
    {
        DEBUG_PRINT_ERROR("ion device open failed \n");
        return OMX_ErrorInsufficientResources;
    }
#if DUMPS_ENABLE
    bfd = open("/data/vendor/audio/bitstreamdump.dat", O_RDWR|O_CREAT, 0666);
    if (bfd < 0) {
        DEBUG_PRINT_ERROR("cannot open bitstreamdump file");
    }
    pfd = open("/data/vendor/audio/pcmdump.dat", O_RDWR|O_CREAT, 0666);
    if (pfd < 0) {
...
void* COmxAlacDec::alloc_ion_buffer(unsigned int bufsize)
{
...
#else
    heap_id_mask = ION_HEAP(ION_AUDIO_HEAP_ID);
#endif

#if TARGET_ION_ABI_VERSION >= 2
    rc = ion_alloc_fd(ionfd, ion_data->map_buf_size,
                      ION_ALLOC_ALIGN, heap_id_mask,
                      0, &ion_data->map_fd);
    if (rc < 0)
    {
        DEBUG_PRINT_ERROR("ION ALLOC failed\n");
        free(ion_data);
        ion_data = NULL;
        return ion_data;
    }
    ...
    ion_data->ion_alloc_data.heap_id_mask = heap_id_mask;
    ion_data->ion_alloc_data.flags = 0;
    rc = ioctl(ionfd,ION_IOC_ALLOC, &(ion_data->ion_alloc_data));
    if (rc < 0)
    {
...
void COmxAlacDec::create_timer_thread() {
    ...
    pthread_condattr_init(&attr);
    pthread_condattr_setclock(&attr, CLOCK_MONOTONIC);
    if((pthread_cond_init(&m_timer.loop_cond, &attr) != 0)
