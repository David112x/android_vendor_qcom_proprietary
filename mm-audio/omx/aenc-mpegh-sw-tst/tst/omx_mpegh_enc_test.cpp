                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if(((pBuffer->nFlags & OMX_BUFFERFLAG_EOS) == OMX_BUFFERFLAG_EOS)) {
         DEBUG_DETAIL("FBD::EOS on output port\n ");
         bOutputEosReached = true;
         return OMX_ErrorNone;
    }
    if(bInputEosReached_tunnel || bOutputEosReached) {
       DEBUG_DETAIL("EOS REACHED NO MORE PROCESSING OF BUFFERS\n");
       return OMX_ErrorNone;
    }
    ...
    while (num_of_frames > 0) {
        ...
        if(bytes_writen < len) {
           DEBUG_PRINT_ERROR("error: invalid MPEGH encoded data \n");
           return OMX_ErrorNone;
        }
                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if(!etb_done) {
       DEBUG_DETAIL("Wait till first set of buffers are given to component\n");
       etb_done++;
    ...
    if(bInputEosReached) {
       DEBUG_DETAIL("   EBD::EOS on input port\n ");
       return OMX_ErrorNone;
    } else if (bFlushing == true) {
       DEBUG_DETAIL("bFlushing is set to TRUE used_ip_buf_cnt=%d\n",used_ip_buf_cnt);
       if(used_ip_buf_cnt == 0) {
          bFlushing = false;
       } else {
          DEBUG_DETAIL("more buffer to come back used_ip_buf_cnt=%d\n",used_ip_buf_cnt);
        return OMX_ErrorNone;
      }
    }
    ...
    if((readBytes = Read_Buffer(pBuffer)) > 0) {
        ...
        OMX_EmptyThisBuffer(hComponent,pBuffer);
        DEBUG_DETAIL("EBD..Either EOS or Some Error while reading file\n");
    }
    return OMX_ErrorNone;
}
...
int main(int argc, char **argv)
{
    ...
    if((bInputEosReached_tunnel) || ((bOutputEosReached))) {
       ...
       /* No need to wait for state to change to Loaded
          No need to explicitly free up I/P and O/P buffers as
          OMX_FreeHandle will clean them
       */

       DEBUG_DETAIL("Releasing component handle");
       ...
       DEBUG_DETAIL("*****************************************\n");
       DEBUG_DETAIL("******...MPEGH ENC TEST COMPLETED...***************\n");
       DEBUG_DETAIL("*****************************************\n");
    }
    return 0;
}
...
void Release_Encoder(int)
{
    ...
    DEBUG_DETAIL("END OF MPEGH ENCODING: EXITING PLEASE WAIT\n");
...
int Init_Encoder(OMX_STRING audio_component)
{
    ...
    DEBUG_PRINT("mpegh_enc_test: Before entering OMX_GetComponentOfRole");
...
void etb_step( unsigned int input_buf_cnt, int Size,
    ...
    for (i = 0; i < input_buf_cnt; i++) {
         ...
         if (OMX_ErrorNone != ret) {
             DEBUG_PRINT_ERROR("OMX_EmptyThisBuffer failed with result %d\n", ret);
         } else {
    ...
    pthread_mutex_lock(&etb_lock);
    if (etb_done) {
        DEBUG_DETAIL("Component is waiting for EBD to be released.\n");
        etb_event_complete();
    } else {
        DEBUG_DETAIL("EBD not yet happened ...\n");
        etb_done++;
    }
    pthread_mutex_unlock(&etb_lock);
}
...
unsigned int ftb_step(unsigned int output_buf_cnt,
    ...
    *ret = OMX_FillThisBuffer(mpegh_enc_handle, pOutputBufHdrs[i]);
    if (OMX_ErrorNone != *ret) {
      DEBUG_PRINT_ERROR("OMX_FillThisBuffer failed with result %d\n", *ret);
    } else {
...
int Play_Encoder()
{
    ...
    etb_step( input_buf_cnt, Size, bInputEosReached, used_ip_buf_cnt, ret,
       pInputBufHdrs, mpegh_enc_handle);
...
static int open_audio_file ()
{
   ...
   if(parse_pcm_header() != 0x00) {
      DEBUG_PRINT_ERROR("PCM parser failed \n");
      return -1;
   }
...
static OMX_ERRORTYPE parse_pcm_header()
{
    ...
    if (hdr->coding != WAVE_FMT_IEEE) {
        DEBUG_DETAIL("Wav file is not IEEE 32-bit float format %d\n", hdr->coding);
    }

    DEBUG_DETAIL("Samplerate is %d\n", hdr->sampleRate);
    DEBUG_DETAIL("Channel Count is %d\n", hdr->numChannels);
    DEBUG_DETAIL("hoaorder is %d\n", objFileUtil_c._hoaOrder);
    DEBUG_DETAIL("Bitwidth is %d\n", hdr->bitsPerSample);
    DEBUG_DETAIL("\n***************************************************************\n");
