...
/* This field is used as the requested sched_priority value for the pthread
   which is to be created, from MM_Thread_CreateEx function above.
   Therefore, this should not be the Android priority values, but the linux
   scheduler priority values as defined in include/linux/sched/prio.h.

   MM_Thread_CreateEx function also sets the scheduling policy to SCHED_RR and
   performs sanity check and clipping of the priority value.
 */
const int MM_Thread_CallingThreadPriority = 0;
const int MM_Thread_BackgroundPriority    = 0;
const int MM_Thread_NormalPriority        = 0;
const int MM_Thread_DefaultPriority       = 0;
const int MM_Thread_HighPriority          = 10;
const int MM_Thread_ComponentPriority     = 25;
const int MM_Thread_RealtimePriority      = 50;
const int MM_Thread_ISTPriority           = 75;
const int MM_Thread_ISRPriority           = 85;
...
{
    UNUSED(handle);
    void* exitCode = (void*)((long)nExitCode);
