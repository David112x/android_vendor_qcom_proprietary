...
void rpcmem_init() {
   ...
   VERIFY_EPRINTF("rpcmem_init:open remote handle for sensorspd");
   sns_remote_handles remote_handles;
   if (0 != init_rpc_symbols(&remote_handles)) {
    VERIFY_EPRINTF("init_rpc_symbols failed");
    return ;
   }
   ...
   if (!remote_handle_fd) {
      if (remote_handle_open(ITRANSPORT_PREFIX "createstaticpd:sensorspd", &remote_handle_fd)){
         VERIFY_EPRINTF("rpcmem_init:Failed to open remote handle for sensorspd");
      } else {
         VERIFY_EPRINTF("rpcmem_init:opened remote handle for sensorspd");
         remote_handle_fd = 1;
      }
   } else {
      VERIFY_EPRINTF("rpcmem_init:using opened remote handle for sensorspd");
   }

   if(stat("/dev/ion",&st) == 0){
     rpcmem_flag=0;
     QList_Ctor(&lst);
   ...
   VERIFY_EPRINTF("stat Failed on /dev/ion ");
   rpcmem_flag=1;
}
...
void register_buf(void* buf, int size, int fd, int attr) {
   VERIFY_EPRINTF("rpcmem:edge:remote_register_buf: %p", remote_register_buf);
   VERIFY_EPRINTF("open remote handle for sensorspd");
   if (!remote_handle_fd) {
      if (remote_handle_open(ITRANSPORT_PREFIX "createstaticpd:sensorspd", &remote_handle_fd)){
         VERIFY_EPRINTF("Failed to open remote handle for sensorspd");
      } else {
         VERIFY_EPRINTF("opened remote handle for sensorspd");
         remote_handle_fd = 1;
      }
   } else {
      VERIFY_EPRINTF("using opened remote handle for sensorspd");
   }
...
void* rpcmem_alloc(int heapid, uint32 flags, int size) {
   ...
   if(heapid == RPCMEM_DEFAULT_HEAP) {
	   VERIFY_IPRINTF("rpcmem: allocating from contig heap\n");
	  ...
	  if(!po) {
         ...
         VERIFY(0 == (nErr = rpcmem_contig_alloc(m, heap_mask, rpc_flags, ion_flags, size, &po)));
      }
   } else {
