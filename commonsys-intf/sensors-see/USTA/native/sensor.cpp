...
usta_rc Sensor::send_attrib_request()
{
  ...
  ssc_attrib_client_obj->send_request(sns_request_encoded, false);
    ...
    size_t encoded_string_size)
{
  ...
  if(streaming_status_flag == true)
  {
    if(stream_event_cb)
    {
      ...
      if(data_type.compare("registry") == 0)
        is_registry_sensor = true;
         ...
         is_registry_sensor = false;
    string adaptedS=sensor_parser->get_event_json_string();
    adaptedS+=",\n \"Event Counter\" : "+to_string(event_count)+",\n";
    if(time_start_ct==1){
     time(&time_start);
     time_start_ct=0;
    }
    time_t time_end;
    time(&time_end);
    double dif=difftime(time_end, time_start);
    int time_elapsed=(int)dif;
    adaptedS+="\"Time Elapsed\" : "+to_string(time_elapsed)+"\n}";
    stream_event_cb(adaptedS,is_registry_sensor);
    ...
    string& in_logfile_name , string &usta_mode)
{
  ...
  ssc_stream_client_obj->send_request(main_request_encoded,false);
...
void Sensor::send_disable_requst()
{
  ...
  ssc_stream_client_obj->send_request(sns_request_encoded,false);
