...
usta_rc MessageParser::formulate_request_msg(send_req_msg_info& req_msg,string& request_string)
{
  ...
  if(message == NULL ) {
    ERROR_LOG(log_instance,  " failed to create profo buffer memory for message");
    ...
    const string& message,int index)
{
  ...
  for (int i = 0; i < desc->field_count(); i++)
  {
    ...
    if (fd->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE)
    {
      ...
      if(fd->message_type()->name().compare("sns_registry_data") == 0 || fd->message_type()->name().compare("item") == 0){
        DEBUG_LOG(log_instance , "Registry message");
        parse_request_msg_fields(nested_msgs_pool, fd->message_type()->name(), nested_msg_index++);
      }
      else{
        const Descriptor* desc_check = pool.FindMessageTypeByName(fd->message_type()->name());
        if(NULL != desc_check){
          parse_request_msg_fields(nested_msgs_pool, fd->message_type()->name(), nested_msg_index++);
        }
        else{
          parse_request_msg_fields(nested_msgs_pool, message + "." + fd->message_type()->name(), nested_msg_index++);
        }
      }
    }
    else
