...
void accel_cal::request_registry_write() {
  ...
  pb_req_msg.SerializeToString(&pb_req_msg_encoded);
  uint64_t current_ts = android::elapsedRealtime();
  if((current_ts - _last_written_ts) > 2*MSEC_PER_SEC) {
      sns_logi("sending ssn request registry write");
      ssc_registry_write_obj->send_request(pb_req_msg_encoded);
  }
  _last_written_ts = current_ts;
}
