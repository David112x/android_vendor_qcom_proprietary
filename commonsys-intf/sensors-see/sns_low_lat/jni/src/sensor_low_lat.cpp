...
_cb(const uint8_t *data, size_t size)
{
  ...
  for(int cnt = 0; cnt < pb_event_msg.events_size(); cnt++)
  {
    const sns_client_event_msg_sns_client_event &pb_event= pb_event_msg.events(cnt);
    ALOGE("event[%d] msg_id=%d, ts=%llu", cnt, pb_event.msg_id(),
    ...
    for (int i = 0; i < attr_event.attributes_size(); i++) {
      ...
      if(attr_event.attributes(i).attr_id() == SNS_STD_SENSOR_ATTRID_ADDITIONAL_LOW_LATENCY_RATES) {
        ...
        for (int j = 0; j< attr_value.values_size(); j++) {
          if (attr_value.values(j).has_flt()) {
            ALOGE("ll_rate %d", attr_value.values(j).flt());
            _supported_rates.push_back(attr_value.values(j).flt());
          }
      }
        }
        /*MAG do not support SNS_STD_SENSOR_ATTRID_ADDITIONAL_LOW_LATENCY_RATES pick from normal attributes*/
        if(attr_event.attributes(i).attr_id() == SNS_STD_SENSOR_ATTRID_RATES) {
          const sns_std_attr_value& attr_value = attr_event.attributes(i).value();
          ALOGE("recevied SNS_STD_SENSOR_ATTRID_RATES ,number of supported rates %d", (int)attr_value.values_size());
          for (int k = 0; k< attr_value.values_size(); k++) {
            if (attr_value.values(k).has_flt()) {
                ALOGE("rate %d", attr_value.values(k).flt());
              _supported_rates.push_back(attr_value.values(k).flt());
            }
          }
        }
      }
...
void get_supportedrates(sensor_uid suid) {
  ...
  _mutex.lock();
  if (!_cv.waitRelative(_mutex, 2*NSEC_PER_SEC_VAL)) {
    if (_supported_rates.empty()) {
...
(JNIEnv *env, jobject obj, jlongArray suids)
{
    int       fd;
    int       error_code;
    jclass    mClass = env->FindClass("com/qualcomm/qti/ustaservice/JavaNativeInterfaceAPIs");
    ...
    if (!_supported_rates.empty()) {
      ...
      result = std::max_element(_supported_rates.begin(), _supported_rates.end());
      max_supported_rate = (jint)*result;
    }
    else
...
JNIEXPORT void JNICALL Java_com_qualcomm_qti_ustaservice_JavaNativeInterfaceAPIs_start
    (JNIEnv *env, jobject obj, jlongArray suids, jint sample_period_us, jint flags, jint stream_handle, jint sensor_handle)
{
    int       error_code;
    jclass    mClass = env->FindClass( "com/qualcomm/qti/ustaservice/JavaNativeInterfaceAPIs");
    jfieldID  field_id;
    ...
    memset(&sns_suid, 0x00, sizeof(sns_std_suid_t));
    // Validate input
    if (suids_len <= 0) {
        jclass input_exception;
    ...
    LOGE("%s: stream handle 0x%x, sensor handle 0x%x\n", __FUNCTION__, (int)stream_handle, (int)sensor_handle);
