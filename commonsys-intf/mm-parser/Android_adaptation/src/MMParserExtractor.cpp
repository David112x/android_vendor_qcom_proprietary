...
status_t FileSourceWrapper::Init(iStreamPort& aStreamPort,
    ...
    if(iDrmEncrypted && fileformat != FILE_SOURCE_WEBM) {
...
void MMParserExtractor::setCryptoTrackMeta(Track* track) {
    AMediaFormat *meta = track->meta;
    ...
    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CRYPTO_KEY,
                           cryptoInfo.ucKeyID, cryptoInfo.ucKeyIDSize);
}
...
media_status_t MMParserSource::read(
    ...
    *out = NULL;
    ...
    if (newBuffer && mBuffer) {
        ...
        if (mIsEncrypted) {
            setEncryptionMetaData(meta, sInfo);
        }
...
void MMParserSource::setEncryptionMetaData(AMediaFormat *meta, FileSourceSampleInfo &info)
{
    ...
    if (pCP->ucIsEncrypted) {
        AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CRYPTO_KEY,
                               pCP->ucKeyID, pCP->ucKeyIDSize);
        AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CRYPTO_IV,
                               pCP->ucInitVector, pCP->ucInitVectorSize);
    }
