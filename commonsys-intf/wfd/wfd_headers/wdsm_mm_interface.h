...
typedef struct WFD_extended_video_codec
{
    std::string name;
    uint8_t  profile;
    uint8_t  level;
    uint8_t  latency;
    uint8_t  frameRateCtlSupp;
    uint16_t minSliceSize;
    uint16_t sliceEnc;
    uint16_t width;
    uint16_t height;
    uint16_t fps;
    WFD_extended_video_codec
    (
        decltype(name) nameV,
        decltype(profile) profileV,
        decltype(level) levelV,
        decltype(latency) latencyV,
        decltype(frameRateCtlSupp) frameRateCtlSuppV,
        decltype(minSliceSize) minSliceSizeV,
        decltype(sliceEnc) sliceEncV,
        decltype(width) widthV,
        decltype(height) heightV,
        decltype(fps) fpsV
    ):
    name(nameV),
    profile(profileV),
    level(levelV),
    latency(latencyV),
    frameRateCtlSupp(frameRateCtlSuppV),
    minSliceSize(minSliceSizeV),
    sliceEnc(sliceEncV),
    width(widthV),
    height(heightV),
    fps(fpsV){}
    ...
    WFD_video_type getVideoMethod() const;
    private:
    WFD_extended_video_codec() = delete;
}WFD_ext_vid_codec_t;

typedef struct WFD_extended_video_capability
{
    std::vector<WFD_ext_vid_codec_t> codecList;
    void dump() const;
}WFD_ext_vid_cap_t;
...
typedef struct {
    ...
    WFD_ext_vid_cap_t               ext_video_capability;
...
typedef enum {
    ...
    WFD_STATUS_AUDIO_STREAM_CREATED,
    WFD_STATUS_AUDIO_STREAM_DELETED,
    WFD_STATUS_VIDEO_STREAM_CREATED,
    WFD_STATUS_VIDEO_STREAM_DELETED,
    WFD_STATUS_IMAGE_STREAM_CREATED,
    WFD_STATUS_IMAGE_STREAM_DELETED,
    ...
    WFD_STATUS_IMAGE_STREAM_ENABLED,
    WFD_STATUS_IMAGE_STREAM_DISABLED,
    WFD_STATUS_INVALID
} WFD_status_t;
