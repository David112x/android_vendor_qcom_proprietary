...
HDCPManager::~HDCPManager()
{
    ...
    */
    ...
    LOCK_MUTEX
    gHDCPManagerList.remove(this);
#ifdef WFD_HDCP_ENABLED
    if(gHDCPManagerList.empty())
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "Finalize HDCP engine");
                                   ...
                                   int nStreamId)
{
    ...
    if( pIn && pIn->handle && pOut && pOut->handle && nLen )
    {
#ifdef HDCP_DEBUG
        if(pesPrivateData)
        {
        ...
        sInBufInfo.handle = (uint64_t)pIn->handle;
        sInBufInfo.type = pIn->isFd?
            ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE : ARM_HDCP_BUFFER_TYPE_VIRTUAL_POINTER;
        ...
        sOutBufInfo.handle = (uint64_t)pOut->handle;
        sOutBufInfo.type = pOut->isFd?
            ARM_HDCP_BUFFER_TYPE_SHARED_MEMORY_HANDLE : ARM_HDCP_BUFFER_TYPE_VIRTUAL_POINTER;
    ...
    {
        MM_MSG_PRIO6(MM_GENERAL, MM_PRIO_ERROR,
               "HDCPManager:Decrypt Err Invalid Args,PES Private Data"
                     " = %p, pIn = %p:%p, pOut = %p:%p, pLen = %lu",
                                     pesPrivateData, pIn, pIn->handle,
                                     pOut, pOut->handle, nLen );
    }
    m_eState = HDCP_STATE_CONNECTED;
    HDCPMAN_CRITICAL_SECT_LEAVE
#else
...
HDCPStatusType HDCPManager::decrypt(const unsigned char *pesPrivateData,
                                   ...
                                   int nStreamId)
{
    HDCPIOBufferType inbuf = {true, (uint64_t)pIn};
    HDCPIOBufferType outbuf = {true, (uint64_t)pOut};
    return decrypt(pesPrivateData,
                   &inbuf,
                   &outbuf,
                   nLen,
                   nPesPvtDataLen,
...
void HDCPManager::constructCodecAndStreamType(int nStream, HDCPCodecType eCodec)
{
    ...
    {
        // then its a LPCM
        m_nAudioStreamType = ((eCodec << 16) | ARM_HDCP_STREAM_MEDIA_TYPE_AUDIO);
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
           "HDCPManager m_nAudioStreamType %x", m_nAudioStreamType);
